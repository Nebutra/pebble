...
description: >-
  Set up, review, debug, or validate Pebble per-workspace environment recipes —
  ...
  per-workspace environments, fix an `environmentRecipes` entry in `pebble.yaml`, scaffold
  provider lifecycle scripts, or resolve an `pebble vm recipe doctor` failure.
---
...
Pebble is a **thin wrapper**: you guide, detect, and scaffold; you never own the user's cloud account,
...
**The one branch that shapes everything — connection mode:** **Pebble-server** (`create` runs `pebble serve`
in the env and emits a `pairingCode`; §7c/§7f) vs **SSH** (`create` runs no server and emits a
`connection.type:"ssh"` block Pebble dials into; §7g/§7h). Settle this first — it changes the `create`
output shape and half the templates.

**Quick-start (happy path):** interview the user (connection mode Pebble-server vs SSH, provider, agent CLI,
git auth — §1.2) + read the provider's CLI docs → scaffold `scripts/pebble-vm/` from §7 → run the
base-snapshot script, then the auth script (you invoke these by hand; not via `pebble.yaml`) → wire
`environmentRecipes` in `pebble.yaml` → `pebble vm recipe doctor <id> --json` (free) → then the `--provision`
...
a long time, or need the user at the keyboard. Never create an Pebble workspace or commit unless asked.

1. **Inspect the repo** for an existing `environmentRecipes` entry, `scripts/pebble-vm/`, a state file, or setup
...
2. **Interview the user up front** — gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** how Pebble attaches to the environment — an **Pebble server** (the VM runs
     `pebble serve` and Pebble pairs over its pairing URL; worked example §7f) or **SSH** (Pebble connects to
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, … For non-obvious providers, also
     ...
     (host/port/user/key or proxy command) or only a provider-mediated interactive shell; Pebble SSH mode
     needs the former.
...
7. **Wire the recipe** so `pebble.yaml` points create/suspend/resume/destroy at the scripts (§8). The
   workspace composer reads `environmentRecipes` from the project's primary checkout of `pebble.yaml`, **not** from
   a feature branch or worktree. So a recipe added only on a branch won't appear as a "Run on" option
   until that `pebble.yaml` change is committed and merged to the project's primary branch. Tell the user
...
8. **Dry-run doctor** — `pebble vm recipe doctor <recipe-id> --repo-path <repo> --json` (free, static; §9).
   Fix every failure before going live.
9. **[CHECKPOINT] Live self-test** — get the user's OK once, then run
   `pebble vm recipe doctor <recipe-id> --provision --json` as a loop: it runs create → validates →
...
- **Connection mode** (Pebble server vs SSH) confirmed with the user — see §1 step 2; it shapes the recipe.
...
A repo-local JSON file (e.g. `scripts/pebble-vm/<provider>-state.json`) threads non-secret values between
...
Scaffold under `scripts/pebble-vm/`. These are **shapes** — fill in the provider's real commands. All
...
- **Local-side** (`create`/`suspend`/`resume`/`destroy` + the base-snapshot/auth scripts the user
  ...
  or require WSL/Git-Bash and point `pebble.yaml` at the right launcher.
...
Worked Vercel commands for this phase are in §7f. You run this script by hand (not via `pebble.yaml`),
...
# name = pebble-${PEBBLE_VM_RECIPE_ID}-${PEBBLE_VM_INSTANCE_ID} (sanitized, length-capped)
...
# 3. remote exec: start pebble serve in the background and read the recipe JSON it writes (see below)
...
**The exact `pebble serve` invocation and its output (verified — do not improvise the flags).** Inside the
VM, run:
...
**Binary name:** in a VM built from source (the Phase-2 flow), run it as `pnpm exec pebble-dev serve …`
from the repo root — `pebble-dev` is the in-repo entrypoint and is what the §7f example uses. Plain
`pebble serve …` is the same command when the built CLI is installed on the VM's PATH. The flags/output
...
```json
{ "schemaVersion": 1, "pairingCode": "<pebble pairing URL>", "projectRoot": "<the --project-root you passed>" }
```
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # Pebble passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in pebble.yaml)
```
...
name="pebble-${PEBBLE_VM_RECIPE_ID:-vercel-sandbox}-${PEBBLE_VM_INSTANCE_ID:-$(date +%s)}"  # sanitize+cap to 63 chars
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  --env "GH_TOKEN=$gh_token" --env "PEBBLE_PROJECT_ROOT=$project_root" \
  --env "PEBBLE_REPO_URL=$repo_url" --env "PEBBLE_REPO_REF=$repo_ref" \
  -- bash -lc 'set -euo pipefail; cd "$PEBBLE_PROJECT_ROOT"; \
    ...
    git fetch origin "$PEBBLE_REPO_REF"; \
...
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  --env "PEBBLE_PORT=$port" --env "PEBBLE_PROJECT_ROOT=$project_root" --env "PEBBLE_PAIRING_ADDRESS=$pairing_ws" \
  -- bash -lc 'set -euo pipefail; cd "$PEBBLE_PROJECT_ROOT"; rm -f /tmp/pebble-recipe.json /tmp/pebble-serve.log; \
    nohup pnpm exec pebble-dev serve --port "$PEBBLE_PORT" --project-root "$PEBBLE_PROJECT_ROOT" \
      --pairing-address "$PEBBLE_PAIRING_ADDRESS" --recipe-json >/tmp/pebble-recipe.json 2>/tmp/pebble-serve.log </dev/null & \
    pid=$!; for _ in $(seq 1 80); do \
      node -e "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/pebble-recipe.json\",\"utf8\"))" >/dev/null 2>&1 && { cat /tmp/pebble-recipe.json; exit 0; }; \
      kill -0 "$pid" 2>/dev/null || { cat /tmp/pebble-serve.log >&2; exit 1; }; sleep 0.25; \
    done; cat /tmp/pebble-serve.log >&2; echo "serve recipe JSON timed out" >&2; exit 1')"
...
`userData.resourceId` from stdin (§7d). This is the **Pebble-server** connection mode (the recipe emits a
...
- **`create` does NOT run `pebble serve` and does NOT emit a `pairingCode`.** Pebble itself connects to the
  host over its SSH relay, brings up the git + filesystem providers, and imports the repo. The script's
  only job is to make the host ready and **print SSH connection details** Pebble will dial.
- The result uses a `connection` block with `type: "ssh"` and a `target`, **not** the flat
  `pairingCode`/`projectRoot` shape. Exact shape (Pebble rejects anything else):
...
- `relayGracePeriodSeconds` (optional): how long Pebble keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO pebble serve here)
...
# 2. print the SSH connection block (NO pairingCode, NO pebble serve). host/port/username tell Pebble's
...
`destroy: none` and omit suspend/resume. (Pebble still disconnects/reconnects its own SSH relay on
...
`connection.type:"ssh"` block above instead of starting `pebble serve`.
...
- If committing from an interactive shell, force the runtime entrypoint back to `sshd`:
  `docker commit --change='ENTRYPOINT ["/usr/local/bin/pebble-docker-ssh-entrypoint"]' …`.
...
docker run -d --name "$name" -p 127.0.0.1::22 -e "PEBBLE_SSH_PUBLIC_KEY=$pubkey" "$auth_image"
...
require WSL/Git-Bash (and point `pebble.yaml` at e.g. `bash ./scripts/pebble-vm/<name>.sh` via a `.cmd`
...
# Pebble-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
...
environmentRecipes:
  - id: cloud-sandbox
    name: Cloud Sandbox
    create: ./scripts/pebble-vm/cloud-sandbox-create.sh
    suspend: ./scripts/pebble-vm/cloud-sandbox-suspend.sh
    resume: ./scripts/pebble-vm/cloud-sandbox-resume.sh
    destroy: ./scripts/pebble-vm/cloud-sandbox-destroy.sh
```
...
**Pebble-server mode** — boot the env, start `pebble serve` in it, and print serve's result:

```json
{
  "schemaVersion": 1,
  "pairingCode": "pebble-pairing-code-or-url",
  "projectRoot": "/absolute/path/to/repo/on/remote",
  "userData": { "provider": "example", "resourceId": "provider-resource-id" }
}
```

Here `pairingCode` (from `pebble serve --recipe-json`) and `projectRoot` are required; `schemaVersion` (`1`)
and `userData` are optional.

**SSH mode** — do **not** run `pebble serve`; print the `connection.type:"ssh"` block instead (full shape +
...
Start Pebble remotely with `pebble serve --port "$PORT" --project-root "$ABS_ROOT" --pairing-address
...
`pebble vm recipe doctor <recipe-id> --repo-path <repo> --json` validates **static wiring only** — it does
...
`pebble vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` actually runs the recipe end
...
- Don't make Pebble own provider lifecycle beyond invoking the configured scripts.
- Don't commit or create an Pebble workspace unless asked.
