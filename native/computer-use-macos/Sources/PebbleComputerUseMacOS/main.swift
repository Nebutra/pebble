...
private let providerName = "pebble-computer-use-macos"
    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrusted() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for Pebble Computer Use. Run `pebble computer permissions` or open Settings > Computer Use, grant Accessibility to Pebble Computer Use, then retry."
            )
        }
        ...
        let screenshotStatus: ScreenshotStatus = if screenshot != nil {
            ...
            .failed("Screen Recording permission is required for Pebble Computer Use; grant permission or pass --no-screenshot to inspect accessibility state only.")
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    let permissionHint = visibleWindowCount > 0
        ? " The app has visible windows, so macOS Accessibility may need Pebble Computer Use toggled off and on again in System Settings."
        : ""
...
private struct WindowCapture {
    ...
    private static func captureImage(windowId: CGWindowID, bounds: CGRect) -> CapturedImage? {
        if (ProcessInfo.processInfo.environment["PEBBLE_COMPUTER_USE_SCK_SCREENSHOTS"] == "1" ||
            ProcessInfo.processInfo.environment["PEBBLE_COMPUTER_USE_SCK_SCREENSHOTS"] == "1"),
...
private final class PermissionWindowController: NSWindowController {
    ...
    convenience init(initialPermission: PermissionKind? = nil, terminateWhenDragAssistantCloses: Bool = false) {
        ...
        window.title = "Enable Pebble Computer Use"
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        switch self {
        case .accessibility:
            "Drag Pebble Computer Use into the list above to allow Accessibility."
        case .screenshots:
            "Drag Pebble Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let title = label(ready ? "Computer Use is Ready" : "Enable Pebble Computer Use", size: 22, weight: .bold)
        let subtitle = label(
            ready ? "Pebble can use local apps when you ask." : "Grant permissions so Pebble can use apps when you ask.",
            size: 12,
            weight: .regular
        )
    ...
    private func permissionRow(permission: PermissionKind, action: @escaping () -> Void) -> NSView {
        ...
        objc_setAssociatedObject(button, "pebble-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag Pebble Computer Use"
...
private final class PermissionDragAssistantView: NSView {
    ...
    private func build() {
        ...
        objc_setAssociatedObject(closeButton, "pebble-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "Pebble Computer Use")
...
private func runStdio() {
    fputs("Pebble Computer Use provider must be launched by Pebble in app-agent mode.\n", stderr)
    exit(13)
}
...
if arguments.first == "--agent" {
    ...
    guard arguments.count >= 2 else {
        fputs("usage: pebble-computer-use-macos --permission-status-file <path>\n", stderr)
        exit(2)
    }
