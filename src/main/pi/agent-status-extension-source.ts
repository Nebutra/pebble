...
// /hook/<kind> using the same PEBBLE_AGENT_HOOK_* + PEBBLE_PANE_KEY env that every
...
// any Pebble dep into the pi runtime.
import type { PiAgentKind } from '../../shared/pi-agent-kind'

export const PEBBLE_PI_AGENT_STATUS_EXTENSION_FILE = 'pebble-agent-status.ts'
...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  // so it cannot import from Pebble's main bundle. fs/http coords come from
  // the same endpoint file the OpenCode plugin reads (process.env is frozen
  // at PTY spawn, so on Pebble restart we have to re-read it from disk).
  return [
    ...
    "      console.warn('[pebble-pi-status] failed to parse endpoint file:', (err as Error).message)",
    '    }',
    '    return null',
    '  }',
    '}',
    ...
    '    })',
    '  } catch {',
    '    // Why: status reporting must never fail the pi run just because Pebble',
    '    // is unavailable or the loopback request failed (e.g. Pebble restart).',
    '  }',
    '}',
    '',
