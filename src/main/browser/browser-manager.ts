...
function releaseAutomationVisibilityToken(renderer: Electron.WebContents, token: string): void {
  ...
  renderer
    .executeJavaScript(
      `(function() {
        var bridge = window.__pebbleBrowserAutomationVisibility;
...
export class BrowserManager {
  ...
  // Why: screenshot sessions target guest page ids, but Pebble's visible browser
        ...
        .executeJavaScript(
          `(function() {
            ...
            if (
              ...
              // Why: Pebble remembers the last browser workspace/page even when
  ...
  async acquireAutomationVisibility(guestWebContentsId: number): Promise<() => void> {
    ...
    // sites, but must not steal the user's visible Pebble tab/worktree.
  ...
  attachGuestPolicies(guest: Electron.WebContents): void {
    ...
    // Pebble window is not the focused foreground app. With throttling enabled,
    ...
    guest.setWindowOpenHandler(({ url }) => {
      ...
      // flows, but Pebble still does not host child windows itself. For normal
      // web URLs, route the request into Pebble's own browser-tab model first so
      // the user stays in the IDE. Only fall back to the system browser when
      // Pebble cannot safely host the destination or when the guest is not yet
      // associated with a trusted browser tab/renderer.
      if (browserTabId && browserUrl && this.openLinkInPebbleTab(browserTabId, browserUrl)) {
  ...
  // Why: headless pebble serve has no renderer window to mount a <webview>, so its
  ...
  unregisterAll(): void {
    ...
    for (const downloadId of this.downloadsById.keys()) {
      this.cancelDownloadInternal(downloadId, 'Pebble is shutting down.')
    }
  ...
  handleGuestWillDownload(args: { guestWebContentsId: number; item: Electron.DownloadItem }): void {
    ...
    try {
      ...
      try {
        ...
        // state; cancellation is best-effort after Pebble has made the UI terminal.
      }
      return
    }
  ...
  // Why: guest browser surfaces are intentionally isolated from Pebble's preload
  ...
  private cancelDownloadInternal(downloadId: string, reason: string): void {
    ...
    try {
      ...
      // source of truth for whether Pebble still considers the request active.
    }
  ...
  private openLinkInPebbleTab(browserTabId: string, rawUrl: string): boolean {
    const renderer = this.resolveRendererForBrowserTab(browserTabId)
    if (!renderer) {
      return false
    }
    ...
    // URL back to that renderer so it can open a sibling Pebble browser tab in
    // the same worktree without letting the guest process mutate app state.
    renderer.send('browser:open-link-in-pebble-tab', {
      browserPageId: browserTabId,
      url: normalizedUrl
    })
    return true
