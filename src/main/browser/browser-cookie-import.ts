...
function getDiagLogPath(): string {
  if (!_diagLog) {
    try {
      ...
      _diagLog = join(tmpdir(), 'pebble-cookie-import-diag.log')
    }
  }
  return _diagLog
}
...
async function importCookiesFromFirefox(
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'pebble-cookie-import-'))
  ...
  try {
    ...
    if (isPermError) {
      return {
        ok: false,
        reason:
          'macOS denied access to Safari cookies. Grant Full Disk Access to Pebble in System Settings → Privacy & Security → Full Disk Access.'
      }
    }
...
export async function importCookiesFromBrowser(
  ...
  try {
    ...
    // cookies prevents stale cookies from a previous Pebble browsing session from
