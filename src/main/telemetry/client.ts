...
const BUILD_IDENTITY: 'stable' | 'rc' | null =
  typeof PEBBLE_BUILD_IDENTITY !== 'undefined'
    ...
    : ((globalThis as { PEBBLE_BUILD_IDENTITY?: 'stable' | 'rc' | null }).PEBBLE_BUILD_IDENTITY ?? null)
const WRITE_KEY: string | null =
  typeof PEBBLE_POSTHOG_WRITE_KEY !== 'undefined'
    ? PEBBLE_POSTHOG_WRITE_KEY
    : ((globalThis as { PEBBLE_POSTHOG_WRITE_KEY?: string | null }).PEBBLE_POSTHOG_WRITE_KEY ?? null)
 ...
 * PEBBLE_TELEMETRY_DISABLED). The SDK flag does not persist across process
...
// official stable/rc builds where CI injects `PEBBLE_BUILD_IDENTITY` and
// `PEBBLE_POSTHOG_WRITE_KEY`.
