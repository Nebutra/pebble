...
describe('resolveConsent', () => {
  ...
  it('returns pebble_disabled when PEBBLE_TELEMETRY_DISABLED=1', () => {
    process.env.PEBBLE_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
        settingsWithTelemetry({
          optedIn: true,
      ...
      effective: 'disabled',
      reason: 'pebble_disabled'
    })
  })

  it('prefers do_not_track over pebble_disabled when both are set', () => {
    process.env.DO_NOT_TRACK = '1'
    process.env.PEBBLE_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
  ...
  it('env-var override wins over a stored pending_banner state', () => {
    ...
    ).toEqual({ effective: 'disabled', reason: 'pebble_disabled' })
  })
})
