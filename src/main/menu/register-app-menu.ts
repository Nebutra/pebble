...
function buildAndApplyMenu(options: RegisterAppMenuOptions): void {
  ...
  const featureTourItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.explorePebble', 'Explore Pebble'),
    click: (_menuItem, window) => onOpenFeatureTour(window)
  }

  const setupGuideItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.gettingStarted', 'Getting Started with Pebble'),
  ...
  // redundant product-name entry with roles that don't apply, so we omit it there
    ...
    submenu: [
      ...
      { role: 'copy' },
      {
        ...
        click: () => {
          ...
          // control, so raw Electron paste cannot know which Pebble surface owns it.
  ...
  const appearanceSubmenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        label: translateMain('menu.showMobileButton', 'Show Pebble Mobile Button'),
        type: 'checkbox',
