...
import watercolorIcon from '../../resources/app-icons/pebble-watercolor.png?asset'
import watercolorMacDockIcon from '../../resources/app-icons/pebble-watercolor.png?asset&asarUnpack'
import blueIcon from '../../resources/app-icons/pebble-blue.png?asset'
import blueMacDockIcon from '../../resources/app-icons/pebble-blue.png?asset&asarUnpack'
...
const MAC_DOCK_ICON_SCRIPT = [
  ...
  'set appPath to system attribute "PEBBLE_APP_BUNDLE_PATH"',
  'set iconPath to system attribute "PEBBLE_APP_ICON_PATH"',
  "set image to current application's NSImage's alloc()'s initWithContentsOfFile:iconPath",
  'if image is missing value then error "Pebble app icon image could not be loaded"',
  "set ok to current application's NSWorkspace's sharedWorkspace()'s setIcon:image forFile:appPath options:0",
  'if ok is false then error "Pebble app icon could not be persisted"'
]
...
const MAC_DOCK_ICON_CLEAR_SCRIPT = [
  ...
  'if ok is false then error "Pebble app icon could not be cleared"'
]
...
function runMacCustomIconCommand(
  ...
  return runBoundedMacDockIconCommand({
    ...
    options: {
      env: {
        ...process.env,
        PEBBLE_APP_BUNDLE_PATH: appBundlePath,
        PEBBLE_APP_ICON_PATH: iconPath
      }
    },
