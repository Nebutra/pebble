...
function writeLegacyLinearToken(token: string | Buffer, viewer: Record<string, unknown>): void {
  const pebbleDir = join(tempHome, '.pebble')
  mkdirSync(pebbleDir, { recursive: true })
  writeFileSync(join(pebbleDir, 'linear-token.enc'), token)
  writeFileSync(join(pebbleDir, 'linear-viewer.json'), JSON.stringify(viewer), {
    encoding: 'utf-8'
  })
}
...
function writeMultiWorkspaceFiles(
  ...
  const pebbleDir = join(tempHome, '.pebble')
  mkdirSync(join(pebbleDir, 'linear-tokens'), { recursive: true })
  writeFileSync(
    join(pebbleDir, 'linear-workspaces.json'),
    JSON.stringify({
      version: 1,
...
beforeEach(() => {
  tempHome = mkdtempLike('pebble-linear-client-')
  fixtures = new Map([
    [
      'token-alpha',
      {
...
describe('Linear client workspace storage', () => {
  ...
  it('migrates legacy token storage to a real workspace id when explicitly tested', async () => {
    ...
    expect(existsSync(join(tempHome, '.pebble', 'linear-token.enc'))).toBe(false)
    expect(readFileSync(join(tempHome, '.pebble', 'linear-workspaces.json'), 'utf-8')).toContain(
      'org-alpha'
    )
  })
  ...
  it('does not pass encrypted safeStorage bytes to the Linear SDK when encryption is unavailable', async () => {
    const tokenPath = join(tempHome, '.pebble', 'linear-token.enc')
