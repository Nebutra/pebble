...
function writeLegacyCopyMarker(relativePath: string, sourcePath: string, targetPath: string): void {
  ...
  const markerPath = join(getRuntimeCodexHomePath(), '.pebble-session-copies', `${relativePath}.json`)
...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'pebble-codex-session-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'pebble-codex-session-user-data-'))
