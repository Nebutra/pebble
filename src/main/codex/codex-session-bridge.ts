...
export function syncSystemCodexSessionsIntoManagedHome(): void {
  ...
  const managedSessionsRoot = join(getPebbleManagedCodexHomePath(), 'sessions')
...
function replaceSymlinkSessionBridgeWithHardlink(
  ...
  try {
    ...
    replacementPath = `${targetPath}.pebble-link-${process.pid}-${Date.now()}`
...
function getLegacySessionCopyMarkerPath(relativePath: string): string {
  return join(getPebbleManagedCodexHomePath(), '.pebble-session-copies', `${relativePath}.json`)
}
