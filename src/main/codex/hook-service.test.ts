...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'pebble-codex-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'pebble-codex-user-data-'))
  previousUserDataPath = process.env.PEBBLE_USER_DATA_PATH
  process.env.PEBBLE_USER_DATA_PATH = userDataDir
...
describe('CodexHookService', () => {
  it('installs PermissionRequest with trust so Codex approval prompts reach Pebble', () => {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'pebble home with spaces')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const metacharHome = join(tmpdir(), 'pebble %PEBBLE_TEST% ^ home')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      ...
      const cmdSafe = /^[A-Za-z0-9_.:\\~-]+$/.test(join(tmpHome, '.pebble', 'agent-hooks'))
      if (cmdSafe) {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      const scriptPath = join(homedir(), '.pebble', 'agent-hooks', 'codex-hook.cmd')
      ...
      try {
        ...
        // Why: this suite may run inside an Pebble-launched terminal whose env
        // already carries PEBBLE_AGENT_HOOK_ENDPOINT/PORT/TOKEN. The managed
        // script sources that endpoint file, so leave it out or the hook posts
        // to the live Pebble instead of this test's listener.
        const cleanEnv = { ...process.env }
        for (const key of Object.keys(cleanEnv)) {
          if (key.startsWith('PEBBLE_')) {
            delete cleanEnv[key]
          }
        }
        const child = spawn('cmd.exe', ['/d', '/c', scriptPath], {
          env: {
            ...
            PEBBLE_AGENT_HOOK_TOKEN: 'tok123',
            PEBBLE_PANE_KEY: '42:leaf-abc',
            PEBBLE_TAB_ID: '42',
            PEBBLE_WORKTREE_ID: 'C:\\work trees\\my repo & co',
        ...
        expect(received.headers['x-pebble-agent-hook-token']).toBe('tok123')
  ...
  it('keeps hooks isolated by Pebble userData instead of mutating system ~/.codex', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    ...
    const devUserDataDir = mkdtempSync(join(tmpdir(), 'pebble-dev-codex-user-data-'))
    const prodUserDataDir = mkdtempSync(join(tmpdir(), 'pebble-prod-codex-user-data-'))
    try {
      getPathMock.mockImplementation((name: string) => {
        if (name === 'userData') {
      ...
      process.env.PEBBLE_USER_DATA_PATH = devUserDataDir
      expect(new CodexHookService().install().state).toBe('installed')
      ...
      process.env.PEBBLE_USER_DATA_PATH = prodUserDataDir
      expect(new CodexHookService().install().state).toBe('installed')
  ...
  it('refreshes runtime user hooks without installing Pebble-managed hooks', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
  ...
  it('removes legacy Pebble-managed hooks from system ~/.codex during install', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
  ...
  it('removes very large legacy Pebble-managed hook lists from system ~/.codex', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    const legacyScriptPath = join(
      tmpHome,
  ...
  it('cleans stale system hooks when runtime hooks.json is malformed during remove', () => {
    const managedCodexHome = join(userDataDir, 'codex-runtime-home', 'home')
    mkdirSync(managedCodexHome, { recursive: true })
  ...
  it('removes managed trust entries when userData resolves through a symlink', () => {
    ...
    process.env.PEBBLE_USER_DATA_PATH = linkedUserDataDir
