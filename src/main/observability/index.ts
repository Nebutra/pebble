...
//   PEBBLE_TELEMETRY_DISABLED=1 → identical to DO_NOT_TRACK for this lane.
//   PEBBLE_DIAGNOSTICS_DISABLED=1 → ALSO disable local file writes. The escape
...
export function resolveObservabilityConsent(): ObservabilityConsent {
  ...
  const pebbleDisabled = envOn('PEBBLE_TELEMETRY_DISABLED')
  const diagnosticsDisabled = envOn('PEBBLE_DIAGNOSTICS_DISABLED')
  const ci = inCI()
  ...
  if (diagnosticsDisabled) {
    return {
      localFileEnabled: false,
      bundleEnabled: false,
      disabledReason: 'pebble_diagnostics_disabled'
    }
  }
  if (dnt || pebbleDisabled) {
    // Local file remains active — DNT is a *network* signal, and the local
    // file never leaves the machine.
    return {
      localFileEnabled: true,
      bundleEnabled: false,
      disabledReason: dnt ? 'do_not_track' : 'pebble_telemetry_disabled'
    }
  }
 ...
 *  `~/Library/Application Support/Pebble/logs/main.trace.ndjson`; we resolve
...
export function getTraceFilePath(): string {
  ...
  try {
    ...
    const home = homedir()
    if (platform() === 'darwin') {
      userData = join(home, 'Library', 'Application Support', 'Pebble')
    } else if (platform() === 'win32') {
      userData = join(process.env.APPDATA ?? home, 'Pebble')
    } else {
      userData = join(home, '.config', 'Pebble')
    }
  }
...
export function initObservability(): ObservabilityConsent {
  ...
  if (!c.localFileEnabled) {
    // Disabled at the CI / PEBBLE_DIAGNOSTICS_DISABLED level — leave the
 ...
 *  `platform` / `arch` / `osRelease` / `pebbleChannel` inputs come from main
...
export function collectDiagnosticBundle(
  meta: Pick<
    ...
    'appVersion' | 'platform' | 'arch' | 'osRelease' | 'pebbleChannel' | 'lookbackMinutes'
  >
): CollectedBundle {
