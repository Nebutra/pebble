 ...
 *
 * Why: when Pebble needs to inject a startup command (e.g. issue command runner),
...
function getShellReadyWrapperRoot(): string {
  const userDataPath =
    ...
    process.env.PEBBLE_USER_DATA_PATH ??
    tmpdir()
...
// itself a Pebble PTY (e.g. the user launched `pn dev` from a terminal inside
// a running Pebble), that ZDOTDIR points at a Pebble shell-ready wrapper dir.
...
// (regardless of whether it came from this app's userData, a packaged Pebble,
...
export function getBashShellReadyRcfileContent(): string {
  return `# Pebble bash shell-ready wrapper
...
# PATH edits, hooks, and prompt init in Pebble startup-command shells.
...
export function getZshShellReadyRcfileContent(): string {
  return `# Pebble zsh shell-ready wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zshrc',
...
[[ -n "\${PEBBLE_MIMOCODE_HOME:-}" ]] && export MIMOCODE_HOME="\${PEBBLE_MIMOCODE_HOME}"
  ${getPosixOmpShellWrapper()}
  # Why: Codex must keep using Pebble's runtime CODEX_HOME after rc files.
  [[ -n "\${PEBBLE_CODEX_HOME:-}" ]] && export CODEX_HOME="\${PEBBLE_CODEX_HOME}"
fi
__pebble_osc133_precmd() {
  local exit_code=$?
...
function ensureShellReadyWrappers(): void {
  ...
  const zshProfile = `# Pebble zsh shell-ready wrapper
${getZshStartupFileSourceBlock({ fileName: '.zprofile' })}
`
  const zshRc = getZshShellReadyRcfileContent()
  const zshLogin = `# Pebble zsh shell-ready wrapper
...
export function writeStartupCommandWhenShellReady(
  ...
  // Why: only Pebble-wrapped bash/zsh have bracketed-paste mode active, so
  ...
  const flush = (): void => {
    ...
    // Why: run startup commands inside the same interactive shell Pebble keeps
    ...
    // unsubmitted, forcing the user to press Enter after Pebble launches the
