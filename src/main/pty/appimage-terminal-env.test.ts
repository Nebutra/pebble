...
describe('removeAppImageRuntimeEnv', () => {
  ...
  it('removes AppImage runtime identity and mount paths from Linux terminal env', () => {
    ...
    const env = {
      APPIMAGE: '/data/apps/pebble.appimage',
      APPDIR: '/tmp/.mount_pebble123',
      ARGV0: '/data/apps/pebble.appimage',
      OWD: '/home/user',
      APPIMAGE_LIBRARY_PATH: '/tmp/.mount_pebble123/usr/lib',
      PATH: ['/tmp/.mount_pebble123', '/tmp/.mount_pebble123/usr/sbin', '/usr/bin'].join(delimiter),
      LD_LIBRARY_PATH: ['/tmp/.mount_pebble123/usr/lib', '/opt/audio/lib'].join(delimiter),
      HOME: '/home/user'
    }
  ...
  it('leaves AppImage-looking env untouched outside Linux', () => {
    ...
    const env = {
      ...
      APPDIR: '/tmp/.mount_pebble123',
      ARGV0: '/data/apps/pebble.appimage',
      PATH: ['/tmp/.mount_pebble123/usr/bin', '/usr/bin'].join(delimiter)
    }
