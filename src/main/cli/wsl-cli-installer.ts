...
const WSL_COMMAND_NAME = 'pebble-ide'
const LEGACY_WSL_COMMAND_NAME = 'pebble'
  ...
  async getStatus(): Promise<CliInstallStatus> {
    ...
    if (content === null) {
      return this.buildStatus({
        ...
        detail: `Register ${ready.commandPath} to use Pebble from WSL.`
      })
    }
    ...
    if (content === 'not_file') {
      return this.buildStatus({
        ...
        currentTarget: null,
        pathConfigured: ready.pathConfigured,
        detail: `${ready.commandPath} exists but is not an Pebble launcher script.`
      })
    }
      ...
      return this.buildStatus({
        ...
        detail:
          bridgeContent === null || bridgeManaged
            ...
            : `${ready.bridgePath} exists but is not managed by Pebble.`
      })
    }
    ...
    return this.buildStatus({
      ...
      pathConfigured: ready.pathConfigured,
      detail: managed
        ? `${ready.commandPath} points to a different Pebble launcher.`
        : `${ready.commandPath} exists but is not managed by Pebble.`
    })
  }
  ...
  async install(): Promise<CliInstallStatus> {
    ...
    await this.run(
      ...
      [
        ...
        // Why: the command was renamed to avoid GNOME Pebble; remove only the
        // old Pebble-managed WSL wrapper so unmanaged `pebble` commands survive.
  ...
  > {
    ...
    if (!hostStatus.launcherPath) {
      return {
        status: this.unsupported(
          ...
          hostStatus.detail ?? 'The Windows Pebble CLI launcher is missing.'
        )
      }
    }
    ...
    if (!interopReady) {
      return {
        status: this.unsupported(
          'launcher_missing',
          'WSL Windows interop is unavailable; Pebble cannot launch the Windows CLI from WSL.'
        )
      }
    }

    const pathDirectory = `${home}/.local/bin`
    // Why: matches the Linux CLI rename to `pebble-ide` (avoids GNOME Pebble conflict).
  ...
  private async readCommandFile(
    ...
    const output = await this.run(
      ...
      [
        ...
        '  printf __PEBBLE_NOT_FILE__',
        `elif [ ! -e ${quoteShell(commandPath)} ]; then`,
        '  printf __PEBBLE_MISSING__',
    ...
    if (output === '__PEBBLE_MISSING__') {
      return null
    }
    if (output === '__PEBBLE_NOT_FILE__') {
      return 'not_file'
    }
    return output
  }
