const APPIMAGE_CLI_SCRIPT = [
  ...
  'if(!appDir){console.error("Pebble AppImage runtime did not set APPDIR.");process.exit(1);}',
...
if [ ! -f "$APPIMAGE" ]; then
  echo "Pebble AppImage not found at $APPIMAGE" >&2
  echo "If you moved the AppImage, re-run CLI registration from Pebble Settings." >&2
  exit 1
fi
export PEBBLE_NODE_OPTIONS="\${NODE_OPTIONS-}"
export PEBBLE_NODE_REPL_EXTERNAL_MODULE="\${NODE_REPL_EXTERNAL_MODULE-}"
# Why: legacy CLI bootstrap reads PEBBLE_*; keep it populated during migration.
export PEBBLE_NODE_OPTIONS="\${NODE_OPTIONS-}"
export PEBBLE_NODE_REPL_EXTERNAL_MODULE="\${NODE_REPL_EXTERNAL_MODULE-}"
unset NODE_OPTIONS
