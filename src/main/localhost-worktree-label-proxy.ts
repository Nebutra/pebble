...
const PEBBLE_LOCALHOST_SUFFIX = '.pebble.localhost'
...
export class LocalhostWorktreeLabelProxy {
  ...
  private buildLabeledUrl(label: string, target: URL): string {
    ...
    url.hostname = `${label}${PEBBLE_LOCALHOST_SUFFIX}`
  ...
  private async handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    if (!route) {
      ...
      response.end('Unknown Pebble localhost label.')
      return
    }
  ...
  private routeForRequest(request: IncomingMessage): RegisteredRoute | null {
    const host =
      String(request.headers.host ?? '')
        .split(':')[0]
        ?.toLowerCase() ?? ''
    if (!host.endsWith(PEBBLE_LOCALHOST_SUFFIX)) {
      return null
    }
    const label = host.slice(0, -PEBBLE_LOCALHOST_SUFFIX.length)
