...
async function performAddWorktree(
  ...
  // every Pebble-created worktree wants the same default. True per-worktree
  ...
  //   with the "benign and idempotent" rationale above — every Pebble-created
 ...
 * for this op. The caller owns migrating Pebble's path-derived worktree identity
...
async function performRemoveWorktree(
  ...
  try {
    // Why: `git worktree remove` only detaches the filesystem entry. Pebble also
