...
// into 429s during long focused Pebble sessions.
...
export class RateLimitService {
  ...
  private shouldBackgroundPoll(): boolean {
    ...
    // Why: these quota fetches only power in-app UI. When Pebble is hidden,
  ...
  private shouldAllowClaudePtyFallback(
    ...
    // Why: system-default Claude is not an Pebble-managed account. Background
    // quota refresh may read existing OAuth, but must not launch Claude and
    // trigger auth/browser flows for users who never configured Claude in Pebble.
