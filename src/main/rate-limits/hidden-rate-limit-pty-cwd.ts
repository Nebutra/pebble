...
const WSL_RATE_LIMIT_PTY_CWD_DIR = 'pebble-rate-limit-pty-cwd'
...
function resolveUserDataRoot(userDataPath?: string | null): string {
  const root =
    userDataPath?.trim() ||
    process.env.PEBBLE_USER_DATA_PATH?.trim() ||
    process.env.PEBBLE_USER_DATA_PATH?.trim()
  if (root && !isRootLikePath(root)) {
    return root
  }
  return join(tmpdir(), 'pebble-rate-limit-pty')
}
...
export function getHiddenRateLimitWslCwdSetupCommands(): string[] {
  return [
    `pebble_rate_limit_cwd="\${TMPDIR:-/tmp}/${WSL_RATE_LIMIT_PTY_CWD_DIR}"`,
    'mkdir -p "$pebble_rate_limit_cwd"',
    'cd "$pebble_rate_limit_cwd"'
  ]
}
