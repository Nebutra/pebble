...
describe('OpenCodeUsageStore', () => {
  ...
  it('reports no data for Pebble scope when only non-Pebble OpenCode usage exists', async () => {
  ...
  it('uses recorded OpenCode costs and token totals without model pricing inference', async () => {
    ...
    const summary = await store.getSummary('pebble', '30d')
    const daily = await store.getDaily('pebble', '30d')
  ...
  it('returns recent sessions with OpenCode event and token fields', async () => {
    ...
    const sessions = await store.getRecentSessions('pebble', '30d', 5)
