...
describe('MimoCodeHookService buildPtyEnv', () => {
  ...
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'pebble-mimocode-userdata-'))
    ...
    mimocodeHome = mkdtempSync(join(tmpdir(), 'pebble-mimocode-home-'))
    ...
    writeFileSync(join(configDir, 'plugins', 'pebble-mimocode-status.js'), 'USER PLUGIN')
  })
  ...
  it('mirrors user config into shared overlay and installs Pebble status plugin', () => {
    ...
    const pebblePlugin = join(overlayHome, 'config', 'plugins', 'pebble-mimocode-status.js')
    expect(existsSync(pebblePlugin)).toBe(true)
    expect(readFileSync(pebblePlugin, 'utf8')).toContain('/hook/mimo-code')

    expect(
      readFileSync(join(mimocodeHome, 'config', 'plugins', 'pebble-mimocode-status.js'), 'utf8')
    ).toBe('USER PLUGIN')
  })
  ...
  it('reuses the overlay home on a second buildPtyEnv call', () => {
    ...
    expect(
      readFileSync(join(overlayHome, 'config', 'plugins', 'pebble-mimocode-status.js'), 'utf8')
