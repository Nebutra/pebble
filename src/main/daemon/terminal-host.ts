...
export class TerminalHost {
  ...
  async createOrAttach(opts: CreateOrAttachOptions): Promise<CreateOrAttachResult> {
    ...
    const session = new Session({
      ...
      cols: size.cols,
      rows: size.rows,
      terminalHandle: opts.env?.PEBBLE_TERMINAL_HANDLE,
      subprocess,
    ...
    if (opts.command && !subprocess.startupCommandDeliveredInShellArgs) {
      ...
      // the user would need to press Enter after Pebble launches the agent or
      ...
      // only for Pebble-wrapped bash/zsh, which is exactly when the shell-ready
  ...
  async getCwd(sessionId: string): Promise<string | null> {
    ...
    // Why: the emulator's cwd is null until the shell emits OSC 7. Pebble's
