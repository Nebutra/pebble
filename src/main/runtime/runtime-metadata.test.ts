...
describe('runtime metadata', () => {
  ...
  it('writes and reads runtime metadata atomically', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'pebble-runtime-metadata-'))
  ...
  it('clears the runtime metadata file', () => {
    ...
    expect(getRuntimeMetadataPath(userDataPath)).toContain('pebble-runtime.json')
  })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'pebble-runtime-secure-files-'))
      tempDirs.push(userDataPath)
      ...
      for (const path of [
        join(userDataPath, 'pebble-devices.json'),
        join(userDataPath, 'pebble-e2ee-keypair.json'),
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'pebble-runtime-existing-secure-files-'))
      tempDirs.push(userDataPath)
      ...
      const devicesPath = join(userDataPath, 'pebble-devices.json')
      const keypairPath = join(userDataPath, 'pebble-e2ee-keypair.json')
  ...
  it('replaces oversized E2EE keypair files instead of reading them as metadata', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'pebble-runtime-large-keypair-'))
    tempDirs.push(userDataPath)
    const keypairPath = join(userDataPath, 'pebble-e2ee-keypair.json')
