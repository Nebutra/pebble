...
// real in-process runtime via PEBBLE_USER_DATA_PATH, and asserts:
...
import { PebbleRuntimeRpcServer } from './runtime-rpc'
...
async function runBuiltCli(
  ...
  const child = spawn(process.execPath, [CLI_PATH, ...args], {
    env: {
      ...process.env,
      PEBBLE_USER_DATA_PATH: userDataPath,
      PEBBLE_TERMINAL_HANDLE: 'term_cli',
      ...extraEnv
    },
...
describeIfBuilt('pebble orchestration check --wait subprocess (§3.4)', () => {
  it('emits newline-flushed JSON heartbeats to stderr while waiting', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'pebble-cli-sub-'))
    ...
    try {
      // Why: use the PEBBLE_HEARTBEAT_INTERVAL_MS escape hatch to shrink the
      ...
      const child = spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            PEBBLE_USER_DATA_PATH: userDataPath,
            PEBBLE_TERMINAL_HANDLE: 'term_nobody',
            PEBBLE_HEARTBEAT_INTERVAL_MS: String(heartbeatMs)
          },
...
describeIfBuilt('pebble orchestration reset subprocess', () => {
  it('validates reset scopes against an isolated runtime through the built CLI', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'pebble-cli-reset-'))
    const runtime = new PebbleRuntimeService()
    const db = new OrchestrationDb(':memory:')
    runtime.setOrchestrationDb(db)
    const server = new PebbleRuntimeRpcServer({ runtime, userDataPath })
