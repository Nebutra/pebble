...
describe('github RPC methods', () => {
  it('resolves the repo slug on the runtime server', async () => {
    const runtime = {
      ...
      getRepoSlug: vi.fn().mockResolvedValue({ owner: 'acme', repo: 'pebble' })
  ...
  it('lists slug-addressed issue metadata on the runtime server', async () => {
    ...
    const labels = await dispatcher.dispatch(
      makeRequest('github.project.listLabelsBySlug', { owner: 'acme', repo: 'pebble' })
    )
    const users = await dispatcher.dispatch(
    ...
    const issueTypes = await dispatcher.dispatch(
      makeRequest('github.project.listIssueTypesBySlug', { owner: 'acme', repo: 'pebble' })
    )

    expect(runtime.listGitHubLabelsBySlug).toHaveBeenCalledWith({ owner: 'acme', repo: 'pebble' })
    expect(runtime.listGitHubAssignableUsersBySlug).toHaveBeenCalledWith({
      owner: 'acme',
      repo: 'pebble',
      seedLogins: ['octo']
    })
    expect(runtime.listGitHubIssueTypesBySlug).toHaveBeenCalledWith({ owner: 'acme', repo: 'pebble' })
