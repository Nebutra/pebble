 ...
 * called solely from `PebbleRuntimeService.closeTerminal` (the explicit user-close IPC).
...
function internals(runtime: PebbleRuntimeService): RuntimeInternals {
  return runtime as unknown as RuntimeInternals
}

function registerTeam(runtime: PebbleRuntimeService, ptyId: string, leaderHandle: string): void {
  const { claudeAgentTeams, handleByPtyId } = internals(runtime)
  claudeAgentTeams.createLaunchEnv({
    leaderHandle,
    baseEnv: {},
    shimDir: '/tmp/pebble-shim',
    shimBin: 'pebble'
  })
