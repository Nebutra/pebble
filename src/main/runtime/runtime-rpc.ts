...
type PebbleRuntimeRpcServerOptions = {
...
export class PebbleRuntimeRpcServer {
  private readonly runtime: PebbleRuntimeService
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // the `pebble` CLI. Close all transports immediately instead of leaving
  ...
  async stop(): Promise<void> {
    ...
    // Pebble processes during restarts, updates, or development, and stale
...
export function createRuntimeTransportMetadata(
  ...
  if (platform === 'win32') {
    return {
      ...
      endpoint: `\\\\.\\pipe\\pebble-${pid}-${endpointSuffix}`
    }
  }
  return {
    kind: 'unix',
