...
// Why: dedicated file for "one surface" separation (emulator), parallel to pebble-runtime-browser.ts. Keeps PebbleRuntimeService focused; emulator routing easy to scan. No max-lines disable (split further if grows; per AGENTS + plan Phase 3).
...
export class RuntimeEmulatorCommands {
  ...
  async emulatorAttach(params: {
    ...
    if (worktreeId) {
      ...
      if (reusable) {
        ...
        serveSimStateWatcher.markPebbleManaged(reusable)
      ...
      if (stoppedUdid) {
        serveSimStateWatcher.unmarkPebbleManaged(stoppedUdid)
      }
    }
  ...
  // devices/AVDs) for the cross-platform `pebble emulator devices` command.
  ...
  async emulatorKill(params: {
    ...
    serveSimStateWatcher.unmarkPebbleManaged(killedUdid)
  ...
  async emulatorShutdown(params: {
    ...
    if (params.managedOnly && worktreeId && !params.device && !params.emulator) {
      ...
      if (shutdownUdid) {
        serveSimStateWatcher.unmarkPebbleManaged(shutdownUdid)
      }
