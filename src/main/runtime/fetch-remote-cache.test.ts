...
// `PebbleRuntimeService.fetchRemoteWithCache` — in particular that a rejected
...
// without disturbing the large pebble-runtime.test.ts mock surface.
...
// Why: pebble-runtime.ts imports heavy modules (hooks, ipc/*, etc.) at top
...
describe('PebbleRuntimeService.fetchRemoteWithCache', () => {
  beforeEach(() => {
  ...
  it('evicts the in-flight Map entry on rejection so the next caller re-fetches', async () => {
    ...
    const runtime = new PebbleRuntimeService(null)
