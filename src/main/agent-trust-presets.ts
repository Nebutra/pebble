 ...
 * first launch.
 *
 * Why: Pebble's "drop URL into agent input as a draft" flow injects the URL
...
export function markCodexProjectTrusted(workspacePath: string): void {
  ...
  // Why: Pebble-launched Codex runs with an Pebble-owned CODEX_HOME, so the trust
  // preset must also update the runtime config Codex will actually read.
  upsertProjectTrustLevel(join(getPebbleManagedCodexHomePath(), 'config.toml'), absPath, 'trusted')
}
...
function canonicalize(p: string): string {
  ...
  // (pebble caches realpath()'d worktree paths) matches the agent's lookup.
  try {
    if (existsSync(p)) {
