...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for Pebble-created worktrees whose branch Pebble is allowed to rename. */
  canRenamePebbleCreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (!deps.canRenamePebbleCreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`, true)
  }
  // Only rename a branch Pebble auto-named — never one the user chose.
