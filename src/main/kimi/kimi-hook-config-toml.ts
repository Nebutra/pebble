...
// so Pebble manages only its own marker-delimited block: install rewrites the
...
// Why: mirror the Claude-compatible events Pebble normalizes for status. Kimi uses
...
const BLOCK_START = '# >>> pebble-managed-kimi-hooks (managed by Pebble; do not edit) >>>'
const BLOCK_END = '# <<< pebble-managed-kimi-hooks <<<'
...
// Pebble generates; escape both defensively anyway.
...
// Pebble-managed script (by filename, so a moved userData path is still swept).
