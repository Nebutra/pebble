...
// homedir()/.pebble, and getKimiHome() honors KIMI_CODE_HOME. Point both at a
// temp dir so the local install/remove cycle never touches the real ~/.pebble or
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'pebble-kimi-hook-'))
...
const scriptPath = (): string => join(home, '.pebble', 'agent-hooks', 'kimi-hook.sh')
...
describe('KimiHookService', () => {
  ...
  it('keeps user config when installing, then restores it on remove', () => {
    ...
    expect((reinstalled.match(/pebble-managed-kimi-hooks \(/g) ?? []).length).toBe(1)
