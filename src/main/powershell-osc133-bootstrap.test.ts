...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('PEBBLE_OPENCODE_CONFIG_DIR')
    expect(script).toContain('PEBBLE_MIMOCODE_HOME')
    expect(script).not.toContain('PEBBLE_PI_CODING_AGENT_DIR')
    expect(script).not.toContain('PEBBLE_OMP_CODING_AGENT_DIR')
    expect(script).toContain('PEBBLE_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('$PebbleStatusExtension = $env:PEBBLE_OMP_STATUS_EXTENSION')
    expect(script).toContain('--extension $PebbleStatusExtension')
    expect(script).toContain('PEBBLE_CODEX_HOME')
