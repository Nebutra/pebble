...
describe('GeminiHookService', () => {
  ...
  beforeAll(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'pebble-gemini-home-'))
    userDataDir = mkdtempSync(join(tmpdir(), 'pebble-gemini-userdata-'))
  ...
  it('removes stale PreToolUse hooks when reinstalling managed Gemini hooks', () => {
    ...
    const staleManagedHookPath =
      process.platform === 'win32'
        ? `C:\\Users\\ramzi\\.pebble\\agent-hooks\\${managedHookFileName}`
        : `/Users/ramzi/.pebble/agent-hooks/${managedHookFileName}`
    const staleManagedCommand =
      process.platform === 'win32'
        ? staleManagedHookPath
        : `if [ -x '${staleManagedHookPath}' ]; then /bin/sh '${staleManagedHookPath}'; fi`
    const managedHookPath = join(homeDir, '.pebble', 'agent-hooks', managedHookFileName)
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'pebble gemini home with spaces')
