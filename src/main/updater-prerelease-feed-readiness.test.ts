...
function buildAtomFeed(tags: string[]): string {
  return `<?xml version="1.0" encoding="UTF-8"?><feed>${tags
    .map(
      (tag) =>
        `<entry><link rel="alternate" type="text/html" href="https://github.com/nebutra/pebble/releases/tag/${tag}"/><title>${tag}</title></entry>`
    )
    .join('')}</feed>`
}
...
function buildManifest(tag: string): string {
  ...
  return [
    `version: ${version}`,
    'files:',
    `  - url: Pebble-${version}-arm64-mac.zip`,
    '    sha512: test',
    `path: Pebble-${version}-arm64-mac.zip`
  ].join('\n')
}
...
function respondWithAtom(
  ...
  netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
    if (url === 'https://github.com/nebutra/pebble/releases.atom') {
...
describe('fetchNewerReleaseTagsWithReadiness', () => {
  ...
  it('requires every asset referenced by the manifest files list to be reachable', async () => {
    netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
          ...
          text: () =>
            Promise.resolve(
              [
                `version: ${version}`,
                'files:',
                `  - url: Pebble-${version}-mac.zip`,
                '    sha512: test',
                `  - url: Pebble-${version}-arm64-mac.zip`,
                '    sha512: test',
                `path: Pebble-${version}-mac.zip`
              ].join('\n')
            )
        })
      }
      ...
      if (init?.method === 'HEAD') {
        return Promise.resolve({
          ok: !url.endsWith('/Pebble-1.4.28-arm64-mac.zip'),
          text: () => Promise.resolve('')
        })
      }
  ...
  it('accepts absolute manifest asset URLs without rewriting them to release asset paths', async () => {
          ...
          text: () =>
            Promise.resolve(
              [
                'version: 1.4.27',
                'files:',
                '  - url: https://downloads.example.com/Pebble-1.4.27-arm64-mac.zip',
                '    sha512: test'
              ].join('\n')
            )
        })
    ...
    expect(assetUrls).toEqual(['https://downloads.example.com/Pebble-1.4.27-arm64-mac.zip'])
  })
