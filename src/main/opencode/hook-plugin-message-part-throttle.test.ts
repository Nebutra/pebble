 ...
 * coalesced and capped before POSTing to Pebble's agent-hook server. The
 * un-throttled plugin re-posted the full accumulated reply per streamed
 * append — O(n²) bytes per turn — which saturated Pebble's main + renderer
...
const ENV_KEYS = ['PEBBLE_PANE_KEY', 'PEBBLE_AGENT_HOOK_PORT', 'PEBBLE_AGENT_HOOK_TOKEN'] as const
...
describe('OpenCode plugin MessagePart throttling', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'pebble-opencode-plugin-test-'))
    posts = []
    savedEnv = {}
    for (const key of ENV_KEYS) {
      savedEnv[key] = process.env[key]
    }
    process.env.PEBBLE_PANE_KEY = 'tab-1:leaf-1'
    process.env.PEBBLE_AGENT_HOOK_PORT = '45678'
    process.env.PEBBLE_AGENT_HOOK_TOKEN = 'test-token'
  ...
  async function loadPluginEventHandler(): Promise<PluginEventHandler> {
    const pluginPath = join(tempDir, 'pebble-opencode-status.mjs')
    writeFileSync(pluginPath, _internals.getOpenCodePluginSource())
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      PebbleOpenCodeStatusPlugin: (ctx: unknown) => Promise<{ event: PluginEventHandler }>
    }
    const client = {
      session: {
    ...
    const hooks = await module.PebbleOpenCodeStatusPlugin({ client })
    return hooks.event
  }
