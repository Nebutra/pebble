...
const PEBBLE_OPENCODE_PLUGIN_FILE = 'pebble-opencode-status.js'
...
const OPENCODE_OVERLAY_MANIFEST_FILE = '.pebble-opencode-overlay-manifest.json'
...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  // same PEBBLE_PANE_KEY / PEBBLE_TAB_ID / PEBBLE_WORKTREE_ID / PEBBLE_AGENT_HOOK_*
  // env vars that Pebble injects into every PTY, so OpenCode panes flow into
  ...
  return [
    ...
    "// OpenCode's Node process (not Pebble's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Pebble restart',
    ...
    '      // writeEndpointFile on the Pebble side) allocates a fresh inode on',
    ...
    '        // digits in the key for forward-compat with future PEBBLE_AGENT_HOOK_*',
    ...
    '      console.warn("[pebble-hook] failed to parse endpoint file:", err.message);',
    '    }',
    '    return null;',
    '  }',
    '}',
    ...
    '  // frozen when OpenCode was fork()ed — stale after a Pebble restart. The',
    '  // file is rewritten on every Pebble start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Pebble',
    ...
    '    env: fileEnv.PEBBLE_AGENT_HOOK_ENV || process.env.PEBBLE_AGENT_HOOK_ENV || "",',
    '    version: fileEnv.PEBBLE_AGENT_HOOK_VERSION || process.env.PEBBLE_AGENT_HOOK_VERSION || "",',
    '  };',
    '}',
    '',
    ...
    '// per turn through Pebble (loopback HTTP -> main JSON parse -> status compare',
    ...
    '// flip the root Pebble pane to done or overwrite the parent turn preview.',
    ...
    '  // rewritten by a newer Pebble since the last call. Pane/tab/worktree IDs',
    '  // stay on process.env because they are per-PTY (stable for the life of',
    '  // the OpenCode process), not per-Pebble-instance.',
    ...
    '    paneKey,',
    '    launchToken: process.env.PEBBLE_AGENT_LAUNCH_TOKEN || "",',
    '    tabId: process.env.PEBBLE_TAB_ID || "",',
    '    worktreeId: process.env.PEBBLE_WORKTREE_ID || "",',
    '    env: coords.env,',
    ...
    '        "X-Pebble-Agent-Hook-Token": coords.token,',
    '      },',
    '      body,',
    '    });',
    '  } catch {',
    '    // Why: OpenCode session events must never fail the agent run just',
    '    // because Pebble is unavailable or the local loopback request failed.',
    '  }',
    '}',
    '',
    ...
    'export const PebbleOpenCodeStatusPlugin = async (_ctx) => {',
...
export class OpenCodeHookService {
  ...
  buildPtyEnv(ptyId: string, existingConfigDir?: string | undefined): Record<string, string> {
    if (!isUsableId(ptyId)) {
      ...
      // user's own config — only the Pebble status plugin is forfeited.
    ...
    // Pebble-owned dir is the exact config-replacement failure mode documented in
  ...
  private readOverlayManifest(overlayDir: string): OpenCodeOverlayManifest {
    try {
      ...
      const parsed = JSON.parse(readFileSync(manifestPath, 'utf8')) as Partial<OpenCodeOverlayManifest>
      return {
  ...
  private clearManifestEntries(overlayDir: string, manifest: OpenCodeOverlayManifest): void {
    ...
    for (const entryName of manifest.pluginEntries) {
      if (entryName === PEBBLE_OPENCODE_PLUGIN_FILE) {
        continue
      }
  ...
  // so Pebble can drop a sibling file alongside the user's plugins; everything
  ...
  private mirrorUserConfig(sourceDir: string, overlayDir: string): void {
    ...
    // Pebble previously mirrored, so deleted/replaced user config cannot stay
      ...
      if (entry.name === 'plugins') {
        ...
        if ((!isSymlink && entry.isDirectory()) || isLinkPointingToDir) {
          ...
          for (const pluginEntry of readdirSync(resolvedSource, { withFileTypes: true })) {
            // Why: skip a user file with the same filename as Pebble's plugin —
            ...
            // semantics: Pebble's status plugin runs and the user's same-named
            // plugin is shadowed for this PTY only — their source file on disk
            // is untouched.
            if (pluginEntry.name === PEBBLE_OPENCODE_PLUGIN_FILE) {
              continue
            }
            mirrorEntry(
  ...
  // Why: write Pebble's status plugin into the overlay's plugins/ dir. The
  ...
  private writePluginIntoOverlay(overlayDir: string): void {
    ...
    const pluginPath = join(pluginsDir, PEBBLE_OPENCODE_PLUGIN_FILE)
    this.removeManagedPluginFiles(pluginsDir)
  ...
  private writeSharedPluginConfig(): string | null {
    ...
    try {
      mkdirSync(pluginsDir, { recursive: true })
      this.removeManagedPluginFiles(pluginsDir)
      writeFileSync(join(pluginsDir, PEBBLE_OPENCODE_PLUGIN_FILE), getOpenCodePluginSource())
    } catch {
  ...
  private removeManagedPluginFiles(pluginsDir: string): void {
    try {
      unlinkSync(join(pluginsDir, PEBBLE_OPENCODE_PLUGIN_FILE))
    } catch {
