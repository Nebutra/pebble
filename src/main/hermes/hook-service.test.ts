...
describe('HermesHookService', () => {
  ...
  beforeEach(() => {
    ...
    homeDir = mkdtempSync(join(tmpdir(), 'pebble-hermes-hooks-'))
  ...
  it('preserves other enabled plugins and removes Pebble from disabled list', () => {
    writeFileSync(
  ...
  it('registered plugin hooks post normalized JSON to Pebble', async () => {
    ...
    const received = new Promise<Record<string, unknown>>((resolve, reject) => {
      const server = createServer((req, res) => {
        ...
        req.on('end', () => {
          try {
            ...
            expect(req.headers['x-pebble-agent-hook-token']).toBe('token-1')
            res.writeHead(204)
            res.end()
      ...
      server.listen(0, '127.0.0.1', () => {
        ...
        const script = [
          ...
          `spec = importlib.util.spec_from_file_location("pebble_status", ${JSON.stringify(initPath)})`,
        ...
        execFile(
          ...
          ['-c', script],
          {
            env: {
              ...
              PEBBLE_AGENT_HOOK_TOKEN: 'token-1',
              PEBBLE_AGENT_HOOK_ENDPOINT: '',
              PEBBLE_PANE_KEY: PANE_KEY,
              PEBBLE_TAB_ID: 'tab-1',
