...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'nebutra.pebble.computer-use'
...
async function openComputerUsePermissionsAsync(
  ...
  if (!helperAppPath) {
    throw new RuntimeClientError('accessibility_error', 'Pebble Computer Use.app was not found')
  }
...
function closeExistingPermissionHelpers(): void {
  ...
  const setupHelperPatterns = [
    'pebble-computer-use-macos[[:space:]]+--permission([[:space:]]|$)',
    'pebble-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'
  ]
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to Pebble Computer Use, then retry get-app-state.`
}
