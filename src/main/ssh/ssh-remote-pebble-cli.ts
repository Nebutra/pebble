...
export type RemotePebbleCliRequest = {
  argv: string[]
  cwd: string
...
export type RemotePebbleCliResult = {
  stdout: string
  stderr: string
  exitCode: number
}
...
export async function runRemotePebbleCli(
  runtime: PebbleRuntimeService,
  request: RemotePebbleCliRequest
): Promise<RemotePebbleCliResult> {
...
async function dispatchRemoteCli(
  ...
  switch (command) {
    ...
    default:
      throw new Error(`Unsupported SSH Pebble CLI command: ${command}`)
  }
}
