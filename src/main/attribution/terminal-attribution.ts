...
import { PEBBLE_GIT_COMMIT_TRAILER } from '../../shared/pebble-attribution'

const ATTRIBUTION_ROOT_DIR = 'pebble-terminal-attribution'
const ATTRIBUTION_SHIM_VERSION = '6'
const PEBBLE_PRODUCT_URL = 'https://github.com/nebutra/pebble'
const PEBBLE_GH_FOOTER = `Made with [Pebble](${PEBBLE_PRODUCT_URL})`
const SHELL_DOLLAR = '$'
const POWERSHELL_TICK = '`'
const ATTRIBUTION_ENV_KEYS = [
  'PEBBLE_ENABLE_GIT_ATTRIBUTION',
  'PEBBLE_GIT_COMMIT_TRAILER',
  'PEBBLE_GH_PR_FOOTER',
  'PEBBLE_GH_ISSUE_FOOTER',
...
export function applyTerminalAttributionEnv(
  ...
  // cannot recursively point PEBBLE_REAL_* at themselves.
  ...
  // Why: these wrappers should affect only Pebble-managed PTYs. Prepending the
  // shim directory here keeps the attribution behavior scoped to Pebble's live
  ...
  baseEnv.PEBBLE_ENABLE_GIT_ATTRIBUTION = '1'
  baseEnv.PEBBLE_GIT_COMMIT_TRAILER = PEBBLE_GIT_COMMIT_TRAILER
  baseEnv.PEBBLE_GH_PR_FOOTER = PEBBLE_GH_FOOTER
  baseEnv.PEBBLE_GH_ISSUE_FOOTER = PEBBLE_GH_FOOTER
  if (shellFamily === 'posix') {
    baseEnv.PEBBLE_ATTRIBUTION_SHIM_DIR = posixDir
  } else {
  ...
  if (platform === 'win32') {
    if (resolvedGit) {
      baseEnv.PEBBLE_REAL_GIT = resolvedGit
    }
    if (resolvedGh) {
      baseEnv.PEBBLE_REAL_GH = resolvedGh
    }
  }
}
...
function stripAttributionPathEntries(pathValue: string, pathDelimiter: string): string {
  return pathValue
    ...
    .filter((entry) => {
      ...
      return !normalized.includes('/pebble-terminal-attribution/')
    })
    .join(pathDelimiter)
}
...
clean_path() {
  ...
  for entry in "${SHELL_DOLLAR}{entries[@]}"; do
    case "$entry" in
      "$script_dir"|*/pebble-terminal-attribution/posix|*/pebble-terminal-attribution/win32|*\\pebble-terminal-attribution\\posix|*\\pebble-terminal-attribution\\win32)
        ;;
      *)
        cleaned+=("$entry")
        ;;
...
if [[ -z "$real_git" ]]; then
  echo "Pebble attribution wrapper could not locate git on PATH." >&2
  exit 127
fi
...
if [[ "\${PEBBLE_ENABLE_GIT_ATTRIBUTION:-0}" != "1" || "\${PEBBLE_ATTRIBUTION_BYPASS:-0}" == "1" ]] || ! is_commit_command "$@"; then
...
trailer="\${PEBBLE_GIT_COMMIT_TRAILER:-Co-authored-by: Pebble <help@nebutra.ai>}"
...
PEBBLE_ATTRIBUTION_BYPASS=1 PATH="$real_path" exec "$real_git" "${SHELL_DOLLAR}{attributed_args[@]}"
`
...
if [[ -z "$real_gh" ]]; then
  echo "Pebble attribution wrapper could not locate gh on PATH." >&2
  exit 127
fi
...
if [[ "\${PEBBLE_ENABLE_GIT_ATTRIBUTION:-0}" != "1" || "\${PEBBLE_ATTRIBUTION_BYPASS:-0}" == "1" ]]; then
  PATH="$real_path" exec "$real_gh" "$@"
fi

if [[ "\${1:-}" == "pr" && "\${2:-}" == "create" ]]; then
  footer="\${PEBBLE_GH_PR_FOOTER:-Made with [Pebble](https://github.com/nebutra/pebble)}"
...
if [[ "\${1:-}" == "issue" && "\${2:-}" == "create" ]]; then
  footer="\${PEBBLE_GH_ISSUE_FOOTER:-Made with [Pebble](https://github.com/nebutra/pebble)}"
  if has_passthrough_create_args "$@"; then
    PATH="$real_path" exec "$real_gh" "$@"
  fi
  if ! has_noninteractive_create_args "$@"; then
...
setlocal
if not "%PEBBLE_ENABLE_GIT_ATTRIBUTION%"=="1" goto run
if "%PEBBLE_ATTRIBUTION_BYPASS%"=="1" goto run
call :pebble_is_git_commit %*
if errorlevel 1 goto run
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0git-wrapper.ps1" %*
exit /b %ERRORLEVEL%
:run
if defined PEBBLE_REAL_GIT (
  "%PEBBLE_REAL_GIT%" %*
) else (
  echo Pebble attribution wrapper could not locate git on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%

:pebble_is_git_commit
if "%~1"=="" exit /b 1
if /I "%~1"=="commit" exit /b 0
set "pebble_git_arg=%~1"
if /I "%pebble_git_arg%"=="-c" goto skip_two
if /I "%pebble_git_arg%"=="--config" goto skip_two
if /I "%pebble_git_arg%"=="-C" goto skip_two
if /I "%pebble_git_arg%"=="--git-dir" goto skip_two
if /I "%pebble_git_arg%"=="--work-tree" goto skip_two
if /I "%pebble_git_arg%"=="--namespace" goto skip_two
if /I "%pebble_git_arg:~0,9%"=="--config=" goto skip_one
if /I "%pebble_git_arg:~0,10%"=="--git-dir=" goto skip_one
if /I "%pebble_git_arg:~0,12%"=="--work-tree=" goto skip_one
if /I "%pebble_git_arg:~0,12%"=="--namespace=" goto skip_one
if "%pebble_git_arg:~0,1%"=="-" goto skip_one
exit /b 1
:skip_two
shift
shift
goto pebble_is_git_commit
:skip_one
shift
goto pebble_is_git_commit
`
...
exit /b %ERRORLEVEL%
:run
if defined PEBBLE_REAL_GH (
  "%PEBBLE_REAL_GH%" %*
) else (
  echo Pebble attribution wrapper could not locate gh on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%
`
...
$realGit = if ($env:PEBBLE_REAL_GIT) { $env:PEBBLE_REAL_GIT } else { 'git' }
$trailer = if ($env:PEBBLE_GIT_COMMIT_TRAILER) { $env:PEBBLE_GIT_COMMIT_TRAILER } else { 'Co-authored-by: Pebble <help@nebutra.ai>' }
...
$env:PEBBLE_ATTRIBUTION_BYPASS = '1'
try {
...
$realGh = if ($env:PEBBLE_REAL_GH) { $env:PEBBLE_REAL_GH } else { 'gh' }
...
if ($isPrCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:PEBBLE_GH_PR_FOOTER) { $env:PEBBLE_GH_PR_FOOTER } else { 'Made with [Pebble](https://github.com/nebutra/pebble)' }
...
if ($isIssueCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:PEBBLE_GH_ISSUE_FOOTER) { $env:PEBBLE_GH_ISSUE_FOOTER } else { 'Made with [Pebble](https://github.com/nebutra/pebble)' }
    if ($null -ne $body -and $body -notmatch [Regex]::Escape($footer)) {
      $tmpFile = [System.IO.Path]::GetTempFileName()
      try {
        $trimmed = $body.TrimEnd("${POWERSHELL_TICK}r", "${POWERSHELL_TICK}n")
