...
vi.mock('electron', () => ({
  app: {
    getPath: vi.fn(() => '/tmp/pebble-test-userdata')
  }
}))
...
describe('ClaudeUsageStore', () => {
  ...
  it('reports no data for Pebble scope when only non-Pebble usage exists', async () => {
  ...
  it('filters sessions by local calendar day instead of raw UTC date prefixes', async () => {
    ...
    const recentSessions = await store.getRecentSessions('pebble', '7d', 10)
