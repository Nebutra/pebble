...
const REDIRECT_ATTEMPT_ENV = 'PEBBLE_PACKAGED_CLI_ENTRY_REDIRECTED'
const LEGACY_REDIRECT_ATTEMPT_ENV = 'PEBBLE_PACKAGED_CLI_ENTRY_REDIRECTED'

/**
 * Why: on Windows the bundled `pebble.cmd` runs `Pebble.exe <unpacked CLI entry>`
 * with ELECTRON_RUN_AS_NODE=1. When that env var is dropped (e.g. a wrapper or
 * shell that resets it), Pebble boots as a GUI, loses the single-instance lock to
 ...
 * CLI's status.
 *
 * Security: the spawned program is always `execPath` (Pebble.exe) and the script
...
export function maybeRedirectPackagedCliEntryLaunch(options: RedirectOptions = {}): RedirectResult {
  ...
  if (env[REDIRECT_ATTEMPT_ENV] === '1' || env[LEGACY_REDIRECT_ATTEMPT_ENV] === '1') {
    process.stderr.write('Unable to start the Pebble CLI through Electron node mode.\n')
    return { redirected: true, status: 1 }
  }
  if (!exists(cliEntryPath)) {
    process.stderr.write(`Unable to locate the Pebble CLI entrypoint at ${cliEntryPath}\n`)
    return { redirected: true, status: 1 }
  }
...
function buildElectronRunAsNodeEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const childEnv = { ...env }
  // Why: the CLI re-reads these from the prefixed copies; clearing the
  // originals keeps Electron's own node bootstrap from inheriting them.
  childEnv.PEBBLE_NODE_OPTIONS = env.NODE_OPTIONS ?? ''
  childEnv.PEBBLE_NODE_REPL_EXTERNAL_MODULE = env.NODE_REPL_EXTERNAL_MODULE ?? ''
  childEnv.PEBBLE_NODE_OPTIONS = env.NODE_OPTIONS ?? ''
  childEnv.PEBBLE_NODE_REPL_EXTERNAL_MODULE = env.NODE_REPL_EXTERNAL_MODULE ?? ''
  childEnv.ELECTRON_RUN_AS_NODE = '1'
  childEnv[REDIRECT_ATTEMPT_ENV] = '1'
  childEnv[LEGACY_REDIRECT_ATTEMPT_ENV] = '1'
