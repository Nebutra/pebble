...
describe('AppImage CLI redirect', () => {
  it('detects direct AppImage CLI commands', () => {
    expect(
      getAppImageCliArgs(
        ['pebble-linux.AppImage', 'status', '--json'],
        { APPIMAGE: '/opt/pebble' },
        {
          platform: 'linux',
          isPackaged: true,
  ...
  it('allows CLI global flags before the command', () => {
    expect(
      getAppImageCliArgs(
        ['pebble-linux.AppImage', '--pairing-code', 'abc123', '--json', 'terminal', 'list'],
        {
          APPIMAGE: '/opt/pebble'
        },
        {
  ...
  it('spawns the unpacked CLI entrypoint with Electron node mode', async () => {
    const root = await mkdtemp(join(tmpdir(), 'pebble-appimage-cli-redirect-'))
    ...
    const result = maybeRedirectAppImageCliLaunch({
      argv: ['pebble-linux.AppImage', 'status', '--json'],
      env: {
        APPIMAGE: '/opt/pebble/pebble-linux.AppImage',
    ...
    expect(spawn).toHaveBeenCalledWith('/opt/pebble/pebble-ide', [cliEntryPath, 'status', '--json'], {
      env: expect.objectContaining({
        APPIMAGE: '/opt/pebble/pebble-linux.AppImage',
        ELECTRON_RUN_AS_NODE: '1',
        PEBBLE_NODE_OPTIONS: '--inspect',
        PEBBLE_NODE_REPL_EXTERNAL_MODULE: '/tmp/repl.js'
      }),
      stdio: 'inherit'
    })
