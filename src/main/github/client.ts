...
const PEBBLE_REPO = 'nebutra/pebble'
...
/**
 * Check if the authenticated user has starred the Pebble repo.
 * Returns true if starred, false if not, null if unable to determine (gh unavailable).
 */
export async function checkPebbleStarred(): Promise<boolean | null> {
  await acquire()
  try {
    const { stdout, stderr } = await execFileAsync(
      'gh',
      ['api', '--include', `user/starred/${PEBBLE_REPO}`],
      { encoding: 'utf-8' }
    )
...
/**
 * Star the Pebble repo for the authenticated user.
 */
export async function starPebble(): Promise<boolean> {
  await acquire()
  try {
    await execFileAsync('gh', ['api', '-X', 'PUT', `user/starred/${PEBBLE_REPO}`], {
      encoding: 'utf-8'
    })
    return true
  } catch {
...
function sameOwnerRepo(left: OwnerRepo | null, right: OwnerRepo | null): boolean {
  ...
  // with different casing (Nebutra/Pebble vs nebutra/pebble) point at the same
...
export async function createGitHubPullRequest(
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'pebble-pr-body-'))
  await acquire()
...
export async function getPRForBranchOutcome(
  ...
  try {
    ...
    // Why: a currently visible PR can be merged outside Pebble; when the caller
