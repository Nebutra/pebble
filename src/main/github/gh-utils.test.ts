...
describe('github owner/repo resolution', () => {
  ...
  it('parses GitHub HTTPS and SSH remotes', () => {
    ...
    expect(parseGitHubOwnerRepo('git@github.com:nebutra/pebble.git')).toEqual({
      owner: 'nebutra',
      repo: 'pebble'
    })
    ...
    expect(parseGitHubOwnerRepo('ssh://git@github.com/nebutra/pebble.git')).toEqual({
      owner: 'nebutra',
      repo: 'pebble'
    })
    expect(parseGitHubOwnerRepo('ssh://git@ssh.github.com:443/nebutra/pebble.git')).toEqual({
      owner: 'nebutra',
      repo: 'pebble'
    })
    expect(parseGitHubOwnerRepo('git@example.com:nebutra/pebble.git')).toBeNull()
  })

  it('parses GitHub Enterprise host identity', () => {
    expect(parseGitHubRemoteIdentity('https://ghe.acme.internal/acme/pebble.git')).toEqual({
      host: 'ghe.acme.internal',
      owner: 'acme',
      repo: 'pebble'
    })
    expect(parseGitHubRemoteIdentity('git@ghe.acme.internal:acme/pebble.git')).toEqual({
      host: 'ghe.acme.internal',
      owner: 'acme',
      repo: 'pebble'
    })
    expect(parseGitHubOwnerRepo('https://ghe.acme.internal/acme/pebble.git')).toBeNull()
  })

  it('keeps getOwnerRepo origin-based', async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:fork/pebble.git\n'
    })

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'fork', repo: 'pebble' })
  ...
  it('prefers upstream for issue owner/repo resolution', async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:nebutra/pebble.git\n'
    })

    await expect(getIssueOwnerRepo('/repo')).resolves.toEqual({ owner: 'nebutra', repo: 'pebble' })
  ...
  it('falls back to origin when upstream is missing or non-GitHub', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:nebutra/pebble.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/pebble.git\n' })

    await expect(getIssueOwnerRepo('/repo')).resolves.toEqual({ owner: 'fork', repo: 'pebble' })
  ...
  it('does not mix origin and upstream cache entries for the same repo path', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/pebble.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:nebutra/pebble.git\n' })
  ...
  it('resolves SSH repo remotes through the registered SSH git provider', async () => {
    const sshProvider = {
      exec: vi.fn().mockResolvedValue({ stdout: 'git@github.com:nebutra/pebble.git\n', stderr: '' })
    }
    getSshGitProviderMock.mockReturnValue(sshProvider)

    await expect(getOwnerRepo('/home/user/pebble', 'openclaw-2')).resolves.toEqual({
      owner: 'nebutra',
      repo: 'pebble'
    })
  ...
  it('keeps local and SSH owner/repo cache entries separate for the same path', async () => {
    const sshProvider = {
      exec: vi.fn().mockResolvedValue({ stdout: 'git@github.com:remote/pebble.git\n', stderr: '' })
    }
    gitExecFileAsyncMock.mockResolvedValueOnce({ stdout: 'git@github.com:local/pebble.git\n' })
    getSshGitProviderMock.mockReturnValue(sshProvider)

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'local', repo: 'pebble' })
    await expect(getOwnerRepo('/repo', 'ssh-1')).resolves.toEqual({ owner: 'remote', repo: 'pebble' })
  })

  it('keeps local host and local WSL owner/repo cache entries separate for the same path', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:host/pebble.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:wsl/pebble.git\n' })

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'host', repo: 'pebble' })
  ...
  it('prunes expired distinct owner/repo cache entries on later lookups', async () => {
    ...
    try {
      ...
      await expect(getOwnerRepo('/repo-a')).resolves.toEqual({ owner: 'nebutra', repo: 'pebble' })
  ...
  it('resolves PR candidates as upstream then origin and de-dupes matching slugs', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:Acme/Pebble.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:acme/pebble.git\n' })

    await expect(resolvePRRepositoryCandidates('/repo')).resolves.toEqual({
      candidates: [{ owner: 'Acme', repo: 'Pebble' }],
      headRepo: { owner: 'acme', repo: 'pebble' }
    })
  })

  it('ignores non-GitHub upstream while keeping origin as the head repo', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:Acme/Pebble.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/pebble.git\n' })

    await expect(resolvePRRepositoryCandidates('/repo')).resolves.toEqual({
      candidates: [{ owner: 'fork', repo: 'pebble' }],
  ...
  it('expires cached remote owner/repo entries after the TTL', async () => {
    ...
    try {
      gitExecFileAsyncMock
        .mockResolvedValueOnce({ stdout: 'git@github.com:old/pebble.git\n' })
        .mockResolvedValueOnce({ stdout: 'git@github.com:new/pebble.git\n' })
  ...
  it('keeps local missing-remote probes cached beyond the short positive TTL', async () => {
    const repoPath = await mkdtemp(join(tmpdir(), 'pebble-gh-utils-'))
...
describe('resolveIssueSource', () => {
  ...
  it("'auto' + upstream exists → upstream, fellBack=false", async () => {
    ...
    await expect(resolveIssueSource('/repo', 'auto')).resolves.toEqual({
      source: { owner: 'nebutra', repo: 'pebble' },
      fellBack: false
    })
  })
  ...
  it("'auto' + no upstream → origin, fellBack=false", async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:nebutra/pebble.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:solo/pebble.git\n' })

    await expect(resolveIssueSource('/repo', 'auto')).resolves.toEqual({
      source: { owner: 'solo', repo: 'pebble' },
      fellBack: false
    })
  })
  ...
  it("'origin' + no upstream → origin, fellBack=false", async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:solo/pebble.git\n'
    })
...
describe('gh error classification', () => {
  ...
  it('classifies "has disabled issues" stderr as issues_disabled', () => {
    const stderr =
      "Command failed: gh issue list --limit 36 --json number,title,state --repo brennanb2025/pebble --state open\nthe 'brennanb2025/pebble' repository has disabled issues"
