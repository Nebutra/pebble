...
describe('GitLab IPC handlers', () => {
  ...
  it('rejects source context for a different host', async () => {
    registerGitLabHandlers(
      storeWithRepos([repo({ id: 'repo-local', path: '/local/pebble' })]) as Store
    )
  ...
  it('resolves pasted URL lookups by repoId and source host context', async () => {
    ...
    expect(getWorkItemByProjectRefMock).toHaveBeenCalledWith(
      '/ssh/pebble',
      { host: 'gitlab.com', path: 'nebutra/pebble' },
      42,
      'issue',
      'builder'
    )
  ...
  it('routes local WSL project GitLab issue, MR, work-item, and todo IPC through project git options', async () => {
    ...
    getProjectSlugMock.mockResolvedValue({ host: 'gitlab.com', path: 'nebutra/pebble' })
    ...
    await ipcHandlers.get('gitlab:projectSlug')?.(null, { repoPath: '/local/pebble' })
    await ipcHandlers.get('gitlab:mrForBranch')?.(null, {
      repoPath: '/local/pebble',
      branch: 'feature/wsl'
    })
    await ipcHandlers.get('gitlab:mr')?.(null, { repoPath: '/local/pebble', iid: 8 })
    await ipcHandlers.get('gitlab:listMRs')?.(null, {
      repoPath: '/local/pebble',
      state: 'opened',
      page: 1,
    ...
    await ipcHandlers.get('gitlab:issue')?.(null, { repoPath: '/local/pebble', number: 7 })
    ...
    await ipcHandlers.get('gitlab:listLabels')?.(null, { repoPath: '/local/pebble' })
    await ipcHandlers.get('gitlab:listAssignableUsers')?.(null, { repoPath: '/local/pebble' })
    await ipcHandlers.get('gitlab:todos')?.(null, { repoPath: '/local/pebble' })

    const hostedReviewOptions = { localGitExecOptions: localGitOptions }
    expect(getProjectSlugMock).toHaveBeenCalledWith('/local/pebble', null, hostedReviewOptions)
    ...
    expect(getMergeRequestMock).toHaveBeenCalledWith('/local/pebble', 8, null, hostedReviewOptions)
    expect(listMergeRequestsMock).toHaveBeenCalledWith(
      '/local/pebble',
      'opened',
      1,
    ...
    expect(getIssueMock).toHaveBeenCalledWith('/local/pebble', 7, null, localGitOptions)
    ...
    expect(listLabelsMock).toHaveBeenCalledWith('/local/pebble', undefined, null, localGitOptions)
    ...
    expect(listTodosMock).toHaveBeenCalledWith('/local/pebble', null, localGitOptions)
  })
  ...
  it('routes local WSL project GitLab MR details, review, job, and pasted URL IPC through project git options', async () => {
    ...
    await ipcHandlers.get('gitlab:closeMR')?.(null, { repoPath: '/local/pebble', iid: 8 })
    await ipcHandlers.get('gitlab:reopenMR')?.(null, { repoPath: '/local/pebble', iid: 8 })
    ...
    await ipcHandlers.get('gitlab:jobTrace')?.(null, { repoPath: '/local/pebble', jobId: 99 })
    await ipcHandlers.get('gitlab:retryJob')?.(null, { repoPath: '/local/pebble', jobId: 99 })
