...
import type { RemoteFetchResult, RemoteTrackingBase } from '../runtime/pebble-runtime'
...
async function spawnLocalStartupAndSetupTerminals(args: {
  runtime: PebbleRuntimeService | undefined
...
async function getOrStartSshWorktreeCreateFetch(
  ...
  const promise = enqueueSshWorktreeCreateFetch(queueKey, async () => {
    ...
    await fetch()
    // Why: SSH creation has no PebbleRuntimeService instance to share, but
...
async function prepareWorktreePushTargetSsh(
  ...
  if (target.remoteUrl) {
    ...
    if (existingRemote) {
      ...
      // Why: if a later PR worktree reuses an Pebble-created fork remote, it
...
async function readRemoteEffectiveHooks(
  ...
  return getEffectiveHooksFromConfig(repo, await readRemotePebbleYaml(fsProvider, hooksRootPath))
}

async function readRemotePebbleYaml(
  fsProvider: IFilesystemProvider,
  hooksRootPath: string
): Promise<ReturnType<typeof parsePebbleYaml>> {
  try {
    const result = await fsProvider.readFile(joinWorktreeRelativePath(hooksRootPath, 'pebble.yaml'))
    return result.isBinary ? null : parsePebbleYaml(result.content)
  } catch {
    return null
  }
}
...
async function createRemoteSetupRunnerScript(
  ...
  const runnerRelativePath = useWindowsFormat ? 'pebble/setup-runner.cmd' : 'pebble/setup-runner.sh'
...
export async function createRemoteWorktree(
  ...
  if (fsProvider) {
    await timing.time('prepare_setup', async () => {
      const yamlHooks = await readRemotePebbleYaml(fsProvider, created.path)
...
export async function createLocalWorktree(
  ...
  // Why: the worktree's own `pebble.yaml` (at the tip of the base branch) is
