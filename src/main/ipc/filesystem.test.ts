...
describe('registerFilesystemHandlers', () => {
  ...
  it('allows readDir when a registered worktree resolves to a macOS canonical alias', async () => {
    const aliasWorktreePath = path.resolve('/var/folders/pebble/worktrees/feature')
    const canonicalWorktreePath = path.resolve('/private/var/folders/pebble/worktrees/feature')
  ...
  it('prepares the Pebble-managed Codex home for the default system selection', async () => {
    const context = {
      branch: 'feature/ai',
      stagedSummary: 'M\tREADME.md',
      stagedPatch: '+hello'
    ...
    registerFilesystemHandlers(store as never, {
      prepareForCodexLaunch: () => '/pebble-managed/codex-home'
    })
    ...
    expect(generateCommitMessageFromContextMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        kind: 'local',
        cwd: WORKTREE_FEATURE_PATH,
        env: expect.objectContaining({ CODEX_HOME: '/pebble-managed/codex-home' })
      })
    )
  })
