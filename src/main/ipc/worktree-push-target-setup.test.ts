...
const FORK_SSH = 'git@github.com:contributor/pebble.git'
const FORK_HTTPS = 'https://github.com/contributor/pebble.git'
...
describe('prepareWorktreePushTargetWithExec', () => {
  it('adds a new fork remote and fetches its head when none matches', async () => {
    const exec = makeRepoExec({ origin: 'git@github.com:nebutra/pebble.git' })
    ...
    expect(callsMatching(exec, ['remote', 'add'])).toEqual([
      ['remote', 'add', 'pr-contributor-pebble', FORK_SSH]
    ])
    expect(callsMatching(exec, ['fetch'])).toEqual([
      [
        'fetch',
        'pr-contributor-pebble',
        '+refs/heads/contributor/fix:refs/remotes/pr-contributor-pebble/contributor/fix'
      ]
    ])
  ...
  it('reuses an existing remote pointing at the same fork (SSH vs HTTPS) without adding', async () => {
    const exec = makeRepoExec({
      origin: 'git@github.com:nebutra/pebble.git',
      'pr-contributor-pebble': FORK_HTTPS
    })
  ...
  it('disambiguates with a numeric suffix when the preferred remote name is taken by a different URL', async () => {
    const exec = makeRepoExec({ 'pr-contributor-pebble': 'git@github.com:someone-else/pebble.git' })

    const result = await prepareWorktreePushTargetWithExec(exec, REPO, forkTarget(), () => false)

    expect(callsMatching(exec, ['remote', 'add'])).toEqual([
      ['remote', 'add', 'pr-contributor-pebble-2', FORK_SSH]
    ])
    expect(result.remoteName).toBe('pr-contributor-pebble-2')
    expect(result.remoteCreated).toBe(true)
  })
...
describe('configureCreatedWorktreePushTargetWithExec', () => {
  it('points the new branch upstream at the fork remote', async () => {
    ...
    expect(exec).toHaveBeenCalledWith(
      ['branch', '--set-upstream-to', 'pr-contributor-pebble/contributor/fix', 'local-branch'],
      '/wt/path'
    )
