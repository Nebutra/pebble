...
describe('diagnostics IPC handlers', () => {
  beforeEach(() => {
    ...
    delete (globalThis as { PEBBLE_BUILD_IDENTITY?: unknown }).PEBBLE_BUILD_IDENTITY
    delete (globalThis as { PEBBLE_DIAGNOSTICS_TOKEN_URL?: unknown }).PEBBLE_DIAGNOSTICS_TOKEN_URL
    process.env.PEBBLE_DIAGNOSTICS_TOKEN_URL = 'https://diagnostics.example.com/diagnostics/token'
  ...
  it('pins official builds to the compile-time diagnostics endpoint', async () => {
    ...
    const globalOverrides = globalThis as {
      PEBBLE_BUILD_IDENTITY?: 'stable'
      PEBBLE_DIAGNOSTICS_TOKEN_URL?: string
    }
    globalOverrides.PEBBLE_BUILD_IDENTITY = 'stable'
    globalOverrides.PEBBLE_DIAGNOSTICS_TOKEN_URL = 'https://official.example.com/diagnostics/token'
    process.env.PEBBLE_DIAGNOSTICS_TOKEN_URL = 'https://attacker.example.com/diagnostics/token'
    collectDiagnosticBundleMock.mockReturnValue(bundle)
    readFileSyncMock.mockReturnValue(bundle.payload)
    const collect = handlers.get('diagnostics:collectBundle')!
    const openPreview = handlers.get('diagnostics:openBundlePreview')!
