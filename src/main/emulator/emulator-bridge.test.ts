...
import { RuntimeEmulatorCommands } from '../runtime/pebble-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous Pebble-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous Pebble-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills Pebble-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
