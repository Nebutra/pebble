...
import { checkPebbleStarred } from '../github/client'
...
export class StarNagService {
  ...
  registerIpcHandlers(): void {
    ...
    ipcMain.handle('star-nag:starPebble', () => this.starPebbleFromNag())
  ...
  private async maybeShow(
    ...
    try {
      // Why: checkPebbleStarred lets us skip users who already starred outside
      ...
      // button.
      const starred = await checkPebbleStarred()
  ...
  private async starPebbleFromNag(): Promise<boolean> {
    const session = this.promptSession
    if (!session) {
      return false
    }
    ...
    const attempt = this.runStarPebbleAttempt(session)
  ...
  private async runStarPebbleAttempt(session: StarNagPromptSession): Promise<boolean> {
