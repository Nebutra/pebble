...
const { getPathMock } = vi.hoisted(() => ({
  getPathMock: vi.fn(() => '/tmp/pebble-test-userdata')
}))
...
describe('CodexUsageStore', () => {
  ...
  beforeEach(() => {
    tempUserData = mkdtempSync(join(tmpdir(), 'pebble-codex-usage-store-'))
  ...
  it('persists a successful refresh with one compact disk write', async () => {
    ...
    const persistedJson = readFileSync(join(tempUserData, 'pebble-codex-usage.json'), 'utf-8')
  ...
  it('reports no data for Pebble scope when only non-Pebble Codex usage exists', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const summary = await store.getSummary('pebble', '30d')
  ...
  it('prices current Codex models with current model rates', async () => {
    ...
    const summary = await store.getSummary('pebble', '30d')
    const breakdown = await store.getBreakdown('pebble', '30d', 'model')
  ...
  it('uses only Pebble-scoped models when projecting mixed-scope sessions', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const breakdown = await store.getBreakdown('pebble', '30d', 'model')
    const recentSessions = await store.getRecentSessions('pebble', '30d', 10)
