...
function doctorRecipe(repoPath: string, recipeId: string): DoctorResult {
  ...
  if (!existsSync(yamlPath)) {
    return {
      ...
      checks: [
        {
          id: 'pebble_yaml.exists',
          status: 'fail',
          message: `No pebble.yaml found at ${yamlPath}`,
          remediation: 'Add environmentRecipes to the repo pebble.yaml.'
        }
      ]
    }
  }

  const hooks = parsePebbleYaml(readTextFile(yamlPath))
  const parseCheck: EphemeralVmRecipeDoctorCheck = {
    id: 'pebble_yaml.parse',
    status: hooks ? 'pass' : 'fail',
    message: hooks ? 'pebble.yaml parsed successfully.' : 'pebble.yaml has no supported Pebble config.',
    ...(hooks ? {} : { remediation: 'Add an environmentRecipes entry to pebble.yaml.' })
  }
...
function loadRecipe(repoPath: string, recipeId: string): PebbleVmRecipe | null {
  const hooks = parsePebbleYaml(readTextFile(join(repoPath, 'pebble.yaml')))
