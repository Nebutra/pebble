...
export const SETUP_AGENT_SEQUENCE_STARTUP_COMMAND_ENV = 'PEBBLE_SEQUENCED_STARTUP_COMMAND'
...
function buildWindowsSetupCommand(setupCommand: string, markerPath: string, nonce: string): string {
  return wrapCmd([
    `set "PEBBLE_SETUP_MARKER=${escapeCmdSetValue(markerPath)}"`,
    `set "PEBBLE_SETUP_NONCE=${escapeCmdSetValue(nonce)}"`,
    'del /f /q "!PEBBLE_SETUP_MARKER!" "!PEBBLE_SETUP_MARKER!.tmp" 2>nul',
    `call ${setupCommand}`,
    'set "PEBBLE_SETUP_STATUS=!ERRORLEVEL!"',
    '> "!PEBBLE_SETUP_MARKER!.tmp" echo !PEBBLE_SETUP_NONCE!:!PEBBLE_SETUP_STATUS!',
    'move /y "!PEBBLE_SETUP_MARKER!.tmp" "!PEBBLE_SETUP_MARKER!" >nul',
    'exit /b !PEBBLE_SETUP_STATUS!'
  ])
}
...
function buildWindowsStartupCommand(
  ...
  const script = [
    '$marker = $env:PEBBLE_SETUP_MARKER',
    '$tmp = $marker + ".tmp"',
    '$nonce = $env:PEBBLE_SETUP_NONCE',
