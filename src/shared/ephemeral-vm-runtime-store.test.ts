...
describe('ephemeral VM runtime store', () => {
  ...
  function makeUserDataPath(): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'pebble-ephemeral-vm-store-'))
  ...
  it('updates lifecycle and cleanup state', () => {
    ...
    const failed = updateEphemeralVmRuntimeStatus(userDataPath, 'pebble-instance-1', {
    ...
    const recovered = updateEphemeralVmRuntimeStatus(userDataPath, 'pebble-instance-1', {
      status: 'cleaned',
