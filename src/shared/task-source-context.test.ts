...
describe('task source context', () => {
  ...
  it('keeps provider cache scopes separate by host and provider identity', () => {
    ...
    const differentRepo = getTaskSourceCacheScope({
      ...
      hostId: 'local',
      repoId: 'repo-1',
      providerIdentity: { provider: 'github', owner: 'other', repo: 'pebble' }
    })
  ...
  it('serializes provider identities for GitLab, Linear, and Jira cache scopes', () => {
    ...
    expect(
      getTaskSourceCacheScope({
        ...base,
        provider: 'gitlab',
        providerIdentity: { provider: 'gitlab', namespace: 'nebutra', project: 'pebble' }
      })
    ).toContain(encodeURIComponent('nebutra/pebble'))
    expect(
      getTaskSourceCacheScope({
        ...base,
        provider: 'linear',
