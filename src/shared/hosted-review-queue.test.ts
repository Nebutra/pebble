...
function baseSummary(overrides: Partial<HostedReviewQueueSummary> = {}): HostedReviewQueueSummary {
  return {
    identity: { provider: 'github', host: 'github.com', owner: 'acme', repo: 'pebble', number: 42 },
...
describe('classifyHostedReview', () => {
  it('classifies mine/requested/agent/teammate', () => {
    ...
    expect(
      classifyHostedReview(baseSummary({ author: { login: 'pebble-ci' } }), {
        agentAuthorLogins: ['pebble-ci']
      }).state
    ).toBe('agent')
