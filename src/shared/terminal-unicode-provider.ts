...
const PEBBLE_UNICODE_VERSION = 'pebble-11-zwj'
...
class PebbleUnicodeProvider implements IUnicodeVersionProvider {
  public readonly version = PEBBLE_UNICODE_VERSION
...
export function activatePebbleTerminalUnicodeProvider(terminal: XtermTerminalWithUnicodeCore): void {
  const { unicode } = terminal
  if (unicode.activeVersion === PEBBLE_UNICODE_VERSION) {
    return
  }
  ...
  if (!unicode.versions.includes(PEBBLE_UNICODE_VERSION)) {
    unicode.register(new PebbleUnicodeProvider(baseProvider))
  }
  unicode.activeVersion = PEBBLE_UNICODE_VERSION
}
