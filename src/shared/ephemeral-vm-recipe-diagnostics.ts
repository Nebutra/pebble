...
export function redactEphemeralVmRecipeDiagnosticText(text: string): string {
  ...
  return text
    .replace(/pebble:\/\/pair\?code=[A-Za-z0-9_-]+/g, 'pebble://pair?code=[redacted]')
    .replace(
...
export function redactEphemeralVmRecipeResultForDiagnostics(
  ...
  return {
    ...result,
    pairingCode: 'pebble://pair?code=[redacted]',
    ...(userData ? { userData } : {})
  }
}
...
function redactConnection(connection: EphemeralVmRecipeConnection): EphemeralVmRecipeConnection {
  if (connection.type === 'pebble-server') {
    return { ...connection, pairingCode: 'pebble://pair?code=[redacted]' }
  }
  return {
    ...connection,
    target: {
