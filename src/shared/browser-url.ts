...
export function normalizeBrowserNavigationUrl(
  ...
  if (trimmed.length === 0 || trimmed === 'about:blank' || trimmed === PEBBLE_BROWSER_BLANK_URL) {
    return PEBBLE_BROWSER_BLANK_URL
  }
...
export function normalizeExternalBrowserUrl(rawUrl: string): string | null {
  ...
  if (normalized === null || normalized === PEBBLE_BROWSER_BLANK_URL) {
    return null
  }
