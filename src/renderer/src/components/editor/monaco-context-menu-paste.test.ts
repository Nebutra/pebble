...
import { runPebbleContextMenuPaste, type PebbleContextMenuPasteDeps } from './monaco-context-menu-paste'
...
function makeDeps(
  overrides: Partial<PebbleContextMenuPasteDeps> & { editor?: ReturnType<typeof makeEditor> }
): PebbleContextMenuPasteDeps {
...
describe('runPebbleContextMenuPaste', () => {
  ...
  it('reads via IPC and dispatches a native paste for writable focused editors', async () => {
    ...
    const outcome = runPebbleContextMenuPaste(deps)
  ...
  it('applies in-app copy metadata (empty-selection + multicursor) when enabled', async () => {
    ...
    await runPebbleContextMenuPaste(deps)
    expect(handle.trigger).toHaveBeenCalledWith('keyboard', 'paste', {
      text: 'line text',
      pasteOnNewLine: true,
      multicursorText: ['a', 'b'],
  ...
  it('does nothing on an empty clipboard', async () => {
    ...
    await expect(runPebbleContextMenuPaste(deps)).resolves.toEqual({
      status: 'noop',
      reason: 'empty'
    })
  ...
  it('routes oversized pastes through the chunked inserter instead of a native trigger', async () => {
    ...
    const outcome = await runPebbleContextMenuPaste(deps)
