...
export function connectPanePty(
  ...
  const scheduleReattachIdleAgentCursorReset = (): void => {
    ...
    reattachIdleAgentCursorResetTimer = setTimeout(() => {
      ...
      // fire because the agent was already idle before Pebble restarted.
  ...
  const dropCommandFinishedStatusIfSameTurn = (
    ...
    const state = useAppStore.getState()
    if (!entry) {
      // Why: an Pebble-started agent can exit before its first hook status. The
  ...
  const onBell = (): void => {
    ...
    // standalone BEL some time after daemon snapshot reattach, even when Pebble
  ...
  // Why: inject PEBBLE_PANE_KEY so global Claude/Codex hooks can attribute their
  // callbacks to the correct Pebble pane without resolving worktrees from cwd.
  // The key matches the `${tabId}:${leafId}` composite used for cacheTimerByKey
  // and agentStatusByPaneKey. Treat it as opaque outside Pebble.
  const state = useAppStore.getState()
  ...
  const workspaceEnv: Record<string, string> = {
    PEBBLE_WORKSPACE_ID: deps.worktreeId
  }
  if (folderWorkspace) {
    workspaceEnv.PEBBLE_PROJECT_GROUP_ID = folderWorkspace.projectGroupId
    workspaceEnv.PEBBLE_WORKSPACE_ROOT = folderWorkspace.folderPath
  }
  const paneIdentityEnv = {
    ...workspaceEnv,
    PEBBLE_PANE_KEY: cacheKey,
    PEBBLE_TAB_ID: deps.tabId,
    PEBBLE_WORKTREE_ID: deps.worktreeId,
  ...
  const transportOptions = {
    ...
    onAgentExited,
    // Why: local IPC terminals are now model-owned in main: PebbleRuntimeService
  ...
  const runDeferredConnect = (): void => {
    ...
    const buildColdRestoreAgentResumeStartup = (): ColdRestoreAgentResumeStartup | null => {
        ...
        if (fallbackLaunchConfig?.agentCommand?.trim()) {
          ...
          return {
            ...
            env: {
              ...
              PEBBLE_AGENT_LAUNCH_TOKEN: freshLaunchToken
            },
            launchConfig: {
      ...
      return {
        ...
        env: {
          ...startupPlan.env,
          PEBBLE_AGENT_LAUNCH_TOKEN: coldRestoreLaunchToken
        },
    ...
    const mergeStartupEnvWithPaneIdentity = (
      ...
      env
        ? {
            ...env,
            ...paneIdentityEnv,
            ...(env.PEBBLE_AGENT_LAUNCH_TOKEN
              ? {
                  PEBBLE_AGENT_LAUNCH_TOKEN: env.PEBBLE_AGENT_LAUNCH_TOKEN
                }
              : {})
          }
        : undefined
    ...
    const schedulePendingStartupCommandDelivery = (): void => {
      ...
      if (!sshStartupShellReady) {
        if (sshShellReadyFallbackTimer === null) {
          // Why: some SSH shells cannot emit Pebble's ready marker. Prefer the
