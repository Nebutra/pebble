...
export type EnvBlockedReason = 'do_not_track' | 'pebble_disabled' | 'ci'
...
export function isEnvBlocked(consent: TelemetryConsentState | null): consent is {
  ...
  return (
    ...
    (consent.reason === 'do_not_track' ||
      consent.reason === 'pebble_disabled' ||
      consent.reason === 'ci')
  )
}
...
export function envVarNameForReason(reason: EnvBlockedReason): string {
  ...
  if (reason === 'pebble_disabled') {
    return 'PEBBLE_TELEMETRY_DISABLED'
  }
  return 'CI'
}
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'Help us figure out what to build next. Pebble sends anonymous counts of which features you use and where things break.'
            )}{' '}
            <button
              type="button"
