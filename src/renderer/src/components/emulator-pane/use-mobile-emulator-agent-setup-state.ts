...
import { PEBBLE_CLI_SKILL_NAME } from '@/lib/agent-feature-install-commands'
import {
...
function getCliActionLabel(status: CliInstallStatus | null, busy: boolean): string {
  ...
  if (isPebbleCliAvailableOnPath(status)) {
    return translate(
...
export function useMobileEmulatorAgentSetupState(enabled = true): {
  ...
  } = useInstalledAgentSkill(PEBBLE_CLI_SKILL_NAME, {
    enabled,
  ...
  const cliEnabled = isPebbleCliAvailableOnPath(cliInstallStatus)
  ...
  const recheckSetup = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const cliReady = isPebbleCliAvailableOnPath(cliStatus)
      if (!mountedRef.current) {
        return
      }
      ...
      if (skillInstalled) {
        toast.message(
          translate(
            ...
            'Skill is installed. Enable the Pebble CLI to finish setup.'
          )
        )
        return
      }
      if (cliReady) {
        toast.message(
          translate(
            ...
            'Pebble CLI is ready. Install the skill to finish setup.'
          )
        )
        return
      }
  ...
  const handleEnableCli = useCallback(async (): Promise<void> => {
    setCliBusy(true)
    try {
      const next = await ensurePebbleCliAvailableForAgentSkillTerminal({
        onStatusChange: setCliInstallStatus
      })
      if (mountedRef.current && isPebbleCliAvailableOnPath(next)) {
        toast.success(
          translate(
            'auto.components.emulator.pane.use.mobile.emulator.agent.setup.state.2b519eed94',
            'Registered the Pebble CLI in PATH.'
          )
        )
      }
    } finally {
