...
describe('task source context summary', () => {
  it('shows provider, host, and provider identity for a single repo-backed source', () => {
    ...
    expect(summary.label).toBe('GitHub · devbox · nebutra/pebble')
    expect(summary.title).toBe('GitHub · Host: devbox · Source: nebutra/pebble')
  })
  ...
  it('shows repo-backed provider account labels when accounts can differ by host', () => {
    ...
    expect(summary.title).toBe(
      `GitHub · Host: ${LOCAL_HOST_LABEL}, builder · Account: personal-gh, work-gh · Source: nebutra/pebble · 2 selected projects`
    )
  })
  ...
  it('shows disconnected source-host availability for a single SSH repo source', () => {
    ...
    expect(summary.label).toBe('GitHub · devbox · disconnected · nebutra/pebble')
    expect(summary.title).toBe(
      'GitHub · Host: devbox · Availability: devbox disconnected · Source: nebutra/pebble'
    )
  })
  ...
  it('shows blocked remote-server source-host availability', () => {
    ...
    expect(summary.label).toBe('GitHub · old-server · server update needed · nebutra/pebble')
    expect(summary.title).toBe(
      'GitHub · Host: old-server · Availability: old-server server update needed · Source: nebutra/pebble'
    )
  })
  ...
  it('shows remote-server task-source capability checks', () => {
    ...
    expect(summary.label).toBe('GitHub · old-server · checking server capabilities · nebutra/pebble')
    expect(summary.title).toBe(
      'GitHub · Host: old-server · Availability: old-server checking server capabilities · Source: nebutra/pebble'
    )
  })
  ...
  it('uses saved remote server labels in repo-backed source summaries and notices', () => {
    ...
    expect(
      ...
      label: 'GitHub · dev box · server update needed · nebutra/pebble',
      title:
        'GitHub · Host: dev box · Availability: dev box server update needed · Source: nebutra/pebble'
    })
  ...
  it('shows account-backed Linear and Jira sources', () => {
    expect(
      getTaskSourceContextSummary({
        ...
        accountHostId: 'local',
        linearWorkspaceName: 'Nebutra'
      }).label
    ).toBe(`Linear · ${LOCAL_HOST_LABEL} · Nebutra`)
    ...
    expect(
      getTaskSourceContextSummary({
        ...
        jiraSiteName: 'Nebutra Jira'
      }).label
    ).toBe('Jira · server · Nebutra Jira')
  })
  ...
  it('shows account-backed source host availability', () => {
    ...
    expect(summary.label).toBe('Linear · old-server · server update needed · Nebutra')
    expect(summary.title).toBe(
      'Linear source · Host: old-server · Availability: old-server server update needed · Account: Nebutra'
    )
  })
  ...
  it('shows provider-specific source availability reasons', () => {
    expect(
      ...
      label: 'GitHub · devbox · provider auth needed · nebutra/pebble',
      title:
        'GitHub · Host: devbox · Availability: devbox provider auth needed · Source: nebutra/pebble'
    })
