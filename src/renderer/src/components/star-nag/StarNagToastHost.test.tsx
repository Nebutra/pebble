...
type StarNagApi = {
  ...
  starPebble: ReturnType<typeof vi.fn>
}
...
describe('StarNagToastHost', () => {
  ...
  beforeEach(() => {
    ...
    starNag = {
      ...
      starPebble: vi.fn().mockResolvedValue(true)
    }
    shell = {
  ...
  it('renders exact onboarding toast copy and confirms only after direct star succeeds', async () => {
    ...
    expect(toastContainer.textContent).toContain(
      'If you’re enjoying Pebble so far, a GitHub star helps other developers discover it.'
    )
    ...
    expect(starNag.starPebble).toHaveBeenCalledTimes(1)
  ...
  it('opens GitHub fallback without calling direct star success path', async () => {
    ...
    expect(shell.openUrl).toHaveBeenCalledWith('https://github.com/nebutra/pebble')
    expect(starNag.openWeb).toHaveBeenCalledTimes(1)
    expect(starNag.starPebble).not.toHaveBeenCalled()
  ...
  it('switches to the explicit GitHub fallback when direct star fails', async () => {
    starNag.starPebble.mockResolvedValueOnce(false)
    ;({ root, container } = renderHost())
  ...
  it('does not allow unresolved close while the primary action is busy', async () => {
    ...
    starNag.starPebble.mockReturnValueOnce(pendingStar.promise)
    ;({ root, container } = renderHost())
