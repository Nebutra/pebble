...
function getAutomationSetupSource(
  ...
  yamlHooks: PebbleHooks | null | undefined
...
export function getVisibleAutomationSetupDecision(args: {
  ...
  if (args.createTarget !== 'pebble' || args.workspaceMode !== 'new_per_run') {
    return undefined
  }
...
export function resolveAutomationSetupDecisionForSave(args: {
  ...
  if (args.yamlHooks === undefined) {
    // Why: automations cannot pause later for an pebble.yaml trust prompt; when
