...
import {
  PEBBLE_TERMINAL_COMMAND_FINISHED_EVENT,
...
export function useGitStatusPushSignalRefresh({
  ...
  useEffect(() => {
    ...
    window.addEventListener(PEBBLE_TERMINAL_COMMAND_FINISHED_EVENT, handleCommandFinished)
    return () => {
      window.removeEventListener(PEBBLE_TERMINAL_COMMAND_FINISHED_EVENT, handleCommandFinished)
    }
