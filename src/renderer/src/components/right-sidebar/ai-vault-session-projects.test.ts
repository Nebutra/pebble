...
const baseSession: AiVaultSession = {
  ...
  totalTokens: 1200,
  previewMessages: [],
  resumeCommand: "cd '/Users/ada/pebble' && claude --resume 'session-1'"
}
...
describe('buildAiVaultProjectContext', () => {
  it('uses durable worktree project ids before repo fallback', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Legacy Repo', path: '/Users/ada/pebble' })
    const project = makeProject({ id: 'project-1', displayName: 'Canonical Pebble' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
      projectId: project.id,
      path: '/Users/ada/pebble'
    })
    ...
    expect(context.sessionProjectById.get(baseSession.id)).toMatchObject({
      ...
      label: 'Canonical Pebble'
    })
  })

  it('normalizes compatibility project ids to repo keys', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Pebble', path: '/Users/ada/pebble' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
    ...
    const context = buildAiVaultProjectContext({
      ...
      worktrees: [worktree],
      projectHostSetupProjection: makeProjection({
        projects: [makeProject({ id: 'repo:repo-1', displayName: 'Compatibility Pebble' })],
    ...
    expect(context.projectLabelByKey.get('repo:repo-1')).toBe('Pebble')
  })
  ...
  it('inherits setup project ids for legacy worktrees without project metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Pebble Repo', path: '/repo/pebble' })
    const worktree = makeWorktree({
      id: 'wt-legacy',
      repoId: repo.id,
      path: '/repo/pebble'
    })
    const session = makeSession({ id: 'claude:legacy-worktree', cwd: '/repo/pebble/src' })

    const context = buildAiVaultProjectContext({
      repos: [repo],
      worktrees: [worktree],
      projectHostSetupProjection: makeProjection({
        projects: [makeProject({ id: 'github:nebutra/pebble', displayName: 'Canonical Pebble' })],
        setups: [
          makeSetup({
            repoId: repo.id,
    ...
    expect(context.activeProjectKey).toBe('project:github:nebutra/pebble')
    expect(context.sessionProjectById.get(session.id)).toMatchObject({
      kind: 'repo',
  ...
  it('uses active worktree setup project ids when active repo is unavailable', () => {
    ...
    const session = makeSession({ id: 'claude:restored', cwd: '/repo/pebble/src' })
    ...
    expect(context.activeProjectKey).toBe('project:github:nebutra/pebble')
    expect(context.sessionProjectById.get(session.id)?.key).toBe('project:github:nebutra/pebble')
  })

  it('inherits setup host ids for legacy worktrees without host metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Runtime Repo', path: '/runtime/pebble' })
    const worktree = makeWorktree({
      id: 'wt-runtime',
      repoId: repo.id,
      path: '/runtime/pebble'
  ...
  it('uses the session host when matching overlapping local and SSH project paths', () => {
    const localRepo = makeRepo({ id: 'local', displayName: 'Local', path: '/srv/pebble' })
  ...
  it('falls back to folder when a legacy hostless session matches multiple host buckets', () => {
    const localRepo = makeRepo({ id: 'local', displayName: 'Local', path: '/srv/pebble' })
    const runtimeRepo = makeRepo({ id: 'runtime', displayName: 'Runtime', path: '/srv/pebble' })
    const session = makeSession({
  ...
  it('maps null cwd sessions to unknown', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Pebble', path: '/repo' })
