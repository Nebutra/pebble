...
describe('buildSourceControlManualReviewUrl', () => {
  it('builds a GitHub compare URL for the current remote branch', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'git@github.com:nebutra/pebble.git',
        upstreamName: 'origin/native-chat-does-not-auto-open'
      })
    ).toBe(
      'https://github.com/nebutra/pebble/compare/main...native-chat-does-not-auto-open?expand=1'
    )
  })
  ...
  it('qualifies GitHub fork heads when the push target remote differs from the base repo', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteUrl: 'https://github.com/nebutra/pebble.git',
        pushTarget: {
          remoteName: 'fork',
          branchName: 'feature/fork-head',
    ...
    ).toBe('https://github.com/nebutra/pebble/compare/main...contributor:feature/fork-head?expand=1')
  })
  ...
  it('keeps slashes literal in a GitHub compare URL for a slash-containing branch name', () => {
    expect(
      ...
      'https://github.com/nebutra/pebble/compare/main...slashdevcorpse/identifying-pwsh.exe-error?expand=1'
    )
  })
  ...
  it('builds a GitLab merge request URL for a self-hosted GitLab remote', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'git@gitlab.company.test:group/sub/pebble.git',
        provider: 'gitlab',
        upstreamName: 'origin/feature/gitlab'
      })
    ).toBe(
      'https://gitlab.company.test/group/sub/pebble/-/merge_requests/new?merge_request%5Bsource_branch%5D=feature%2Fgitlab&merge_request%5Btarget_branch%5D=release%2Fnext'
    )
  })
  ...
  it('builds a Gitea compare URL when the provider is known', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'ssh://git@gitea.company.test/team/pebble.git',
        provider: 'gitea',
        upstreamName: 'origin/feature/gitea'
      })
    ).toBe('https://gitea.company.test/team/pebble/compare/main...feature/gitea')
  })
  ...
  it('suppresses the link when the branch tracks a fork remote with no resolvable push URL', () => {
    ...
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'https://github.com/nebutra/pebble.git',
        upstreamName: 'pr-devajmeireles-pebble/quick-commands'
      })
    ).toBeNull()
  })
  ...
  it('uses the pushed upstream branch name when the local branch tracks a differently named branch on the base remote', () => {
    ...
    ).toBe('https://github.com/nebutra/pebble/compare/main...feature/pushed-name?expand=1')
  })

  it('still qualifies the fork head when Pebble resolved the fork push URL', () => {
    expect(
      buildSourceControlManualReviewUrl({
        baseRef: 'refs/remotes/origin/main',
        branchName: 'quick-commands',
        ...
        upstreamName: 'pr-devajmeireles-pebble/quick-commands',
        pushTarget: {
          remoteName: 'pr-devajmeireles-pebble',
          branchName: 'quick-commands',
          remoteUrl: 'git@github.com:devajmeireles/pebble.git'
        }
      })
    ).toBe('https://github.com/nebutra/pebble/compare/main...devajmeireles:quick-commands?expand=1')
  })
  ...
  it('does not guess a provider for unknown hosts without a provider hint', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'git@git.company.test:team/pebble.git',
