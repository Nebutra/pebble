...
type LinkRoutingPreferenceDialogRequest = {
  ...
  resolve: (openInPebble: boolean) => void
}
...
const PREVIEW_STORAGE_KEY = 'pebble.previewLinkRoutingPreferenceDialog'
...
export function LinkRoutingPreferenceDialogProvider({
  ...
  useEffect(() => {
    ...
    void requestPreference({
      openLinksInAppDefault: previewDefault === 'pebble',
      preview: true,
      url: 'https://github.com/nebutra/pebble/pull/1234'
    })
  }, [requestPreference])

  const settleActiveRequest = useCallback((openInPebble: boolean) => {
    const request = activeRequestRef.current
    if (!request) {
      return
    }
    request.resolve(openInPebble)
        ...
        >
                ...
                <DialogTitle className="text-xl leading-tight">
                  {openLinksInAppDefault
                    ? translate(
                        ...
                        "Keep terminal links in Pebble's browser?"
                      )
                    : translate(
                        'auto.components.link.routing.preference.dialog.title',
                        "Open terminal links in Pebble's browser?"
                      )}
                </DialogTitle>
                <DialogDescription className="text-sm leading-relaxed">
                  {openLinksInAppDefault
                    ...
                    : translate(
                        ...
                        "Use Pebble's browser for terminal links, or keep your system browser."
                      )}
                </DialogDescription>
              </div>
            </DialogHeader>
          ...
          <div className="space-y-3 px-6">
            ...
            <div className="flex gap-2 rounded-lg border border-border bg-muted/20 p-3 text-xs leading-relaxed text-muted-foreground">
              ...
              <div className="space-y-1">
                <p>
                  {translate(
                    'auto.components.link.routing.preference.dialog.pebble.note',
                    'Pebble can use imported cookies for logged-in sites.'
                  )}
                </p>
                <p>
                  {translate(
          ...
          <DialogFooter className="border-t border-border bg-muted/20 px-6 py-4 sm:justify-between">
            ...
            <Button autoFocus onClick={() => settleActiveRequest(true)}>
              {openLinksInAppDefault
                ? translate(
                    'auto.components.link.routing.preference.dialog.keep.pebble.button',
                    'Keep Pebble'
                  )
                : translate(
                    'auto.components.link.routing.preference.dialog.pebble.button',
                    'Open in Pebble'
                  )}
            </Button>
          </DialogFooter>
