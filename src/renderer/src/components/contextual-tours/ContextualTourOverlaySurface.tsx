...
if (typeof window !== 'undefined') {
  const guardedWindow = window as Window & {
    __pebbleContextualTourGlobalKeyGuardInstalled?: boolean
  }
  if (!guardedWindow.__pebbleContextualTourGlobalKeyGuardInstalled) {
    guardedWindow.__pebbleContextualTourGlobalKeyGuardInstalled = true
    window.addEventListener('keydown', handleContextualTourGlobalKeyDown, true)
  }
}

const PANEL_BASE_CLASSES =
  'pebble-contextual-tour-panel rounded-lg border border-border text-popover-foreground backdrop-blur-[2px]'
  ...
  return (
    ...
    >
      {showTargetRings ? (
        <div
          aria-hidden="true"
          className="pebble-contextual-tour-target-rings fixed z-[75]"
