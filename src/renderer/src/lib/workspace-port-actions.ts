...
export function shouldOpenWorkspacePortInPebbleBrowser(
...
export function resolvePortOpenInPebbleBrowser({
  settings,
  event,
  isMac
}: {
  ...
  return shouldOpenWorkspacePortInPebbleBrowser(settings)
}
...
export async function openWorkspacePortInBrowser(args: {
  ...
  if (args.openInPebbleBrowser === false && args.runtimeTarget.kind === 'local') {
    try {
