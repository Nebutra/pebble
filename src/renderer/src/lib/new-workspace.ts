...
import type { FolderWorkspaceLinkedTask, PebbleHooks, TaskViewPresetId } from '../../../shared/types'
...
// Why: when a repo has no `pebble.yaml` issueCommand and no per-user override,
...
function getDefaultTabCommandPreview(yamlHooks: PebbleHooks | null): string {
 ...
 * their `pebble.yaml` / `.pebble/issue-command` yet.
 */
