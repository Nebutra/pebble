...
function makeRepo(overrides: Partial<Repo> = {}): Repo {
  return {
    id: 'repo-1',
    path: '/Users/test/src/pebble',
    displayName: 'nebutra/pebble',
    badgeColor: '#22c55e',
    addedAt: 0,
    ...overrides
  }
...
describe('repo-search', () => {
  ...
  it('matches display names case-insensitively', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'nebutra/pebble', path: '/repos/pebble' }),
      makeRepo({ id: '2', displayName: 'nebutra/noqa', path: '/repos/noqa' })
    ]

    expect(searchRepos(repos, 'PEBBLE').map((repo) => repo.id)).toEqual(['1'])
  })

  it('falls back to matching repo paths', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'frontend', path: '/src/team-a/pebble' }),
  ...
  it('keeps display-name matches ahead of path-only matches', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'misc', path: '/src/pebble-tools/misc' }),
      makeRepo({ id: '2', displayName: 'pebble', path: '/src/team-a/project' })
    ]

    expect(searchRepos(repos, 'pebble').map((repo) => repo.id)).toEqual(['2', '1'])
  })
