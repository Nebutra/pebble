...
describe('web pairing input', () => {
  ...
  it('parses query-form pairing URLs', () => {
    expect(parseWebPairingInput(`pebble://pair?code=${encodeOffer()}`)).toEqual(offer)
  })

  it('still parses legacy hash-form pairing URLs', () => {
    expect(parseWebPairingInput(`pebble://pair#${encodeOffer()}`)).toEqual(offer)
  })

  it('preserves optional device scope metadata', () => {
    expect(parseWebPairingInput(`pebble://pair?code=${encodeOffer({ scope: 'mobile' })}`)).toEqual({
      ...offer,
      scope: 'mobile'
    })
  })
  ...
  it('treats invalid device scope metadata as unknown', () => {
    expect(parseWebPairingInput(`pebble://pair?code=${encodeOffer({ scope: 'admin' })}`)).toEqual(
      offer
    )
  })
  ...
  it('auto-saves scoped runtime offers during web startup', () => {
    const input = `pebble://pair?code=${encodeOffer({ scope: 'runtime' })}`
    expect(
  ...
  it('shows the connect screen for mobile-scope and legacy unknown-scope offers', () => {
    const mobileInput = `pebble://pair?code=${encodeOffer({ scope: 'mobile' })}`
    const legacyInput = `pebble://pair?code=${encodeOffer()}`
