...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      const zshRoot = join(homeDir, '.pebble-relay', 'shell-ready', 'zsh')
      ...
      expect(readFileSync(join(zshRoot, '.zprofile'), 'utf8')).toContain(
        '_pebble_home="${PEBBLE_USER_ZDOTDIR:-${PEBBLE_ORIG_ZDOTDIR:-$HOME}}"'
      )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.args).toEqual(['-l'])
      expect(config.env.ZDOTDIR).toBe(zshRoot)
      expect(config.env.PEBBLE_SHELL_READY_MARKER).toBe('1')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.PEBBLE_SHELL_READY_MARKER).toBe('1')
      expect(bashRc).toContain('__pebble_append_prompt_command "__pebble_prompt_mark"')
      expect(bashRc).toContain('printf "\\033]777;pebble-shell-ready\\007"')
    }
  )
  ...
  // Pebble inherits ends in a ";"+whitespace separator. Prepend/append must not
