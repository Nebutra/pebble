...
export async function resolveCurrentWorktreeSelector(
  ...
  if (!enclosingWorktree) {
    throw new RuntimeClientError(
      'selector_not_found',
      `No Pebble-managed worktree contains the current directory: ${currentPath}`
    )
  }
...
// like `pebble terminal send --text "hello" --enter` Just Work.
...
export type EmulatorCliTarget = {
  worktree?: string
  device?: string
  emulator?: string // Pebble id from list
}
