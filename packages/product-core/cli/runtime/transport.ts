  ...
  return await new Promise((resolve, reject) => {
    ...
    if (!transport) {
      reject(
        new RuntimeClientError(
          'runtime_unavailable',
          'No compatible transport found in Pebble runtime metadata.'
        )
      )
      return
    }
    ...
    socket.once('error', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'Could not connect to the running Pebble app. Restart Pebble and try again.'
        )
      })
    })
    ...
    socket.once('close', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'The Pebble runtime closed the connection before responding. Restart Pebble and try again.'
        )
      })
    })
    socket.on('data', (chunk) => {
      ...
      while (newlineIndex !== -1 && !settled) {
        ...
        try {
          ...
          finish({
            ...
            error: new RuntimeClientError(
              ...
              'The Pebble runtime returned an invalid response frame.'
            )
          })
          return
        }
        ...
        // the decode boundary so version skew between the CLI and the Pebble
        ...
        if (response.id !== requestId) {
          finish({
            ...
            error: new RuntimeClientError(
              'invalid_runtime_response',
              'The Pebble runtime returned a mismatched response id.'
            )
          })
          return
        }
        if (response._meta?.runtimeId && response._meta.runtimeId !== metadata.runtimeId) {
          finish({
            ...
            error: new RuntimeClientError(
              'runtime_unavailable',
              'The Pebble runtime changed while the request was in flight. Retry the command.'
            )
          })
          return
        }
