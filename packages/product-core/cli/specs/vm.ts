...
export const VM_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage:
      'pebble vm recipe doctor <recipe-id> [--repo-path <path>] [--provision|--connect] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'recipe-id', 'repo-path', 'provision', 'connect'],
    positionalArgs: ['recipe-id'],
    notes: [
      'Reads environmentRecipes from pebble.yaml in the repo path, validates the selected recipe, and reports agent-friendly checks.',
    ...
    examples: [
      'pebble vm recipe doctor cloud-sandbox',
      'pebble vm recipe doctor cloud-sandbox --repo-path /path/to/repo --json',
      'pebble vm recipe doctor cloud-sandbox --provision --json'
    ]
  }
]
