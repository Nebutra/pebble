// Self-contained relay-host protocol; the desktop channel contract lives in
// packages/product-core/shared/ssh-relay-protocol.ts.
// but has no Electron dependencies. Deployed standalone to remote hosts.

export const RELAY_VERSION = '0.1.0'
export const RELAY_SENTINEL = `PEBBLE-RELAY v${RELAY_VERSION} READY\n`
...
export type HandshakeMessage =
  | { type: 'pebble-relay-handshake'; version: string }
  | { type: 'pebble-relay-handshake-ok'; version: string }
  | { type: 'pebble-relay-handshake-mismatch'; expected: string; got: string }
...
export function parseHandshakeMessage(payload: Buffer): HandshakeMessage {
  ...
  if (
    t !== 'pebble-relay-handshake' &&
    t !== 'pebble-relay-handshake-ok' &&
    t !== 'pebble-relay-handshake-mismatch'
  ) {
