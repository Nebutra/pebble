...
} from '../shared/shell-ready-templates'

const RELAY_SHELL_READY_DIR = '.pebble-relay/shell-ready'
const POSIX_LOGIN_ARGS = ['-l']
const SHELL_READY_MARKER_ESCAPED = '\\033]777;pebble-shell-ready\\007'
...
function hasOverlayRestoreEnv(env: Record<string, string>): boolean {
  return Boolean(
    env.PEBBLE_OPENCODE_CONFIG_DIR ||
    env.PEBBLE_MIMOCODE_HOME ||
    env.PEBBLE_REMOTE_CLI_BIN_DIR ||
    env.PEBBLE_OMP_STATUS_EXTENSION
  )
}
...
function resolveOriginalZdotdir(env: Record<string, string>): string {
  return (
    ...
    normalizeOriginalZdotdirCandidate(env.PEBBLE_ORIG_ZDOTDIR) ||
    env.HOME ||
    process.env.HOME ||
    ''
  )
...
function ensureOverlayRestoreWrappers(root: string): void {
  ...
  const zshEnv = `# Pebble relay zsh overlay wrapper
export PEBBLE_ORIG_ZDOTDIR="\${PEBBLE_ORIG_ZDOTDIR:-$HOME}"
case "\${PEBBLE_ORIG_ZDOTDIR%/}" in
  */shell-ready/zsh) export PEBBLE_ORIG_ZDOTDIR="$HOME" ;;
esac
[[ -f "$PEBBLE_ORIG_ZDOTDIR/.zshenv" ]] && source "$PEBBLE_ORIG_ZDOTDIR/.zshenv"
export PEBBLE_USER_ZDOTDIR="\${ZDOTDIR:-\${PEBBLE_ORIG_ZDOTDIR:-$HOME}}"
case "\${PEBBLE_USER_ZDOTDIR%/}" in
  */shell-ready/zsh) export PEBBLE_USER_ZDOTDIR="$HOME" ;;
esac
export ZDOTDIR=${quotePosixSingle(zshDir)}
`
  const zshProfile = `# Pebble relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zprofile',
  homeExpression: '"${PEBBLE_USER_ZDOTDIR:-${PEBBLE_ORIG_ZDOTDIR:-$HOME}}"'
})}
`
  const zshRc = `# Pebble relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zshrc',
  homeExpression: '"${PEBBLE_USER_ZDOTDIR:-${PEBBLE_ORIG_ZDOTDIR:-$HOME}}"',
...
if [[ ! -o login ]]; then
  ...
  [[ -n "\${PEBBLE_REMOTE_CLI_BIN_DIR:-}" ]] && case ":$PATH:" in *:"\${PEBBLE_REMOTE_CLI_BIN_DIR}":*) ;; *) export PATH="\${PEBBLE_REMOTE_CLI_BIN_DIR}:$PATH" ;; esac
  ${getPosixOmpShellWrapper()}
fi
if [[ ! -o login ]]; then
${getZshFinalZdotdirRestoreBlock('"${PEBBLE_USER_ZDOTDIR:-${PEBBLE_ORIG_ZDOTDIR:-$HOME}}"')}
fi
`
  const zshLogin = `# Pebble relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zlogin',
  homeExpression: '"${PEBBLE_USER_ZDOTDIR:-${PEBBLE_ORIG_ZDOTDIR:-$HOME}}"',
  interactiveOnly: true
...
`
  const bashRc = `# Pebble relay bash overlay wrapper
...
__pebble_osc133_preexec() {
  ...
  case "$BASH_COMMAND" in
    *__pebble_osc133_precmd*|*__pebble_osc133_prompt_done*) return ;;
  esac
...
__pebble_normalize_prompt_command() {
  ...
  # a ";"/whitespace separator; trim it so Pebble's prepend/append never form ";;".
...
`
  ...
  for (const [path, content] of files) {
    ...
    // Why: relay wrapper files persist under ~/.pebble-relay across app
...
export function getRelayShellLaunchConfig(
  ...
  if (shellName === 'zsh') {
    return {
      ...
      env: {
        PEBBLE_ORIG_ZDOTDIR: resolveOriginalZdotdir(env),
        ZDOTDIR: join(root, 'zsh'),
        ...(emitReadyMarker ? { PEBBLE_SHELL_READY_MARKER: '1' } : {})
      }
    }
  }
  ...
  return {
    ...
    env: emitReadyMarker ? { PEBBLE_SHELL_READY_MARKER: '1' } : {}
  }
}
