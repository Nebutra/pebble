...
function getProjectId(
  repo: Pick<Repo, 'id' | 'upstream' | 'repoIcon' | 'gitRemoteIdentity' | 'logicalProjectId'>
): string {
  return repo.logicalProjectId?.trim() || getProjectIdentityKey(repo)
}
...
function createProjectFromRepo(repo: Repo, now: number): Project {
  ...
  return {
    ...
    ...(repo.localWindowsRuntimePreference
      ? { localWindowsRuntimePreference: repo.localWindowsRuntimePreference }
      : {}),
