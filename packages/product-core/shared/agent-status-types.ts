...
// These types define the normalized status that Pebble receives from Claude,
...
export type AgentStateHistoryEntry = {
   ...
   *  (for example Claude Code `is_interrupt`) or Pebble's guarded interrupt
...
export type AgentStatusEntry = {
   ...
   *  inferred by Pebble's guarded interrupt fallback.
   ...
   *  Used only for exact CLI resume; Pebble terminal ids are not agent-session ids. */
