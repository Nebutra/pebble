import type { GlobalSettings, TerminalColorOverrides, GhosttyImportPreview } from '../types'
...
export function mapGhosttyToPebble(
  ...
  const FIELD_PARSERS: Record<string, FieldParser> = {
    ...
    'focus-follows-mouse': (v) => {
      // Why: Ghostty's focus-follows-mouse is semantically identical to Pebble's
  ...
  for (const [key, rawValue] of Object.entries(parsed)) {
    ...
    // Why: Pebble's windowBackgroundBlur is a boolean; the numeric radius is lost.
