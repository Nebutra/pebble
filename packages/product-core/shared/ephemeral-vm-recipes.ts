...
const EphemeralVmRecipePebbleServerConnectionSchema = z
  .object({
    type: z.literal('pebble-server'),
...
export const EphemeralVmRecipeConnectionSchema = z.discriminatedUnion('type', [
  EphemeralVmRecipePebbleServerConnectionSchema,
...
export function parseEphemeralVmRecipeResult(stdout: string): EphemeralVmRecipeResultParseResult {
  ...
  if (connection.type === 'pebble-server' && !parsePairingCode(connection.pairingCode)) {
    return { ok: false, error: 'Recipe result pairingCode is not a valid Pebble pairing code.' }
  }
...
export function getEphemeralVmRecipeResultPairingCode(
  ...
  return connection.type === 'pebble-server' ? connection.pairingCode : null
}
