...
import { buildKnownPebbleWorkspaceLayouts, classifyWorktreeOwnership } from './worktree-ownership'
...
describe('repo-specific worktree ownership layouts', () => {
  it('resolves the same relative base path from each repo root', () => {
    ...
    expect(buildKnownPebbleWorkspaceLayouts(settings, repoA)[0]).toEqual({
      path: '/projects/a/worktrees',
      nestWorkspaces: true
    })
    expect(buildKnownPebbleWorkspaceLayouts(settings, repoB)[0]).toEqual({
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        knownPebbleLayouts: buildKnownPebbleWorkspaceLayouts(settings, repoA)
      })
    ).toBe('external')
    expect(
      classifyWorktreeOwnership({
        ...
        settings,
        worktree: makeWorktree('/projects/a/worktrees/repo/feature'),
        knownPebbleLayouts: buildKnownPebbleWorkspaceLayouts(settings, repoB)
      })
    ).toBe('external')
  })
  ...
  it('includes relative global layouts for SSH repos without applying absolute desktop paths', () => {
    ...
    expect(buildKnownPebbleWorkspaceLayouts(relativeSettings, repo)[0]).toEqual({
      path: '/remote/worktrees',
      nestWorkspaces: true
    })
    expect(
      buildKnownPebbleWorkspaceLayouts(absoluteSettings, repo).some(
