/* GitLab-specific shared types. Split out of `packages/product-core/shared/types.ts` so
...
// target_type narrows to the four shapes Pebble renders meaningfully —
...
// tab so users can see which job failed and where without leaving Pebble.
...
export type GitLabWorkItemDetails = {
   ...
   *  picker) — main-process doesn't know Pebble's Repo.id. */
