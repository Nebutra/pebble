...
import type {
  PebbleDefaultTabTemplate,
  PebbleHooks,
  PebbleVmRecipe,
  PebbleVmRecipeDiagnostic
} from './types'
...
export const PEBBLE_VM_RECIPE_ID_PATTERN = /^[a-z0-9][a-z0-9._-]{0,63}$/
export const PEBBLE_VM_RECIPE_ID_RULE =
  'Use 1-64 lowercase letters, numbers, dots, underscores, or hyphens, starting with a letter or number.'

function normalizeDefaultTabs(value: unknown): PebbleDefaultTabTemplate[] {
  ...
  return value
    ...
    .filter((entry): entry is PebbleDefaultTabTemplate => entry !== null)
}

type VmRecipeParseResult = {
  recipes: PebbleVmRecipe[]
  diagnostics: PebbleVmRecipeDiagnostic[]
}

function normalizeVmRecipes(value: unknown): VmRecipeParseResult {
  const diagnostics: PebbleVmRecipeDiagnostic[] = []
  if (!Array.isArray(value)) {
  ...
  const recipes = value
    .map((entry, index) => {
      ...
      if (!PEBBLE_VM_RECIPE_ID_PATTERN.test(id)) {
        diagnostics.push({
          index,
          field: 'id',
          message: `Invalid recipe id "${id}". ${PEBBLE_VM_RECIPE_ID_RULE}`
        })
        return null
      }
      if (seenIds.has(id)) {
    ...
    .filter((entry): entry is PebbleVmRecipe => entry !== null)
  return { recipes, diagnostics }
}

/**
 * Parse the supported project defaults from `pebble.yaml`.
 */
export function parsePebbleYaml(content: string): PebbleHooks | null {
  let root: unknown
  try {
    root = parse(content)
  } catch {
