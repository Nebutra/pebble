...
function resolveBaseCommand(args: {
  ...
  // Why: Codex status hooks live in Pebble's runtime CODEX_HOME; adding
...
function buildSleepingAgentLaunchConfig(args: {
  ...
  return {
    ...
    // durable resume snapshot is limited to Pebble-managed agent env inputs.
...
export function buildAgentResumeStartupPlan(args: {
  ...
  agentEnv?: Record<string, string> | null
  agentCommand?: string | null
  /** Why: see buildAgentStartupPlan — remote launches use the plain `pebble` shim. */
  isRemote?: boolean
}): AgentStartupPlan | null {
