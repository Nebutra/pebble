...
import { buildGuestOverlayScript } from './browser-grab-guest-script'
...
describe('buildGuestOverlayScript', () => {
  ...
  it('arm script contains shadow DOM setup', () => {
    ...
    expect(script).toContain('__pebble-grab-host')
    expect(script).toContain('__pebbleGrab')
  })
  ...
  it('teardown script cancels pending awaitClick', () => {
    ...
    expect(buildGuestOverlayScript('awaitClick')).toContain('__pebbleCancelled')
  })
