...
// the relay sends to Pebble. Consumed by `packages/product-core/relay/agent-hook-server.ts` (which
// produces it after the shared listener parses an HTTP POST) and by
// the desktop/runtime status owner (which ingests it at the local transport boundary).
//
// Lives in `shared/` because the relay deliberately has no Electron dependency
// (cf. `packages/product-core/relay/protocol.ts` header). `agent-hook-types.ts` is reserved for
// the renderer-bound IPC + installer contract; this module is the wire envelope
// between Pebble's main process and the remote relay.
//
// Per the design doc:
// - The relay normalizes; Pebble routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Pebble's `ingestRemote` re-runs the canonical payload normalizer at
//   the SSH trust boundary before caching or persisting, so relay skew or a
//   buggy remote process cannot poison desktop/runtime state.
// - The wire `connectionId` is **always `null`**: a `connectionId` is Pebble's
//   local handle on an `ssh2` connection, not a wire identity. Pebble stamps the
//   real value on receive from the authenticated connection identity.
// - The wire `version` and `env` fields are forwarded from the agent CLI's
//   POST body so Pebble's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Pebble can attribute the event back to the right CLI.
// The shared package owns this source catalog so desktop shells, Go runtime,
// and the relay cannot drift into transport-specific discriminators.
...
/** Wire envelope for a single hook event flowing relay → Pebble. */
export type AgentHookRelayEnvelope = {
  source: AgentHookSource
  paneKey: string
  /** Ephemeral Pebble launch identity stamped into the PTY env for this process. */
  launchToken?: string
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Pebble's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Pebble can
  ...
  /** True when the relay is replaying its cache after Pebble reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Pebble's warn-once
   ...
   *  Pebble validates it again at the SSH trust boundary. */
...
/** JSON-RPC request method Pebble issues after `--connect` reattach to ask the
...
/** JSON-RPC request method Pebble issues at session-ready to ship the
...
/** Feature-flag env var. Read once at process start by Pebble and the relay.
 *  Remote agent hooks ship as the default SSH behavior; set "0" to opt out. */
export const PEBBLE_FEATURE_REMOTE_AGENT_HOOKS_ENV = 'PEBBLE_FEATURE_REMOTE_AGENT_HOOKS' as const

export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[PEBBLE_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
