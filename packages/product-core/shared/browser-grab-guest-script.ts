...
// itself on teardown.
//
// Why a string builder rather than a bundled file: Pebble's browser guests have
...
// It stores state on window.__pebbleGrab so finalize/teardown can access it.
// ---------------------------------------------------------------------------
const ARM_SCRIPT = `(function() {
  'use strict';

  // Why: always tear down any pre-existing state before arming. A malicious
  // guest page could predefine window.__pebbleGrab with a fake extractPayload
  // function. By tearing down unconditionally we ensure our freshly installed
  // extraction logic is the only code that runs.
  if (window.__pebbleGrab) {
    try {
      if (typeof window.__pebbleGrab.cleanup === 'function') {
        window.__pebbleGrab.cleanup();
      }
    } catch(e) {}
    delete window.__pebbleGrab;
  }
...
const AWAIT_CLICK_SCRIPT = `new Promise(function(resolve, reject) {
  ...
  function onContext(e) {
    ...
    if (!payload) return;
    grab.freezeHighlight();
    resolve({ __pebbleContextMenu: true, payload: payload });
  }
  ...
  grab.cancelAwait = function() {
    ...
    resolve({ __pebbleCancelled: true });
  };
})`
