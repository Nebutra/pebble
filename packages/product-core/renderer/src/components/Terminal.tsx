...
import {
  PEBBLE_EDITOR_REQUEST_FILE_CLOSE_EVENT,
  PEBBLE_EDITOR_SAVE_AND_CLOSE_EVENT,
  PEBBLE_EDITOR_REQUEST_CMD_SAVE_EVENT,
...
function Terminal(): React.JSX.Element | null {
  ...
  const terminalShortcutPolicy = useAppStore(
    (s) => s.settings?.terminalShortcutPolicy ?? 'pebble-first'
  )
  ...
  // the same row as the "Pebble" title. The target element is created by App.tsx.
  ...
  // PEBBLE_EDITOR_REQUEST_FILE_CLOSE_EVENT) must not re-open the dialog over
  ...
  const windowCloseAfterDirtyRef = useRef<WindowCloseRequest | null>(null)
  const windowCloseAfterChildProcessRef = useRef<WindowCloseRequest | null>(null)

  const proceedToNativeWindowClose = useCallback((request: WindowCloseRequest) => {
        ...
        void Promise.all(localPtyIds.map((id) => window.api.pty.hasChildProcesses(id))).then(
          (results) => {
            if (results.some(Boolean)) {
              windowCloseAfterChildProcessRef.current = request
  ...
  const advanceEditorCloseQueue = useCallback(() => {
    ...
    if (pendingWindowClose) {
      ...
      proceedToNativeWindowClose(pendingWindowClose)
    }
  }, [
  ...
  const queueEditorCloseRequests = useCallback(
    (fileIds: string[], pendingWindowClose?: WindowCloseRequest) => {
      if (pendingWindowClose) {
  ...
  const handleSaveDialogSave = useCallback(async () => {
    ...
    window.dispatchEvent(
      new CustomEvent(PEBBLE_EDITOR_SAVE_AND_CLOSE_EVENT, { detail: { fileId } })
    )
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent): void => {
      ...
      const notifyTerminalCapture = (actionId: KeybindingActionId): void => {
        if (context !== 'terminal' || terminalShortcutPolicy !== 'pebble-first') {
          return
        }
      ...
      if (!e.repeat && matchShortcut('editor.save')) {
        ...
        if (!inEditor) {
          ...
          if (state.activeTabType === 'editor' && state.activeFileId) {
            ...
            window.dispatchEvent(new Event(PEBBLE_EDITOR_REQUEST_CMD_SAVE_EVENT))
            return
          }
        }
      }
  ...
  useEffect(() => {
    setWindowCloseRequestHandler((request) => {
      if (isIntentionalAppRestartInProgress()) {
        window.api.ui.confirmWindowClose(request.requestId)
        return
      }
      ...
      if (windowCloseAfterDirtyRef.current) {
        if (request.isQuitting && !windowCloseAfterDirtyRef.current.isQuitting) {
          windowCloseAfterDirtyRef.current = request
        }
        return
      }
      if (windowCloseAfterChildProcessRef.current) {
        if (request.isQuitting) {
          windowCloseAfterChildProcessRef.current = null
          setWindowCloseDialogOpen(false)
          proceedToNativeWindowClose(request)
        }
        return
      }
        ...
        <DialogContent className="max-w-sm" showCloseButton={false}>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              ...
              autoFocus
              onClick={() => {
                const request = windowCloseAfterChildProcessRef.current
                windowCloseAfterChildProcessRef.current = null
                setWindowCloseDialogOpen(false)
                window.api.ui.confirmWindowClose(request?.requestId)
              }}
