...
function GitHubStarButton({ hasRepos }: { hasRepos: boolean }): React.JSX.Element | null {
  ...
  useEffect(() => {
    let cancelled = false
    void window.api.gh.checkPebbleStarred().then((result) => {
      if (cancelled) {
        return
      }
      if (result === null) {
  ...
  const handleClick = async (): Promise<void> => {
    ...
    const ok = await window.api.gh.starPebble('landing')
    if (!ok) {
...
function PreflightBanner({
  ...
  return (
    ...
    <div
      data-parity-volatile="landing-preflight"
...
export default function Landing(): React.JSX.Element {
  ...
  const createsGitWorktree = repos.every((repo) => isGitRepoKind(repo))
  ...
  useEffect(() => {
    ...
    // Why: users often install/authenticate gh outside Pebble. Re-check when the
  ...
  useEffect(() => {
    ...
    let cancelled = false
    // Why: some users complete `gh auth login` without ever leaving the Pebble
    ...
    return [
      {
        ...
        action: createsGitWorktree
          ? translate('auto.components.Landing.createParallelUniverse', 'Create worktree')
          : translate('auto.components.Landing.createFolderSpace', 'Create workspace')
      },
      {
        id: 'up',
        shortcut: previousWorktreeShortcut,
        action: translate('auto.components.Landing.moveUpParallelUniverse', 'Move up workspace')
      },
      {
        id: 'down',
        shortcut: nextWorktreeShortcut,
        action: translate('auto.components.Landing.moveDownParallelUniverse', 'Move down workspace')
      }
    ]
  }, [createsGitWorktree, createWorktreeShortcut, nextWorktreeShortcut, previousWorktreeShortcut])

  return (
    <div
      data-parity-surface="landing"
    ...
    >
      ...
      {showGitHubSupportFooter && (
        <div
          data-parity-volatile="landing-github-support"
