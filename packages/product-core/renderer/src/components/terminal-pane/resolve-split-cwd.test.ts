...
describe('resolveSplitCwd', () => {
  ...
  it('uses the worktree root when the Tauri provider reports an expired PTY', async () => {
    installGetCwd(async () => '')
    const result = await resolveSplitCwd({
      paneCwdMap: new Map(),
      sourcePaneId: 1,
      sourcePtyId: 'expired-runtime-pty',
      fallbackCwd: '/project/parallel-universe'
    })
    expect(result).toBe('/project/parallel-universe')
  })
