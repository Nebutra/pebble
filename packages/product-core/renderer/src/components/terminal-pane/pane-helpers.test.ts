...
describe('shellEscapePath', () => {
  ...
  it('keeps safe Windows paths unquoted', () => {
    expect(shellEscapePath('C:\\Users\\pebble\\file.txt', 'windows')).toBe(
      'C:\\Users\\pebble\\file.txt'
    )
  })

  it('double-quotes Windows paths with spaces', () => {
    expect(shellEscapePath('C:\\Users\\pebble\\my file.txt', 'windows')).toBe(
      '"C:\\Users\\pebble\\my file.txt"'
    )
  })

  it('double-quotes Windows paths with cmd separators', () => {
    expect(shellEscapePath('C:\\Users\\pebble\\a&b.txt', 'windows')).toBe(
      '"C:\\Users\\pebble\\a&b.txt"'
    )
  })
  ...
  it('uses POSIX escaping for SSH drops regardless of client OS', () => {
    ...
    expect(shellEscapePath("/home/u/wt/.pebble/drops/my file's $draft.txt", 'posix')).toBe(
      "'/home/u/wt/.pebble/drops/my file'\\''s $draft.txt'"
    )
  })
})
