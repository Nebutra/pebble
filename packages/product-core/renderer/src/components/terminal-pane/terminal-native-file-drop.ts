...
import {
  isWorktreeUsingLocalWslRuntime,
  toLocalWslDropPath
} from './terminal-drop-local-wsl-path'
 ...
 * or IPC). SSH worktrees: upload each file into `${worktreePath}/.pebble/drops`
...
async function uploadRemoteDropPaths(
  ...
  try {
    ...
    await pasteResolvedDropPaths({ ...args, paths: importedPaths, targetShell: args.targetShell })
    reportTerminalDropUploadSkipsAndFailures(
      results.filter((result) => result.status === 'skipped'),
      results.filter((result) => result.status === 'failed')
    )
