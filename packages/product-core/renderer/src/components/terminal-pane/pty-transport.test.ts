...
describe('createIpcPtyTransport', () => {
  ...
  it('delivers initial replay returned by a newly spawned native PTY', async () => {
    const { createIpcPtyTransport } = await import('./pty-transport')
    const spawn = window.api.pty.spawn as unknown as ReturnType<typeof vi.fn>
    const onReplayData = vi.fn()
    spawn.mockResolvedValueOnce({ id: 'pty-native', replay: 'pebble$ ' })

    const transport = createIpcPtyTransport({})
    const result = await transport.connect({ url: '', callbacks: { onReplayData } })

    expect(onReplayData).toHaveBeenCalledWith('pebble$ ')
    expect(result).toBe('pty-native')
  })
  ...
  it('suppresses the SSH-not-active toast for a runtime-owned (per-workspace-env) target', async () => {
    ...
    const spawnMock = vi
      .fn()
      .mockRejectedValue(new Error('No PTY provider for connection runtime-ssh-pebble-1'))
    ;(globalThis as { window: typeof window }).window = {
      ...originalWindow,
      api: {
        ...originalWindow?.api,
    ...
    const onError = vi.fn()
    await createIpcPtyTransport({ connectionId: 'runtime-ssh-pebble-1' }).connect({
      url: '',
      callbacks: { onError }
    })
