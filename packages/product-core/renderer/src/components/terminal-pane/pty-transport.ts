...
export function createPtyOutputProcessor({
  ...
  function scheduleSideEffectDrain(): void {
    ...
    // Why: xterm.write() buffers parsing onto its own timer. Defer Pebble's
  ...
  function processData(
    ...
    // Why: OSC 9999 is an Pebble control protocol. Parse it before xterm sees
...
export function createIpcPtyTransport(opts: IpcPtyTransportOptions = {}): PtyTransport {
  ...
  // unread marks, or notifications for unrelated worktrees just because Pebble
  ...
  return {
    async connect(options) {
      ...
      try {
        ...
        if (!spawnResult.isReattach && !spawnResult.coldRestore && spawnResult.replay) {
          // Why: native runtimes can finish the login-shell prompt before the
          // per-PTY data handler binds; fresh callers do not run reattach replay handling.
          storedCallbacks.onReplayData?.(spawnResult.replay)
        }
