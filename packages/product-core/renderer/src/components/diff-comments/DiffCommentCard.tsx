...
type Props = {
  ...
  // Why: only Monaco view-zone mounts need ResizeObserver; popover/list cards
  // pass the callback for edit-mode height without continuous observation.
...
export function DiffCommentCard({
  ...
  return (
    <div ref={cardRef} className="pebble-diff-comment-card">
      <div className="pebble-diff-comment-content-col">
        {/* Header Row */}
        <div className="pebble-diff-comment-header">
          <div className="pebble-diff-comment-meta-group">{metaText}</div>

          {/* Action buttons pill (only shown if not editing) */}
          {!editing && (
            <div
              className="pebble-diff-comment-actions-pill"
              onMouseDown={(ev) => ev.stopPropagation()}
            >
              {headerActions}
              {headerActions && (url || onSubmitEdit || onDelete) && (
                <span className="pebble-diff-comment-pill-divider" />
              )}
              {url && (
                <>
                  <button
                    type="button"
                    className="pebble-diff-comment-pill-btn"
                    title={translate(
              ...
              {onSubmitEdit && (
                <>
                  ...
                  {onDelete && <span className="pebble-diff-comment-pill-divider" />}
                </>
              )}
              {onDelete && (
                <button
                  type="button"
                  className="pebble-diff-comment-pill-btn pebble-diff-comment-pill-btn-danger"
                  title={translate(
        ...
        {/* Quote Block */}
        {quote ? (
          <div className="pebble-diff-comment-quote">
            <div className="pebble-diff-comment-quote-text">{quote}</div>
          </div>
        ) : null}
        ...
        {editing ? (
          <div className="flex flex-col gap-2 mt-1">
            <textarea
              ref={textareaRef}
              className="pebble-diff-comment-popover-textarea"
              value={draft}
              onChange={(e) => {
              ...
              rows={3}
            />
            <div className="pebble-diff-comment-popover-footer">
          ...
          <div className="pebble-diff-comment-body">{body}</div>
        )}
      </div>
    </div>
  )
