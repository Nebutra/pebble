...
export function deriveRepoHookProbeIds(
  neededRepoIds: readonly string[],
  resolvedRepoIds: ReadonlySet<string>,
  inFlightRepoIds: ReadonlySet<string>
): string[] {
  return neededRepoIds.filter(
    (repoId) => !resolvedRepoIds.has(repoId) && !inFlightRepoIds.has(repoId)
  )
}
