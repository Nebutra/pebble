...
export function MobileEmulatorAgentControlRow(): React.JSX.Element {
  ...
  const cliSkillInstallCommand = buildSkillCommandForRuntime(PEBBLE_CLI_SKILL_INSTALL_COMMAND)
  const cliSkillUpdateCommand = buildSkillCommandForRuntime(PEBBLE_CLI_SKILL_UPDATE_COMMAND)
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'Let coding agents control the active mobile emulator with Pebble CLI commands.'
            )}
          </p>
        </div>
        <span
          ...
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              {translate(
                ...
                'Registers the Pebble CLI command so agents can control the active emulator from their shell.'
              )}
            </p>
        ...
        <div className={cn('py-3', setup.step2Blocked && 'opacity-60')}>
          <AgentSkillSetupPanel
            ...
            description={translate(
              ...
              'Enables agents to use Pebble CLI commands, including mobile emulator control.'
            )}
            command={cliSkillInstallCommand}
            installedCommand={cliSkillUpdateCommand}
            terminalTitle="Pebble CLI skill setup"
            terminalAriaLabel="Pebble CLI skill install terminal"
            terminalWorktreeId="settings-mobile-emulator-pebble-cli-skill-terminal"
            ...
            openingHint={translate(
              ...
              'Checking Pebble CLI before opening skill setup.'
            )}
            onBeforeOpenTerminal={async () => {
              await ensurePebbleCliAvailableForAgentSkillTerminal()
            }}
