...
import type { GlobalSettings, PebbleHooks } from '../../../../shared/types'
...
function Settings(): React.JSX.Element {
  ...
  const [repoHooksMap, setRepoHooksMap] = useState<
    Record<string, { hasHooks: boolean; hooks: PebbleHooks | null; mayNeedUpdate: boolean }>
  >({})
  ...
  const orchestrationSkill = useInstalledAgentSkill(ORCHESTRATION_SKILL_NAME, {
    enabled: activeSectionId === 'orchestration',
    discoveryTarget: activeSkillRuntime.discoveryTarget,
    sourceKinds: GLOBAL_AGENT_SKILL_SOURCE_KINDS
  })
  const computerUseSkill = useInstalledAgentSkill(COMPUTER_USE_SKILL_NAME, {
    enabled: showDesktopOnlySettings && activeSectionId === 'computer-use',
    discoveryTarget: activeSkillRuntime.discoveryTarget,
    sourceKinds: GLOBAL_AGENT_SKILL_SOURCE_KINDS
  })
  const [voiceModelStatesLoading, setVoiceModelStatesLoading] = useState(false)
  const [voiceModelStatesChecked, setVoiceModelStatesChecked] = useState(false)
  ...
  const repoHooksInFlightRef = useRef(new Set<string>())
  const repoHooksResolvedRef = useRef(new Set<string>())
  ...
  useSettingsKeyboardScope({
    activeSectionId,
    closeSettingsPage: closeSettingsPageWithPromptGuard,
    enabled: settingsPageOpen,
    keybindings,
    searchInputRef
  })

  useEffect(() => {
    if (!showDesktopOnlySettings || activeSectionId !== 'voice') {
    ...
    void refreshModelStates().finally(() => {
      if (!canceled) {
        setVoiceModelStatesLoading(false)
        setVoiceModelStatesChecked(true)
      }
    })
    return () => {
      canceled = true
    }
  }, [activeSectionId, refreshModelStates, showDesktopOnlySettings])
  ...
  const capabilityInstallStatusBySectionId = useMemo(() => {
    ...
    if (activeSectionId === 'orchestration') {
      next.set(
        'orchestration',
    ...
    if (showDesktopOnlySettings && activeSectionId === 'computer-use') {
      next.set(
        'computer-use',
        getSkillNavInstallStatus({
    ...
    if (
      ...
      settings &&
      (voiceModelStatesChecked || voiceModelStatesLoading)
    ) {
      next.set(
        'voice',
  ...
  // Why: General owns the Pebble CLI controls, including WSL skill-location setup.
  ...
  useEffect(() => {
    ...
    repoHooksResolvedRef.current = new Set(
      [...repoHooksResolvedRef.current].filter((repoId) => repoIdSet.has(repoId))
    )
    repoHooksInFlightRef.current = new Set(
      [...repoHooksInFlightRef.current].filter((repoId) => repoIdSet.has(repoId))
    )
    setRepoHooksMap((previous) => {
      const next = Object.fromEntries(
        Object.entries(previous).filter(([repoId]) => repoIdSet.has(repoId))
      ) as Record<string, { hasHooks: boolean; hooks: PebbleHooks | null; mayNeedUpdate: boolean }>
  ...
  useEffect(() => {
    if (repoHooksRuntimeIdentityRef.current !== runtimeTargetIdentity) {
      ...
      repoHooksInFlightRef.current.clear()
      repoHooksResolvedRef.current.clear()
      setRepoHooksMap({})
    }
  ...
  useEffect(() => {
    ...
    void Promise.all(
      deriveRepoHookProbeIds(
        neededRepoIds,
        repoHooksResolvedRef.current,
        repoHooksInFlightRef.current
      ).map(async (repoId) => {
        ...
        if (isFolderRepo(repo)) {
          repoHooksResolvedRef.current.add(repoId)
          setRepoHooksMap((previous) => {
            if (previous[repoId]) {
              return previous
            }
        ...
        repoHooksInFlightRef.current.add(repoId)
        try {
          ...
          if (requestSeq !== repoHooksRequestSeqRef.current) {
            return
          }
          repoHooksResolvedRef.current.add(repoId)
          setRepoHooksMap((previous) => {
          ...
          repoHooksInFlightRef.current.delete(repoId)
        }
      })
    )
    ...
    async (
      ...
      startTransition(() => {
        setMountedSectionIds((previous) => {
          ...
          return new Set(previous).add(sectionId)
        })
  ...
  const prepareSection = useCallback((sectionId: string): void => {
    // Why: pointer travel should warm the code chunk without mounting hidden
    // panes and running their effects before the user actually selects them.
    preloadSettingsPane(sectionId)
  }, [])
  ...
  return (
    <div className="settings-view-shell flex min-h-0 flex-1 overflow-hidden bg-background">
      <SettingsSidebar
        ...
        onPrepareSection={prepareSection}
              ...
              <ActiveSettingsSectionProvider value={activeSectionId}>
                ...
                <SettingsSection
                  ...
                  description={translate(
                    ...
                    'Optional. Pebble works with your existing provider logins; add accounts only if you want Pebble to help switch between them.'
                  )}
                  badge={translate(
                ...
                <SettingsSection
                  ...
                  description={translate(
                    ...
                    'Coordinate multiple coding agents through Pebble.'
                  )}
                ...
                <SettingsSection
                  ...
                  description={translate(
                    ...
                    'Finish the core workflows that make Pebble useful for parallel agent work.'
                  )}
                ...
                {showDesktopOnlySettings ? (
                  <SettingsSection
                    ...
                    description={translate(
                      ...
                      'Configure mobile emulator support for Pebble and coding agents.'
                    )}
                ...
                <SettingsSection
                  ...
                  description={translate(
                    ...
                    'Pebble stats plus Claude, Codex, and OpenCode usage analytics.'
                  )}
                  ...
                  description={
                    isWebClient
                      ? translate(
                          ...
                          'Connect this browser to a saved Pebble server.'
                        )
                      : translate(
                          'auto.components.settings.Settings.b5ee17826b',
                          'Pair remote Pebble runtimes for persistent sessions, richer remote state, and web or mobile handoff.'
                        )
                  }
