...
export function VoicePane({ settings, updateSettings }: VoicePaneProps): React.JSX.Element {
  ...
  const updateVoiceSettingsRef = useRef<(updates: Partial<VoiceSettings>) => void>(() => {})
  ...
  updateVoiceSettingsRef.current = updateVoiceSettings
  ...
  useEffect(() => {
    ...
    void window.api.speech
      ...
      .then((status) => {
        if (!cancelled && status.configured !== voiceSettingsRef.current.openAiApiKeyConfigured) {
          updateVoiceSettingsRef.current({ openAiApiKeyConfigured: status.configured })
  ...
  const toggleVoiceDictation = async (): Promise<void> => {
    if (voiceDictationUnavailableReason) {
      toast.message(voiceDictationUnavailableReason)
      return
    }
  ...
  return (
    <div ref={handlePaneRef} className="space-y-1">
      <VoiceDictationSettingsSection
        ...
        unavailableReason={voiceDictationUnavailableReason}
