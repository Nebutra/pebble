...
export function VoiceDictationSettingsSection({
  ...
  const controlsDisabled = permissionPending || unavailableReason !== null
  const modeDisabled = !voiceSettings.enabled || unavailableReason !== null
      ...
      <div className="flex items-center justify-between gap-4 py-2">
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {unavailableReason ??
              `${translate('auto.components.settings.VoicePane.4465596675', 'Press')} ${shortcutLabel} ${translate(
        ...
        <button
          ...
          className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border border-transparent transition-colors ${
            ...
            unavailableReason ? 'cursor-not-allowed opacity-50' : ''
          }`}
        >
          <span
