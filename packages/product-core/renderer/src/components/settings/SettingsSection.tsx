...
import { Activity, createContext, Suspense, useContext } from 'react'
...
export function SettingsSection({
  ...
  const visible = forceVisible || (hasQuery ? sectionIsActive && matchesQuery : sectionIsActive)
  ...
  return (
    <Activity mode={visible ? 'visible' : 'hidden'}>
      ...
      >
        ...
        >
          <Suspense fallback={<SettingsSectionLoadingFallback />}>{children}</Suspense>
        </div>
      </section>
    </Activity>
  )
...
function SettingsSectionLoadingFallback(): React.JSX.Element {
  return (
    <div
      aria-busy="true"
      className="min-h-32 rounded-md bg-muted/30"
