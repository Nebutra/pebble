...
/**
 * Settings-search entries for the status bar's usage providers.
 *
 * Why split from the rest of the catalogue: this list grows by one entry per
 * supported provider, while ssh/resource-usage/ports are a fixed set. Keeping
 * them apart stops the combined file from crossing the length ceiling each
 * time a provider is added.
 */
export function getUsageProviderToggleEntries(): readonly StatusBarToggleSearchEntry[] {
  return [
    {
      id: 'claude',
    ...
    {
      id: 'grok',
      title: translate('settings.appearance.statusBar.grokUsageTitle', 'Grok Usage'),
      description: translate(
        'settings.appearance.statusBar.grokUsageDescription',
        'Show Grok credit usage in the status bar.'
      ),
      keywords: [
        ...translateSearchKeyword(
          'auto.components.settings.appearance.search.896eb53fd4',
        ...
        ...translateSearchKeyword('settings.appearance.statusBar.grokKeyword', 'grok'),
        ...translateSearchKeyword('auto.components.settings.appearance.search.00a028f25f', 'usage'),
        ...translateSearchKeyword('settings.appearance.statusBar.grokCreditsKeyword', 'credits'),
        ...translateSearchKeyword('settings.appearance.statusBar.grokXaiKeyword', 'xai')
      ],
      toggleDescription: translate(
        'settings.appearance.statusBar.grokToggleDescription',
        'Show Grok credit usage for the active workspace.'
      )
    }
  ]
}
