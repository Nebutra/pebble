...
export function FileExplorerRow({
  ...
  const handleOpenInPebbleBrowser = useCallback(() => {
    if (!activeWorktreeId) {
      return
    }
  ...
  const handleDownload = useCallback(() => {
    // Why: structured worktree identity lets Tauri's legacy SSH relay use the
    // same bounded runtime stream as paired hosts instead of the Web fallback.
    ...
    >
      ...
      >
        ...
        {!node.isDirectory && activeWorktreeId && (
          <ContextMenuItem onSelect={handleOpenInPebbleBrowser}>
            <Globe />
            {translate(
              'auto.components.right.sidebar.FileExplorerRow.dd112c81d2',
              'Open in Pebble Browser'
            )}
          </ContextMenuItem>
        )}
