...
export default function ChecksPanel(): React.JSX.Element {
  ...
  const resolvedPullRequestAi = useMemo(() => {
    if (!settings) {
      return null
    }
    return resolveSourceControlAiForOperation({
      ...
      discoveryHostKey: getCommitMessageModelDiscoveryHostKeyForScope(
  ...
  const prCreationDefaults = useMemo(() => {
    ...
    return resolvedPullRequestAi?.ok
      ? resolvedPullRequestAi.value.prCreationDefaults
      : resolveSourceControlAiPrCreationDefaults({
          settings,
          repo,
          prCreationProductDefaults: DEFAULT_SOURCE_CONTROL_AI_PR_CREATION_DEFAULTS
        })
  }, [repo, resolvedPullRequestAi, settings])
  ...
  // opened outside Pebble, externally force-pushed heads, and stale checks/comments
  ...
  const resolveSelectedThreadsAfterLaunch = useCallback(
    async (resolution: NonNullable<ChecksAgentComposerState['commentResolution']>) => {
      ...
      for (const threadId of resolution.selectedThreadIds) {
        ...
        const ok = await handleResolve(threadId, true, {
  ...
  // when the actual plain-click path would open inside Pebble.
  ...
  return (
    <div
      ref={setChecksPanelContentRef}
      data-testid="checks-panel"
      className="flex-1 overflow-auto scrollbar-sleek"
    >
