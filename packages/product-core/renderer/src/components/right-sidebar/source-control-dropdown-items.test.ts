...
describe('resolveDropdownItems', () => {
  ...
  it.each([
    ['azure-devops', 'Set PEBBLE_AZURE_DEVOPS_TOKEN in this environment'],
    ['gitea', 'Set PEBBLE_GITEA_TOKEN in this environment']
  ...
  it('uses environment-unavailable copy when hosted review creation is unsupported', () => {
    const items = resolveDropdownItems(
      inputs({
        upstreamStatus: { hasUpstream: true, ahead: 0, behind: 0 },
        hostedReviewCreation: {
    ...
    expect(byKind.create_pr.hint).toBe('Unavailable in this environment')
