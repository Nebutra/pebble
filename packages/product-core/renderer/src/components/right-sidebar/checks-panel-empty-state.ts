...
export function getChecksPanelEmptyStateCopy(
  ...
  if (blockedReason === 'unsupported_provider') {
    return {
      title: translate(
        'auto.components.right.sidebar.checks.panel.empty.state.0a7d6a8e2b',
        'Hosted review unavailable'
      ),
      description: translate(
        'auto.components.right.sidebar.checks.panel.empty.state.1a2d4f9730',
