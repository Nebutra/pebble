...
describe('browser notice formatting', () => {
  it('formats denied permissions with safe copy', () => {
    ...
    ).toBe('https://example.com asked for camera or microphone access, and Pebble denied it.')
  })
  ...
  it('formats popup outcomes', () => {
    ...
    ).toBe('https://example.com opened a new page in Pebble.')
    ...
    ).toBe('A site tried to open a popup Pebble does not support here.')
  })
