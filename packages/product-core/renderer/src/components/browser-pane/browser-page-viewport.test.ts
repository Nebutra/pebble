...
import {
  ...
  applyBrowserPageViewportOverride,
...
describe('applyBrowserPageViewportOverride', () => {
  it('sizes and centers the native guest container, then restores fill layout', () => {
    mountSlotViewport('workspace-1')
    const viewport = ensureBrowserPageViewport('page-1', 'workspace-1')!

    applyBrowserPageViewportOverride('page-1', { width: 375, height: 667 })
    expect(viewport.container.style.width).toBe('375px')
    expect(viewport.container.style.height).toBe('667px')
    expect(viewport.container.style.maxWidth).toBe('100%')
    expect(viewport.container.style.alignSelf).toBe('center')
    expect(viewport.container.style.flex).toBe('0 1 auto')

    applyBrowserPageViewportOverride('page-1', null)
    expect(viewport.container.style.width).toBe('')
    expect(viewport.container.style.height).toBe('')
    expect(viewport.container.style.flex).toBe('')
  })
})
