...
export function applyBrowserPageViewportOverride(
  browserPageId: string,
  override: { width: number; height: number } | null
): void {
  const container = browserPageViewports.get(browserPageId)?.container
  if (!container) {
    return
  }
  if (!override) {
  ...
  // Why: Tauri has no CDP device-metrics override. Sizing the stable guest
  // container changes the native child WebView bounds without reparenting it.
  container.style.width = `${override.width}px`
  container.style.height = `${override.height}px`
