...
export function BrowserImportHintButton({
  ...
  const browserGuestAvailability = getBrowserGuestFeatureAvailability()
  const browserCookieImportUnavailableReason =
    browserGuestAvailability.cookieImportUnavailableReason
  const cookieFileImportUnavailableReason =
    browserGuestAvailability.cookieFileImportUnavailableReason
  const shouldShow =
  ...
  const handleOpenChange = useCallback(
    (nextOpen: boolean): void => {
      ...
      if (nextOpen && !browserCookieImportUnavailableReason) {
    ...
    [browserCookieImportUnavailableReason, fetchDetectedBrowsers]
  )
  ...
  const handleImportFromBrowser = useCallback(
    async (browserFamily: string, browserProfile?: string): Promise<void> => {
      setOpen(false)
      setImportMenuOpen(false)
      if (browserCookieImportUnavailableReason) {
        toast.message(browserCookieImportUnavailableReason)
        return
      }
    ...
    [
      browserCookieImportUnavailableReason,
      detectedBrowsers,
      effectiveProfileId,
  ...
  const handleImportFromFile = useCallback(async (): Promise<void> => {
    setOpen(false)
    setImportMenuOpen(false)
    if (cookieFileImportUnavailableReason) {
      toast.message(cookieFileImportUnavailableReason)
      return
    }
  ...
  }, [cookieFileImportUnavailableReason, effectiveProfileId, importCookiesToProfile])
          ...
          <div className="flex items-center gap-3">
            <DropdownMenu modal={false} open={importMenuOpen} onOpenChange={setImportMenuOpen}>
              ...
              <DropdownMenuContent align="start" className="w-52">
                ...
                <DropdownMenuItem
                  onSelect={() => void handleImportFromFile()}
                  disabled={Boolean(cookieFileImportUnavailableReason)}
                >
                  {cookieFileImportUnavailableReason ??
                    translate(
