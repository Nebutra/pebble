...
import {
  ...
  PEBBLE_BROWSER_FOCUS_REQUEST_EVENT,
...
function getBrowserOverlayAnchor(
  ...
  webview: BrowserPageWebview | null,
...
function toDisplayUrl(url: string): string {
  return url === PEBBLE_BROWSER_BLANK_URL ? 'about:blank' : redactKagiSessionToken(url)
}
...
function getCurrentBrowserUrl(webview: BrowserPageWebview | null, fallbackUrl: string): string {
  let currentUrl = fallbackUrl
  if (webview) {
    try {
      currentUrl = webview.getURL() || fallbackUrl
...
function RemoteBrowserPagePane({
  ...
  const ensureRemotePage = useCallback(
    async (token: RemoteBrowserOperationToken): Promise<string | null> => {
      ...
      const createRemotePage = async (): Promise<string | null> => {
        ...
        const initialUrl =
          currentUrl === PEBBLE_BROWSER_BLANK_URL ? 'about:blank' : currentUrl || 'about:blank'
  ...
  useEffect(() => {
    ...
    window.addEventListener(PEBBLE_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
    return () =>
      window.removeEventListener(PEBBLE_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
...
function BrowserPagePane({
  ...
  const webviewRef = useRef<BrowserPageWebview | null>(null)
  ...
  // Match both so the "New Browser Tab" overlay and toolbar gates agree.
  const isBlankTab = browserTab.url === 'about:blank' || browserTab.url === PEBBLE_BROWSER_BLANK_URL
  const browserGuestFeatureAvailability = getBrowserGuestFeatureAvailability()
  const browserElementGrabUnavailableReason =
    browserGuestFeatureAvailability.elementGrabUnavailableReason
  const browserAnnotationUnavailableReason =
    browserGuestFeatureAvailability.annotationUnavailableReason
  const browserFindUnavailableReason = browserGuestFeatureAvailability.findInPageUnavailableReason
  const browserElementGrabDisabled = isBlankTab || !browserGuestFeatureAvailability.canGrabElement
  const browserAnnotationDisabled =
    isBlankTab || !browserGuestFeatureAvailability.canAnnotateElement
  ...
  useEffect(() => {
    const handleRuntimeViewportPreset = (event: Event): void => {
      const detail = (
      ...
      if (detail?.browserPageId !== browserTab.id) {
        return
      }
      useAppStore.getState().setBrowserPageViewportPreset(browserTab.id, detail.presetId ?? null)
    }
    window.addEventListener('pebble:browser-viewport-preset', handleRuntimeViewportPreset)
    return () =>
      window.removeEventListener('pebble:browser-viewport-preset', handleRuntimeViewportPreset)
  }, [browserTab.id])
  ...
  const sessionProfile = sessionProfileId
    ...
    : null
  const webviewPartition = sessionPartition ?? sessionProfile?.partition ?? PEBBLE_BROWSER_PARTITION
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent): void => {
      ...
      e.preventDefault()
      e.stopPropagation()
      if (browserFindUnavailableReason) {
        setResourceNotice(browserFindUnavailableReason)
        return
      }
      setFindOpen(true)
    }
    window.addEventListener('keydown', handleKeyDown, true)
    return () => window.removeEventListener('keydown', handleKeyDown, true)
  }, [browserFindUnavailableReason, isActive, keybindings])
  ...
  }, [browserFindUnavailableReason, isActive])
  ...
  useEffect(() => {
    ...
    const applyActivePageZoom = (direction: BrowserPageZoomDirection): void => {
      ...
      // Why: reset targets 100% like Chromium; the configured default is a new-tab seed.
  ...
  const syncNavigationState = useCallback(
    (webview: BrowserPageWebview): void => {
      try {
        onUpdatePageStateRef.current(browserTab.id, {
          ...
          // Only explicit navigation/load events should drive Pebble's loading UI.
          canGoBack: webview.canGoBack(),
          canGoForward: webview.canGoForward()
        })
      } catch {
        // Why: Electron only exposes these getters after the guest fully
        // attaches. Ignoring the transient failure avoids crashing Pebble while
  ...
  useEffect(() => {
    ...
    const handleDidStopLoading = (): void => {
      ...
      if (activeLoadFailure) {
        ...
        if (normalizedAttemptedUrl === normalizedCurrentUrl) {
          ...
          // navigation falls back to a blank Chromium surface even though Pebble
      ...
      if (keepAddressBarFocusRef.current && currentUrl === PEBBLE_BROWSER_BLANK_URL) {
        focusAddressBarNow()
      } else {
        keepAddressBarFocusRef.current = false
      }
    ...
    const handleFailLoad = (event: {
      ...
      if (event.errorCode === -3) {
        ...
        // when the replacement navigation succeeds. Ignore that noise so Pebble
    ...
    if (needsInitialNavigation) {
      ...
      // Pebble never misses the initial did-fail-load signal for a new tab.
      // Only non-blank initial tabs should light up Pebble's loading indicator.
      const initialUrl =
        normalizeBrowserNavigationUrl(initialBrowserUrlRef.current) ?? PEBBLE_BROWSER_BLANK_URL
      trackNextLoadingEventRef.current = initialUrl !== PEBBLE_BROWSER_BLANK_URL
  ...
  useEffect(() => {
    ...
    if (
      ...
      // Why: browserTab.url changes are Pebble-driven navigations (address bar,
      ...
      trackNextLoadingEventRef.current = normalizedUrl !== PEBBLE_BROWSER_BLANK_URL
      lastKnownWebviewUrlRef.current = normalizedUrl
      webview.src = normalizedUrl
      if (normalizedUrl !== PEBBLE_BROWSER_BLANK_URL) {
        keepAddressBarFocusRef.current = false
        if (document.activeElement === addressBarInputRef.current) {
          focusWebviewNow()
        }
  ...
  useEffect(() => {
    ...
    // Polling only while the active tab is "loading" gives Pebble a last-resort
  ...
  const startGrabIntent = useCallback(
    (nextIntent: GrabIntent): void => {
      const unavailableReason =
        nextIntent === 'annotate'
          ? browserAnnotationUnavailableReason
          : browserElementGrabUnavailableReason
      if (unavailableReason) {
        setResourceNotice(unavailableReason)
        return
      }
      recordFeatureInteraction('browser-grab')
  ...
  useEffect(() => {
    ...
    if (!isActive || browserElementGrabDisabled) {
      return
    }
    const shortcutPlatform = getShortcutPlatform()
    const handleKeyDown = (e: KeyboardEvent): void => {
  ...
  }, [browserElementGrabDisabled, isActive, keybindings, startGrabIntent])
  ...
  useEffect(() => {
    return window.api.browser.onGrabModeToggle((tabId) => {
      if (tabId === browserTab.id) {
        if (browserElementGrabUnavailableReason) {
          setResourceNotice(browserElementGrabUnavailableReason)
          return
        }
        startGrabIntent('copy')
      }
    })
  }, [browserElementGrabUnavailableReason, browserTab.id, startGrabIntent])
  ...
  const navigateToUrl = useCallback(
    (url: string): void => {
      const navigateBrowserUrl = (targetUrl: string): void => {
        ...
        trackNextLoadingEventRef.current = targetUrl !== PEBBLE_BROWSER_BLANK_URL
        lastKnownWebviewUrlRef.current =
          normalizeBrowserNavigationUrl(browserModelUrl) ?? browserModelUrl
        webview.src = targetUrl
        if (targetUrl !== PEBBLE_BROWSER_BLANK_URL) {
          focusWebviewNow()
        }
      }
            ...
            <TooltipTrigger asChild>
              <span className="inline-flex">
                <Button
                  ...
                  disabled={browserElementGrabDisabled}
                  aria-label={translate(
            ...
            <TooltipTrigger asChild>
              ...
              <span className="inline-flex">
                <Button
                  ...
                  disabled={browserAnnotationDisabled}
                  aria-label={translate(
      ...
      {pageViewport?.container
        ? createPortal(
            <>
              ...
              <BrowserFind
                isOpen={findOpen && !browserFindUnavailableReason}
                    ...
                    <div className="mt-5 flex items-center gap-2">
                      ...
                      {externalUrl ? (
                        <Button
                          ...
                          onClick={() => {
                            // Why: page failures inside Pebble can still be recoverable
