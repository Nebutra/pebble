        ...
        <div role="listbox" aria-label={addHostLabel} onMouseLeave={() => setHoveredKey(null)}>
          ...
          {onAddRemoteServer ? (
            <RunTargetRow
              icon={<Cloud className="size-3.5 shrink-0 text-muted-foreground" />}
              label={translate(
                'auto.components.NewWorkspaceComposerCard.addRemoteServer',
                'Add remote server'
              )}
              detail={translate(
                'auto.components.NewWorkspaceComposerCard.addRemoteServerHint',
                'Pair with Pebble running on another computer'
              )}
