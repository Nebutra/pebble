...
const pebbleDemo = project('pebble-demo', 'pebble-demo', 'stablyai/pebble-demo')
const relay = project('relay', 'pebble-relay', 'stablyai/pebble-relay')
...
describe('rankProjectOptions', () => {
  it('ranks a name-prefix match above a mid-name match', () => {
    const ranked = rankProjectOptions([relay, pebbleDemo], 'pebble-demo', [])
    expect(ranked[0]?.option.id).toBe('pebble-demo')
  })
  ...
  it('orders an unfiltered list by recency', () => {
    const ranked = rankProjectOptions([pebbleDemo, relay, gateway], '', ['gateway', 'relay'])
    expect(ranked.map((r) => r.option.id)).toEqual(['gateway', 'relay', 'pebble-demo'])
  })

  it('returns nothing for an oversized query rather than scanning it', () => {
    expect(rankProjectOptions([pebbleDemo], 'x'.repeat(4096), [])).toEqual([])
  })
})
...
describe('sectionProjectOptions', () => {
  ...
  it('keeps a short list unsectioned', () => {
    const matches = rankProjectOptions([pebbleDemo, relay], '', ['relay'])
...
describe('getAmbiguousProjectOptionIds', () => {
  it('flags only ids whose display name repeats', () => {
    ...
    const ids = getAmbiguousProjectOptionIds([a, b, pebbleDemo])
...
describe('splitDetailForElision', () => {
  ...
  it('leaves short or shallow details alone', () => {
    expect(splitDetailForElision('stablyai/pebble-demo')).toBeNull()
