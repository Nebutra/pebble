...
describe('getFeatureWallSetupProgress', () => {
  ...
  it('renders Setup before Milestones and numbers Milestones after Setup', () => {
    const source = readFileSync(
      join(
        process.cwd(),
        'packages/product-core/renderer/src/components/feature-wall/FeatureWallSetupChecklist.tsx'
      ),
      'utf8'
    )
  ...
  it('marks the Pebble CLI setup row complete when installed skills are ready and Computer Use is unavailable', () => {
    const progress = getFeatureWallSetupProgress(
      makeInput({
        browserUseSkillInstalled: true,
        computerUseSkillInstalled: true,
