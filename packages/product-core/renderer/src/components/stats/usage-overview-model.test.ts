...
describe('usage overview model', () => {
  ...
  it('projects sorted provider histories without sorting the combined result', () => {
    ...
    try {
      ...
      expect(overview.daily.map(({ day, totalTokens }) => ({ day, totalTokens }))).toEqual([
        { day: '2026-05-13', totalTokens: 10 },
        { day: '2026-05-14', totalTokens: 20 },
        { day: '2026-05-15', totalTokens: 70 }
      ])
  ...
  it('preserves ordered aggregation for unsorted and duplicate provider days', () => {
    const overview = buildUsageOverview({
      claude: { scanState: null, summary: null, daily: [] },
      codex: {
        scanState: null,
