...
import { PEBBLE_EDITOR_REQUEST_CMD_SAVE_EVENT } from './editor-autosave'
...
export function useEditorCmdSaveRequest({
  ...
  useEffect(() => {
    ...
    window.addEventListener(PEBBLE_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
    return () => window.removeEventListener(PEBBLE_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
