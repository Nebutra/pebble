...
export function selectTabAgentTypesByTabId(
...
): Record<string, AgentType | null> {
  const projection: Record<string, AgentType | null> = {}
  for (const [paneKey, entry] of Object.entries(agentStatusByPaneKey)) {
    ...
    if (Object.prototype.hasOwnProperty.call(projection, tabId)) {
      continue
    }
    projection[tabId] = entry.agentType ?? null
  }
  return projection
}
