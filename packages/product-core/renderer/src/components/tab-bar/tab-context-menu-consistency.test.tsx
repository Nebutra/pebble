...
// Pebble routes terminal splits through TabWorkspaceLayoutMenuSection instead of
// upstream's TerminalTabSplitMenuSection, so that file has no counterpart here.
...
describe('tab context menu consistency', () => {
  it.each(TAB_MENU_SOURCES)('gives every action item an icon in %s', (fileName) => {
    ...
    for (const body of bodies) {
      ...
      // Pebble sizes them as either `size-3.5` or the older `w-3.5 h-3.5`, and
      // carries an `mr-1.5` gutter, so match the size token anywhere in the class.
      const hasIcon =
        /<[A-Z][A-Za-z0-9]*\s+className="[^"]*(?:\bsize-3\.5|\bw-3\.5)/.test(body) ||
        /Icon\(/.test(body)
