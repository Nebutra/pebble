...
describe('WorktreeCard quick actions', () => {
  ...
  it('renders repo identity in the detailed metadata row', () => {
    ...
    expect(markup).not.toContain('aria-label="Project pebble"')
    expect(markup).toContain('>pebble</span>')
  ...
  it('renders detached HEAD identity in detailed card metadata', () => {
    ...
    const markup = renderToStaticMarkup(
      <WorktreeCard
        worktree={makeWorktree({ displayName: 'pebble', branch: '' })}
        repo={makeRepo()}
        isActive={false}
        hideRepoBadge
      />
    ...
    expect(markup).toContain('pebble')
    expect(markup).toContain('data-worktree-card-meta-row=""')
