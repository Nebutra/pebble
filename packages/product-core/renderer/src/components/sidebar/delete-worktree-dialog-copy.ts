  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Pebble. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Pebble. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Pebble workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Pebble. The project folder on disk will not be deleted.'
