...
describe('getCyclableWorktreeIds', () => {
  ...
  it('keeps the rendered sidebar order', () => {
    const rows: HostSectionRow[] = [worktree('pinned-a', true), worktree('plain-b')]

    expect(getCyclableWorktreeIds(rows)).toEqual(['pinned-a', 'plain-b'])
  })

  it('counts a worktree rendered in two places once', () => {
    ...
    expect(getCyclableWorktreeIds(rows)).toEqual(['dup', 'plain-b'])
  })
  ...
  it('leaves folder workspaces out of the rotation', () => {
    ...
    expect(getCyclableWorktreeIds(rows)).toEqual(['plain-b'])
  })
  ...
  it('drops worktrees the sidebar elided inside a collapsed host section', () => {
    ...
    expect(getCyclableWorktreeIds(rows)).toEqual(['visible-after-host'])
  })
})
...
describe('WorktreeList keyboard cycling', () => {
  it('cycles over the rendered rows instead of rebuilding a parallel layout', () => {
    ...
    expect(navigateWorktree).toContain('getCyclableWorktreeIds(rows)')
