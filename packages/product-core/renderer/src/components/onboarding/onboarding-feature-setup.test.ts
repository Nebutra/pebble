...
const OPENED_COMPUTER_USE_SETUP: ComputerUsePermissionSetupResult = {
  platform: 'darwin',
  helperAppPath: '/Applications/Pebble.app',
  openedSettings: true,
  launchedHelper: true
}
...
describe('onboarding feature setup runner', () => {
  ...
  it('builds one skill command for selected onboarding feature setup skills', () => {
    ...
    expect(text).toBe(
      'npx skills add https://github.com/nebutra/pebble --skill pebble-cli computer-use orchestration pebble-linear --global'
    )
  })
  ...
  it('shows CLI registration context before installing a missing CLI during onboarding', async () => {
    const staleStatus: CliInstallStatus = {
      ...
      state: 'stale',
      currentTarget: '/tmp/other-pebble',
      detail: '/usr/local/bin/pebble points to a different launcher.'
    }
