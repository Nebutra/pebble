...
import { showPebbleCliRegistrationPromptToast } from '@/lib/agent-skill-cli-prerequisite'
import {
...
export function createOnboardingFeatureSetupDeps(): OnboardingFeatureSetupDeps {
  ...
  return {
    ...
    showCliRegistrationPrompt: showPebbleCliRegistrationPromptToast,
...
export async function runOnboardingFeatureSetup(
  ...
  try {
    ...
    if (!status.supported) {
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Pebble CLI registration is not available on this platform.'
      })
      ...
      if (next.state !== 'installed') {
        warnings.push({
          featureId: 'cli',
          message: next.detail ?? 'Pebble CLI registration needs attention.'
        })
