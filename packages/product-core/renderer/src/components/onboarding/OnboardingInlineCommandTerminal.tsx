...
import {
  ...
  terminalReadyForCommand
} from './onboarding-inline-terminal-command-delivery'

export {
  READY_MAX_ATTEMPTS,
  INSERT_ACK_TIMEOUT_MS,
  INSERT_NATIVE_ACK_TIMEOUT_MS,
  dispatchTerminalTextWithAcknowledgement,
  focusInlineTerminalSurface,
  getNextTerminalReadyRetryAttempt
} from './onboarding-inline-terminal-command-delivery'
...
export function OnboardingInlineCommandTerminal({
    ...
    (onSettled: (pasted: boolean) => void) => {
      ...
      dispatchTerminalTextWithAcknowledgement({
        ...
        onSettled: (pasted) => {
          ...
          if (pasted) {
            focusInlineTerminalSurface(tabId)
          }
          onSettled(pasted)
        }
      })
  ...
  useEffect(() => {
    ...
    const scheduleInsert = (): void => {
      ...
      const attemptInsert = (): void => {
        insertionTimer = null
        if (canceled || autoInsertedRef.current === command) {
          return
        }
        insertCommand((pasted) => {
          if (canceled) {
            return
          }
          if (pasted) {
          ...
          {cwdError ? (
            <div className="flex h-full flex-col items-center justify-center gap-3 px-6 text-center">
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                onClick={() => setCwdAttempt((attempt) => attempt + 1)}
              >
                <RotateCcw className="size-3.5" />
                {translate(
                  'auto.components.onboarding.OnboardingInlineCommandTerminal.retry',
                  'Retry'
                )}
              </Button>
            </div>
