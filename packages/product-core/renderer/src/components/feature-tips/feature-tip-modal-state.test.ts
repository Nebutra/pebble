...
describe('feature tip modal state', () => {
  ...
  it('does not render a pinned tip that is suppressed for the current shell', () => {
    const tip = getFeatureTipForModal({
      cliInstalled: false,
      modalData: { tipId: 'voice-dictation' },
      seenTipIds: [],
  ...
  it('returns no tip when every tip is already seen and no modal tip id is pinned', () => {
    const tip = getFeatureTipForModal({
      cliInstalled: false,
      modalData: {},
      seenTipIds: ['voice-dictation', 'pebble-cli', 'cmd-j-palette'],
      featureInteractions: {},
      settings: makeSettings()
    })
