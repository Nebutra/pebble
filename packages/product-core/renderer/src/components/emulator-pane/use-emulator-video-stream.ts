...
type EmulatorVideoApi = {
  ...
  onVideoStreamError?: (
    cb: (msg: { streamId: string; deviceId: string; message: string }) => void
  ) => () => void
}
...
export function useEmulatorVideoStream(
  ...
  useEffect(() => {
    ...
    unsubFrame = api.onVideoStreamFrame?.((msg) => {
      ...
      try {
        const parsedPts = msg.pts === undefined ? Number.NaN : Number(msg.pts)
        fallbackTimestamp += 1
        // scrcpy PTS values are already microseconds, which is WebCodecs'
        // timestamp unit; preserve them so decode pacing and diagnostics match
        // the native stream rather than inventing one tick per access unit.
        const timestamp =
          Number.isSafeInteger(parsedPts) && parsedPts >= 0 ? parsedPts : fallbackTimestamp
        decoder.decode(
          new ChunkCtor({
