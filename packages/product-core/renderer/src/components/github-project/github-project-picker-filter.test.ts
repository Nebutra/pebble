...
describe('github-project-picker-filter', () => {
  it('excludes pinned and recent projects from browse results', () => {
    const projects = [
      project('nebutra', 1, 'Pinned Project'),
      project('nebutra', 2, 'Recent Project'),
      project('nebutra', 3, 'Browse Project')
    ]

    expect(
      filterGitHubProjectPickerProjects({
        projects,
        pinned: [{ owner: 'nebutra', ownerType: 'organization', number: 1 }],
        recent: [
          {
            owner: 'nebutra',
  ...
  it('enforces the query budget by UTF-8 byte length', () => {
    ...
    expect(
      filterGitHubProjectPickerProjects({
        projects: [project('nebutra', 1, 'Roadmap')],
        pinned: [],
        recent: [],
        query
      })
