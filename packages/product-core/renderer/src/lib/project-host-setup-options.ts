...
/** A disconnected or failed host can be reconnected straight from the run-target row. */
...
function getHostSetupAvailability(host: ExecutionHostRegistryEntry): {
  ...
  if (host.health === 'blocked') {
    return {
      isAvailable: false,
      detail: 'Pebble server version is incompatible'
    }
  }
  if (host.kind === 'runtime') {
    ...
    if (
      ...
      return {
        isAvailable: false,
        detail: 'Update Pebble on this host to set up projects'
      }
    }
  }
  return {
