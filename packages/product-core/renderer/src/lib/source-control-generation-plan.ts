...
> = {
  commitMessage: {
    ...
    stagedFiles: 'M packages/product-core/example.ts',
    stagedPatch: 'diff --git a/packages/product-core/example.ts b/packages/product-core/example.ts'
  },
  pullRequest: {
    ...
    patch: 'diff --git a/packages/product-core/example.ts b/packages/product-core/example.ts'
  },
  branchName: {
...
export function planSourceControlTextGeneration(
  ...
  return {
    ...
    delivery,
    caveat:
      'This checks Pebble’s planner only. It does not invoke the CLI, prove PATH or binary availability, or reproduce main-process Windows .cmd resolution.'
  }
}
