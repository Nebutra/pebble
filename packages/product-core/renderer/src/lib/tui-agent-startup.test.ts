...
describe('buildAgentDraftLaunchPlan', () => {
  ...
  it('uses PEBBLE_PI_PREFILL env var for pi (no CLI flag exists)', () => {
    // Why: pi has no `--prefill` flag, and bracketed-paste-after-ready races
    // against pi's lengthy startup output. The Pebble overlay installs a
    // `pebble-prefill` extension that reads PEBBLE_PI_PREFILL on session_start
    ...
    expect(
      ...
      agent: 'pi',
      launchCommand: 'pi; unset PEBBLE_PI_PREFILL',
      expectedProcess: 'pi',
      env: { PEBBLE_PI_PREFILL: 'https://github.com/acme/repo/issues/42' },
