 ...
 * (chromium/packages/product-core/third_party/blink/renderer/modules/keyboard/keyboard.idl).
 ...
 * global shortcut) transfers focus out of Pebble and back, so focus-in is a
 * reliable proxy. The only missed case is a layout change triggered by a
 * key pressed while Pebble is focused (e.g. a Karabiner rule), which is
