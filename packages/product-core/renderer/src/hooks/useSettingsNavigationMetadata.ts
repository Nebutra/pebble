...
import { PebbleLogoSettingsIcon } from '@/components/settings/pebble-logo-settings-icon'
  ...
  return [
      ...
      : []),
    {
      ...
      description: translate(
        ...
        'Finish the onboarding checklist for core Pebble workflows.'
      ),
      icon: PebbleLogoSettingsIcon,
      searchEntries: [
        {
          title: translate(
            'auto.hooks.useSettingsNavigationMetadata.ded9e9032f',
    ...
    ...(showDesktopOnlySettings && !isTauri
      ? [
          {
            id: 'advanced',
...
export function useSettingsNavigationMetadata(): SettingsNavSection[] {
  ...
  const isTauri = isPebbleTauriShell()
  ...
  return useMemo(
    ...
    [isMac, isTauri, isWindows, isWindowsTerminalHost, isWebClient, repos, activeLocale]
  )
}
