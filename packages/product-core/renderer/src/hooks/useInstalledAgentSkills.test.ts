...
describe('hasInstalledAgentSkill', () => {
  it('matches installed skills by summarized name', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'pebble-cli' })], 'pebble-cli')).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkill(
        [
          skill({
            name: 'Pebble CLI',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\pebble-cli'
          })
        ],
        'pebble-cli'
      )
  ...
  it('ignores non-installed discovery entries', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'pebble-cli', installed: false })], 'pebble-cli')
    ).toBe(false)
  })
      ...
      hasInstalledAgentSkill(
        [
          skill({
            ...
            directoryPath: '/repo/.agents/skills/pebble-cli',
            skillFilePath: '/repo/.agents/skills/pebble-cli/SKILL.md'
          }),
          skill({
            id: 'skill-2',
            name: 'pebble-cli',
            ...
            directoryPath: '/Users/test/.codex/plugins/cache/vendor/pebble-cli',
            skillFilePath: '/Users/test/.codex/plugins/cache/vendor/pebble-cli/SKILL.md'
          })
        ],
        'pebble-cli',
  ...
  it('counts home skills when matching global installs', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'pebble-cli' })], 'pebble-cli', {
        sourceKinds: GLOBAL_AGENT_SKILL_SOURCE_KINDS
      })
    ).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkillNamed(
        [
          skill({
            name: 'Linear Tickets',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\pebble-linear'
          })
        ],
        ['pebble-linear', 'linear-tickets']
      )
  ...
  it('keeps aliases opt-in for unrelated single-name checks', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'linear-tickets' })], 'pebble-linear')).toBe(false)
  })
})
...
describe('discoverInstalledAgentSkills', () => {
  ...
  it('starts a fresh scan when a forced refresh arrives during a background scan', async () => {
    ...
    const freshResult = discoveryResult([skill({ name: 'pebble-cli' })])
