...
} from '../../../shared/native-chat-api-types'
...
const SESSION_STORAGE_KEY = 'pebble.web.workspaceSession.v1'
const ONBOARDING_STORAGE_KEY = 'pebble.web.onboarding.v1'
const GITHUB_CACHE_STORAGE_KEY = 'pebble.web.githubCache.v1'
const KEYBINDINGS_STORAGE_KEY = 'pebble.web.keybindings.v1'
...
export function installWebPreloadApi(): void {
  ...
  const webWindow = window as unknown as { __PEBBLE_WEB_CLIENT__?: boolean }
  webWindow.__PEBBLE_WEB_CLIENT__ = true
...
function createWebPreloadApi(): Partial<PreloadApi> {
  return {
    ...
    starNag: {
      ...
      starPebble: () => Promise.resolve(false),
...
function createRuntimeEnvironmentsApi(): NonNullable<Partial<PreloadApi>['runtimeEnvironments']> {
  return {
    ...
    addFromPairingCode: async ({ name, pairingCode }) => {
      ...
      if (!offer) {
        throw new Error('Invalid Pebble pairing code.')
      }
...
function createBrowserApi(): NonNullable<Partial<PreloadApi>['browser']> {
  return {
    ...
    onOpenLinkInPebbleTab: () => noopUnsubscribe,
...
function createGitHubApi(): WebGitHubApi {
  ...
  const githubApi = {
    ...
    checkPebbleStarred: () => Promise.resolve(null),
    starPebble: () => Promise.resolve(false),
    rateLimit: (args) =>
...
function createCliApi(): NonNullable<Partial<PreloadApi>['cli']> {
  ...
  const status = {
    ...
    detail: 'CLI registration is managed on the Pebble server, not in the web browser.'
  } as const
  return {
...
function createAgentHooksApi(): NonNullable<Partial<PreloadApi>['agentHooks']> {
  const status = (
    ...
    Promise.resolve({
      ...
      detail: 'Agent hook status is only available on the Pebble server.'
    } as const)
  return {
  ...
  return {
    ...
    openSetup: (args) =>
      callRuntimeResult<ComputerUsePermissionSetupResult>(
        ...
        nextStep: 'Computer-use permissions are managed on the Pebble server.'
      })),
    reset: () =>
      Promise.resolve({
        platform: getBrowserPlatform(),
...
function resolveEnvironment(selector: string): StoredWebRuntimeEnvironment {
  ...
  throw new Error(`Unknown Pebble runtime environment: ${selector}`)
}
...
function requireActiveEnvironment(): StoredWebRuntimeEnvironment {
  ...
  if (!activeEnvironment) {
    throw new Error('Pair this web client with an Pebble server first.')
  }
...
function getStoredOnboarding(): OnboardingState {
  ...
  // Why: pairing already means the user has an Pebble server. Desktop first-run
...
function mapRepoPathArg(args: unknown): unknown {
  ...
  return {
    ...
    // Pebble's repo id on task calls, so prefer the explicit selector.
