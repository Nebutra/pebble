...
describe('web viewport shell', () => {
  it('uses dynamic viewport height for document and app shell containers', () => {
    const css = readSource('packages/product-core/renderer/src/assets/main.css')
  ...
  it('uses dynamic viewport Tailwind utilities for web shell entry points', () => {
    const source = [
      readSource('packages/product-core/renderer/src/App.tsx'),
      readSource('packages/product-core/renderer/src/web/main.tsx'),
      readSource('packages/product-core/renderer/src/web/WebConnect.tsx')
    ].join('\n')
