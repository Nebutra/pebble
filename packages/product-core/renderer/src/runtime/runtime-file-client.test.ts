...
beforeEach(() => {
  delete (globalThis as { __PEBBLE_WEB_CLIENT__?: boolean }).__PEBBLE_WEB_CLIENT__
...
describe('runtime file client', () => {
  ...
  it('uploads staged local drops into the selected runtime environment', async () => {
    ...
    expect(smallWriteCall.params.relativePath).toMatch(
      /^uploads\/assets\/\.logo\.png\.pebble-upload-/
    )
  ...
  it('chunks large staged runtime uploads below the WebSocket frame budget', async () => {
    ...
    expect(chunkWriteCall.params.relativePath).toMatch(/^uploads\/\.large\.bin\.pebble-upload-/)
    expect(runtimeEnvironmentCall).toHaveBeenNthCalledWith(4, {
      selector: 'env-1',
  ...
  it('routes Tauri legacy SSH imports through the local Go relay runtime', async () => {
    ;(window as Window & { __PEBBLE_TAURI_SHELL__?: boolean }).__PEBBLE_TAURI_SHELL__ = true
    fsStageExternalPathsForRuntimeUpload.mockResolvedValue({
      sources: [
        {
          sourcePath: '/Users/me/note.txt',
          status: 'staged',
          name: 'note.txt',
          kind: 'file',
          entries: [{ relativePath: '', kind: 'file', contentBase64: 'bm90ZQ==' }]
        }
      ]
    })
    runtimeCall
      ...
      .mockResolvedValueOnce({ id: 'dest-create', ok: true, result: { ok: true } })
      .mockResolvedValueOnce({
        id: 'drops-stat',
        ok: false,
        error: { code: 'not_found', message: 'not found' }
      })
      .mockResolvedValueOnce({ id: 'drops-create', ok: true, result: { ok: true } })
      .mockResolvedValueOnce({
        id: 'file-stat',
        ok: false,
        error: { code: 'not_found', message: 'not found' }
      })
      .mockResolvedValueOnce({ id: 'write', ok: true, result: { ok: true } })
      .mockResolvedValueOnce({ id: 'commit', ok: true, result: { ok: true } })
      .mockResolvedValueOnce({ id: 'cleanup', ok: true, result: { ok: true } })
    ...
    await expect(
      importExternalPathsToRuntime(
        ...
        ['/Users/me/note.txt'],
        '/remote/repo/.pebble/drops'
      )
    ).resolves.toMatchObject({
      results: [{ status: 'imported', destPath: '/remote/repo/.pebble/drops/note.txt' }]
    })

    expect(runtimeCall).toHaveBeenCalledWith({
      method: 'files.createDir',
      params: { worktree: 'id:wt-ssh', relativePath: '.pebble' }
    })
    expect(runtimeCall).toHaveBeenCalledWith(
      expect.objectContaining({ method: 'files.writeBase64' })
    )
  ...
  it('removes a created runtime directory import root when a nested file upload fails', async () => {
    ...
    expect(writeCall.params.relativePath).toMatch(/^uploads\/assets\/\.logo\.png\.pebble-upload-/)
    expect(runtimeEnvironmentCall).toHaveBeenLastCalledWith({
      selector: 'env-1',
      method: 'files.delete',
