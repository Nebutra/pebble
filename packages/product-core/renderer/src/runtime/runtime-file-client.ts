...
export async function importExternalPathsToRuntime(
  ...
  const target = getActiveRuntimeTarget(context.settings)
  const legacySshRuntime =
    target.kind === 'local' && Boolean(context.connectionId) && isTauriDesktopShell()
  if (
    (!legacySshRuntime && target.kind !== 'environment') ||
    !context.worktreeId ||
  ...
  for (const source of staged.sources as StagedRuntimeImportSource[]) {
    ...
    try {
      ...
      if (createdDirectoryImportRoot) {
        ...
        // Pebble created when a nested runtime upload fails halfway through.
        await callRuntimeRpc(
          target,
          'files.delete',
          {
...
function makeRuntimeUploadTempPath(relativePath: string): string {
  ...
  return `${dir}.${leaf}.pebble-upload-${nonce}`
}
...
function isWebRuntimeFileWatchSharedSocket(): boolean {
  return Boolean((globalThis as { __PEBBLE_WEB_CLIENT__?: boolean }).__PEBBLE_WEB_CLIENT__)
}
...
function getRemoteFileArgs(
  ...
  const legacySshRuntime =
    target.kind === 'local' && Boolean(context.connectionId) && isTauriDesktopShell()
  if ((!legacySshRuntime && target.kind !== 'environment') || !context.worktreeId) {
    return null
  }
...
function hasRemoteRuntimeOwner(context: RuntimeFileOperationArgs): boolean {
  ...
  return Boolean(
    context.worktreeId &&
    (target.kind === 'environment' ||
      (target.kind === 'local' && context.connectionId && isTauriDesktopShell()))
  )
}

function isTauriDesktopShell(): boolean {
  return Boolean((window as Window & { __PEBBLE_TAURI_SHELL__?: boolean }).__PEBBLE_TAURI_SHELL__)
}
