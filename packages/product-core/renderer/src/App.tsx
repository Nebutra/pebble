...
import { getTauriUnavailableFeatureTipIds } from './components/feature-tips/tauri-unavailable-feature-tips'
import {
...
const StatusBar = lazy(() =>
  import('./components/status-bar/StatusBar').then((module) => ({
...
function App(): React.JSX.Element {
  ...
  const settingsVisibleRef = useRef(useAppStore.getState().settingsPageOpen)
  useEffect(
    () =>
      useAppStore.subscribe((state) => {
        settingsVisibleRef.current = state.settingsPageOpen
      }),
    []
  )
  const renderedAppView = activeView
  ...
  const creationLayoutActive = shouldShowWorktreeCreationSurface({
    activeView: renderedAppView,
    activePendingCreationId,
    hasActivePendingCreation: activePendingCreationExists
  })
  const workspaceChromeActive =
    renderedAppView === 'terminal' && activeWorktreeId !== null && !creationLayoutActive
  ...
  const onboardingSettingsDetourActive =
    onboardingSettingsDetour && settingsVisibleRef.current && shouldRenderOnboarding
  ...
  useEffect(() => {
    const featureTipsDecision = getFeatureTipsAppOpenDecision({
      ...
      settings,
      suppressedTipIds: getTauriUnavailableFeatureTipIds(),
      ...
      try {
        ...
        if (!cancelled) {
          ...
          if (connectionIds.length > 0) {
            ...
            logRendererStartupDiagnostic('ssh-reconnect-skipped', {
              connectionIds: 0
            })
          }
    ...
    return createSessionWriteSubscriber({
      ...
      persist: ({ patch }) => {
        ...
        if (hydratedTargetIds.length > 0) {
          void localWrite
            .then(() =>
              window.api.remoteWorkspace?.setForConnectedTargets({
                hydratedTargetIds
              })
            )
            .then((results) => {
  ...
  useEffect(() => {
    ...
    const captureAndFlush = (): void => {
      ...
      for (const capture of shutdownBufferCaptures.values()) {
        try {
          // Why: Electron moves local scrollback into sync snapshot files;
          // Tauri persists the already-bounded buffers in its native session document.
          capture({
            includeLocalBuffers: Boolean(
              (window as Window & { __PEBBLE_TAURI_SHELL__?: boolean }).__PEBBLE_TAURI_SHELL__
            )
          })
        } catch {
  ...
  const showSidebar =
    renderedAppView !== 'activity' && renderedAppView !== 'space' && renderedAppView !== 'skills'
  ...
  const showRightSidebarControls =
    !creationLayoutActive && canShowRightSidebarForView(renderedAppView)
  ...
  useEffect(() => {
    ...
    const dispatchShortcutInput = (input: ShortcutDispatchInput): void => {
      ...
      const notifyTerminalCapture = (actionId: KeybindingActionId): void => {
        if (
          context !== 'terminal' ||
          (terminalShortcutPolicy ?? 'pebble-first') !== 'pebble-first'
        ) {
          return
        }
      ...
      if (matchShortcut('workspace.openBoard') && !useAppStore.getState().settingsPageOpen) {
        input.preventDefault()
      ...
      if (matchShortcut('view.tasks') && !useAppStore.getState().settingsPageOpen) {
        const store = useAppStore.getState()
          ...
          <LinkRoutingPreferenceDialogProvider>
            ...
            >
              <div className="relative flex flex-row flex-1 min-h-0 overflow-hidden">
                <SettingsOverlay />
                {/* Why: Settings fully occludes the workbench. Retaining the app
                on its own compositor layer avoids a full-window repaint on return. */}
                <div className="app-workbench-layer flex flex-row flex-1 min-w-0 min-h-0 overflow-hidden">
                ...
                page keep the titlebar. */}
                    ...
                    a few pixels, which reads as layout jitter. */}
                          ...
                          <div className="flex flex-1 min-w-0 min-h-0 flex-col">
                            ...
                            <Suspense fallback={null}>
                              <RecoverableRenderErrorBoundary
                                boundaryId={`page.${renderedAppView}`}
                                surface="page"
                                resetKey={renderedAppView}
                              ...
                              >
                                {renderedAppView === 'skills' ? <SkillsPage /> : null}
                                {renderedAppView === 'tasks' ? <TaskPage /> : null}
                                {renderedAppView === 'automations' ? <AutomationsPage /> : null}
                                {renderedAppView === 'activity' ? <ActivityPrototypePage /> : null}
                                {renderedAppView === 'space' ? <WorkspaceSpacePage /> : null}
                                {renderedAppView === 'mobile' ? <MobilePage /> : null}
                                {renderedAppView === 'terminal' &&
                                creationLayoutActive &&
                                activePendingCreationId ? (
