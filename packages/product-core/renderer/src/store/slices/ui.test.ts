...
describe('createUISlice settings navigation', () => {
  ...
  it('keeps the tasks page mounted while settings is open', () => {
    const store = createUIStore()
    ...
    expect(store.getState().settingsPageOpen).toBe(true)

    store.getState().closeSettingsPage()

    expect(store.getState().activeView).toBe('tasks')
    expect(store.getState().settingsPageOpen).toBe(false)
  })

  it('keeps the workbench route stable when settings is reopened', () => {
    const store = createUIStore()
