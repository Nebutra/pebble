...
async function assertProjectHostSetupRuntimeCapability(
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    PROJECT_HOST_SETUP_RUNTIME_CAPABILITY,
    'The selected Pebble server does not support project host setup yet. Update Pebble on the server and try again.',
    15_000
  )
}
...
async function assertProjectHostSetupMutationRuntimeCapabilities(
  ...
  await assertRuntimeEnvironmentCapability(
    target.environmentId,
    WORKSPACE_RUN_CONTEXT_RUNTIME_CAPABILITY,
    'The selected Pebble server does not support explicit workspace run hosts yet. Update Pebble on the server and try again.',
    15_000
  )
}
...
async function fetchRuntimeAddProjectPathStatus(args: {
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    translate(
      ...
      'Update Pebble server to open non-Git folders on this runtime.'
    ),
    15_000
  )
  try {
...
export const createRepoSlice: StateCreator<AppState, [], [], RepoSlice> = (set, get) => ({
    ...
    for (const projectId of targets.projectIds) {
      ...
      if (stillExists) {
        failedProjectRemovals.push({
          projectId,
          reason: 'Project remained in Pebble after removeProject completed.'
        })
      } else {
  ...
  addRepoPath: async (path, kind = 'git', options) => {
    try {
      ...
      if (alreadyAdded) {
        get().clearPebbleHookTrustForRepo(repo.id)
      }
      set((s) => {
  ...
  removeProject: async (projectId) => {
    try {
      ...
      get().clearPebbleHookTrustForRepo(projectId)
      ...
      set((s) => {
        ...
        return {
          ...
          // Why: removing the last repo should reveal Landing rather than leave
          // a retained Settings overlay above an empty workbench.
          ...(nextRepos.length === 0
            ? {
