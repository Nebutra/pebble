...
describe('renderer startup runtime routing', () => {
  it('loads settings before repo and worktree hydration', () => {
    const source = readFileSync(
      join(process.cwd(), 'packages/product-core/renderer/src/App.tsx'),
      'utf8'
    )
  ...
  it('keeps crash-report listeners eager while lazy-loading the dialog surface', () => {
    ...
    const hostSource = readFileSync(
      join(
        process.cwd(),
        'packages/product-core/renderer/src/components/crash-report/CrashReportDialog.tsx'
      ),
      'utf8'
    )
  ...
  it('clears stale crash-report state before opening the lazy manual report surface', () => {
    ...
    const manualOpenStart = hostSource.indexOf('const openLatestCrashReport = (): void => {')
    const manualOpenEnd = hostSource.indexOf('\n    }', manualOpenStart)
  ...
  it('keeps the new-workspace composer eager because it is a critical create surface', () => {
    ...
    const lazyModalSource = readFileSync(
      join(process.cwd(), 'packages/product-core/renderer/src/lazy-modal-mount-state.ts'),
      'utf8'
    )
  ...
  it('does not eagerly import inactive sidebar dialog flows on startup', () => {
    ...
    const sidebarSource = readFileSync(
      join(process.cwd(), 'packages/product-core/renderer/src/components/sidebar/index.tsx'),
      'utf8'
    )
  ...
  it('does not eagerly import optional status-bar segments on startup', () => {
    const source = readFileSync(
      join(process.cwd(), 'packages/product-core/renderer/src/components/status-bar/StatusBar.tsx'),
      'utf8'
    )
