...
// (PEBBLE_BUILD_IDENTITY='stable' | 'rc', PEBBLE_POSTHOG_WRITE_KEY=phc_...);
...
const pebbleBuildIdentity = process.env.PEBBLE_BUILD_IDENTITY
const PEBBLE_BUILD_IDENTITY_LITERAL =
  pebbleBuildIdentity === 'stable' || pebbleBuildIdentity === 'rc'
    ? JSON.stringify(pebbleBuildIdentity)
    : 'null'
const pebblePostHogWriteKey = process.env.PEBBLE_POSTHOG_WRITE_KEY
const PEBBLE_POSTHOG_WRITE_KEY_LITERAL =
  typeof pebblePostHogWriteKey === 'string' && pebblePostHogWriteKey.length > 0
    ? JSON.stringify(pebblePostHogWriteKey)
    : 'null'
const pebbleDiagnosticsTokenUrl = process.env.PEBBLE_DIAGNOSTICS_TOKEN_URL
const PEBBLE_DIAGNOSTICS_TOKEN_URL_LITERAL =
  typeof pebbleDiagnosticsTokenUrl === 'string' && pebbleDiagnosticsTokenUrl.length > 0
    ? JSON.stringify(pebbleDiagnosticsTokenUrl)
    : 'null'
...
;(() => {
  ...
  const mode = env?.PEBBLE_STARTUP_DIAGNOSTICS
  ...
  const diagnosticFile = env?.PEBBLE_STARTUP_DIAGNOSTICS_FILE
  ...
  if (!globalThis.__PEBBLE_BOOTSTRAP_EXIT_LOG_INSTALLED__) {
    globalThis.__PEBBLE_BOOTSTRAP_EXIT_LOG_INSTALLED__ = true
  ...
  if (mode === 'trace' && !globalThis.__PEBBLE_BOOTSTRAP_REQUIRE_TRACE_INSTALLED__) {
    globalThis.__PEBBLE_BOOTSTRAP_REQUIRE_TRACE_INSTALLED__ = true
    try {
      const Module = require('node:module')
      const originalLoad = Module._load
      const parsedTraceLimit = Number(env?.PEBBLE_STARTUP_DIAGNOSTICS_TRACE_LIMIT ?? 20000)
...
function createStartupDiagnosticsBootstrapPlugin() {
  return {
    name: 'pebble-startup-diagnostics-bootstrap',
...
export default defineConfig({
  main: {
    build: {
      ...
      rollupOptions: {
        input: {
          ...
          // this path for `pebble agent hooks ...`, so it must survive rebuilds.
    ...
    define: {
      PEBBLE_BUILD_IDENTITY: PEBBLE_BUILD_IDENTITY_LITERAL,
      PEBBLE_POSTHOG_WRITE_KEY: PEBBLE_POSTHOG_WRITE_KEY_LITERAL,
      PEBBLE_DIAGNOSTICS_TOKEN_URL: PEBBLE_DIAGNOSTICS_TOKEN_URL_LITERAL
    },
