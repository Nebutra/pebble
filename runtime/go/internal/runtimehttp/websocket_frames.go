...
func (c *websocketConn) readText(requireMasked bool) (string, error) {
	opcode, payload, err := c.readMessage(requireMasked)
	if err != nil {
		return "", err
	}
	if opcode != 0x1 {
		return "", errors.New("expected websocket text message")
	}
	return string(payload), nil
}

func (c *websocketConn) readMessage(requireMasked bool) (byte, []byte, error) {
	var message []byte
	...
	for {
		...
		case 0x1, 0x2:
			if message != nil {
				return 0, nil, errors.New("websocket data frame interrupted fragmented message")
			}
			if fin {
...
func (c *websocketConn) writeFrame(opcode byte, payload []byte) error {
	...
	// Why: writes are synchronous, so at most one frame is queued per socket;
	// a deadline bounds slow-client backpressure without an unbounded buffer.
