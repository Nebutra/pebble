...
func (s *Server) handleProjectGroupImportRemoteNested(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if errors.Is(err, runtimecore.ErrRemoteNestedScanRequired) ||
			errors.Is(err, runtimecore.ErrRemoteNestedScanIDRequired) ||
			errors.Is(err, runtimecore.ErrRemoteNestedScanMismatch) ||
			errors.Is(err, runtimecore.ErrRemoteNestedScanIncomplete) {
