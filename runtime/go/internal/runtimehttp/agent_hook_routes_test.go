...
func TestAgentHookIngestRouteUpdatesSessionHookState(t *testing.T) {
	...
	for _, step := range []struct {
		event string
		state runtimecore.SessionHookState
	}{
		{"UserPromptSubmit", runtimecore.SessionHookWorking},
		{"PermissionRequest", runtimecore.SessionHookPermission},
		{"PreToolUse", runtimecore.SessionHookWorking},
		{"PermissionRequest", runtimecore.SessionHookPermission},
		{"PostToolUse", runtimecore.SessionHookWorking},
		{"Stop", runtimecore.SessionHookIdle},
	} {
		form := url.Values{}
		form.Set("launchToken", "hook-launch-token")
		form.Set("paneKey", "tab-1:leaf-1")
		form.Set("payload", `{"hook_event_name":"`+step.event+`"}`)
		...
		if rec.Code != http.StatusOK {
			t.Fatalf("%s: expected 200, got %d: %s", step.event, rec.Code, rec.Body.String())
		}
		var result runtimecore.AgentHookIngestResult
		if err := json.Unmarshal(rec.Body.Bytes(), &result); err != nil {
			t.Fatal(err)
		}
		if !result.Accepted || result.SessionID != session.ID || result.State != step.state {
			t.Fatalf("%s: expected accepted %q ingest for %s, got %#v", step.event, step.state, session.ID, result)
		}
		...
		if snapshot.HookAgentState != step.state {
			t.Fatalf("%s: expected hook state %q, got %q", step.event, step.state, snapshot.HookAgentState)
		}
	}
}
...
func TestAutomationRunDispatchResultRoute(t *testing.T) {
	...
	body := strings.NewReader(`{"status":"completed","workspaceId":"ws-1","terminalSessionId":"sess-1","outputSnapshot":{"format":"plain_text","content":"Real terminal output","capturedAt":1768000000000,"truncated":false}}`)
	rec := httptest.NewRecorder()
	...
	if updated.DispatchState == nil || updated.DispatchState.WorkspaceID != "ws-1" ||
		...
		updated.DispatchState.OutputSnapshot == nil ||
		updated.DispatchState.OutputSnapshot.Content != "Real terminal output" {
