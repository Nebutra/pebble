package providercli

import (
	"context"
	"fmt"
	"os"
	"path/filepath"
	"strings"
	"testing"
)

func stringPtr(s string) *string { return &s }
func boolPtr(value bool) *bool   { return &value }

func recordingReviewCLI(t *testing.T, name string) (string, string) {
	t.Helper()
	dir := fakeCLIStub(t, name, "", 0)
	logPath := filepath.Join(t.TempDir(), "review-update-args.log")
	script := fmt.Sprintf("#!/bin/sh\nprintf '%%s\\n' \"$*\" >> %q\n", logPath)
	if err := os.WriteFile(filepath.Join(dir, name), []byte(script), 0o755); err != nil {
		t.Fatalf("write recording %s: %v", name, err)
	}
	return dir, logPath
}

func recordedReviewCLIArgs(t *testing.T, logPath string) []string {
	t.Helper()
	contents, err := os.ReadFile(logPath)
	if err != nil {
		t.Fatalf("read recorded review CLI args: %v", err)
	}
	return strings.Split(strings.TrimSpace(string(contents)), "\n")
}

func TestUpdateGitHubPullRequestEditsTitleAndBody(t *testing.T) {
	dir := fakeCLIStub(t, "gh", "", 0)
	withPath(t, dir)

	result := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "github",
		Number:   42,
		Title:    stringPtr("New title"),
		Body:     stringPtr("New body"),
	})
	if !result.OK {
		t.Fatalf("expected update to succeed, got %+v", result)
	}
}

func TestUpdateGitHubPullRequestRejectsMissingNumber(t *testing.T) {
	result := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "github",
		Title:    stringPtr("New title"),
	})
	if result.OK || result.Code != "validation" {
		t.Fatalf("expected validation error for missing number, got %+v", result)
	}
}

func TestUpdateGitHubPullRequestRejectsEmptyTitle(t *testing.T) {
	result := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "github",
		Number:   1,
		Title:    stringPtr("   "),
	})
	if result.OK || result.Code != "validation" {
		t.Fatalf("expected validation error for blank title, got %+v", result)
	}
}

func TestUpdateGitHubPullRequestClosesAndReopens(t *testing.T) {
	dir := fakeCLIStub(t, "gh", "", 0)
	withPath(t, dir)

	closed := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "github",
		Number:   7,
		State:    "closed",
	})
	if !closed.OK {
		t.Fatalf("expected close to succeed, got %+v", closed)
	}

	reopened := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "github",
		Number:   7,
		State:    "open",
	})
	if !reopened.OK {
		t.Fatalf("expected reopen to succeed, got %+v", reopened)
	}
}

func TestUpdateGitHubPullRequestRejectsInvalidState(t *testing.T) {
	result := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "github",
		Number:   7,
		State:    "merged",
	})
	if result.OK || result.Code != "validation" {
		t.Fatalf("expected validation error for unsupported state, got %+v", result)
	}
}

func TestUpdateGitHubPullRequestAddsAndRemovesReviewers(t *testing.T) {
	dir := fakeCLIStub(t, "gh", "", 0)
	withPath(t, dir)

	added := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider:     "github",
		Number:       3,
		AddReviewers: []string{"octocat", " hubot "},
	})
	if !added.OK {
		t.Fatalf("expected add-reviewers to succeed, got %+v", added)
	}

	removed := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider:        "github",
		Number:          3,
		RemoveReviewers: []string{"octocat"},
	})
	if !removed.OK {
		t.Fatalf("expected remove-reviewers to succeed, got %+v", removed)
	}
}

func TestUpdateGitHubPullRequestRetargetsAndMarksDraft(t *testing.T) {
	dir, logPath := recordingReviewCLI(t, "gh")
	withPath(t, dir)

	result := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "github", Number: 42, Owner: "upstream", Repo: "project",
		Base: stringPtr(" release/next "), Draft: boolPtr(true),
	})
	if !result.OK {
		t.Fatalf("expected retarget and draft update to succeed, got %+v", result)
	}
	want := []string{
		"pr edit 42 --base release/next --repo upstream/project",
		"pr ready 42 --undo --repo upstream/project",
	}
	if got := recordedReviewCLIArgs(t, logPath); strings.Join(got, "|") != strings.Join(want, "|") {
		t.Fatalf("unexpected GitHub commands: got %#v want %#v", got, want)
	}
}

func TestUpdateGitHubPullRequestMarksReady(t *testing.T) {
	dir, logPath := recordingReviewCLI(t, "gh")
	withPath(t, dir)

	result := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "github", Number: 42, Draft: boolPtr(false),
	})
	if !result.OK {
		t.Fatalf("expected ready update to succeed, got %+v", result)
	}
	if got := recordedReviewCLIArgs(t, logPath); len(got) != 1 || got[0] != "pr ready 42" {
		t.Fatalf("unexpected GitHub ready command: %#v", got)
	}
}

func TestUpdateGitHubPullRequestClassifiesAuthFailure(t *testing.T) {
	dir := fakeCLIStub(t, "gh", "gh auth login required: not authenticated", 1)
	withPath(t, dir)

	result := UpdateGitHubPullRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "github",
		Number:   1,
		Title:    stringPtr("New title"),
	})
	if result.OK || result.Code != "auth_required" {
		t.Fatalf("expected auth-required result, got %+v", result)
	}
}

func TestUpdateGitLabMergeRequestEditsTitleAndBody(t *testing.T) {
	dir := fakeCLIStub(t, "glab", "", 0)
	withPath(t, dir)

	result := UpdateGitLabMergeRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "gitlab",
		Number:   17,
		Title:    stringPtr("New title"),
		Body:     stringPtr("New body"),
	})
	if !result.OK {
		t.Fatalf("expected update to succeed, got %+v", result)
	}
}

func TestUpdateGitLabMergeRequestClosesAndReopens(t *testing.T) {
	dir := fakeCLIStub(t, "glab", "", 0)
	withPath(t, dir)

	closed := UpdateGitLabMergeRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "gitlab",
		Number:   5,
		State:    "closed",
	})
	if !closed.OK {
		t.Fatalf("expected close to succeed, got %+v", closed)
	}

	reopened := UpdateGitLabMergeRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "gitlab",
		Number:   5,
		State:    "open",
	})
	if !reopened.OK {
		t.Fatalf("expected reopen to succeed, got %+v", reopened)
	}
}

func TestUpdateGitLabMergeRequestTreatsAlreadyInStateAsSuccess(t *testing.T) {
	dir := fakeCLIStub(t, "glab", "merge request is already closed", 1)
	withPath(t, dir)

	result := UpdateGitLabMergeRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "gitlab",
		Number:   5,
		State:    "closed",
	})
	if !result.OK {
		t.Fatalf("expected already-closed to be treated as success, got %+v", result)
	}
}

func TestUpdateGitLabMergeRequestReplacesReviewers(t *testing.T) {
	dir := fakeCLIStub(t, "glab", `{"reviewers":[{"id":7,"username":"tanuki","name":"Tanuki","avatar_url":"https://example/avatar.png","state":"active"}]}`, 0)
	withPath(t, dir)
	reviewerIDs := []int{7}
	result := UpdateGitLabMergeRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "gitlab", Number: 5, ReviewerIDs: &reviewerIDs,
	})
	if !result.OK || len(result.Reviewers) != 1 || result.Reviewers[0].Username != "tanuki" {
		t.Fatalf("expected mapped reviewer response, got %+v", result)
	}
}

func TestUpdateGitLabMergeRequestClearsReviewers(t *testing.T) {
	dir := fakeCLIStub(t, "glab", `{"reviewers":[]}`, 0)
	withPath(t, dir)
	reviewerIDs := []int{}
	result := UpdateGitLabMergeRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "gitlab", Number: 5, ReviewerIDs: &reviewerIDs,
	})
	if !result.OK || len(result.Reviewers) != 0 {
		t.Fatalf("expected reviewer clear to succeed, got %+v", result)
	}
}

func TestUpdateGitLabMergeRequestRetargetsAndMarksReady(t *testing.T) {
	dir, logPath := recordingReviewCLI(t, "glab")
	withPath(t, dir)

	result := UpdateGitLabMergeRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "gitlab", Number: 17, Base: stringPtr(" release/next "), Draft: boolPtr(false),
	})
	if !result.OK {
		t.Fatalf("expected retarget and ready update to succeed, got %+v", result)
	}
	if got := recordedReviewCLIArgs(t, logPath); len(got) != 1 || got[0] != "mr update 17 --target-branch release/next --ready --yes" {
		t.Fatalf("unexpected GitLab update command: %#v", got)
	}
}

func TestUpdateGitLabMergeRequestMarksDraft(t *testing.T) {
	dir, logPath := recordingReviewCLI(t, "glab")
	withPath(t, dir)

	result := UpdateGitLabMergeRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "gitlab", Number: 17, Draft: boolPtr(true),
	})
	if !result.OK {
		t.Fatalf("expected draft update to succeed, got %+v", result)
	}
	if got := recordedReviewCLIArgs(t, logPath); len(got) != 1 || got[0] != "mr update 17 --draft --yes" {
		t.Fatalf("unexpected GitLab draft command: %#v", got)
	}
}

func TestUpdateGitLabMergeRequestRejectsMissingNumber(t *testing.T) {
	result := UpdateGitLabMergeRequest(context.Background(), t.TempDir(), UpdateReviewRequest{
		Provider: "gitlab",
		Title:    stringPtr("New title"),
	})
	if result.OK || result.Code != "validation" {
		t.Fatalf("expected validation error for missing number, got %+v", result)
	}
}
