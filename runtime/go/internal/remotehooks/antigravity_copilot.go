...
func containsManagedScript(command, script string) bool {
	...
	needle := "agent-hooks/" + script
	if len(normalized) > 0 && filepath.Base(script) != "" && contains(normalized, needle) {
		return true
	}
	// Windows Git-Bash-safe launchers encode the exact PowerShell invocation.
	// Decode only the final bounded token so stale Pebble entries still converge.
	fields := strings.Fields(command)
	if len(fields) == 0 || !strings.Contains(command, "EncodedCommand") || len(fields[len(fields)-1]) > 32<<10 {
		return false
	}
	...
	for index := 0; index < len(encoded); index += 2 {
		words = append(words, uint16(encoded[index])|uint16(encoded[index+1])<<8)
	}
	decoded := strings.ReplaceAll(string(utf16.Decode(words)), `\`, "/")
	return contains(decoded, needle)
}
