...
func TestInstallAllWritesGeminiCursorAndDroidSchemas(t *testing.T) {
	...
	expectedAgents := map[string]bool{
		"claude": true, "openclaude": true, "gemini": true, "cursor": true,
		"droid": true, "amp": true, "antigravity": true, "copilot": true,
		"grok": true, "command-code": true, "hermes": true, "devin": true,
		"kimi": true, "codex": true,
	}
	for _, status := range statuses {
		if status.State != "installed" {
			t.Fatalf("unexpected status: %+v", status)
		}
		if !expectedAgents[status.Agent] {
			t.Fatalf("unexpected or duplicate agent status: %+v", status)
		}
		delete(expectedAgents, status.Agent)
	}
	if len(expectedAgents) != 0 {
		t.Fatalf("missing remote managed-hook agents: %+v", expectedAgents)
	}
