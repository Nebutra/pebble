...
func InstallAll(home string) []InstallStatus {
	return InstallAllForPlatform(home, runtime.GOOS)
}

// InstallAllForPlatform keeps target-platform selection explicit for SSH
// installs: the relay worker's GOOS, rather than the Pebble client's GOOS,
// determines which provider-native hook payload is written.
func InstallAllForPlatform(home, goos string) []InstallStatus {
	if goos == "windows" {
		return installAllWindows(home)
	}
