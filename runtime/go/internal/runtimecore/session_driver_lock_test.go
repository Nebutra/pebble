...
func TestMobileFitPreservesDesktopBaselineAcrossRefits(t *testing.T) {
	manager := newDriverLockTestManager(t)
	session := startDriverLockTestSession(t, manager)
	baseline, err := manager.ResizeSession(session.ID, SessionResizeRequest{Cols: 132, Rows: 43})
	if err != nil {
		t.Fatal(err)
	}
	if _, override, err := manager.ApplyMobileSessionFit(session.ID, "phone-1", 72, 22); err != nil {
		t.Fatal(err)
	} else if override.PreviousCols != baseline.Cols || override.PreviousRows != baseline.Rows {
		t.Fatalf("unexpected desktop baseline: %+v", override)
	}
	if _, override, err := manager.ApplyMobileSessionFit(session.ID, "phone-1", 68, 20); err != nil {
		t.Fatal(err)
	} else if override.PreviousCols != baseline.Cols || override.PreviousRows != baseline.Rows {
		t.Fatalf("repeat fit replaced desktop baseline: %+v", override)
	}
	reclaimed, err := manager.ReclaimSessionFitForDesktop(session.ID)
	if err != nil || !reclaimed {
		t.Fatalf("expected desktop reclaim, reclaimed=%v err=%v", reclaimed, err)
	}
	restored, err := manager.SessionStatus(session.ID)
	if err != nil || restored.Cols != baseline.Cols || restored.Rows != baseline.Rows {
		t.Fatalf("desktop size was not restored: %+v, %v", restored, err)
	}
	if _, exists := manager.GetSessionFitOverride(session.ID); exists {
		t.Fatal("fit override survived desktop reclaim")
	}
}
