...
func TestRecordGitBaseStatusPersistsProjectionAndEmitsContracts(t *testing.T) {
	...
	worktree, err := manager.CreateWorktree(context.Background(), CreateWorktreeRequest{
		ProjectID: project.ID, Path: t.TempDir(), Branch: "feature", Base: "origin/main",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	result := GitBaseStatusResult{
		Status: "drift", Base: "origin/main", Remote: "origin", Behind: 2,
		RecentSubjects: []string{"newer"},
		Conflict:       &GitRemoteBranchConflict{Remote: "fork", BranchName: "feature"},
	}
	if err := manager.recordGitBaseStatus(GitBaseStatusRequest{
		ProjectID: project.ID, WorktreeID: worktree.ID, BaseRef: "origin/main",
	}, result); err != nil {
		t.Fatal(err)
	}
	projection, ok := manager.cachedSourceControlProjection(project.ID, worktree.ID)
	if !ok || projection.BaseStatus == nil || projection.BaseStatus.Behind != 2 {
		t.Fatalf("base status was not persisted in projection: %#v", projection)
	}
	wantTopics := map[string]bool{
		"source-control.changed":                  false,
		"worktree.base-status.changed":            false,
		"worktree.remote-branch-conflict.changed": false,
	}
	deadline := time.After(time.Second)
	for remaining := len(wantTopics); remaining > 0; {
		select {
		case event := <-events:
			if seen, tracked := wantTopics[event.Topic]; tracked && !seen {
				wantTopics[event.Topic] = true
				remaining--
			}
		case <-deadline:
			t.Fatalf("missing runtime contracts: %#v", wantTopics)
		}
	}
}
