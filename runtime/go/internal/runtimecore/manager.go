...
type Manager struct {
	...
	projectHostSetups        map[string]ProjectHostSetup
	projectGroups            map[string]ProjectGroup
	folderWorkspaces         map[string]FolderWorkspace
	worktrees                map[string]Worktree
	preservedBranchCleanup   map[string]PreservedBranchCleanupTicket
	...
	terminalArtifactGrants   map[string]SshTerminalArtifactGrant
	...
	browserDrivers           map[string]BrowserDriverState
	browserProfiles          map[string]BrowserProfile
	sparsePresets            map[string][]SparsePreset
	...
	sshRelayWorkers          map[string]sshRelayWorkerDeployment
	sshRelayWorkerMu         sync.Mutex
	...
	localhostLabelForwards  map[string]SshPortForwardEntry
	localhostLabelForwardMu sync.Mutex
	sessionTabLayouts       map[string]SessionTabLayout
	sessions                map[string]*processSession
	sessionDrivers          map[string]SessionDriverState
	sessionFitOverrides     map[string]SessionFitOverride
	stats                   StatsSummary
	...
	workspaceSpaceMu      sync.Mutex
	workspaceSpaceCancel  context.CancelFunc
	ephemeralVMMu         sync.Mutex
	ephemeralVMCancels    map[string]context.CancelFunc
	textGenerationMu      sync.Mutex
	textGenerationCancels map[string]textGenerationCancellation
}
...
func NewManager(dataDir string, unavailableTools []string) (*Manager, error) {
	...
	manager := &Manager{
		...
		projectHostSetups:        make(map[string]ProjectHostSetup),
		projectGroups:            make(map[string]ProjectGroup),
		folderWorkspaces:         make(map[string]FolderWorkspace),
		worktrees:                make(map[string]Worktree),
		preservedBranchCleanup:   make(map[string]PreservedBranchCleanupTicket),
		...
		automationDispatchTokens: make(map[string]automationDispatchTokenRecord),
		...
		terminalArtifactGrants:   make(map[string]SshTerminalArtifactGrant),
		...
		browserDrivers:           make(map[string]BrowserDriverState),
		browserProfiles:          make(map[string]BrowserProfile),
		sparsePresets:            make(map[string][]SparsePreset),
		...
		sshRelayWorkers:          make(map[string]sshRelayWorkerDeployment),
		sshPortForwards:          make(map[string]*sshPortForwardProcess),
		localhostLabelForwards:   make(map[string]SshPortForwardEntry),
		sessionTabLayouts:        make(map[string]SessionTabLayout),
		sessions:                 make(map[string]*processSession),
		sessionDrivers:           make(map[string]SessionDriverState),
		sessionFitOverrides:      make(map[string]SessionFitOverride),
		stats:                    state.Stats,
		ephemeralVMCancels:       make(map[string]context.CancelFunc),
		textGenerationCancels:    make(map[string]textGenerationCancellation),
		accountsSnapshot:         append(json.RawMessage(nil), state.AccountsSnapshot...),
		legacySharedControl:      state.LegacySharedControl,
		liveAgentStats:           make(map[string]time.Time),
	...
	for _, preset := range state.SparsePresets {
		manager.sparsePresets[preset.RepoID] = append(manager.sparsePresets[preset.RepoID], preset)
	}
	for _, setup := range state.ProjectHostSetups {
		manager.projectHostSetups[setup.ID] = setup
	}
	...
	for _, ticket := range state.PreservedBranchCleanup {
		manager.preservedBranchCleanup[ticket.WorktreeID] = ticket
	}
...
func (m *Manager) CreateProject(req CreateProjectRequest) (Project, error) {
	...
	project := Project{
		...
		LogicalProjectID:       strings.TrimSpace(req.LogicalProjectID),
		WorktreeBasePath:       strings.TrimSpace(req.WorktreeBasePath),
		GitUsername:            strings.TrimSpace(req.GitUsername),
		IssueSourcePreference:  normalizeIssueSourcePreference(req.IssueSourcePreference),
		ProjectHostSetupMethod: normalizeProjectHostSetupMethod(req.ProjectHostSetupMethod, "imported-existing-folder"),
		CreatedAt:              now,
		UpdatedAt:              now,
	}
	m.mu.Lock()
...
func (m *Manager) CloneProject(ctx context.Context, req CloneProjectRequest) (Project, error) {
	if strings.TrimSpace(req.HostID) != "" {
		return m.cloneSshProject(ctx, req)
	}
	...
	output := readCloneProgress(stderr, func(phase string, percent int) {
		m.emit("project.cloneProgress", map[string]interface{}{
	...
	return m.CreateProjectWithMainWorktree(ctx, CreateProjectRequest{
...
func (m *Manager) CancelClone() bool {
	m.cloneMu.Lock()
	cancel := m.cloneCancel
	m.cloneMu.Unlock()
	if cancel == nil {
		return false
	}
...
func (m *Manager) beginClone(cancel context.CancelFunc) bool {
	m.cloneMu.Lock()
	defer m.cloneMu.Unlock()
...
func (m *Manager) finishClone() {
	m.cloneMu.Lock()
	defer m.cloneMu.Unlock()
	// Why: a future clone may start immediately after this one exits; only the
	// currently registered cancellation slot may be cleared.
	if m.cloneCancel != nil {
		m.cloneCancel = nil
	}
}

func readCloneProgress(reader io.Reader, emit func(string, int)) string {
	...
	for scanner.Scan() {
		...
		if phase, percent, ok := parseCloneProgress(line); ok {
			emit(phase, percent)
		}
	}
...
func parseCloneProgress(line string) (string, int, bool) {
...
func (m *Manager) UpdateProject(id string, req UpdateProjectRequest) (Project, error) {
	...
	if logicalProjectID := strings.TrimSpace(req.LogicalProjectID); logicalProjectID != "" {
		project.LogicalProjectID = logicalProjectID
	}
	applyOptionalString(req.WorktreeBasePath, &project.WorktreeBasePath)
	applyOptionalString(req.GitUsername, &project.GitUsername)
	if req.IssueSourcePreference != nil {
		preference, err := validateIssueSourcePreference(*req.IssueSourcePreference)
		if err != nil {
			m.mu.Unlock()
			return Project{}, err
		}
		project.IssueSourcePreference = preference
	}
	if req.ProjectHostSetupMethod != nil {
		project.ProjectHostSetupMethod = normalizeProjectHostSetupMethod(*req.ProjectHostSetupMethod, project.ProjectHostSetupMethod)
	}
	if req.LocalWindowsRuntimePreference != nil {
		preference, err := normalizeLocalWindowsRuntimePreference(*req.LocalWindowsRuntimePreference)
		if err != nil {
			m.mu.Unlock()
			return Project{}, err
		}
...
func normalizeIssueSourcePreference(value string) string {
	preference, _ := validateIssueSourcePreference(value)
	return preference
}

func validateIssueSourcePreference(value string) (string, error) {
	switch preference := strings.TrimSpace(value); preference {
	case "", "auto":
		return "", nil
	case "origin", "upstream":
		return preference, nil
	default:
		return "", errors.New("invalid issue source preference")
	}
}

func normalizeLocalWindowsRuntimePreference(input LocalWindowsRuntimePreference) (LocalWindowsRuntimePreference, error) {
	preference := LocalWindowsRuntimePreference{
		Kind:   strings.TrimSpace(input.Kind),
		Distro: strings.TrimSpace(input.Distro),
	}
	switch preference.Kind {
	case "inherit-global", "windows-host":
		preference.Distro = ""
		return preference, nil
	case "wsl":
		if preference.Distro == "" {
			return LocalWindowsRuntimePreference{}, errors.New("wsl runtime preference requires a distro")
		}
		return preference, nil
	default:
		return LocalWindowsRuntimePreference{}, errors.New("invalid local Windows runtime preference")
	}
}
...
func applyWorktreeMetadataUpdate(worktree *Worktree, req UpdateWorktreeRequest) {
	...
	if req.SparseDirectories != nil {
		worktree.SparseDirectories = append([]string(nil), (*req.SparseDirectories)...)
	}
	if req.SparseBaseRef != nil {
		worktree.SparseBaseRef = strings.TrimSpace(*req.SparseBaseRef)
	}
	if req.SparsePresetID != nil {
		worktree.SparsePresetID = strings.TrimSpace(*req.SparsePresetID)
	}
	if req.CreatedWithAgent != nil {
		worktree.CreatedWithAgent = strings.TrimSpace(*req.CreatedWithAgent)
	}
	if req.PendingFirstAgentMessageRename != nil {
		worktree.PendingFirstAgentMessageRename = *req.PendingFirstAgentMessageRename
	}
	if req.AutomationProvenance != nil {
		provenance := *req.AutomationProvenance
		worktree.AutomationProvenance = &provenance
	}
...
func (m *Manager) DeleteWorktree(ctx context.Context, id string, req DeleteWorktreeRequest) (DeleteWorktreeResponse, error) {
	...
	if preserved != nil {
		m.preservedBranchCleanup[id] = PreservedBranchCleanupTicket{
			WorktreeID: id, ProjectID: project.ID, BranchName: preserved.BranchName,
			Head: preserved.Head, CreatedAt: time.Now().UTC(),
		}
	} else {
		delete(m.preservedBranchCleanup, id)
	}
	err := m.saveLocked()
	m.mu.Unlock()
	if err != nil {
...
func (m *Manager) ForceDeletePreservedBranchForWorktree(ctx context.Context, worktreeID, branchName, expectedHead string) (ForceDeletePreservedBranchResponse, error) {
	m.mu.RLock()
	ticket, found := m.preservedBranchCleanup[strings.TrimSpace(worktreeID)]
	m.mu.RUnlock()
	if !found {
		return ForceDeletePreservedBranchResponse{}, errors.New("no preserved branch cleanup is tracked for worktree")
	}
	if normalized := normalizeLocalBranchRef(branchName); normalized != "" && normalized != ticket.BranchName {
		return ForceDeletePreservedBranchResponse{}, errors.New("preserved branch cleanup does not match branch")
	}
	if expected := strings.TrimSpace(expectedHead); expected != "" && ticket.Head != "" && !strings.EqualFold(expected, ticket.Head) {
		return ForceDeletePreservedBranchResponse{}, errors.New("preserved branch cleanup does not match expected head")
	}
	result, err := m.ForceDeletePreservedBranch(ctx, ForceDeletePreservedBranchRequest{
		ProjectID: ticket.ProjectID, BranchName: ticket.BranchName, ExpectedHead: ticket.Head,
	})
	if err != nil {
		return result, err
	}
	m.mu.Lock()
	delete(m.preservedBranchCleanup, ticket.WorktreeID)
	err = m.saveLocked()
	m.mu.Unlock()
	return result, err
}
...
func (m *Manager) StartSession(ctx context.Context, req StartSessionRequest) (Session, error) {
	...
	resolvedReq, err := m.resolveSessionStartRequest(ctx, req)
	if err != nil {
		return Session{}, err
	}
...
func (m *Manager) FindSessionByLaunchToken(worktreeID, launchToken string) (Session, bool) {
	launchToken = strings.TrimSpace(launchToken)
	if launchToken == "" {
		return Session{}, false
	}
	for _, session := range m.ListSessions() {
		if session.WorktreeID == worktreeID && session.LaunchToken == launchToken && session.Status != SessionStopped {
			return session, true
		}
	}
	return Session{}, false
...
func (m *Manager) SignalSession(id string, req SessionSignalRequest) error {
	m.mu.RLock()
	session, ok := m.sessions[id]
	m.mu.RUnlock()
	if !ok {
...
func (m *Manager) TailSession(id string, limit int) (TailSessionResponse, error) {
	...
	return session.tailResponse(limit), nil
}

func (m *Manager) ReadSessionTranscript(id string, cursor *uint64, limit int) (TerminalTranscriptRead, error) {
	m.mu.RLock()
	session, ok := m.sessions[id]
	m.mu.RUnlock()
	if !ok {
		return TerminalTranscriptRead{}, ErrSessionNotFound
	}
	return session.transcriptRead(cursor, limit), nil
}

func (m *Manager) SessionScreenSnapshot(id string) (TerminalScreenSnapshot, error) {
	m.mu.RLock()
	session, ok := m.sessions[id]
	m.mu.RUnlock()
	if !ok {
		return TerminalScreenSnapshot{}, ErrSessionNotFound
	}
...
func (m *Manager) DispatchTask(req DispatchTaskRequest) (Dispatch, error) {
	...
	if req.Inject {
		...
		dispatch.Preamble = buildDispatchPreambleForContext(
			task,
			dispatch.ID,
			req.CoordinatorHandle,
			req.DevMode,
		)
	}
...
func (m *Manager) CreateBrowserTab(req CreateBrowserTabRequest) (BrowserTab, error) {
	...
	if id != "" {
		...
		if err != nil || parsed.String() != strings.ToLower(id) {
			return BrowserTab{}, errors.New("browser tab id must be a canonical UUID")
		}
	} else {
		id = newID("bt")
	}
	...
	m.mu.Lock()
	if existing, ok := m.browserTabs[tab.ID]; ok {
		// Why: React may remount a live WebView; preserving creation time keeps
		// registration idempotent without inventing a second runtime identity.
...
func (m *Manager) DeleteBrowserTab(id string) (BrowserTab, error) {
	...
	failedActions := make([]ComputerAction, 0)
	for actionID, action := range m.computerActions {
		if action.Target != id || (action.Status != ComputerActionQueued && action.Status != ComputerActionRunning) {
			continue
		}
		action.Status = ComputerActionFailed
		action.Error = "browser tab was closed before the command completed"
		action.UpdatedAt = time.Now().UTC()
		m.computerActions[actionID] = action
	...
	m.setBrowserDriver(id, BrowserDriverState{Kind: "idle"})
...
func (m *Manager) UpdateComputerAction(id string, req UpdateComputerActionRequest) (ComputerAction, error) {
	...
	if strings.HasPrefix(action.Kind, "browser.") {
		if action.Status == ComputerActionCompleted || action.Status == ComputerActionFailed {
			m.mu.Unlock()
			return ComputerAction{}, errors.New("browser action is already terminal and no longer owned by this WebView executor")
		}
		if req.Status != ComputerActionCompleted && req.Status != ComputerActionFailed {
			m.mu.Unlock()
			return ComputerAction{}, errors.New("running browser action must complete or fail")
		}
	}
...
func (m *Manager) GetComputerAction(id string) (ComputerAction, error) {
	m.mu.RLock()
	action, ok := m.computerActions[id]
	m.mu.RUnlock()
	if !ok {
...
func (m *Manager) ClaimComputerActions(req ClaimComputerActionsRequest) ([]ComputerAction, error) {
	...
	for _, target := range req.Targets {
		if clean := strings.TrimSpace(target); clean != "" {
	...
	for _, action := range m.computerActions {
		...
		if len(targets) > 0 {
			if _, allowed := targets[action.Target]; !allowed {
				continue
			}
		}
...
func (m *Manager) RegisterEmulatorDevice(req RegisterEmulatorDeviceRequest) (EmulatorDevice, error) {
	...
	nativeID, err := normalizeEmulatorNativeID(req.NativeID)
	if err != nil {
...
func (m *Manager) UpdateEmulatorDevice(id string, req UpdateEmulatorDeviceRequest) (EmulatorDevice, error) {
	...
	if strings.TrimSpace(req.NativeID) != "" {
		nativeID, normalizeErr := normalizeEmulatorNativeID(req.NativeID)
		if normalizeErr != nil {
			m.mu.Unlock()
			return EmulatorDevice{}, normalizeErr
		}
...
func normalizeEmulatorNativeID(value string) (string, error) {
	value = strings.TrimSpace(value)
	if len(value) > 255 || strings.IndexByte(value, 0) >= 0 || strings.ContainsAny(value, "\r\n") {
		return "", errors.New("emulator nativeId contains unsupported characters")
	}
	return value, nil
}
...
func (m *Manager) GitBaseStatus(ctx context.Context, req GitBaseStatusRequest) (GitBaseStatusResult, error) {
	...
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			result, relayErr := m.sshGitBaseStatus(ctx, req)
			if relayErr != nil {
				// Why: a disconnected SSH host must not erase the last status the
				// relay reported. With no prior report, return an honest unknown.
				if cached, ok := m.cachedGitBaseStatus(req); ok {
					return cached, nil
				}
				return GitBaseStatusResult{
					Status: "unknown",
					Base:   strings.TrimSpace(req.BaseRef),
				}, nil
			}
			if err := m.recordGitBaseStatus(req, result); err != nil {
	...
	result := ComputeGitBaseStatus(ctx, base, req)
	if err := m.recordGitBaseStatus(req, result); err != nil {
		return GitBaseStatusResult{}, err
	}
	return result, nil
...
func (m *Manager) cachedGitBaseStatus(req GitBaseStatusRequest) (GitBaseStatusResult, bool) {
	...
	if !ok || projection.BaseStatus == nil {
		return GitBaseStatusResult{}, false
	}
	status := normalizeSourceControlBaseStatus(projection.BaseStatus)
	return GitBaseStatusResult{
...
func (m *Manager) GitCheckIgnored(ctx context.Context, req GitCheckIgnoredRequest) ([]string, error) {
	...
	if err != nil {
		...
		// Why: check-ignore uses exit 1 for "one or more paths did not match";
		// stdout still contains valid matches from the same batch.
...
var knownHugeFolderNames = []string{"node_modules", ".next", "dist", "build", "target", "vendor"}

func (m *Manager) GitFindHugeFoldersToIgnore(ctx context.Context, req GitHugeFolderRequest) ([]string, error) {
	base, err := m.resolveWorkspacePath(req.ProjectID, req.WorktreeID)
	if err != nil {
		return nil, err
	}
	existing := make([]string, 0, len(knownHugeFolderNames))
	for _, name := range knownHugeFolderNames {
	...
	ignored, err := m.GitCheckIgnored(ctx, GitCheckIgnoredRequest{ProjectID: req.ProjectID, WorktreeID: req.WorktreeID, Paths: existing})
	if err != nil {
		ignored = []string{}
	}
	ignoredSet := make(map[string]struct{}, len(ignored))
	for _, name := range ignored {
		ignoredSet[strings.TrimSuffix(strings.TrimSpace(name), "/")] = struct{}{}
	}
	result := make([]string, 0, len(existing))
	for _, name := range existing {
		if _, found := ignoredSet[name]; !found {
...
func (m *Manager) GitAppendHugeFolderToIgnore(_ context.Context, req GitHugeFolderRequest) (bool, error) {
	base, err := m.resolveWorkspacePath(req.ProjectID, req.WorktreeID)
	if err != nil {
		return false, err
	}
	...
	allowed := false
	for _, candidate := range knownHugeFolderNames {
		allowed = allowed || candidate == folderName
	}
	if !allowed || strings.ContainsAny(folderName, "/\\\r\n") {
		return false, fmt.Errorf("refusing to add unrecognized folder to .gitignore: %s", req.FolderName)
	}
	gitignorePath := filepath.Join(base, ".gitignore")
	if info, statErr := os.Lstat(gitignorePath); statErr == nil && info.Mode()&os.ModeSymlink != 0 {
		return false, errors.New("refusing to write a symlinked .gitignore")
	...
	for _, existing := range strings.Split(strings.ReplaceAll(string(content), "\r\n", "\n"), "\n") {
		...
		if trimmed == folderName || trimmed == line {
			return false, nil
		}
	}
	prefix := ""
...
func (m *Manager) saveLocked() error {
	state := persistedState{
		RelayID:                m.relayID,
		Projects:               make([]Project, 0, len(m.projects)),
		SparsePresets:          make([]SparsePreset, 0),
		ProjectHostSetups:      make([]ProjectHostSetup, 0, len(m.projectHostSetups)),
		ProjectGroups:          make([]ProjectGroup, 0, len(m.projectGroups)),
		FolderWorkspaces:       make([]FolderWorkspace, 0, len(m.folderWorkspaces)),
		Worktrees:              make([]Worktree, 0, len(m.worktrees)),
		PreservedBranchCleanup: make([]PreservedBranchCleanupTicket, 0, len(m.preservedBranchCleanup)),
		Agents:                 make([]AgentProfile, 0, len(m.agents)),
		...
		Stats:                  m.stats,
		AccountsSnapshot:       append(json.RawMessage(nil), m.accountsSnapshot...),
		LegacySharedControl:    m.legacySharedControl,
	}
	for _, project := range m.projects {
		state.Projects = append(state.Projects, project)
	}
	for _, presets := range m.sparsePresets {
		state.SparsePresets = append(state.SparsePresets, presets...)
	}
	for _, setup := range m.projectHostSetups {
		state.ProjectHostSetups = append(state.ProjectHostSetups, setup)
	}
	...
	for _, ticket := range m.preservedBranchCleanup {
		state.PreservedBranchCleanup = append(state.PreservedBranchCleanup, ticket)
	}
	for _, agent := range m.agents {
...
func (m *Manager) emit(topic string, payload interface{}) {
	if topic == "session.status" {
		if session, ok := payload.(Session); ok {
			m.recordSessionStats(session)
		}
	}
	event := RuntimeEvent{
	...
	m.mu.RLock()
	// Why: unsubscribe closes channels under the write lock. Sending while the
	// read lock is held prevents a copied channel from being closed mid-send.
...
func isBrowserCommand(command string) bool {
	switch command {
	case "goto", "reload", "goBack", "goForward", "stop", "screenshot", "fullScreenshot", "pdf",
		"snapshot", "click", "dblclick", "fill", "type", "focus", "clear", "keypress", "keyDown", "keyUp", "scroll",
		"scrollIntoView", "select", "check", "hover", "selectAll", "drag", "upload", "get", "is",
		"find", "keyboardInsertText", "wait", "captureStart", "captureStop", "console", "network",
		"harStart", "harStop", "profilerStart", "profilerStop", "interceptEnable", "interceptDisable", "interceptList", "geolocation", "setMedia", "download",
		"pushState", "eval", "viewport", "setHeaders", "setOffline", "setCredentials",
		"storageLocalGet", "storageLocalSet", "storageLocalClear", "storageSessionGet", "storageSessionSet", "storageSessionClear",
		"highlight", "mouseMove", "mouseDown", "mouseUp", "mouseClick", "mouseWheel",
		"clipboardRead", "clipboardWrite", "clipboardCopy", "clipboardPaste",
		"initScriptAdd", "initScriptRemove",
		"screencastStart", "screencastStop",
		"recordingStart", "recordingStop",
		"cookieGet", "cookieSet", "cookieDelete", "cookieClear", "dialogAccept", "dialogDismiss":
		return true
	default:
		return false
	}
...
func isEmulatorCommand(command string) bool {
	switch command {
	case "tap", "swipe", "gesture", "type", "install", "launch", "screenshot", "logs", "pressKey", "button", "rotate", "ax", "exec", "shutdown":
		return true
	default:
		return false
	}
