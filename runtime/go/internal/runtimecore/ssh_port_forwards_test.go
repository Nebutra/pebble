...
func TestSshLabelPortAllowlistRequiresObservedLoopbackListener(t *testing.T) {
	ports := []SshDetectedPort{{Host: "0.0.0.0", Port: 4173}, {Host: "192.168.1.4", Port: 9000}}
	if !sshLabelPortIsAdvertised(ports, "127.0.0.1", 4173) {
		t.Fatal("expected wildcard listener to authorize its loopback target")
	}
	if sshLabelPortIsAdvertised(ports, "127.0.0.1", 4174) {
		t.Fatal("unobserved port must be rejected")
	}
	if sshLabelPortIsAdvertised(ports, "192.168.1.4", 9000) {
		t.Fatal("non-loopback listener must be rejected")
	}
}
