...
func LoadWorktreeArchiveHookScript(repoPath string) string {
	...
	return parsePebbleYamlScript(string(content), "archive")
}

func LoadWorktreeSetupHookScript(worktreePath string) string {
	content, err := os.ReadFile(filepath.Join(worktreePath, "pebble.yaml"))
	if err != nil {
		return ""
	}
	return parsePebbleYamlScript(string(content), "setup")
}
...
func runWorktreeArchiveHookScript(
	...
	output, timedOut, err := runWorktreeHookScript(ctx, repoPath, worktreePath, script, timeout)
	if err == nil {
		return nil
	}
	return &ArchiveHookError{Output: output, TimedOut: timedOut, cause: err}
}

func RunWorktreeSetupHookOnHost(ctx context.Context, repoPath, worktreePath string) error {
	script := LoadWorktreeSetupHookScript(worktreePath)
	if script == "" {
		return nil
	}
	output, timedOut, err := runWorktreeHookScript(ctx, repoPath, worktreePath, script, archiveHookTimeout)
	if err == nil {
		return nil
	}
	reason := "setup hook failed"
	if timedOut {
		reason = "setup hook timed out after " + archiveHookTimeout.String()
	} else {
		reason += ": " + err.Error()
	}
	if output != "" {
...
func runWorktreeHookScript(
	...
	return strings.TrimSpace(string(output)), errors.Is(hookCtx.Err(), context.DeadlineExceeded), err
}
...
func parsePebbleYamlArchiveScript(content string) string {
	return parsePebbleYamlScript(content, "archive")
}

func parsePebbleYamlScript(content string, scriptName string) string {
	...
	for index := 0; index < len(lines); index++ {
		...
		key, rest, found := strings.Cut(trimmed, ":")
		if !found || strings.TrimSpace(key) != scriptName {
			continue
		}
