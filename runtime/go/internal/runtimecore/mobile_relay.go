...
const (
	...
	ProjectionAccounts      ProjectionKind = "accounts"
)
...
func (m *Manager) MobileRelaySnapshot(kinds []ProjectionKind, outputLimit int) MobileRelayProjectionSnapshot {
	...
	if hasProjectionKind(kinds, ProjectionAccounts) {
		snapshot.Accounts = m.GetAccountsSnapshot()
	}
	return snapshot
}
...
func (m *Manager) MobileRelayEvent(event RuntimeEvent, kinds []ProjectionKind) (RuntimeEvent, bool) {
	...
	case "notification.dispatched":
		// Why: notifications are transient push events rather than a snapshot
		// projection; every paired ready client receives them regardless of diet.
		return event, true
	case "session.output":
		if !hasProjectionKind(kinds, ProjectionTerminal) {
	...
	case "accounts.changed":
		...
		event.Payload = map[string]interface{}{"accounts": m.GetAccountsSnapshot()}
		return event, true
	default:
		return RuntimeEvent{}, false
	}
