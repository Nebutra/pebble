...
func (s *processSession) signal(signal string) error {
	normalized := strings.ToUpper(strings.TrimSpace(signal))
	if normalized == "" {
		return errors.New("session signal is required")
	}
	s.mu.RLock()
	status := s.status
	pid := s.pid
	...
	if normalized == "SIGWINCH" {
		// Why: resizing is the portable terminal-window notification. It also crosses
		// ConPTY and SSH PTY boundaries where host POSIX signals are not meaningful.
		if resizePty != nil {
			return resizePty(cols, rows)
		}
		return nil
	}
	return signalPlatformSessionProcess(pid, stdin, normalized)
}
...
func (s *processSession) stop() (Session, error) {
	...
	// Why: callers retry failed SSH teardown. Publishing `stopped` before a
	// successful kill makes the retry skip the still-live child process.
	s.mu.Lock()
...
func (s *processSession) screenSnapshot() TerminalScreenSnapshot {
...
func (s *processSession) tailResponse(limit int) TailSessionResponse {
	...
	return TailSessionResponse{
		SessionID:  s.id,
		Chunks:     chunks,
		Transcript: s.transcript.snapshot(),
	}
}

func (s *processSession) transcriptRead(cursor *uint64, limit int) TerminalTranscriptRead {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.transcript.read(cursor, limit)
}
