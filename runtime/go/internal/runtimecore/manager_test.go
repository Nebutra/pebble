...
func TestEventSubscriberCloseDoesNotRaceConcurrentEmit(t *testing.T) {
	...
	for iteration := 0; iteration < 500; iteration++ {
		id, _ := manager.Subscribe(1)
		var group sync.WaitGroup
		group.Add(2)
		go func() {
			defer group.Done()
			manager.emit("test.concurrent", iteration)
		}()
		go func() {
			defer group.Done()
...
func TestProjectWindowsRuntimePreferencePersists(t *testing.T) {
	stateDir := t.TempDir()
	...
	if _, err := manager.UpdateProject(project.ID, UpdateProjectRequest{
		LocalWindowsRuntimePreference: &LocalWindowsRuntimePreference{Kind: "wsl"},
	}); err == nil {
		t.Fatal("expected WSL preference without a distro to be rejected")
	}
	updated, err := manager.UpdateProject(project.ID, UpdateProjectRequest{
		LocalWindowsRuntimePreference: &LocalWindowsRuntimePreference{Kind: "wsl", Distro: "Ubuntu-24.04"},
	})
	if err != nil {
		t.Fatal(err)
	}
	if updated.LocalWindowsRuntimePreference == nil || updated.LocalWindowsRuntimePreference.Distro != "Ubuntu-24.04" {
		t.Fatalf("unexpected runtime preference: %#v", updated.LocalWindowsRuntimePreference)
	}
	...
	projects := reloaded.ListProjects()
	if len(projects) != 1 || projects[0].LocalWindowsRuntimePreference == nil || projects[0].LocalWindowsRuntimePreference.Kind != "wsl" {
		t.Fatalf("runtime preference did not survive reload: %#v", projects)
	}
}

func TestProjectIssueSourcePreferencePersistsAndClears(t *testing.T) {
	stateDir := t.TempDir()
	manager, err := NewManager(stateDir, nil)
	if err != nil {
		t.Fatal(err)
	...
	invalid := "fork"
	if _, err := manager.UpdateProject(project.ID, UpdateProjectRequest{IssueSourcePreference: &invalid}); err == nil {
		t.Fatal("expected invalid issue source preference to be rejected")
	}
	upstream := "upstream"
	updated, err := manager.UpdateProject(project.ID, UpdateProjectRequest{IssueSourcePreference: &upstream})
	if err != nil || updated.IssueSourcePreference != "upstream" {
		t.Fatalf("unexpected issue source preference: %#v err=%v", updated, err)
	}
	reloaded, err := NewManager(stateDir, nil)
	if err != nil {
		t.Fatal(err)
	}
	if projects := reloaded.ListProjects(); len(projects) != 1 || projects[0].IssueSourcePreference != "upstream" {
		t.Fatalf("issue source preference did not survive reload: %#v", projects)
	}
	auto := "auto"
	cleared, err := reloaded.UpdateProject(project.ID, UpdateProjectRequest{IssueSourcePreference: &auto})
	if err != nil || cleared.IssueSourcePreference != "" {
		t.Fatalf("auto did not clear issue source preference: %#v err=%v", cleared, err)
	}
}
...
func TestDeleteWorktreePreservesBranchWithUnmergedCommits(t *testing.T) {
	...
	// Why: cleanup is commonly accepted after a runtime restart, so the
	// worktree-to-project safety ticket must survive persistence.
	restarted, err := NewManager(filepath.Dir(manager.store.path), nil)
	if err != nil {
		t.Fatal(err)
	}
	result, err := restarted.ForceDeletePreservedBranchForWorktree(
		context.Background(), worktree.ID, deleted.PreservedBranch.BranchName, deleted.PreservedBranch.Head,
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFileServiceReadsWritesAndBlocksEscapes(t *testing.T) {
	...
	if _, err := manager.WriteFile(WriteFileRequest{
		ProjectID: project.ID,
		Path:      "docs/upload.tmp",
		Content:   "replacement recording",
	}); err != nil {
		t.Fatal(err)
	}
	if _, err := manager.WriteFile(WriteFileRequest{
		ProjectID: project.ID,
		Path:      "docs/recording.webm",
		Content:   "old recording",
	}); err != nil {
		t.Fatal(err)
	}
	if err := manager.CommitUpload(FileRenameRequest{
		ProjectID:       project.ID,
		SourcePath:      "docs/upload.tmp",
		DestinationPath: "docs/recording.webm",
	}); err != nil {
		t.Fatal(err)
	}
	recording, err := manager.ReadFile(ReadFileRequest{ProjectID: project.ID, Path: "docs/recording.webm"})
	if err != nil || recording.Content != "replacement recording" {
		t.Fatalf("unexpected committed upload: %#v err=%v", recording, err)
	}
	if _, err := manager.StatFile(ReadFileRequest{ProjectID: project.ID, Path: "docs/upload.tmp"}); err == nil {
		t.Fatal("expected committed upload temporary file to be removed")
	}
...
func TestFileServiceRequiresRegisteredTargetForLiveRemoteReads(t *testing.T) {
	manager, err := NewManager(t.TempDir(), nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !errors.Is(err, ErrNotFound) {
		t.Fatalf("expected missing SSH target error, got %v", err)
	}
}
...
func TestCreateBrowserTabAcceptsCanonicalRendererIdentityIdempotently(t *testing.T) {
	manager, err := NewManager(t.TempDir(), nil)
	if err != nil {
		t.Fatal(err)
	}
	id := "7f608e5c-19c0-4df7-b5f2-00f65ef367c8"
	first, err := manager.CreateBrowserTab(CreateBrowserTabRequest{
		ID: id, WorktreeID: "worktree-1", Title: "First", URL: "about:blank",
	})
	if err != nil {
		t.Fatal(err)
	}
	second, err := manager.CreateBrowserTab(CreateBrowserTabRequest{
		ID: id, WorktreeID: "worktree-2", Title: "Second", URL: "https://example.test",
	})
	if err != nil {
		t.Fatal(err)
	}
	if second.ID != id || second.WorktreeID != "worktree-2" || second.Title != "Second" {
		t.Fatalf("renderer identity was not updated in place: %#v", second)
	}
	if !second.CreatedAt.Equal(first.CreatedAt) || len(manager.ListBrowserTabs()) != 1 {
		t.Fatalf("renderer identity registration was not idempotent: %#v", manager.ListBrowserTabs())
	}
	if _, err := manager.CreateBrowserTab(CreateBrowserTabRequest{
		ID: "browser-page-1", URL: "about:blank",
	}); err == nil {
		t.Fatal("expected malformed renderer identity to be rejected")
	}
}
...
func TestQueueBrowserCommandRejectsUnknownTabsAndCommands(t *testing.T) {
	...
	for _, command := range []string{
		"keyDown", "keyUp", "harStart", "harStop", "pushState", "storageLocalSet",
		"storageSessionGet", "highlight", "mouseWheel", "clipboardPaste", "eval",
		"viewport", "setHeaders", "setOffline", "setCredentials",
		"cookieGet", "cookieSet", "cookieDelete", "cookieClear", "dialogAccept", "dialogDismiss",
	} {
		action, queueErr := manager.QueueBrowserCommand(tab.ID, BrowserCommandRequest{
			Command: command,
			Payload: map[string]interface{}{"key": "Shift"},
		})
		if queueErr != nil {
			t.Fatalf("expected %s to be accepted: %v", command, queueErr)
		}
		if action.Kind != "browser."+command || action.Payload["key"] != "Shift" {
			t.Fatalf("unexpected %s action: %#v", command, action)
		}
	}
...
func TestBrowserActionClaimRespectsMountedWebviewTargets(t *testing.T) {
	manager, err := NewManager(t.TempDir(), nil)
	if err != nil {
		t.Fatal(err)
	}
	visible, _ := manager.CreateBrowserTab(CreateBrowserTabRequest{URL: "https://visible.test"})
	hidden, _ := manager.CreateBrowserTab(CreateBrowserTabRequest{URL: "https://hidden.test"})
	visibleAction, _ := manager.QueueBrowserCommand(visible.ID, BrowserCommandRequest{Command: "reload"})
	hiddenAction, _ := manager.QueueBrowserCommand(hidden.ID, BrowserCommandRequest{Command: "reload"})

	claimed, err := manager.ClaimComputerActions(ClaimComputerActionsRequest{
		KindPrefix: "browser.", Targets: []string{visible.ID},
	})
	if err != nil || len(claimed) != 1 || claimed[0].ID != visibleAction.ID {
		t.Fatalf("unexpected target-aware claim: %#v err=%v", claimed, err)
	}
	remaining, _ := manager.GetComputerAction(hiddenAction.ID)
	if remaining.Status != ComputerActionQueued {
		t.Fatalf("hidden WebView action was claimed: %#v", remaining)
	}
}

func TestDeletingBrowserTabFailsOutstandingActionsAndRejectsLateCompletion(t *testing.T) {
	manager, err := NewManager(t.TempDir(), nil)
	if err != nil {
		t.Fatal(err)
	}
	tab, _ := manager.CreateBrowserTab(CreateBrowserTabRequest{URL: "https://example.test"})
	action, _ := manager.QueueBrowserCommand(tab.ID, BrowserCommandRequest{Command: "reload"})
	claimed, err := manager.ClaimComputerActions(ClaimComputerActionsRequest{
		KindPrefix: "browser.", Targets: []string{tab.ID},
	})
	if err != nil || len(claimed) != 1 {
		t.Fatalf("failed to claim action: %#v err=%v", claimed, err)
	}
	if _, err := manager.DeleteBrowserTab(tab.ID); err != nil {
		t.Fatal(err)
	}
	failed, _ := manager.GetComputerAction(action.ID)
	if failed.Status != ComputerActionFailed || failed.Error == "" {
		t.Fatalf("destroy did not terminate action: %#v", failed)
	}
	if _, err := manager.UpdateComputerAction(action.ID, UpdateComputerActionRequest{
		Status: ComputerActionCompleted, Result: map[string]interface{}{"ok": true},
	}); err == nil {
		t.Fatal("late executor completion replaced the destroy failure")
	}
}
...
func TestQueueEmulatorCommandRejectsInactiveSessionsAndUnknownCommands(t *testing.T) {
	...
	if _, err := manager.QueueEmulatorCommand(session.ID, EmulatorCommandRequest{Command: "shutdown"}); err != nil {
		t.Fatalf("expected native shutdown command to queue: %v", err)
	}
...
func TestEmulatorDeviceUpdateAndDetach(t *testing.T) {
	...
	if device.NativeID != "emulator-5554" || device.Runtime != "adb" || device.Status != EmulatorDeviceError || device.Error != "offline" {
...
func TestClearSessionBufferDropsTail(t *testing.T) {
	...
	read, err := manager.ReadSessionTranscript(session.ID, nil, 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(read.Tail) != 0 || read.OldestCursor != "0" || read.NextCursor != "0" || read.LatestCursor != "0" || read.Truncated || read.Limited {
		t.Fatalf("expected cleared transcript epoch, got %#v", read)
	}
}
...
func TestParseCloneProgressHandlesGitCarriageReturnLines(t *testing.T) {
	phase, percent, ok := parseCloneProgress("Receiving objects: 73% (73/100), 1.2 MiB")
	if !ok || phase != "Receiving objects" || percent != 73 {
		t.Fatalf("unexpected progress parse: phase=%q percent=%d ok=%v", phase, percent, ok)
	}
	if _, _, ok := parseCloneProgress("fatal: authentication failed"); ok {
		t.Fatal("fatal errors must not be reported as progress")
	}
}

func TestCloneCancellationSlotIsSingleOwner(t *testing.T) {
	manager, err := NewManager(t.TempDir(), nil)
	if err != nil {
		t.Fatal(err)
	}
	ctx, cancel := context.WithCancel(context.Background())
	if !manager.beginClone(cancel) {
		t.Fatal("first clone should own the cancellation slot")
	}
	if manager.beginClone(func() {}) {
		t.Fatal("second clone must not replace the active cancellation slot")
	}
	if !manager.CancelClone() {
		t.Fatal("active clone should be cancellable")
	}
	select {
	case <-ctx.Done():
	case <-time.After(time.Second):
		t.Fatal("clone cancellation did not cancel the context")
	}
	manager.finishClone()
	if manager.CancelClone() {
