...
const (
	...
	maxFileSearchReadBytes          = 4 * 1024 * 1024
	maxFileSearchLineBytes          = 8 * 1024
)
...
type sshFileMutationPayload struct {
...
type FileWatchSnapshotEntry struct {
	Path        string `json:"path"`
	Size        int64  `json:"size"`
...
func (m *Manager) ListFiles(req ListFilesRequest) ([]FileEntry, error) {
	return m.ListFilesContext(context.Background(), req)
}

func (m *Manager) ListFilesContext(ctx context.Context, req ListFilesRequest) ([]FileEntry, error) {
	...
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			entries, relayErr := m.listSshFiles(ctx, req)
			if relayErr == nil {
				return entries, nil
			}
			if ctx.Err() != nil {
...
func (m *Manager) ReadFile(req ReadFileRequest) (FileContent, error) {
	return m.ReadFileContext(context.Background(), req)
}

func (m *Manager) ReadFileContext(ctx context.Context, req ReadFileRequest) (FileContent, error) {
	if err := ctx.Err(); err != nil {
	...
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			content, relayErr := m.readSshFile(ctx, req)
			if relayErr == nil {
				return content, nil
			}
			if ctx.Err() != nil {
				return FileContent{}, relayErr
			}
			cachedContent, ok, cacheErr := m.cachedRemoteFileContent(req)
			if cacheErr != nil {
...
func (m *Manager) readSshFile(parent context.Context, req ReadFileRequest) (FileContent, error) {
	project, root, err := m.sshFileRelayScope(req.ProjectID, req.WorktreeID)
	if err != nil {
		return FileContent{}, err
	}
	limit := normalizedFileReadLimit(req.MaxBytes)
	ctx, cancel := context.WithTimeout(parent, 40*time.Second)
	defer cancel()
	output, err := m.runSshRelayWorker(ctx, project.HostID, []string{
		"file-read-json",
		"--project", project.ID,
		"--worktree", strings.TrimSpace(req.WorktreeID),
		"--root", root,
		"--path", req.Path,
	...
	if err := json.Unmarshal(output, &content); err != nil {
		return FileContent{}, errors.New("relay worker returned malformed file content")
	}
	return content, nil
}

func (m *Manager) listSshFiles(parent context.Context, req ListFilesRequest) ([]FileEntry, error) {
	project, root, err := m.sshFileRelayScope(req.ProjectID, req.WorktreeID)
	if err != nil {
		return nil, err
	}
	...
	if err := json.Unmarshal(output, &entries); err != nil {
		return nil, errors.New("relay worker returned malformed file tree")
	}
	return entries, nil
}

func (m *Manager) sshFileRelayScope(projectID string, worktreeID string) (Project, string, error) {
	m.mu.RLock()
	project, projectOK := m.projects[strings.TrimSpace(projectID)]
	worktree, worktreeOK := m.worktrees[strings.TrimSpace(worktreeID)]
	m.mu.RUnlock()
	if !projectOK || project.LocationKind != "ssh" || strings.TrimSpace(project.HostID) == "" {
		return Project{}, "", ErrRemoteNeedsRelay
	}
	root := project.Path
	if worktreeOK && worktree.ProjectID == project.ID && strings.TrimSpace(worktree.Path) != "" {
		root = worktree.Path
	}
...
func (m *Manager) ReadFileChunk(req ReadFileChunkRequest) (FileChunk, error) {
	return m.ReadFileChunkContext(context.Background(), req)
}

func (m *Manager) ReadFileChunkContext(ctx context.Context, req ReadFileChunkRequest) (FileChunk, error) {
	if err := ctx.Err(); err != nil {
	...
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			return m.readSshFileChunk(ctx, req)
		}
		return FileChunk{}, err
	}
...
func (m *Manager) readSshFileChunk(parent context.Context, req ReadFileChunkRequest) (FileChunk, error) {
	project, root, err := m.sshFileRelayScope(req.ProjectID, req.WorktreeID)
	if err != nil {
		return FileChunk{}, err
	}
	...
	if err := json.Unmarshal(output, &chunk); err != nil {
		return FileChunk{}, errors.New("relay worker returned malformed file chunk")
	}
	return chunk, nil
}

func (m *Manager) WriteFile(req WriteFileRequest) (FileContent, error) {
	return m.WriteFileContext(context.Background(), req)
}

func (m *Manager) WriteFileContext(ctx context.Context, req WriteFileRequest) (FileContent, error) {
	if err := ctx.Err(); err != nil {
		return FileContent{}, err
	}
	base, err := m.resolveWorkspacePath(req.ProjectID, req.WorktreeID)
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			if err := m.mutateSshFile(ctx, req.ProjectID, req.WorktreeID, sshFileMutationPayload{
				Operation: "write", Path: req.Path, Content: req.Content, CreateDirs: req.CreateDirs,
			}); err != nil {
				return FileContent{}, err
			}
			content, err := m.readSshFile(ctx, ReadFileRequest{
				ProjectID: req.ProjectID, WorktreeID: req.WorktreeID, Path: req.Path,
	...
	content, err := m.ReadFileContext(ctx, ReadFileRequest{
...
func (m *Manager) WriteFileBase64(req WriteFileBase64Request) error {
	return m.WriteFileBase64Context(context.Background(), req)
}

func (m *Manager) WriteFileBase64Context(ctx context.Context, req WriteFileBase64Request) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	...
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			return m.mutateSshFile(ctx, req.ProjectID, req.WorktreeID, sshFileMutationPayload{
				Operation: "write-base64", Path: req.Path, ContentBase64: req.ContentBase64, Append: req.Append,
			})
		}
		return err
	}
...
func (m *Manager) CreateFile(req FileMutationRequest) error {
	return m.CreateFileContext(context.Background(), req)
}

func (m *Manager) CreateFileContext(ctx context.Context, req FileMutationRequest) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	base, relPath, err := m.resolveLocalMutationTarget(req.ProjectID, req.WorktreeID, req.Path)
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			return m.mutateSshFile(ctx, req.ProjectID, req.WorktreeID, sshFileMutationPayload{Operation: "create-file", Path: req.Path})
		}
		return err
	}
...
func (m *Manager) CreateDirectory(req FileMutationRequest) error {
	return m.CreateDirectoryContext(context.Background(), req)
}

func (m *Manager) CreateDirectoryContext(ctx context.Context, req FileMutationRequest) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	base, relPath, err := m.resolveLocalMutationTarget(req.ProjectID, req.WorktreeID, req.Path)
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			return m.mutateSshFile(ctx, req.ProjectID, req.WorktreeID, sshFileMutationPayload{Operation: "create-dir", Path: req.Path})
		}
		return err
	}
...
func (m *Manager) RenamePath(req FileRenameRequest) error {
	return m.RenamePathContext(context.Background(), req)
}

func (m *Manager) RenamePathContext(ctx context.Context, req FileRenameRequest) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	...
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			return m.mutateSshFile(ctx, req.ProjectID, req.WorktreeID, sshFileMutationPayload{
				Operation: "rename", OldPath: req.OldPath, NewPath: req.NewPath,
			})
		}
		return err
	}
...
func (m *Manager) CopyPath(req FileRenameRequest) error {
	return m.CopyPathContext(context.Background(), req)
}

func (m *Manager) CopyPathContext(ctx context.Context, req FileRenameRequest) error {
	if err := ctx.Err(); err != nil {
		return err
	}
...
// CommitUpload atomically publishes a completed workspace upload while ordinary
// copy operations retain their no-clobber behavior.
func (m *Manager) CommitUpload(req FileRenameRequest) error {
	return m.CommitUploadContext(context.Background(), req)
}

func (m *Manager) CommitUploadContext(ctx context.Context, req FileRenameRequest) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	sourcePath := req.SourcePath
	...
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			return m.mutateSshFile(ctx, req.ProjectID, req.WorktreeID, sshFileMutationPayload{
				Operation: "commit-upload", SourcePath: sourcePath, DestinationPath: destinationPath,
			})
		}
		return err
	}
	...
	destination, err := resolveWorkspaceReplaceTarget(base, destinationRelPath)
	if err != nil {
		return err
	}
	return replaceRemoteWorkspaceFile(source, destination)
}

func (m *Manager) DeletePath(req FileMutationRequest) error {
	return m.DeletePathContext(context.Background(), req)
}

func (m *Manager) DeletePathContext(ctx context.Context, req FileMutationRequest) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	base, relPath, err := m.resolveLocalMutationTarget(req.ProjectID, req.WorktreeID, req.Path)
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			return m.mutateSshFile(ctx, req.ProjectID, req.WorktreeID, sshFileMutationPayload{
				Operation: "delete", Path: req.Path, Recursive: req.Recursive,
			})
		}
		return err
	}
...
func (m *Manager) mutateSshFile(parent context.Context, projectID string, worktreeID string, payload sshFileMutationPayload) error {
	project, root, err := m.sshFileRelayScope(projectID, worktreeID)
	if err != nil {
		return err
	}
	payload.Root = root
	...
	defer cancel()
	_, err = m.runSshRelayWorkerWithInput(ctx, project.HostID, []string{"file-mutate-json"}, input)
	return err
}

func (m *Manager) StatFile(req ReadFileRequest) (FileStat, error) {
	return m.StatFileContext(context.Background(), req)
}

func (m *Manager) StatFileContext(ctx context.Context, req ReadFileRequest) (FileStat, error) {
	if err := ctx.Err(); err != nil {
		return FileStat{}, err
	}
	base, err := m.resolveWorkspacePath(req.ProjectID, req.WorktreeID)
...
func (m *Manager) statSshFile(parent context.Context, req ReadFileRequest) (FileStat, error) {
	project, root, err := m.sshFileRelayScope(req.ProjectID, req.WorktreeID)
	if err != nil {
		return FileStat{}, err
	}
	...
	if err := json.Unmarshal(output, &stat); err != nil {
		return FileStat{}, errors.New("relay worker returned malformed file stat")
	}
	return stat, nil
}

func (m *Manager) ListAllFiles(req ListAllFilesRequest) (FileListResult, error) {
	return m.ListAllFilesContext(context.Background(), req)
}

func (m *Manager) ListAllFilesContext(ctx context.Context, req ListAllFilesRequest) (FileListResult, error) {
	if err := ctx.Err(); err != nil {
	...
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			return m.listAllSshFiles(ctx, req)
		}
		return FileListResult{}, err
	}
	limit := req.Limit
...
func (m *Manager) listAllSshFiles(parent context.Context, req ListAllFilesRequest) (FileListResult, error) {
	project, root, err := m.sshFileRelayScope(req.ProjectID, req.WorktreeID)
	if err != nil {
		return FileListResult{}, err
	}
	excluded, err := json.Marshal(req.ExcludePaths)
	if err != nil {
		return FileListResult{}, err
	}
	limit := req.Limit
	...
	if err := json.Unmarshal(output, &result); err != nil {
		return FileListResult{}, errors.New("relay worker returned malformed file list")
	}
	return result, nil
}
...
func (m *Manager) FileWatchSnapshot(req ListFilesRequest) ([]FileWatchSnapshotEntry, error) {
	return m.FileWatchSnapshotContext(context.Background(), req)
}

func (m *Manager) FileWatchSnapshotContext(ctx context.Context, req ListFilesRequest) ([]FileWatchSnapshotEntry, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	base, err := m.resolveWorkspacePath(req.ProjectID, req.WorktreeID)
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			return m.sshFileWatchSnapshot(ctx, req)
		}
		return nil, err
	}
	return SnapshotWorkspaceFiles(base)
}

func SnapshotWorkspaceFiles(root string) ([]FileWatchSnapshotEntry, error) {
	base, err := filepath.Abs(strings.TrimSpace(root))
	if err != nil || strings.TrimSpace(root) == "" {
		return nil, errors.New("workspace root is required")
	}
	entries := make([]FileWatchSnapshotEntry, 0, 256)
	err = filepath.WalkDir(base, func(path string, entry os.DirEntry, walkErr error) error {
		if walkErr != nil || path == base {
			return nil
		}
		...
		entries = append(entries, FileWatchSnapshotEntry{
			Path: filepath.ToSlash(rel), Size: info.Size(), MtimeNanos: info.ModTime().UnixNano(), IsDirectory: entry.IsDir(),
		})
		return nil
	})
	return entries, err
...
func (m *Manager) sshFileWatchSnapshot(parent context.Context, req ListFilesRequest) ([]FileWatchSnapshotEntry, error) {
	project, root, err := m.sshFileRelayScope(req.ProjectID, req.WorktreeID)
	if err != nil {
		return nil, err
	}
	ctx, cancel := context.WithTimeout(parent, 60*time.Second)
	defer cancel()
	output, err := m.runSshRelayWorker(ctx, project.HostID, []string{"file-watch-snapshot-json", "--root", root})
	if err != nil {
		return nil, err
	}
	var entries []FileWatchSnapshotEntry
	if err := json.Unmarshal(output, &entries); err != nil {
		return nil, errors.New("relay worker returned malformed file watch snapshot")
	}
	return entries, nil
}

func (m *Manager) SearchFiles(req FileSearchRequest) (SearchResult, error) {
	return m.SearchFilesContext(context.Background(), req)
}

func (m *Manager) SearchFilesContext(ctx context.Context, req FileSearchRequest) (SearchResult, error) {
	base, err := m.resolveWorkspacePath(req.ProjectID, req.WorktreeID)
	if err != nil {
		if errors.Is(err, ErrRemoteNeedsRelay) {
			return m.searchSshFiles(ctx, req)
		}
		return SearchResult{}, err
	}
	return SearchWorkspaceFilesContext(ctx, base, req)
}

func SearchWorkspaceFiles(base string, req FileSearchRequest) (SearchResult, error) {
	return SearchWorkspaceFilesContext(context.Background(), base, req)
}

func SearchWorkspaceFilesContext(ctx context.Context, base string, req FileSearchRequest) (SearchResult, error) {
	if err := ctx.Err(); err != nil {
		return SearchResult{}, err
	}
	...
	err = filepath.WalkDir(base, func(path string, entry os.DirEntry, walkErr error) error {
		...
		fileResult, ok := searchFile(ctx, path, relativePath, matcher, limit-result.TotalMatches)
		if !ok {
			return nil
		}
...
func (m *Manager) searchSshFiles(ctx context.Context, req FileSearchRequest) (SearchResult, error) {
	project, root, err := m.sshFileRelayScope(req.ProjectID, req.WorktreeID)
	if err != nil {
		return SearchResult{}, err
	}
	input, err := json.Marshal(struct {
		Root    string            `json:"root"`
		Request FileSearchRequest `json:"request"`
	}{Root: root, Request: req})
	if err != nil {
		return SearchResult{}, err
	}
	ctx, cancel := context.WithTimeout(ctx, 60*time.Second)
	defer cancel()
	output, err := m.runSshRelayWorkerWithInput(ctx, project.HostID, []string{"file-search-json"}, input)
	if err != nil {
		return SearchResult{}, err
	}
	var result SearchResult
	if err := json.Unmarshal(output, &result); err != nil {
		return SearchResult{}, errors.New("relay worker returned malformed file search result")
	}
	return result, nil
}
...
func resolveWorkspaceReplaceTarget(base string, relPath string) (string, error) {
	resolvedBase, err := filepath.EvalSymlinks(base)
	if err != nil {
		return "", err
	}
	parent, err := resolveWorkspaceParentPath(resolvedBase, filepath.Dir(relPath), true)
	if err != nil {
		return "", err
	}
	target := filepath.Join(parent, filepath.Base(relPath))
...
func searchFile(ctx context.Context, path string, relativePath string, matcher runtimeFileMatcher, remaining int) (SearchFileResult, bool) {
	if remaining <= 0 {
	...
	content, readErr := io.ReadAll(io.LimitReader(file, maxFileSearchReadBytes+1))
	closeErr := file.Close()
	// Why: remote search runs on the workspace host; one generated artifact must
	// not monopolize the relay worker after the Tauri request has been cancelled.
	if readErr != nil || closeErr != nil || len(content) > maxFileSearchReadBytes || ctx.Err() != nil || isLikelyBinary(content) {
		return SearchFileResult{}, false
	}
	...
	for index, line := range lines {
		...
		// Repeating a minified multi-megabyte line for every match can make an
		// otherwise bounded result consume gigabytes when serialized over SSH.
		if len(line) > maxFileSearchLineBytes {
			continue
		}
