...
func TestImportRemoteNestedReposRequiresMatchingCompletedScan(t *testing.T) {
	manager, err := NewManager(t.TempDir(), nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	requests := []struct {
		...
		{name: "missing", want: ErrRemoteNestedScanIDRequired},
		{name: "stale", scanID: "scan-stale", want: ErrRemoteNestedScanMismatch},
		{name: "partial", scanID: "scan-current", want: ErrRemoteNestedScanIncomplete},
	}
