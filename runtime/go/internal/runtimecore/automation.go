...
func (m *Manager) triggerAutomationAt(ctx context.Context, id string, req TriggerAutomationRequest, triggerTime time.Time) (AutomationRun, error) {
	...
	dispatchToken, err := m.issueAutomationDispatchToken(automation.ID, run.ID)
	if err != nil {
	...
	if hasRendererAutomationPayload(run.Payload) {
		return run, nil
	}
	return m.executeAutomationRun(ctx, automation, run)
}

func hasRendererAutomationPayload(payload map[string]interface{}) bool {
	snapshot, ok := payload[AutomationRendererPayloadKey].(map[string]interface{})
	if !ok {
		return false
	}
	mode, _ := snapshot["workspaceMode"].(string)
	return mode == "existing" || mode == "new_per_run"
}
