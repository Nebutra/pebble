 ...
 * parses `PEBBLE_STARTUP_DIAGNOSTICS=1` milestone lines from stderr, and
 ...
 *     [--exe <path-to-packaged-Pebble>] [--timeout-ms 240000]
 *
 ...
 * tiny. Layout mirrors Chromium cache dirs plus a few Pebble-owned dirs.
 */
  ...
  for (let i = 0; i < githubRepos; i++) {
    ...
    const remote = spawnSync(
      ...
      [
        ...
        'add',
        'origin',
        `https://github.com/pebble-bench/bench-gh-repo-${i}.git`
      ],
      { stdio: 'ignore' }
    )
...
function writePersistedStateFixture(fixtureDir, { stateProfile, sessionTabs, githubRepos }) {
  const dataPath = join(fixtureDir, 'pebble-data.json')
...
function buildLaunchEnvironment({ fixtureDir, githubRepos, ghShimDir }) {
  const env = {
    ...
    PEBBLE_E2E_USER_DATA_DIR: fixtureDir,
    PEBBLE_E2E_HEADLESS: '1'
  }
  if (ghShimDir) {
