...
function createLocalRepoFixture() {
  const baseDir = mkdtempSync(path.join(os.tmpdir(), 'pebble-termperf-'))
  const repoPath = path.join(baseDir, 'repo')
  mkdirSync(repoPath, { recursive: true })
  git(repoPath, 'init', '--initial-branch=main')
  git(repoPath, 'config', 'user.email', 'bench@pebble.local')
  git(repoPath, 'config', 'user.name', 'Pebble Bench')
...
async function setupWorkspaces(page, fixture) {
  return await runWithTimeout(
    'fixture registration in Pebble',
    () =>
      page.evaluate(
...
async function main() {
  ...
  try {
    ...
    if (launched) {
      ...
      // Main-process phase attribution (requires PEBBLE_PTY_SPAWN_TIMING=1 in the
