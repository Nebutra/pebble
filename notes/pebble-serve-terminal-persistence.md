# Pebble Serve Terminal Persistence

## Problem

`pebble serve` exposes terminal tabs to paired web clients through the runtime
...
- Persist `pebble serve` terminal tab, leaf, and PTY/session bindings in the
...
- Do not make browser panes supported in headless `pebble serve`.
