...
This profile investigated high Pebble renderer memory while working in
`/Users/nwparker/pebble/workspaces/pebble/goal`. The user suspected the browser
...
- Pebble runtime was reachable through the packaged CLI fallback. The public
  `/usr/local/bin/pebble` shim pointed at a removed development app path, so it
  failed before contacting the runtime.
- `pebble tab list --worktree all --json` returned `tabs: []`. There were no live
  Pebble browser tabs in the measured session.
...
- `vmmap -summary` on the renderer showed about 218 MB physical footprint and a
  ...
  library mappings.
- `pebble terminal list --worktree active --json` showed the active Codex terminal
...
2. The browser-pane retention fix is still useful: inactive worktree browser
   ...
   state remains in Pebble, and automation-visible webviews stay mounted so
   agent-browser can keep driving them.
3. Resource Usage was using `app.getAppMetrics().memory.workingSetSize` for
   Pebble app buckets. On macOS this can count large shared Electron/Chromium
...
- Browser panes now mount their backing webview only when the pane is active or
  ...
  without sleeping the main Pebble renderer.
...
The current packaged Pebble app was not running this worktree's patched code
...
`/usr/local/bin/pebble` command was still broken because it was a regular
...
conflicts. That meant Settings could not replace an Pebble-owned stale launcher,
forcing profiling to use the packaged CLI fallback.

The follow-up fix teaches the installer to recognize only generated Pebble Unix
...
and host process output by hand. This branch adds `pebble diagnostics memory`,
...
run with `--json`, including host memory, Pebble app process buckets, worktree
