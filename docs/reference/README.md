...
- How the runtime detects a wedged state lock and what bounds a usage scan on a long-lived machine, including where both deliberately differ from the Electron predecessor. See [Runtime Hang Detection and Scan Bounds](./runtime-hang-detection-and-scan-bounds.md).
