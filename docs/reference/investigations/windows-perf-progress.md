...
Historical harness: the retired Electron CDP terminal benchmark (renderer-clock phase
timings; scenarios tab-create / tab-switch / workspace-switch; local git fixture).
Main-process spawn attribution: `PEBBLE_PTY_SPAWN_TIMING=1` → `[pty-spawn-timing]` lines
...
PATH assertions in migration/electron-reference/src/main/ipc/pty.test.ts (path-separator artifacts).
...
- Electron app, entry: `migration/electron-reference/src/main/index.ts` (~1557 lines)
- Existing startup diagnostics: `PEBBLE_STARTUP_DIAGNOSTICS=1` writes `[startup] <event>` lines to stderr
  (`migration/electron-reference/src/main/startup/startup-diagnostics.ts`)
...
1. **`grantDirAcl(userData, { recursive: true })`** — `migration/electron-reference/src/main/index.ts:517-523`, win32 only,
...
the electron-vite build with `PEBBLE_STARTUP_DIAGNOSTICS=1`, milestones parsed from stderr).
...
freshly-written small files → real %APPDATA%\Pebble measured 62s for the same command.)
...
2. `readHooksJson` + JSON.parse re-read per agent-status IPC call across ~10 hook services
   (`migration/electron-reference/src/main/*/hook-service.ts` via `agent-hooks/installer-utils.ts:50`) — 10-100ms per
...
- User report: UI freezes ~5s after sending prompt; OpenCode session itself continues fine
  (visible from external terminal). So the agent process is healthy — the freeze is in Pebble's
...
1. **ConPTY output flood vs PTY batching/backpressure** (HIGH): Windows ConPTY re-renders
   full TUI frames → 10-100x output volume vs macOS. Batching in `migration/electron-reference/src/main/ipc/pty.ts`
...
2. **Sync `runtime.onPtyData` per data event before batching** (MED-HIGH):
   `migration/electron-reference/src/main/ipc/pty.ts:1376-1430` → `pebble-runtime.ts:3256-3420`: normalizeTerminalChunk +
...
3. **`mirrorUserConfig` recursive fs work in `buildPtyEnv` on PTY spawn** (MED):
   `migration/electron-reference/src/main/opencode/hook-service.ts:359-524` + `pty/overlay-mirror.ts:63-110` —
...
The actual mechanism (migration/electron-reference/src/main/opencode/hook-service.ts plugin source):
...
- Pebble's plugin POSTed that full text to the agent-hook server on EVERY event →
...
1. **Plugin throttle + cap (source fix)** — `migration/electron-reference/src/main/opencode/hook-service.ts`:
   ...
   prompts bypass the throttle slot). Plugin file is rewritten on every Pebble-launched
   OpenCode spawn, so the fix deploys to new sessions immediately.
2. **Listener-side cap (stale-plugin defense)** — `packages/product-core/shared/agent-hook-listener.ts`:
...
3. **Benchmark/regression test** — `migration/electron-reference/src/main/agent-hooks/opencode-message-part-flood-bench.test.ts`
...
4. Behavioral plugin tests — `migration/electron-reference/src/main/opencode/hook-plugin-message-part-throttle.test.ts`
...
- This machine's real packaged-Pebble userData: `%APPDATA%\Pebble` = **28,650 files / 2.06 GB**
  (mostly Chromium caches: Cache, Code Cache, GPUCache, blob_storage…).
- Measured the exact command Pebble runs in `openMainWindow()` (migration/electron-reference/src/main/index.ts:517-523):
...
- `PEBBLE_STARTUP_DIAGNOSTICS=1` → `[startup] <event>` lines on stderr (startup-diagnostics.ts).
...
- Hermetic benchmark launch path: `PEBBLE_E2E_USER_DATA_DIR=<dir>` redirects userData
  (works packaged + dev), `PEBBLE_E2E_HEADLESS=1` keeps window hidden. Dev/preview mode
  skips single-instance lock → safe alongside installed Pebble.
...
- Files: migration/electron-reference/src/main/startup/windows-user-data-acl.ts (+tests), migration/electron-reference/src/main/index.ts (wire-up +
  startup milestones), migration/electron-reference/src/main/win32-utils.ts (export identity resolver),
