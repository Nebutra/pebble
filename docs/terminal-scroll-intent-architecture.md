...
an escalation point for Pebble because private internals increase xterm upgrade
risk.

## Current Pebble Risk Points

The current Pebble branch already has several independent scroll actors:
...
The design must preserve Pebble's terminal performance priorities:
...
- Whether Pebble should patch private xterm bottom-scroll internals. The reference
  pattern does this for strict ownership, but Pebble should first try public API
