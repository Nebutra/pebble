...
// sshProbeTimeout bounds the system-ssh connectivity probe. The probe runs
// non-interactively (BatchMode=yes) so it can never hang on a password prompt;
// this ceiling additionally guards against a stalled TCP connect.
const sshProbeTimeout = 12 * time.Second

// SshTarget mirrors src/shared/ssh-types.ts SshTarget for the fields the
...
// ProbeSshTarget performs a bounded, non-interactive connectivity check using
// the system ssh binary. BatchMode=yes disables all password/passphrase prompts
// so the probe can never hang; a successful auth-free reachability check is the
// desktop's testConnection contract. Reachable-but-auth-required is reported as
// an error string, not a crash, mirroring the Electron surface.
func (m *Manager) ProbeSshTarget(ctx context.Context, id string) (SshProbeResult, error) {
	...
	// Why: never inherit an interactive askpass helper; the probe must fail
	// closed rather than pop a GUI prompt when a key needs a passphrase.
	cmd.Env = append(os.Environ(), "SSH_ASKPASS=", "DISPLAY=")
...
// findSystemSshBinary mirrors src/main/ssh/system-ssh-binary.ts resolution so
