...
} from '../../../src/shared/modifier-double-tap-detector'
...
} from '../../../src/shared/window-shortcut-policy'
import {
  emitTauriEmptyUiEvent,
  emitTauriIndexedUiEvent,
  emitTauriTerminalShortcutCaptured,
...
function sendTauriWindowShortcutAction(action: WindowShortcutAction): void {
  switch (action.type) {
    case 'zoom':
...
function canDispatchTauriDictationShortcut(): boolean {
  ...
  return Boolean(
    voice?.enabled && voice.sttModel && (voice.dictationMode ?? 'toggle') === 'toggle'
  )
}
