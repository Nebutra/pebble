...
import {
  createPebbleClaudeAccountsApi,
  createPebbleCodexAccountsApi,
...
export function installPebbleTauriPreloadApi(): void {
  ...
  api.git = createPebbleGitTextGenerationApi(api.git);
  api.runtime = createPebbleRuntimeApi(api.runtime);
  api.runtimeEnvironments = createPebbleRuntimeEnvironmentsApi(
  ...
  api.computerUsePermissions = createPebbleComputerUsePermissionsApi(
    api.computerUsePermissions,
  );
...
function createPebblePreflightApi(
  base: PreloadApi["preflight"],
): PreloadApi["preflight"] {
  return {
    ...base,
    check: async () => {
    ...
    detectRemoteWindowsTerminalCapabilities: () =>
      readHostTerminalCapabilities(requestRuntimeJson),
  };
}
