...
pub fn start_runtime_process(
    ...
    let current = refresh_process_status(&mut process);
    if current.running {
    ...
    let (child, display_executable) =
        spawn_runtime_process(&executable, &args, bearer_token.as_deref())?;
    let pid = child.id();
...
pub fn stop_runtime_process(
    ...
    let kill_error = handle.child.kill().err().map(|error| error.to_string());
...
pub fn runtime_process_status(
    ...
    Ok(refresh_process_status(&mut process))
}
...
fn spawn_runtime_process(
    ...
    match spawn_runtime_plan(&primary, bearer_token) {
        ...
        Err(error) if should_try_go_run_fallback(executable, &error) => {
            ...
            spawn_runtime_plan(&fallback, bearer_token)
...
fn spawn_runtime_plan(plan: &RuntimeSpawnPlan, bearer_token: Option<&str>) -> io::Result<Child> {
...
fn go_run_runtime_spawn_plan(runtime_args: &[String]) -> Option<RuntimeSpawnPlan> {
    ...
    let go_runtime_dir = root.join("go-runtime");
...
fn find_pebble_source_root() -> Option<PathBuf> {
    runtime_search_start_dirs()
        ...
        .find(|candidate| candidate.join("go-runtime/cmd/pebble-runtime").is_dir())
}
