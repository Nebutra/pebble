...
//!
//! Mirrors the credential *reads* Electron performs in
//! `src/main/codex-accounts/service.ts` and `src/main/rate-limits/claude-fetcher.ts`:
...
//! login/add flows are intentionally NOT here — they require the CLI's
//! browser dance and stay an explicit gap in the TS bridge.
...
/// legacy/default item (mirrors src/main/claude-accounts/keychain.ts).
...
fn read_keychain_password(service: &str) -> Result<Option<String>, String> {
...
fn parse_claude_oauth_json(raw: &str, source: &'static str) -> ClaudeOauthCredentials {
    let parsed: serde_json::Value = match serde_json::from_str(raw) {
        Ok(parsed) => parsed,
