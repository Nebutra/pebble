...
Thanks for contributing to Pebble.
...
- **Include your X (Twitter) handle!** We love giving shoutouts to our contributors when we merge features on [@pebble_build](https://x.com/pebble_build).
...
```bash
brew install --cask nebutra/pebble/pebble@rc
```

The two casks conflict because both install `Pebble.app`. Switch channels with a
normal `brew uninstall --cask` followed by the install for the other channel.
Do not use `--zap` unless you intentionally want to remove local Pebble state.
