...
The shipping Pebble repository no longer presents a root Electron application, an ambiguous root
`src/` tree, or a secondary desktop-shell workspace. The canonical layout is now `apps/desktop`,
`apps/mobile`, `packages/product-core`, `runtime/go`, and `native/*`. Repository verification
rejects restoration of the deleted Electron reference paths, dependencies, scripts, and projects.

One retirement boundary remains explicit:
...
helper, and desktop staging outputs; generated caches and obsolete desktop build copies are not part
of the source tree. Tauri, Rust, Go, browser-renderer, and immutable Tauri baseline gates now own the
cross-platform visual and behavioral evidence.

The product API contract is physically owned by
`packages/product-core/shared/preload-api-types.ts`. The canonical renderer has no relative import
that resolves into a retired desktop-main or preload implementation, and its browser surface uses a
shell-neutral WebView contract. Browser grab payloads, guest scripts, and worktree-name sanitization
are shared product modules. `verify-tauri-mainline` resolves renderer and Tauri imports and rejects
new legacy-shell dependencies, while native bridge coverage proves all 74 namespaces and 243
renderer methods are mapped.
...
2. **Complete:** shared contracts and the canonical renderer no longer depend on a legacy desktop
   main or preload implementation.
3. **Complete:** the Electron reference directory, dependencies, scripts, workflow jobs, generated
   output, and executable fixtures are removed. Cross-platform behavioral and pixel evidence is
   Tauri-owned; symlinks and compatibility forwarding directories are not acceptable.
   Relay extractions include audited PTY overlay mirroring/removal, POSIX shell
   ...
   roundtrip now lives under native runtime integration ownership rather than relay production
   ownership. CLI hook installation is also
   extracted: 14 managed agent installers and their platform-neutral file mutation primitives live
   in `packages/product-core/agent-hooks`, and `packages/product-core/cli` has no import or tsconfig
   whitelist into the desktop shell. The repository-layout gate enforces both boundaries and forbids
   retired main/preload directories from returning. Worktree path, metadata merge,
   linked-work-item, and branch-name logic were promoted to `packages/product-core/shared`, so the canonical Web
   project no longer includes implementation files from the deleted reference.
4. **Complete:** negative regression fixtures may retain rejected legacy strings because they prove
   those identifiers and dependencies cannot return to shipping source; they are not runtime
   dependencies.
