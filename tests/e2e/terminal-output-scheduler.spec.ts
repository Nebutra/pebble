...
test.describe('Terminal output scheduler', () => {
  test('background tab output bursts use the shared drain while the active tab renders', async ({
    ...
    const ptyIdsByTabId: Record<string, string> = {
      [firstTabId]: await waitForTabPtyId(pebblePage, firstTabId)
    }

    while (tabIds.length < TAB_COUNT) {
      const tabId = await createTerminalTab(pebblePage)
      await waitForActiveTerminalManager(pebblePage, 30_000)
      tabIds.push(tabId)
      ptyIdsByTabId[tabId] = await waitForTabPtyId(pebblePage, tabId)
    }

    await tabLocator(pebblePage, firstTabId).click()
    await expect
    ...
    await sendPtyCommands(pebblePage, [
      {
    ...
    await expect
      .poll(async () => (await getTerminalContent(pebblePage)).includes(foregroundMarker), {
        timeout: 5_000,
    ...
    await tabLocator(pebblePage, firstBackgroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(pebblePage), {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getTerminalContent(pebblePage)).includes(firstBackground.marker), {
        timeout: 5_000,
  ...
  test('visible bulk output uses the high-priority drain instead of synchronous xterm writes', async ({
    ...
    const activeTabId = await createTerminalTab(pebblePage)
    if (!activeTabId) {
      throw new Error('Expected a fresh terminal tab')
    }
    const ptyId = await waitForTabPtyId(pebblePage, activeTabId)
    await resetSchedulerDebug(pebblePage)
    ...
    await sendPtyCommands(pebblePage, [{ ptyId, command: floodCommand }])

    await expect
      .poll(async () => (await getTerminalContent(pebblePage, 12_000)).includes(marker), {
        timeout: 30_000,
  ...
  test('hidden overflow restores from main-owned terminal state when the tab becomes visible', async ({
    ...
    const foregroundTabId = await getActiveTabId(pebblePage)
    if (!foregroundTabId) {
      throw new Error('Expected an initial terminal tab')
    }
    const hiddenTabId = await createTerminalTab(pebblePage)
    await waitForActiveTerminalManager(pebblePage, 30_000)
    const hiddenPtyId = await waitForTabPtyId(pebblePage, hiddenTabId)

    await tabLocator(pebblePage, foregroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(pebblePage), {
        timeout: 5_000,
    ...
    await sendPtyCommands(pebblePage, [{ ptyId: hiddenPtyId, command: floodCommand }])

    await expect
      .poll(async () => mainSnapshotContains(pebblePage, hiddenPtyId, marker), {
        timeout: 30_000,
        message: 'Main-owned terminal snapshot did not capture the hidden flood marker'
      })
      .toBe(true)

    await tabLocator(pebblePage, hiddenTabId).click()
    await expect
      .poll(() => getDomActiveTabId(pebblePage), {
        timeout: 5_000,
    ...
    expect(await getTerminalContent(pebblePage)).not.toContain('Pebble skipped hidden terminal output')
  })
})
