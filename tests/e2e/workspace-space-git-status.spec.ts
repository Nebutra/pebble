...
test.describe('Workspace Space git status checks', () => {
  test('checks every scanned deletable row, including rows after the first 50', async ({
    ...
    // Why: on symlinked tmpdirs (/var→/private/var on macOS, /tmp→… on CI) Pebble
    ...
    const worktreeParent = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), 'pebble-space-git-status-'))
    )
    ...
    try {
      ...
      const repoId = await pebblePage.evaluate((testRepoPath) => {
      ...
      await loadWorktreesUntilPathsPresent(pebblePage, repoId, registeredWorktreePaths)
      ...
      await expect(pebblePage.getByText('Keep: git not checked')).toHaveCount(0)
    } finally {
      for (const worktreePath of worktreePaths) {
        try {
