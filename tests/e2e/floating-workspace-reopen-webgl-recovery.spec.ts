...
test.describe('floating workspace reopen WebGL recovery @headful', () => {
  test('reopening the floating workspace recovers a corrupted glyph atlas', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(pebblePage, 'REOPEN')
    ...
    expect(await instrumentRecoveryCounters(pebblePage)).toBe(true)

    await toggleFloatingPanel(pebblePage, false)
    ...
    const webglAttachedWhileClosed = await pebblePage.evaluate((worktreeId) => {
      const state = window.__store?.getState()
      const tab = (state?.tabsByWorktree?.[worktreeId] ?? [])[0]
      const manager = tab ? window.__paneManagers?.get(tab.id) : null
      const diagnostics = manager?.getRenderingDiagnostics?.() ?? []
    ...
    await toggleFloatingPanel(pebblePage, true)
    await settleRecoveryWindows(pebblePage)

    const counters = await readRecoveryCounters(pebblePage)
    const afterReopen = await screenshotFloatingTerminal(pebblePage)
  ...
  test('window focus regain recovers the corrupted atlas (harness control)', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(pebblePage, 'CONTROL')
    test.skip(!shots, 'WebGL was not active or atlas corruption could not be injected')
    const { baseline, corrupted } = shots!
    expect(corrupted.equals(baseline)).toBe(false)
    ...
    const afterFocus = await screenshotFloatingTerminal(pebblePage)
