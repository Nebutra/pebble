...
test('mobile subscribe mounts overlay; collapse → chip; Take back dismisses', async ({
  ...
  const overlay = pebblePage.locator('.mobile-driver-banner')
  ...
  await expectExpandedOverlayLeavesPaneReadable(pebblePage, ptyId)
  ...
  await captureAttachment(pebblePage, testInfo, 'overlay-loud.png')
  ...
  await expectChipIsCompactInPane(pebblePage, ptyId)

  await captureAttachment(pebblePage, testInfo, 'overlay-collapsed.png')
...
test('held phone-fit state mounts restore overlay without collapse', async ({
  ...
  await captureAttachment(pebblePage, testInfo, 'overlay-held-fit.png')
...
test('restore this terminal refits the active restored pane', async ({ pebblePage, electronApp }) => {
  await waitForSessionReady(pebblePage)
  await waitForActiveWorktree(pebblePage)
  await ensureTerminalVisible(pebblePage)
  await waitForActiveTerminalManager(pebblePage)
  ...
  await expect(pebblePage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })
  await expect
    .poll(() => getPaneTerminalCols(pebblePage, ptyId), {
...
test('restore all refits non-focused restored terminal panes', async ({
  ...
  const ptyIds = await waitForVisiblePanePtyIds(pebblePage, 2)
  const focusPtyId = await waitForActivePanePtyId(pebblePage)
  ...
  await expect(pebblePage.locator('.mobile-driver-banner')).toHaveCount(2, { timeout: 15_000 })

  await forcePaneToOneColumn(pebblePage, inactivePtyId)
  await expect
    .poll(() => getPaneTerminalCols(pebblePage, inactivePtyId), {
...
test('restore all recovers a hidden workspace held at narrow terminal geometry', async ({
  ...
  await ensureTerminalVisible(pebblePage)
  await waitForActiveTerminalManager(pebblePage)
  const hiddenWorkspacePtyId = await waitForActivePanePtyId(pebblePage)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: hiddenWorkspacePtyId, cols: 45, rows: 20 })
  await expect(pebblePage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })

  await forcePaneToOneColumnAndSwitchWorktree(pebblePage, hiddenWorkspacePtyId, secondWorktreeId)
  await expect
  ...
  await expect
    .poll(() => getPaneTerminalCols(pebblePage, hiddenWorkspacePtyId), {
      message: 'test harness should hold workspace 1 in the bad narrow state'
    })
    .toBeLessThanOrEqual(2)
  await ensureTerminalVisible(pebblePage)
  await waitForActiveTerminalManager(pebblePage)
  const activeWorkspacePtyId = await waitForActivePanePtyId(pebblePage)
  await installRestoreTerminalFitAutoRestoreRecorder(electronApp)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: activeWorkspacePtyId, cols: 45, rows: 20 })
  await expect(
    pebblePage.locator(`[data-pty-id="${activeWorkspacePtyId}"] .mobile-driver-banner`)
