...
test.describe('Windows project runtime smoke', () => {
  test('keeps a Windows-host project and WSL project available side by side', async ({
    ...
    const wsl = await pebblePage.evaluate(async () => ({
    ...
    const wslRepoPath = await createGitRepo('pebble-e2e-project-runtime-', 'wsl-runtime-project')

    const smoke = await pebblePage.evaluate(
    ...
    await openRepoSettings(pebblePage, smoke.hostRepoId)
    const hostSection = pebblePage.locator(`[data-settings-section="repo-${smoke.hostRepoId}"]`)
    ...
    await chooseProjectRuntime(pebblePage, smoke.hostRepoId, 'WSL')
    await expect(
      hostSection.getByText(`This project runs in ${smoke.wslDistro} via WSL.`)
    ).toBeVisible()
    const hostAfterWslUiSwitch = await pebblePage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
    ...
    await chooseProjectRuntime(pebblePage, smoke.hostRepoId, 'Windows')
    await expect(hostSection.getByText('This project runs on Windows.')).toBeVisible()
    const hostAfterWindowsUiSwitch = await pebblePage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
        project.sourceRepoIds.includes(hostRepoId)
      )
    ...
    await openRepoSettings(pebblePage, smoke.wslRepoId)
    const wslSection = pebblePage.locator(`[data-settings-section="repo-${smoke.wslRepoId}"]`)
    ...
    await pebblePage.evaluate(async (repoId) => {
