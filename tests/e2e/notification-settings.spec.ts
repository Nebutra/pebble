...
test.describe('Notification settings', () => {
  ...
  test('can be toggled from settings and disables child controls', async ({ pebblePage }) => {
    await openNotificationSettings(pebblePage)

    const notificationsSection = pebblePage.locator('[data-settings-section="notifications"]')
    ...
    await expect
      .poll(async () => (await getSettings(pebblePage)).notifications.agentTaskComplete, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getSettings(pebblePage)).notifications.enabled, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => {
        const settings = await getSettings(pebblePage)
        return {
