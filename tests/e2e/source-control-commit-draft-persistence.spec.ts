...
test.describe('Source Control commit draft persistence', () => {
  test('preserves a typed draft when the sidebar tab remounts', async ({
    ...
    try {
      ...
      await openSourceControlForWorktree(pebblePage, testRepoPath, firstWorktree.worktreePath)

      const textarea = pebblePage.getByRole('textbox', { name: 'Commit message' })
      ...
      await expect
        .poll(
          async () => pebblePage.evaluate(() => window.__store?.getState().rightSidebarTab ?? null),
          { timeout: 5_000 }
        )
        .toBe('explorer')
      ...
      await openSourceControlForWorktree(pebblePage, testRepoPath, secondWorktree.worktreePath)
      await expect(textarea).toBeVisible({ timeout: 10_000 })
