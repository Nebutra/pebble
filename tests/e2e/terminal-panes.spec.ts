...
test.describe('Terminal Panes', () => {
   ...
   * - terminal panes can be split
   */
  test('can split terminal pane right', async ({ pebblePage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(pebblePage)

    await splitActiveTerminalPane(pebblePage, 'vertical')
    await waitForPaneCount(pebblePage, paneCountBefore + 1)

    const paneCountAfter = await countVisibleTerminalPanes(pebblePage)
   ...
   * - terminal panes can be split
   */
  test('can split terminal pane down', async ({ pebblePage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(pebblePage)

    await splitActiveTerminalPane(pebblePage, 'horizontal')
    await waitForPaneCount(pebblePage, paneCountBefore + 1)
  ...
  test('split panes persist PTY bindings by stable UUID leaf id', async ({ pebblePage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(pebblePage)

    await splitActiveTerminalPane(pebblePage, 'vertical')
    await waitForPaneCount(pebblePage, paneCountBefore + 1)

    const snapshot = await waitForPaneIdentitySnapshot(pebblePage, paneCountBefore + 1)
  ...
  test('terminal process receives PEBBLE_PANE_KEY with the active UUID leaf id', async ({
    pebblePage
  }) => {
    const snapshot = await waitForPaneIdentitySnapshot(pebblePage, 1)
    ...
    const marker = `PEBBLE_PANE_KEY_E2E_${Date.now()}`

    await execInTerminal(pebblePage, ptyId, `printf '${marker}=%s\\n' "$PEBBLE_PANE_KEY"`)
    await waitForTerminalOutput(pebblePage, `${marker}=${expectedPaneKey}`)

    expect(activeLeafId).toMatch(UUID_RE)
  })

  test('terminal context menu copies the stable pane ID', async ({ pebblePage }) => {
    const snapshot = await waitForPaneIdentitySnapshot(pebblePage, 1)
    ...
    await pebblePage.getByText('Copy Pane ID', { exact: true }).click()

    await expect
      .poll(() => pebblePage.evaluate(() => window.api.ui.readClipboardText()), { timeout: 3_000 })
      .toBe(expectedPaneKey)
    await expect(pebblePage.getByText('Pane ID copied', { exact: true })).toBeVisible()
  ...
  test('first Set Title from terminal context menu stays open for typing', async ({ pebblePage }) => {
    ...
    await openTerminalContextMenu(pebblePage)
    await pebblePage.getByText('Set Title…', { exact: true }).click()

    const titleInput = pebblePage.locator('.pane-title-input').first()
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toBeFocused()
    await pebblePage.waitForTimeout(250)
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toBeFocused()
    ...
    await expect(titleInput).toHaveCount(0)
    await expect(pebblePage.locator('.pane-title-text', { hasText: title })).toHaveCount(1)
  })

  test('Set Title editor renders in Pebble overlay while terminal reserves title space', async ({
    pebblePage
  }) => {
    const title = `Reserved overlay title ${Date.now()}`
    const terminalBoxBefore = await readVisibleXtermContainerBox(pebblePage)

    await openTerminalContextMenu(pebblePage)
    await pebblePage.getByText('Set Title…', { exact: true }).click()

    const titleInput = pebblePage.locator('.pane-title-overlay-layer .pane-title-input').first()
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toBeFocused()
    await expect(pebblePage.getByText('Set Title…', { exact: true })).toBeHidden()
    await expect(pebblePage.locator('.pane .pane-title-input')).toHaveCount(0)
    await expect(pebblePage.locator('.pane[data-has-title]')).toHaveCount(1)
    await expect
      .poll(() =>
        pebblePage
    ...
    const terminalBoxEditing = await readVisibleXtermContainerBox(pebblePage)
    expectTerminalToReserveTitleSpace(terminalBoxEditing, terminalBoxBefore)

    await titleInput.fill(title)
    await titleInput.press('Enter')
    await expect(pebblePage.locator('.pane-title-text', { hasText: title })).toBeVisible()
    await expect(pebblePage.locator('.pane[data-has-title]')).toHaveCount(1)
    expectTerminalToReserveTitleSpace(
      await readVisibleXtermContainerBox(pebblePage),
      terminalBoxBefore
    )
  })

  test('Set Title context menu opens from the title overlay strip', async ({ pebblePage }) => {
    ...
    await openPaneTitleContextMenu(pebblePage, title)
    await pebblePage.getByText('Set Title…', { exact: true }).click()
    ...
    await expect(pebblePage.locator('.pane-title-text', { hasText: updatedTitle })).toHaveCount(1)
    await expect(pebblePage.locator('.pane-title-text', { hasText: title })).toHaveCount(0)
  })

  test('Set Title strip activates its pane and accepts file-path drops', async ({ pebblePage }) => {
    const title = `Drop target title ${Date.now()}`
    const droppedPath = `/tmp/title-drop-${Date.now()}.txt`

    await setPaneTitleFromTerminalMenu(pebblePage, title)
    const initialSnapshot = await waitForPaneIdentitySnapshot(pebblePage, 1)
    ...
    const splitSnapshot = await waitForPaneIdentitySnapshot(pebblePage, 2)
    ...
    await expect
      .poll(async () => (await readPaneIdentitySnapshot(pebblePage))?.activeLeafId ?? null)
      .toBe(otherPane.leafId)

    const titleBar = pebblePage.locator('.pane-title-bar', { hasText: title }).first()
    ...
    await titleBar.evaluate((element, path) => {
      ...
      dataTransfer.setData('text/x-pebble-file-path', path)
    ...
    await expect
      ...
      .toBe(titledLeafId)
    await expect
      .poll(async () => (await getTerminalContent(pebblePage)).includes(droppedPath), {
        timeout: 5_000,
  ...
  test('Set Title overlay follows its pane after same-count pane move', async ({ pebblePage }) => {
    ...
    await splitActiveTerminalPane(pebblePage, 'vertical')
    await waitForPaneCount(pebblePage, 2)
    const beforeMove = await waitForPaneIdentitySnapshot(pebblePage, 2)
    ...
    const beforeOrder = await readTerminalPaneDomLeafOrder(pebblePage)

    await expectPaneTitleAttachedToLeaf(pebblePage, title, titledLeafId)
    await moveTerminalPaneByLeafId(pebblePage, titledLeafId, target.leafId, 'right')

    await expect
      .poll(async () => readTerminalPaneDomLeafOrder(pebblePage), {
        timeout: 10_000,
  ...
  test('Set Title keeps the pane drag handle available over the title strip', async ({
    ...
    const titleTopHit = await pebblePage.evaluate(
      ({ title, titledLeafId }) => {
    ...
    await pebblePage.locator('.pane-title-bar', { hasText: title }).click({
      position: { x: 20, y: 18 }
    })
  ...
  test('@headful Set Title pane can be dragged from the title strip', async ({ pebblePage }) => {
    ...
    await splitActiveTerminalPane(pebblePage, 'vertical')
    await waitForPaneCount(pebblePage, 2)
    const beforeDrag = await waitForPaneIdentitySnapshot(pebblePage, 2)
    ...
    const titleDragHandle = pebblePage
    ...
    await pebblePage.mouse.move(sourceBox!.x + sourceBox!.width / 2, sourceBox!.y + 4)
    await pebblePage.mouse.down()
    await pebblePage.mouse.move(targetDropX, targetBox!.y + targetBox!.height / 2, {
      steps: 20
    })
    await pebblePage.mouse.up()
    ...
    await expect
      ...
      .not.toEqual(beforeOrder)
    const afterDrag = await waitForPaneIdentitySnapshot(pebblePage, 2)
  ...
  test('Set Title input stays open when clicked in a split terminal', async ({ pebblePage }) => {
    await splitActiveTerminalPane(pebblePage, 'vertical')
    await waitForPaneCount(pebblePage, 2)
    await splitActiveTerminalPane(pebblePage, 'horizontal')
    await waitForPaneCount(pebblePage, 3)
  ...
  test('Set Title survives an early blur during first focus handoff', async ({ pebblePage }) => {
    await openTerminalContextMenu(pebblePage)
    await pebblePage.evaluate(() => {
  ...
  test('Set Title survives delayed terminal focus handoffs', async ({ pebblePage }) => {
    await openTerminalContextMenu(pebblePage)
    await installDelayedTerminalFocusSteals(pebblePage, [50, 150, 300])
    await pebblePage.getByText('Set Title…', { exact: true }).click()
  ...
  test('Set Title preserves draft text across terminal focus steals', async ({ pebblePage }) => {
  ...
  test('Set Title does not submit when synthetic focus restore fails', async ({ pebblePage }) => {
    ...
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toHaveValue(draftTitle)
    await expect(pebblePage.locator('.pane-title-text', { hasText: draftTitle })).toHaveCount(0)
  })
  ...
  test('Set Title still commits by blur after synthetic terminal focus steals', async ({
    ...
    await openTerminalContextMenu(pebblePage)
    await installDelayedTerminalFocusSteals(pebblePage, [50, 150])
    await pebblePage.getByText('Set Title…', { exact: true }).click()

    const titleInput = pebblePage.locator('.pane-title-input').first()
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toBeFocused()
    await pebblePage.waitForTimeout(300)
    await titleInput.fill(title)
    await pebblePage
      .locator('.xterm:visible')
      .first()
  ...
  test('Set Title commits when tabbing away from the title input', async ({ pebblePage }) => {
  ...
  test('Set Title overlay hides with its inactive terminal tab', async ({ pebblePage }) => {
    ...
    await pressShortcut(pebblePage, 't')
    await expect
      .poll(async () => (await getWorktreeTabs(pebblePage, worktreeId)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)
    await expect(pebblePage.locator('.pane-title-text', { hasText: title })).toBeHidden()

    await pressShortcut(pebblePage, 'BracketLeft', { shift: true })
    await expect(pebblePage.locator('.pane-title-text', { hasText: title })).toBeVisible()
  })

  test('Set Title still commits by blur after focus settles', async ({ pebblePage }) => {
  ...
  test('Always-on pane header split button hover stays transparent', async ({ pebblePage }) => {
    const splitButton = pebblePage.getByRole('button', { name: 'Split Terminal Right' })
  ...
  test('Set Title stays pane-local during agent title churn', async ({ pebblePage }) => {
    const worktreeId = (await getActiveWorktreeId(pebblePage))!
    const tabId = (await getActiveTabId(pebblePage))!
    ...
    await setPaneTitleFromTerminalMenu(pebblePage, paneTitle)
    await expect(pebblePage.locator('.pane-title-text', { hasText: paneTitle })).toBeVisible()
    await expectTabCustomTitle(pebblePage, worktreeId, tabId, null)

    await pebblePage.getByRole('button', { name: `Edit pane title: ${paneTitle}` }).focus()
    await pebblePage.keyboard.press('Enter')
    const paneTitleInput = pebblePage.getByRole('textbox', { name: 'Pane title' })
    ...
    // Why: active agents continuously write OSC titles. Set Title is Pebble's
    // pane-local overlay and must remain visible while the tab runtime title
    // continues to follow the active PTY.
    await expect(pebblePage.locator('.pane-title-text', { hasText: paneTitle })).toBeVisible()
    await expect(
      pebblePage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"]`)
    ).toHaveAttribute('data-tab-title', runtimeTitle)
    await expectTabCustomTitle(pebblePage, worktreeId, tabId, null)

    await setPaneTitleFromTerminalMenu(pebblePage, '')
    await expect(pebblePage.locator('.pane-title-text', { hasText: paneTitle })).toBeHidden()
    await expectSavedLayoutNotToContainTitle(pebblePage, tabId, paneTitle)
    ...
    await splitActiveTerminalPane(pebblePage, 'vertical')
    await waitForPaneCount(pebblePage, 2)
    await expect(pebblePage.locator('.pane-title-text', { hasText: splitTitle })).toBeVisible()
  ...
  test('closing a split pane prunes its leaf-keyed PTY binding without remapping siblings', async ({
    ...
    const beforeClose = await waitForPaneIdentitySnapshot(pebblePage, 3)
    ...
    await closeActiveTerminalPane(pebblePage)
    await waitForPaneCount(pebblePage, 2)

    const afterClose = await waitForPaneIdentitySnapshot(pebblePage, 2)
  ...
  test('closing and remaking right/down splits keeps surviving leaf-keyed bindings stable', async ({
    ...
    const afterRemake = await waitForPaneIdentitySnapshot(pebblePage, 3)
  ...
  test('moving panes through the drag-drop handler preserves leaf-keyed PTY bindings', async ({
    ...
    const beforeMove = await waitForPaneIdentitySnapshot(pebblePage, 3)
    const beforeOrder = await readTerminalPaneDomLeafOrder(pebblePage)
    ...
    await moveTerminalPaneByLeafId(pebblePage, source.leafId, target.leafId, 'left')
    ...
    const afterMove = await waitForPaneIdentitySnapshot(pebblePage, 3)
   ...
   * - terminal panes retain state when switching tabs and when you make / close a pane / switch worktrees
   */
  test('terminal pane retains content when switching tabs and back', async ({ pebblePage }) => {
    ...
    const activeType = await getActiveTabType(pebblePage)
    ...
    await expect
      .poll(async () => (await getTerminalContent(pebblePage)).includes(marker), { timeout: 5_000 })
      .toBe(true)

    // Clean up the extra tab
    await pressShortcut(pebblePage, 'BracketRight', { shift: true })
    await pressShortcut(pebblePage, 'w')
  })
  ...
  test('terminal pane retains content when splitting and closing a pane', async ({ pebblePage }) => {
    ...
    await splitActiveTerminalPane(pebblePage, 'vertical')
    await waitForPaneCount(pebblePage, panesBefore + 1)

    await focusLastTerminalPane(pebblePage)
    await closeActiveTerminalPane(pebblePage)
    await waitForPaneCount(pebblePage, panesBefore)
   ...
   * - resizing terminal panes works
   */
  test('shows a pane divider after splitting', async ({ pebblePage }) => {
    ...
    await expect(pebblePage.locator('.pane-divider.is-vertical').first()).toBeVisible({
      timeout: 3_000
    })
  })
  ...
  test('@headful can resize terminal panes by real mouse drag', async ({ pebblePage }) => {
    ...
    const paneWidthsBefore = await pebblePage.evaluate(() => {
    ...
    const divider = pebblePage.locator('.pane-divider.is-vertical').first()
    ...
    await pebblePage.mouse.move(startX, startY)
    await pebblePage.mouse.down()
    await pebblePage.mouse.move(startX + 150, startY, { steps: 20 })
    await pebblePage.mouse.up()
    ...
    await expect
      .poll(
        async () => {
          const widthsAfter = await pebblePage.evaluate(() => {
            const xterms = document.querySelectorAll('.xterm')
            return Array.from(xterms)
              .filter((x) => (x as HTMLElement).offsetParent !== null)
              .map((x) => (x as HTMLElement).getBoundingClientRect().width)
  ...
  test('@headful resizing split panes forwards only the settled PTY size', async ({ pebblePage }) => {
    ...
    await expect
      .poll(
        async () =>
          (await readVisiblePaneContents(pebblePage)).every((content) =>
    ...
    await pebblePage.mouse.move(startX, startY)
    await pebblePage.mouse.down()
    await pebblePage.mouse.move(startX - 350, startY, { steps: 40 })
    await pebblePage.mouse.move(startX + 250, startY, { steps: 40 })
    await pebblePage.mouse.up()
    await pebblePage.waitForTimeout(500)

    const paneContents = await readVisiblePaneContents(pebblePage)
  ...
  test('@headful dragging terminal panes around preserves leaf-keyed PTY bindings', async ({
    ...
    const beforeDrag = await waitForPaneIdentitySnapshot(pebblePage, 3)
    const beforeOrder = await readTerminalPaneDomLeafOrder(pebblePage)
    ...
    const sourceHandle = pebblePage.locator(
    ...
    await pebblePage.mouse.move(sourceBox!.x + sourceBox!.width / 2, sourceBox!.y + 4)
    await pebblePage.mouse.down()
    await pebblePage.mouse.move(targetBox!.x + 8, targetBox!.y + targetBox!.height / 2, {
      steps: 20
    })
    await pebblePage.mouse.up()
    ...
    const afterDrag = await waitForPaneIdentitySnapshot(pebblePage, 3)
    expect(afterDrag.panes.map((pane) => pane.leafId).sort()).toEqual(
      beforeDrag.panes.map((pane) => pane.leafId).sort()
    )
   ...
   * - closing panes works
   */
  test('closing a split pane removes it and remaining pane fills space', async ({ pebblePage }) => {
    const panesBefore = await countVisibleTerminalPanes(pebblePage)
    ...
    const panesAfterSplit = await countVisibleTerminalPanes(pebblePage)
    expect(panesAfterSplit).toBeGreaterThanOrEqual(2)

    await closeActiveTerminalPane(pebblePage)
    await waitForPaneCount(pebblePage, panesAfterSplit - 1)

    // The remaining pane should fill the available space
    const paneWidth = await pebblePage.evaluate(() => {
      const xterms = document.querySelectorAll('.xterm')
