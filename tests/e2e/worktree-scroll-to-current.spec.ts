...
test.describe('Reveal active workspace button', () => {
  test.beforeEach(async ({ pebblePage }) => {
    ...
    await pebblePage.emulateMedia({ reducedMotion: 'reduce' })
  ...
  test('clears sidebar filters before revealing a hidden current workspace', async ({
    ...
    await prepareSidebarForScrollTest(pebblePage)

    const renderedOptions = pebblePage.locator('[data-worktree-sidebar] [role="option"]')
    ...
    const targetRows = pebblePage.locator(
      `[data-worktree-sidebar] [data-worktree-id=${JSON.stringify(targetId)}]`
    )
    const targetRow = targetRows.first()
    const revealButton = pebblePage.getByRole('button', { name: 'Reveal active workspace' })

    await pebblePage.evaluate((targetId) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
  ...
  test('reveals the current workspace when it starts outside the virtualized window', async ({
    ...
    const targetId = await pebblePage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
    ...
    const scroller = pebblePage.locator('[data-worktree-sidebar]')
    await expect
      .poll(
        () =>
          pebblePage.evaluate(() => {
  ...
  test('uses the active workspace key when the legacy active worktree id is not set', async ({
    ...
    const folderWorktreeId = await pebblePage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
