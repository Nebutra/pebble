 ...
 * - PR #677 (`return to Pebble landing screen after closing last terminal`)
...
test.describe('Tab Close Navigation', () => {
   ...
   */
  test('closing the active editor tab activates its visual neighbor', async ({ pebblePage }) => {
    const worktreeId = await waitForActiveWorktree(pebblePage)

    const fileIds = await openSeededEditorTabs(pebblePage, worktreeId, [
      'package.json',
      'README.md',
      'tsconfig.json'
    ])
    ...
    await setActiveFile(pebblePage, fileIds[1])
    await expect.poll(async () => getActiveFileId(pebblePage), { timeout: 3_000 }).toBe(fileIds[1])

    await closeFile(pebblePage, fileIds[1])
    ...
    await expect
      .poll(async () => getActiveFileId(pebblePage), {
        timeout: 5_000,
   ...
   */
  test('closing the active diff tab activates a still-open neighbor', async ({ pebblePage }) => {
    const worktreeId = await waitForActiveWorktree(pebblePage)

    // Seed two editor tabs + one diff tab in the same worktree.
    const editorIds = await openSeededEditorTabs(pebblePage, worktreeId, [
      'package.json',
      'README.md'
    ])
    expect(editorIds.length).toBe(2)

    const diffId = await pebblePage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
        return null
      }
    ...
    await expect.poll(async () => getActiveFileId(pebblePage), { timeout: 3_000 }).toBe(diffId)

    await closeFile(pebblePage, diffId!)
   ...
   */
  test('closing the last visible surface returns the app to Landing', async ({ pebblePage }) => {
    const worktreeId = await waitForActiveWorktree(pebblePage)
    ...
    const editorIds = await openSeededEditorTabs(pebblePage, worktreeId, ['package.json'])
    expect(editorIds.length).toBe(1)

    await setActiveFile(pebblePage, editorIds[0])
    await expect.poll(async () => getActiveFileId(pebblePage), { timeout: 3_000 }).toBe(editorIds[0])
    ...
    await closeFile(pebblePage, editorIds[0])
