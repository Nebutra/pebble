...
test.describe('Diff note delete', () => {
  ...
  test('clicking the trash button removes the saved note', async ({ pebblePage }) => {
    ...
    const deleteButton = pebblePage.getByTitle('Delete note').first()
    ...
    await expect(pebblePage.getByTitle('Delete note')).toHaveCount(0, {
      timeout: 5_000
    })
  })
})
