...
test.describe('floating workspace shared glyph atlas renderer evidence', () => {
  test('switching floating workspace tabs keeps workspace terminal glyphs intact', async ({
    ...
    const scenario = await setUpSharedAtlasScenario(pebblePage)
    ...
    await activateFloatingTab(pebblePage, floatingTabIds[1])
    // Why: the switched-to tab attaching WebGL proves the suspend/resume
    // (and with it the atlas reset trigger) actually ran.
    expect(
      await waitForWebglOnTab(pebblePage, floatingTabIds[1]),
      'switched-to floating tab should resume WebGL'
    ).toBe(true)
    await settleAtlasActivity(pebblePage)
    await toggleFloatingPanel(pebblePage, false)

    const afterSwitch = await captureWorkspaceAfterTrigger(pebblePage, scenario!)
  ...
  test('reopening the floating workspace keeps workspace terminal glyphs intact', async ({
    ...
    const afterReopen = await captureWorkspaceAfterTrigger(pebblePage, scenario!)
    await testInfo.attach('baseline', { body: baseline, contentType: 'image/png' })
