...
test.describe('Tasks page', () => {
  ...
  test('opening the tasks view renders the tasks UI', async ({ pebblePage }) => {
    await openTasksPage(pebblePage)

    await expect
      .poll(async () => getStoreState<string>(pebblePage, 'activeView'), { timeout: 5_000 })
      .toBe('tasks')

    await expect(pebblePage.getByRole('button', { name: 'Close tasks' })).toBeVisible({
      timeout: 10_000
    })
    ...
    await expect
      .poll(
        async () => {
          renderedSources = await getRenderedTaskSources(pebblePage)
    ...
    if (renderedSources.some((source) => source.source === 'github' && source.active)) {
      await expect(pebblePage.getByRole('button', { name: 'Issues', exact: true })).toBeVisible()
      await expect(pebblePage.getByRole('button', { name: 'PRs', exact: true })).toBeVisible()
      await expect(pebblePage.getByRole('button', { name: 'Projects', exact: true })).toBeVisible()
      await expect(pebblePage.getByPlaceholder(/Search GitHub (issues|PRs)/i)).toBeVisible()
    }
  })

  test('closing the tasks page returns to the previous view', async ({ pebblePage }) => {
    const previousView = await getStoreState<string>(pebblePage, 'activeView')
    ...
    await pebblePage.getByRole('button', { name: 'Close tasks' }).click()
    ...
    await expect(pebblePage.getByRole('button', { name: 'Close tasks' })).toHaveCount(0)
    if (previousView === 'terminal') {
      await expect(pebblePage.locator('.xterm').first()).toBeVisible({ timeout: 5_000 })
    }
  })
})
