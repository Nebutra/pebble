...
test.describe('Markdown prose reflow', () => {
  ...
  test('hard-wrapped prose reflows as one document paragraph', async ({ pebblePage }, testInfo) => {
    let filePath: string | null = null

    try {
      filePath = await openHardWrappedFixture(pebblePage, testInfo)
      const metrics = await getGoalParagraphMetrics(pebblePage)
  ...
  test('deleting an inserted empty paragraph keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      filePath = await openHardWrappedFixture(pebblePage, testInfo)
      await placeCaretAtGoalParagraphEnd(pebblePage)
  ...
  test('deleting slash text then the empty paragraph keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      ...
      await placeCaretAtGoalParagraphEnd(pebblePage)
      await pebblePage.keyboard.press('Enter')
      await pebblePage.keyboard.type('/')
      await pebblePage.keyboard.press('Backspace')
      await pebblePage.keyboard.press('Backspace')
  ...
  test('deleting the block boundary before a heading keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      filePath = await openHardWrappedFixture(pebblePage, testInfo)
      await placeCaretAtHeadingStart(pebblePage)
      await pebblePage.keyboard.press('Backspace')
