...
async function addAndActivateRepo(pebblePage: Page, repoPath: string): Promise<string> {
  const repoId = await pebblePage.evaluate(async (pathToRepo: string) => {
  ...
  await expect
    .poll(
      () =>
        pebblePage.evaluate(async (targetRepoId: string) => {
          const store = window.__store
          if (!store) {
            return 0
          }
  ...
  const worktreeId = await pebblePage.evaluate(
...
test.describe('Large diff freeze repro', () => {
  ...
  test('opening a large single-file diff keeps the renderer responsive', async ({ pebblePage }) => {
    await waitForSessionReady(pebblePage)
    const fixture = createIsolatedLargeDiffRepo()
    const lineCount = Number(process.env.PEBBLE_LARGE_DIFF_REPRO_LINES ?? '60000')
    if (!Number.isFinite(lineCount) || lineCount < 0) {
      throw new Error(
        `Invalid PEBBLE_LARGE_DIFF_REPRO_LINES: ${process.env.PEBBLE_LARGE_DIFF_REPRO_LINES}`
      )
    }
    ...
    try {
      ...
      const measurement = await pebblePage.evaluate(
