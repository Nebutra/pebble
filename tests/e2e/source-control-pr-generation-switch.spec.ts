...
test.describe('Source Control AI PR generation worktree switching', () => {
  ...
  test('keeps checks-panel PR generation running after switching worktrees', async ({
    ...
    const { primaryWorktreeId, prWorktreeId, prWorktreePath, primaryBranch } =
      await seedCreatePrComposer(pebblePage)
    ...
    await installDelayedPrGenerator(pebblePage, generatorScriptPath, callLogPath, primaryBranch)

    await openChecks(pebblePage, prWorktreeId)
    const generate = pebblePage.getByRole('button', {
    ...
    await expect(
      pebblePage.getByRole('button', { name: 'Stop generating pull request details' })
    ).toBeVisible()
    await expect.poll(() => readLog(callLogPath)).toContain('start')
    await pebblePage.screenshot({
      path: path.join(screenshotDir, '01-checks-pr-generation-pending-on-a.png')
    })

    await openChecks(pebblePage, primaryWorktreeId)
    await expect(pebblePage.getByText('Generated PR title after switch')).toHaveCount(0)
    await pebblePage.screenshot({
    ...
    await openChecks(pebblePage, prWorktreeId)
    await expect(pebblePage.getByRole('textbox', { name: 'Pull request title' })).toHaveValue(
      'Generated PR title after switch',
      { timeout: 10_000 }
    )
    await expect(pebblePage.getByRole('textbox', { name: 'Pull request description' })).toHaveValue(
  ...
  test('keeps pending PR generation attached to its original worktree', async ({
    ...
    await openSourceControl(pebblePage, prWorktreeId)
    const generate = pebblePage.getByRole('button', {
      name: 'Generate pull request details with AI'
    })
    await expect(generate).toBeVisible({ timeout: 10_000 })
    ...
    await expect
      ...
      .toContain('start')
    const pendingEvidence = await pebblePage.evaluate(() => {
    ...
    await openSourceControl(pebblePage, primaryWorktreeId)
    await expect(pebblePage.getByText('Generated PR title after switch')).toHaveCount(0)
    const switchedEvidence = await pebblePage.evaluate(() => {
      const state = window.__store?.getState()
      return {
        activeWorktreeId: state?.activeWorktreeId,
    ...
    await expect
      ...
      .toContain('finish')
    await waitForPrGenerationStored(pebblePage, prWorktreeId)
    await openSourceControl(pebblePage, prWorktreeId)
    await waitForPrGenerationHydrated(pebblePage, prWorktreeId)
    await expect(pebblePage.getByRole('textbox', { name: 'Pull request title' })).toHaveValue(
      'Generated PR title after switch',
      { timeout: 10_000 }
    )
    ...
    const finalEvidence = await pebblePage.evaluate(() => {
      const state = window.__store?.getState()
      return {
        activeWorktreeId: state?.activeWorktreeId,
  ...
  test('keeps Create PR intent running after switching worktrees', async ({
        ...
        store.setState((current) => ({
          ...
          createHostedReview: async (repoPath, input) => {
            ...
            return {
              ok: true as const,
              number: 74,
              url: 'https://github.com/acme/pebble/pull/74'
            }
          },
          gitStatusByWorktree: {
    ...
    await openSourceControl(pebblePage, prWorktreeId)
    const createPr = pebblePage.getByRole('button', { name: 'Create PR' }).first()
    ...
    const completedWhileSwitchedEvidence = await pebblePage.evaluate(() => {
      const state = window.__store?.getState()
      return {
        activeWorktreeId: state?.activeWorktreeId,
        rightSidebarTab: state?.rightSidebarTab
    ...
    await openSourceControl(pebblePage, prWorktreeId)
    const payloads = await pebblePage.evaluate(
      () =>
        (
  ...
  test('hydrates pending PR generation after Source Control remounts', async ({
    ...
    await waitForSessionReady(pebblePage)
    await waitForActiveWorktree(pebblePage)
    const { prWorktreeId, prWorktreePath, primaryBranch } = await seedCreatePrComposer(pebblePage)
    createBranchCommit(prWorktreePath)
  ...
  test('keeps pending commit message generation attached to its original worktree', async ({
    ...
    const { primaryWorktreeId, commitWorktreeId, commitWorktreePath } =
      await seedCommitMessageComposer(pebblePage)
    ...
    await installDelayedCommitMessageGenerator(pebblePage, generatorScriptPath, callLogPath)

    await openSourceControl(pebblePage, commitWorktreeId)
    await expect(pebblePage.getByText('e2e-commit-message-generation.txt')).toBeVisible({
      timeout: 10_000
    })
    const generate = pebblePage.getByRole('button', {
    ...
    await generate.click()
    await expect(
      pebblePage.getByRole('button', { name: 'Stop generating commit message' })
    ).toBeVisible()
    await expect
      .poll(() => {
        return readLog(callLogPath)
    ...
    await openSourceControl(pebblePage, primaryWorktreeId)
    await expect(pebblePage.getByText('Generated commit message after switch')).toHaveCount(0)
    await expect(
      pebblePage.getByRole('button', { name: 'Stop generating commit message' })
    ).toHaveCount(0)
    const switchedEvidence = await pebblePage.evaluate(() => {
    ...
    await expect
      .poll(() => readFileSync(callLogPath, 'utf8'), { timeout: 10_000 })
      .toContain('finish')
    await waitForCommitGenerationStored(pebblePage, commitWorktreeId)
    await openSourceControl(pebblePage, commitWorktreeId)
    await waitForCommitGenerationHydrated(pebblePage, commitWorktreeId)
    await expect(pebblePage.getByRole('textbox', { name: 'Commit message' })).toHaveValue(
      'Generated commit message after switch\n\nGenerated from staged e2e-commit-message-generation.txt after switching worktrees',
      { timeout: 10_000 }
    )
    const finalEvidence = await pebblePage.evaluate(() => {
  ...
  test('hydrates pending commit message generation after Source Control remounts', async ({
    ...
    await waitForSessionReady(pebblePage)
    await waitForActiveWorktree(pebblePage)
    const { commitWorktreeId, commitWorktreePath } = await seedCommitMessageComposer(pebblePage)
    createStagedCommitMessageChange(commitWorktreePath)
  ...
  test('hides the commit AI composer on a clean branch empty state', async ({
    ...
    await waitForSessionReady(pebblePage)
    await waitForActiveWorktree(pebblePage)
    const primaryWorktreeId = await seedCleanBranchEmptyState(pebblePage)
    ...
    await expect
      .poll(
        async () => {
          ...
          await seedCleanBranchEmptyState(pebblePage, primaryWorktreeId)
          return pebblePage.evaluate(() => {
