...
test.describe('Terminal pinned viewport worktree switch', () => {
  test('does not jump or flash when returning to a viewport pinned just above bottom', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.pebble-pinned-viewport-${runId}.mjs`)
    ...
    try {
      ...
      const pinned = await pinActiveTerminalNearBottom(pebblePage)
      ...
      const samplesPromise = sampleTerminalViewportDuringReturn(pebblePage, pinned.tabId, 450)
