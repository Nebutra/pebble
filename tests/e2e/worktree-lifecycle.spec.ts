...
test.describe('Worktree Lifecycle', () => {
  ...
  test('removing a worktree clears its tabs, open files, and browser tabs', async ({
    ...
    const originalWorktreeId = await waitForActiveWorktree(pebblePage)

    createdWorktreeId = await createIsolatedWorktree(pebblePage)
    const newWorktreeId = createdWorktreeId
    await switchToWorktree(pebblePage, newWorktreeId)
    await expect
    ...
    await pebblePage.evaluate((worktreeId) => {
      const store = window.__store
      if (!store) {
        return
      }
    ...
    await openFileExplorer(pebblePage)
    await clickFileInExplorer(pebblePage, ['README.md', 'package.json'])

    // Baseline: the new worktree now has tabs/browser tabs/open files.
    expect((await getWorktreeTabs(pebblePage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getBrowserTabs(pebblePage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getOpenFiles(pebblePage, newWorktreeId)).length).toBeGreaterThan(0)
    ...
    // is protecting.
    await switchToWorktree(pebblePage, originalWorktreeId)
    await expect
      .poll(async () => getActiveWorktreeId(pebblePage), { timeout: 10_000 })
      .toBe(originalWorktreeId)

    const result = await removeWorktreeViaStore(pebblePage, newWorktreeId)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(pebblePage, newWorktreeId)).length, {
        timeout: 10_000,
        message: 'tabsByWorktree still holds entries for the removed worktree'
      })
      .toBe(0)
    await expect
      .poll(async () => (await getBrowserTabs(pebblePage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)
    await expect
      .poll(async () => (await getOpenFiles(pebblePage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)

    const allIds = await getAllWorktreeIds(pebblePage)
  ...
  test('switching worktrees preserves per-worktree state across a round-trip', async ({
    ...
    const originalState = await pebblePage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
    ...
    const afterRoundTrip = await pebblePage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
   ...
   */
  test('terminal tabs stay scoped to the worktree that created them', async ({ pebblePage }) => {
    const allIds = await getAllWorktreeIds(pebblePage)
    expect(
      allIds.length,
      'fixture should provide primary + e2e-secondary worktrees'
    ...
    const worktreeA = await waitForActiveWorktree(pebblePage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(pebblePage, worktreeA)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    // Switch to B and create a tab there too.
    await switchToWorktree(pebblePage, worktreeB)
    await expect
      .poll(async () => getActiveWorktreeId(pebblePage), { timeout: 10_000 })
      .toBe(worktreeB)
    await ensureTerminalVisible(pebblePage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(pebblePage, worktreeB)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    const tabsA = await getWorktreeTabs(pebblePage, worktreeA)
    const tabsB = await getWorktreeTabs(pebblePage, worktreeB)
