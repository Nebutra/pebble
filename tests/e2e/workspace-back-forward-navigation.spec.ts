...
test.describe('Workspace Back/Forward Navigation', () => {
  ...
  test('buttons are hidden outside the terminal view', async ({ pebblePage }) => {
    await expect(await getBackButton(pebblePage)).toBeVisible()
    await expect(await getForwardButton(pebblePage)).toBeVisible()
    ...
    await expect(await getBackButton(pebblePage)).toHaveCount(0)
    await expect(await getForwardButton(pebblePage)).toHaveCount(0)
  ...
  test('both buttons disabled at cold start with a single history entry', async ({ pebblePage }) => {
    ...
    const activeId = await getActiveWorktreeId(pebblePage)
    expect(activeId).not.toBeNull()

    await resetNavHistory(pebblePage)
    await seedVisit(pebblePage, activeId!)

    const back = await getBackButton(pebblePage)
    const forward = await getForwardButton(pebblePage)
    await expect(back).toBeDisabled()
    await expect(forward).toBeDisabled()
  })

  test('clicking Back and Forward walks the history stack', async ({ pebblePage }) => {
    const worktreeIds = await getAllWorktreeIds(pebblePage)
    ...
    await resetNavHistory(pebblePage)
    await seedVisit(pebblePage, primaryId)
    await seedVisit(pebblePage, secondaryId)
    ...
    const primaryRow = worktreeRow(pebblePage, primaryId)
    const secondaryRow = worktreeRow(pebblePage, secondaryId)

    await back.click()
    await expect
      .poll(async () => getActiveWorktreeId(pebblePage), {
  ...
  test('re-activating the current worktree is a no-op (dedupe)', async ({ pebblePage }) => {
    const activeId = await getActiveWorktreeId(pebblePage)
    expect(activeId).not.toBeNull()
    ...
    const snapshot = await getNavHistorySnapshot(pebblePage)
    expect(snapshot.history).toEqual([activeId])
    expect(snapshot.index).toBe(0)
    await expect(await getBackButton(pebblePage)).toBeDisabled()
  })

  test('new navigation after going back truncates the forward stack', async ({ pebblePage }) => {
    const worktreeIds = await getAllWorktreeIds(pebblePage)
    ...
    await seedVisit(pebblePage, primaryId)
    await seedVisit(pebblePage, secondaryId)
    await (await getBackButton(pebblePage)).click()
    await expect.poll(() => getActiveWorktreeId(pebblePage)).toBe(primaryId)

    // Forward button is live — a forward entry exists.
    await expect(await getForwardButton(pebblePage)).toBeEnabled()
    ...
    await expect(await getForwardButton(pebblePage)).toBeDisabled()
  })

  test(`${isMac ? 'Cmd' : 'Ctrl'}+Alt+Left/Right shortcuts walk history`, async ({ pebblePage }) => {
    const worktreeIds = await getAllWorktreeIds(pebblePage)
    ...
    await pebblePage.keyboard.press(`${mod}+Alt+ArrowRight`)
    await expect
      .poll(async () => getActiveWorktreeId(pebblePage), {
  ...
  test('shortcut is a no-op in settings view', async ({ pebblePage }) => {
    const worktreeIds = await getAllWorktreeIds(pebblePage)
    ...
    await expect
      .poll(async () => pebblePage.evaluate(() => window.__store!.getState().activeView))
      .toBe('settings')

    const idBefore = await getActiveWorktreeId(pebblePage)
    await pebblePage.evaluate(() => document.body.focus())
    await pebblePage.keyboard.press(`${mod}+Alt+ArrowLeft`)

    // Give any erroneous nav a beat to land, then assert the active worktree
    // and the slice index both stayed put.
    await pebblePage.waitForTimeout(150)
    expect(await getActiveWorktreeId(pebblePage)).toBe(idBefore)
    const snapshot = await getNavHistorySnapshot(pebblePage)
    expect(snapshot.index).toBe(1)
  })
})
