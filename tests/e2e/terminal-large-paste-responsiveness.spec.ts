...
function largePastePayload(runId: string): string {
  return `PEBBLE_LARGE_PASTE_${runId}_0123456789abcdef`.repeat(4096)
}
...
test.describe('large terminal paste responsiveness', () => {
  test('chunked keyboard paste keeps the renderer responsive while PTY writes are pending', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.pebble-large-paste-${runId}.cjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(pebblePage, `LARGE_PASTE_READY_${runId}`, 10_000)
      ...
      await installRendererHeartbeat(pebblePage)
      await focusActiveTerminalInput(pebblePage)

      const pasteKey = pebblePage.keyboard.press(keyboardPasteChord())
      await expect
        .poll(
          async () =>
      ...
      const heartbeatBefore = await readRendererHeartbeat(pebblePage)
      await pebblePage.waitForTimeout(150)
      const heartbeatAfter = await readRendererHeartbeat(pebblePage)
      expect(heartbeatAfter).toBeGreaterThan(heartbeatBefore)

      await pasteKey
      await waitForTerminalOutput(pebblePage, doneLine, 20_000, 12_000)
      ...
      await stopRendererHeartbeat(pebblePage).catch(() => undefined)
      if (scriptStarted) {
