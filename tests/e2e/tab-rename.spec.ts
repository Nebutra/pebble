...
test.describe('Tab Rename (Inline)', () => {
  ...
  test('double-clicking a tab opens an inline rename input and Enter commits', async ({
    ...
    const tabLocator = tabLocatorByTitle(pebblePage, originalTitle)
    ...
    await expect
      .poll(async () => getActiveCustomTitle(pebblePage, worktreeId), { timeout: 3_000 })
      .toBe('My Custom Title')
    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(pebblePage, 'My Custom Title')).toBeVisible()
  })
  ...
  test('context-menu Change Title opens a focused select-all rename input', async ({
    ...
    await tabLocatorByTitle(pebblePage, originalTitle).click({ button: 'right' })
    await pebblePage.getByRole('menuitem', { name: 'Change Title', exact: true }).click()
    ...
    await expect(tabLocatorByTitle(pebblePage, 'Context Menu Title')).toBeVisible()
  })

  test('Escape during inline rename discards the edit', async ({ pebblePage }) => {
    const worktreeId = (await getActiveWorktreeId(pebblePage))!
    const originalTitle = await getActiveTabTitle(pebblePage, worktreeId)
  ...
  test('renaming to an empty string resets the tab to its default title', async ({ pebblePage }) => {
    const worktreeId = (await getActiveWorktreeId(pebblePage))!
    ...
    const defaultTitle = await getActiveTabTitle(pebblePage, worktreeId)
    ...
    const tabLocator = tabLocatorByTitle(pebblePage, 'Seeded Custom')
    await tabLocator.dblclick()
    ...
    await expect(tabLocatorByTitle(pebblePage, defaultTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(pebblePage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('clicking away (blur) commits the rename', async ({ pebblePage }) => {
    const worktreeId = (await getActiveWorktreeId(pebblePage))!
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(pebblePage, worktreeId)).length, { timeout: 3_000 })
      .toBeGreaterThanOrEqual(2)

    const tabs = await getWorktreeTabs(pebblePage, worktreeId)
    const activeId = await getActiveTabId(pebblePage)
    const activeTab = tabs.find((t) => t.id === activeId)!
    const otherTab = tabs.find((t) => t.id !== activeId)!

    const tabLocator = tabLocatorByTitle(pebblePage, activeTab.title!)
    await tabLocator.dblclick()
    ...
    await tabLocatorByTitle(pebblePage, otherTab.title!).click()

    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(pebblePage, 'Committed By Blur')).toBeVisible()
    expect(
      await pebblePage.evaluate(
  ...
  test('rename input stays at a usable width when many tabs are open', async ({ pebblePage }) => {
    const worktreeId = (await getActiveWorktreeId(pebblePage))!
    const targetTabId = await getActiveTabId(pebblePage)
    ...
    const tabLocator = tabLocatorByTitle(pebblePage, targetTitle)
  ...
  test('middle-clicking inside the rename input does not close the tab', async ({ pebblePage }) => {
    const worktreeId = (await getActiveWorktreeId(pebblePage))!
    const tabsBefore = (await getWorktreeTabs(pebblePage, worktreeId)).length
    const originalTitle = await getActiveTabTitle(pebblePage, worktreeId)
    ...
    await expect(renameInput).toBeVisible()
    await expect(tabLocatorByTitle(pebblePage, originalTitle)).toBeVisible()
    expect((await getWorktreeTabs(pebblePage, worktreeId)).length).toBe(tabsBefore)
  })
})
