...
test.describe('terminal TUI wheel reports', () => {
  test('notched mouse wheel ticks produce immediate mouse-reporting TUI scroll reports', async ({
    ...
    const samples = await probeSmallMouseWheelReports(pebblePage, 4)
  ...
  test('fullscreen mouse-reporting TUI scroll distance follows wheel magnitude @headful', async ({
    ...
    await expect
      ...
      .toBe(true)
    await pebblePage.waitForTimeout(1200)
    ...
    await execInTerminal(pebblePage, ptyId, `node ${JSON.stringify(VISIBLE_TUI_FIXTURE_PATH)}`)

    await expect
      .poll(() => readVisibleTuiOffset(pebblePage), {
        timeout: 10_000,
        message: 'visible fullscreen TUI did not render numbered rows'
      })
      .toBe(0)

    await dispatchTuiWheel(pebblePage, {
      deltaY: 10,
    ...
    const cellHeight = await pebblePage.evaluate(() => {
      const state = window.__store?.getState()
      const worktreeId = state?.activeWorktreeId
      const tabId =
        state?.activeTabType === 'terminal'
  ...
  test('TUI scroll setting scales notched mouse wheel reports', async ({ pebblePage }) => {
    await waitForSessionReady(pebblePage)
    await waitForActiveWorktree(pebblePage)
    await ensureTerminalVisible(pebblePage)
    await waitForActiveTerminalManager(pebblePage, 30_000)
    ...
    const slow = await probeTimedSmallMouseWheelReports(pebblePage, {
      drainWaitMs: 120,
      intervalMs: 220,
      ticks: 5
    })
    await pebblePage.waitForTimeout(220)
    const paced = await probeTimedSmallMouseWheelReports(pebblePage, {
      drainWaitMs: 220,
      intervalMs: 80,
      ticks: 5
    })
