...
test.describe('Setup script import prompt', () => {
  ...
  test('imports Superset local overlays through the prompt UI', async ({ pebblePage }, testInfo) => {
    const repoPath = createSupersetSetupRepo(testInfo.outputPath('superset-setup-repo'))
    const repoId = await addAndActivateRepo(pebblePage, repoPath)
    ...
    await pebblePage.getByRole('button', { name: 'Save local setup' }).click()

    await expect(
      pebblePage.getByText('2 unsupported fields skipped. Saved the setup command.')
    ).toBeVisible()

    const localCommands = await openImportedSetupSettingsFromToast(pebblePage, repoId)
  ...
  test('imports cmux setup commands through the prompt UI', async ({ pebblePage }, testInfo) => {
    ...
    await expect(
      pebblePage.getByRole('button', { name: "project's settings", exact: true })
    ).toBeVisible()

    const repoSettings = await openRepoSettings(pebblePage, repoId)
