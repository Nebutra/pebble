...
import { WINDOWS_GIT_BASH_SHELL } from '../../packages/product-core/shared/windows-terminal-shell'
...
test.describe('Windows terminal shell paste ownership', () => {
  ...
  test('PowerShell default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await createWindowsDefaultShellTerminalTab(pebblePage, 'powershell.exe')
    await waitForActiveTerminalManager(pebblePage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(pebblePage)
    const runId = randomUUID()
    const sentinel = `PEBBLE_E2E_POWERSHELL_DONE_${runId}`
    const powershellEscape = '`'
    const payload = [
      `PEBBLE_E2E_POWERSHELL_PASTE_${runId}`,
      `PowerShell metacharacters: ${powershellEscape} $ " ' ; | & < > @ { } ( )`,
      'quoted Windows path: C:\\Program Files\\Pebble Test\\file name.txt',
      'cmd metacharacters preserved as text: %PATH% !PROMPT! ^ & | < >',
      'Unicode: café 你好 مرحبا 😀',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.pebble-paste-powershell-shell-${runId}.mjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(pebblePage, `PASTE_READY_${runId}`, 10_000)
      ...
      await pebblePage.keyboard.press('Control+V')
      await waitForTerminalOutput(pebblePage, `PASTE_COMPLETE_${runId}:MATCH`, 10_000, 12_000)
  ...
  test('cmd.exe default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(pebblePage)
    await ensureTerminalVisible(pebblePage)
    await createWindowsDefaultShellTerminalTab(pebblePage, 'cmd.exe')
    await waitForActiveTerminalManager(pebblePage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(pebblePage)
    const runId = randomUUID()
    const sentinel = `PEBBLE_E2E_CMD_DONE_${runId}`
    const payload = [
      `PEBBLE_E2E_CMD_PASTE_${runId}`,
      'cmd metacharacters: %PATH% !PROMPT! ^ & | < >',
      'quoted Windows path: C:\\Program Files\\Pebble Test\\file name.txt',
      'PowerShell metacharacters: ` $ " \' ; @ { } ( )',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.pebble-paste-cmd-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('Git Bash default terminal keyboard paste preserves POSIX-shaped content with one PTY owner', async ({
    ...
    await skipWhenGitBashUnavailable(pebblePage)

    await waitForSessionReady(pebblePage)
    await waitForActiveWorktree(pebblePage)
    await ensureTerminalVisible(pebblePage)
    await createWindowsDefaultShellTerminalTab(pebblePage, WINDOWS_GIT_BASH_SHELL)
    await waitForActiveTerminalManager(pebblePage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(pebblePage)
    const runId = randomUUID()
    const sentinel = `PEBBLE_E2E_GIT_BASH_DONE_${runId}`
    const payload = [
      `PEBBLE_E2E_GIT_BASH_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.pebble-paste-git-bash-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('WSL terminal keyboard paste preserves Linux shell content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(pebblePage)
    await ensureTerminalVisible(pebblePage)
    const wslDistro = await configureActiveProjectWslRuntime(pebblePage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    await createWindowsProjectRuntimeTerminalTab(pebblePage, 'wsl.exe')
    await waitForActiveTerminalManager(pebblePage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(pebblePage)
    const runId = randomUUID()
    const sentinel = `PEBBLE_E2E_WSL_DONE_${runId}`
    const payload = [
      `PEBBLE_E2E_WSL_PASTE_${runId}`,
      'POSIX shell metacharacters: $ ` " \' ; | & < > * ? [ ] ( )',
    ...
    const scriptPath = path.join(testRepoPath, `.pebble-paste-wsl-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('existing WSL terminal keeps paste runtime after default shell changes', async ({
    ...
    const wslDistro = await configureActiveProjectWslRuntime(pebblePage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    const tabId = await createWindowsProjectRuntimeTerminalTab(pebblePage, 'wsl.exe')
    await updateWindowsDefaultShellSetting(pebblePage, 'cmd.exe')
    await expect(
      pebblePage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"] [data-shell-icon]`)
    ...
    const ptyId = await waitForActivePanePtyId(pebblePage)
    const runId = randomUUID()
    const sentinel = `PEBBLE_E2E_WSL_RETENTION_DONE_${runId}`
    const payload = [
      `PEBBLE_E2E_WSL_RETENTION_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.pebble-paste-wsl-retention-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
