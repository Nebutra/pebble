...
test('floating Mobile Emulator tab renders content and closes from the tab strip', async ({
  ...
  const tab = await seedFloatingSimulatorTab(pebblePage)
  await openFloatingPanelIfNeeded(pebblePage)

  const openPanel = pebblePage.locator(OPEN_PANEL_SELECTOR).first()
  ...
  await attachPanelScreenshot(pebblePage, testInfo)
