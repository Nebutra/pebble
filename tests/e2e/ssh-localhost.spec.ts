...
const RUN_LOCALHOST_SSH = process.env.PEBBLE_E2E_SSH_LOCALHOST === '1'
const RUN_REMOTE_HOOKS =
  process.env.PEBBLE_FEATURE_REMOTE_AGENT_HOOKS === undefined ||
  (process.env.PEBBLE_FEATURE_REMOTE_AGENT_HOOKS.trim() !== '' &&
    process.env.PEBBLE_FEATURE_REMOTE_AGENT_HOOKS.trim() !== '0')
...
function parsePort(value: string | undefined): number {
  ...
  throw new Error(`Invalid PEBBLE_E2E_SSH_PORT: ${value}`)
}

function currentUsername(): string {
  return (
    process.env.PEBBLE_E2E_SSH_USER ??
    process.env.USER ??
...
function readLocalhostSshTarget(): LocalhostSshTarget {
  const configHost = process.env.PEBBLE_E2E_SSH_CONFIG_HOST?.trim()
  const host = process.env.PEBBLE_E2E_SSH_HOST?.trim() ?? (configHost ? '' : '127.0.0.1')
  const identityFile = process.env.PEBBLE_E2E_SSH_IDENTITY_FILE?.trim()

  return {
    label: `Localhost SSH E2E ${Date.now()}`,
    host,
    port: parsePort(process.env.PEBBLE_E2E_SSH_PORT),
...
function marker(name: string): string {
  return `__PEBBLE_${name}_${Date.now()}__`
}
...
async function postCodexHook(
  ...
  await execInTerminal(
    ...
    [
      ...
      '  echo __PEBBLE_AGENT_HOOK_ENV_MISSING__',
      'else',
      `  hook_payload=${shellQuote(JSON.stringify(payload))}`,
      '  (',
      '    sleep 0.1',
      '    if curl -sS -X POST "http://127.0.0.1:${PEBBLE_AGENT_HOOK_PORT}/hook/codex" \\',
...
test.describe('Localhost SSH', () => {
  test.skip(
    !RUN_LOCALHOST_SSH,
    'Set PEBBLE_E2E_SSH_LOCALHOST=1 to run this local-machine-only SSH E2E test.'
  )
  test.skip(
    !RUN_REMOTE_HOOKS,
    'Unset PEBBLE_FEATURE_REMOTE_AGENT_HOOKS or set it to 1 so remote PTYs keep pane identity and forward hook events.'
  )
  ...
  test('routes a terminal and agent-hook status over localhost SSH', async ({
    ...
    const remote = await pebblePage.evaluate(
    ...
    await ensureTerminalVisible(pebblePage, 30_000)
    await waitForActiveTerminalManager(pebblePage, 45_000)
    const ptyId = await waitForActivePanePtyId(pebblePage, 45_000)
    const paneKey = await pebblePage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('Store unavailable')
      }
    ...
    await execInTerminal(pebblePage, ptyId, emitMarkerCommand(terminalMarker))
    await waitForTerminalOutput(pebblePage, terminalMarker, 20_000)
    ...
    await execInTerminal(
      ...
      ptyId,
      [
        `if [ "$PEBBLE_PANE_KEY" = ${shellQuote(paneKey)} ] && [ -n "$PEBBLE_AGENT_HOOK_PORT" ] && [ -n "$PEBBLE_AGENT_HOOK_TOKEN" ] && /bin/sh -c 'test -n "$PEBBLE_PANE_KEY" && test -n "$PEBBLE_AGENT_HOOK_PORT" && test -n "$PEBBLE_AGENT_HOOK_TOKEN"'; then`,
        `  ${emitMarkerCommand(envMarker)}`,
        'else',
        '  token_state=${PEBBLE_AGENT_HOOK_TOKEN:+set}',
        `  printf '%s pane=%s port=%s token=%s endpoint=%s\\n' ${shellQuote(envFailedMarker)} "$PEBBLE_PANE_KEY" "$PEBBLE_AGENT_HOOK_PORT" "$token_state" "$PEBBLE_AGENT_HOOK_ENDPOINT"`,
        'fi'
      ].join('\n')
    )
    await waitForTerminalOutput(pebblePage, envMarker, 20_000)
    ...
    await execInTerminal(
      ...
      ptyId,
      [
        'opencode_status_file="$OPENCODE_CONFIG_DIR/plugins/pebble-opencode-status.js"',
        'pi_status_file="$HOME/.pi/agent/extensions/pebble-agent-status.ts"',
    ...
    await waitForTerminalOutput(pebblePage, pluginOverlayMarker, 20_000)

    const prompt = `pebble ssh e2e prompt ${Date.now()}`
    await postCodexHook(
      pebblePage,
      ptyId,
    ...
    const ctrlPrompt = `pebble ssh ctrl-c interrupt ${Date.now()}`
    await postCodexHook(
      pebblePage,
      ptyId,
    ...
    await focusTerminal(pebblePage)
    await pebblePage.keyboard.press('Control+C')
    await pebblePage.waitForTimeout(750)
    expect(
      await pebblePage.evaluate(
        ({ paneKey, prompt, targetId, worktreeId }) => {
          const state = window.__store?.getState()
    ...
    const escapePrompt = `pebble ssh escape interrupt ${Date.now()}`
    await postCodexHook(
      pebblePage,
      ptyId,
