...
test.describe('Source Control AI commit messages', () => {
  test('generates a commit message from staged changes through the Source Control UI', async ({
    ...
    try {
      ...
      await openSourceControlForWorktree(pebblePage, testRepoPath, worktreePath, {
        commitMessageAi: {
          enabled: true,
          agentId: 'custom',
      ...
      const generate = pebblePage.getByRole('button', { name: 'Generate commit message with AI' })
