...
import { POST_REPLAY_MODE_RESET } from '../../packages/product-core/renderer/src/components/terminal-pane/layout-serialization'
...
test.describe('Terminal attention', () => {
  ...
  test('background terminal attention marks a tab unread and clears on focus', async ({
    ...
    const secondTabId = await createTerminalTab(pebblePage)
    await waitForActiveTerminalManager(pebblePage, 30_000)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(pebblePage)).includes(secondTabId), {
        timeout: 10_000,
        message: 'Background tab did not become unread after BEL'
      })
      .toBe(true)

    const secondTabBell = pebblePage
      .locator(
  ...
  test('a BEL on the focused tab raises, then clears on click', async ({ pebblePage }) => {
    await waitForSessionReady(pebblePage)
    await waitForActiveWorktree(pebblePage)
    await ensureTerminalVisible(pebblePage)
    await waitForActiveTerminalManager(pebblePage, 30_000)

    const activeTabId = await getActiveTabId(pebblePage)
    if (!activeTabId) {
      throw new Error('Expected an active terminal tab')
    }
    const activePtyId = await waitForActivePanePtyId(pebblePage)
    await installRendererTitleLog(pebblePage)
    ...
    expect((await getUnreadTerminalTabIds(pebblePage)).includes(activeTabId)).toBe(true)
    const activeTabBell = pebblePage
      .locator(
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(pebblePage)).includes(activeTabId), {
        timeout: 5_000,
  ...
  test('a BEL on the focused tab raises, then clears on plain Escape', async ({ pebblePage }) => {
    await waitForSessionReady(pebblePage)
    await waitForActiveWorktree(pebblePage)
    await ensureTerminalVisible(pebblePage)
    await waitForActiveTerminalManager(pebblePage, 30_000)
    ...
    const activePaneKey = await getActivePaneKey(pebblePage, activeTabId)
    const activePtyId = await waitForActivePanePtyId(pebblePage)
    await installRendererTitleLog(pebblePage)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalPaneKeys(pebblePage)).includes(activePaneKey), {
        timeout: 5_000,
    ...
    await focusActiveXterm(pebblePage, activeTabId)
  ...
  // after restarting Pebble with a Claude Code session open, clicking between
  ...
  test('mode bits replayed into xterm do not leak focus escapes to the shell', async ({
    ...
    try {
      ...
      // shell-startup BELs.
      const emittedFromXterm = await pebblePage.evaluate(
        () =>
