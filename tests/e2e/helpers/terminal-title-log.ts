...
export async function installRendererTitleLog(page: Page): Promise<void> {
  await page.evaluate(() => {
    const w = window as unknown as {
      __pebbleE2eTitleLog?: string[]
      __pebbleE2eTitleUnsubscribe?: () => void
    }
    ...
    w.__pebbleE2eTitleUnsubscribe?.()
    w.__pebbleE2eTitleLog = []
    ...
    const recordTitles = (): void => {
      ...
      for (const title of [...paneTitles, ...tabTitles]) {
        if (typeof title === 'string') {
          w.__pebbleE2eTitleLog!.push(title)
        }
      }
    }
    ...
    recordTitles()
    w.__pebbleE2eTitleUnsubscribe = store.subscribe(recordTitles)
  })
}

export async function getRendererTitleLog(page: Page): Promise<string[]> {
  return page.evaluate(() => {
    const w = window as unknown as { __pebbleE2eTitleLog?: string[] }
    return w.__pebbleE2eTitleLog ?? []
  })
}
