 ...
 *
 * Why: the default `pebblePage` fixture creates a fresh `userDataDir` per test
...
export function createRestartSession(testInfo: TestInfo): RestartSession {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'pebble-e2e-restart-'))
  ...
  const launch = async (): Promise<LaunchedPebble> => {
...
export async function attachRepoAndOpenTerminal(page: Page, repoPath: string): Promise<string> {
  ...
  const repoId = await page.evaluate(async (repoPath) => {
    ...
    // worktree is external to Pebble's workspace root after the visibility rollout.
