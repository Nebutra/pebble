/**
 * Shared Electron fixture for Pebble E2E tests.
 *
...
import { getPebbleElectronLaunchArgs } from './electron-launch-args'
...
type PebbleTestFixtures = {
  ...
  // developer's shell or already-running Pebble instance.
...
// Why: parse + warn at module scope so a bad PEBBLE_E2E_SLOWMO_MS value logs once
// per worker instead of once per test (otherwise hundreds of lines per CI run).
const PEBBLE_E2E_SLOWMO_MS_RAW = process.env.PEBBLE_E2E_SLOWMO_MS
const PEBBLE_E2E_SLOWMO_MS = ((): number => {
  if (PEBBLE_E2E_SLOWMO_MS_RAW === undefined) {
    return 0
  }
  const parsed = Number(PEBBLE_E2E_SLOWMO_MS_RAW)
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[pebble-e2e] PEBBLE_E2E_SLOWMO_MS="${PEBBLE_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
function shouldLaunchHeadful(testInfo: TestInfo): boolean {
  // Why: PEBBLE_E2E_FORCE_HEADFUL lets a developer watch any spec in a real
  // window without retagging it `@headful` or switching projects.
  if (process.env.PEBBLE_E2E_FORCE_HEADFUL === '1') {
    return true
  }
  return testInfo.project.metadata.pebbleHeadful === true
}

function forwardElectronProcessLogs(app: ElectronApplication, testInfo: TestInfo): void {
  if (process.env.PEBBLE_E2E_FORWARD_APP_LOGS !== '1') {
    return
  }
...
/**
 * Extended Playwright test with Pebble-specific fixtures.
 *
 * `pebblePage` — the main Pebble renderer window.
 *
...
export const test = base.extend<PebbleTestFixtures, PebbleWorkerFixtures>({
  ...
  electronApp: async ({ dismissOnboarding, launchEnv }, provideFixture, testInfo) => {
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'pebble-e2e-userdata-'))
    ...
    void _unused
    // Why: PEBBLE_E2E_SLOWMO_MS adds a pause between every Playwright action so a
    // developer running with PEBBLE_E2E_FORCE_HEADFUL=1 can actually watch what
    // the test does. Defaults to 0 (no slowdown) for normal runs.
    const slowMo = PEBBLE_E2E_SLOWMO_MS
    // Why: PEBBLE_E2E_RECORD_VIDEO=1 captures a webm of the renderer so a
    ...
    const recordVideoDir = process.env.PEBBLE_E2E_RECORD_VIDEO === '1' ? testInfo.outputDir : null
    if (recordVideoDir) {
      mkdirSync(recordVideoDir, { recursive: true })
    }
    const app = await electron.launch({
      args: getPebbleElectronLaunchArgs(mainPath, headful),
      ...(slowMo > 0 ? { slowMo } : {}),
      ...(recordVideoDir ? { recordVideo: { dir: recordVideoDir } } : {}),
      // Why: keep NODE_ENV=development so window.__store is exposed and
      // dev-only helpers activate. PEBBLE_E2E_USER_DATA_DIR overrides the usual
      // shared dev profile so every spec gets a clean persistence root.
      // Why: PEBBLE_E2E_HEADLESS suppresses mainWindow.show() so the app
      // window stays hidden during test runs, avoiding focus stealing and
      // screen clutter. Playwright interacts via CDP regardless.
      // Why: PEBBLE_E2E_HEADLESS suppresses mainWindow.show() for CI/headless
      // runs. PEBBLE_E2E_HEADFUL overrides this for tests that need a visible
      ...
      env: {
        ...
        PEBBLE_E2E_USER_DATA_DIR: userDataDir,
        ...((process.env.PEBBLE_E2E_SSH_LOCALHOST === '1' ||
          process.env.PEBBLE_E2E_SSH_DOCKER === '1') &&
        !cleanEnv.PEBBLE_RELAY_PATH
          ? { PEBBLE_RELAY_PATH: path.join(process.cwd(), 'out', 'relay') }
          : {}),
        ...(headful ? { PEBBLE_E2E_HEADFUL: '1' } : { PEBBLE_E2E_HEADLESS: '1' })
      }
    })
...
export { expect } from '@nebutra/playwright-test'
