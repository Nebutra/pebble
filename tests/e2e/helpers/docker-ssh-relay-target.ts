...
const CONTAINER_IMAGE = process.env.PEBBLE_E2E_SSH_DOCKER_IMAGE ?? 'node:22-bookworm'
...
function seedRemoteRepo(target: DockerSshRelayTarget): void {
  dockerExec(
    ...
    [
      ...
      'git config user.name "Pebble Docker SSH E2E"',
...
export function startDockerSshRelayTarget(testInfo: TestInfo): DockerSshRelayTarget {
  const tempDir = mkdtempSync(path.join(os.tmpdir(), 'pebble-ssh-docker-'))
  ...
  const containerName = `pebble-ssh-e2e-${testInfo.workerIndex}-${Date.now()}`
    ...
    run(
      ...
      [
        ...
        [
          ...
          'git config --global user.name "Pebble Docker SSH E2E"',
