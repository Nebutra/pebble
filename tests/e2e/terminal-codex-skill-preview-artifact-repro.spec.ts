...
import type { ElectronApplication, Page, TestInfo } from '@nebutra/playwright-test'
...
const RUN_REPRO = process.env.PEBBLE_E2E_CODEX_SKILL_PREVIEW_REPRO === '1'
const EXPECT_NO_ARTIFACTS = process.env.PEBBLE_E2E_EXPECT_NO_CODEX_SKILL_PREVIEW_ARTIFACTS === '1'
const FULLSCREEN_MIN_SIZE = { width: 1200, height: 760 }
const MIN_REPRO_DIFF_RATIO = 0.006
const MAX_CLEAN_DIFF_RATIO = 0.0015
const PEBBLE_REPO_PATH = realpathSync(process.cwd())
...
const CODEX_SKILL_PREVIEW_RE = /Press enter to insert|esc to close|electron|pebble-cli|pebble-emulator/i
const SETUP_PANE_ACTIVITY_RE = /install-pebble-skills|pnpm|Progress:|Packages:|Lockfile/i
...
async function addRealPebbleRepo(page: Page, repoPath: string): Promise<string> {
  return page.evaluate(async (repoPath) => {
    ...
    if (!repo) {
      throw new Error(`Real Pebble repo did not load: ${repoPath}`)
    }
    ...
    if (!worktree) {
      throw new Error(`Real Pebble worktree did not load: ${repoPath}`)
    }
...
async function createWorkspaceThroughComposer(page: Page, workspaceName: string): Promise<string> {
  ...
  await expect
    .poll(
      ...
      {
        timeout: 60_000,
        message: `Workspace ${workspaceName} did not appear in the real Pebble repo`
      }
    )
    .not.toBeNull()
  ...
  await expect
    .poll(() => getActiveWorktreeId(page), {
      timeout: 30_000,
      message: 'Created real Pebble workspace did not become active'
    })
    .toBe(createdId)
...
test.describe('Codex skill preview terminal artifact repro @headful', () => {
  ...
  test.skip(!RUN_REPRO, 'Set PEBBLE_E2E_CODEX_SKILL_PREVIEW_REPRO=1 to run this repro.')
  ...
  test('captures the real Pebble repo setup-split Codex skill preview overpaint before any click', async ({
    electronApp,
    pebblePage
  }, testInfo) => {
    ...
    await setStableFullscreenWindow(electronApp, pebblePage)
    await waitForSessionReady(pebblePage)
    await addRealPebbleRepo(pebblePage, PEBBLE_REPO_PATH)
    ...
    const worktreeId = await createWorkspaceThroughComposer(pebblePage, workspaceName)
    createdWorktreeIds.push(worktreeId)

    await ensureTerminalVisible(pebblePage, 30_000)
    await waitForActiveTerminalManager(pebblePage, 30_000)
    await waitForPaneCount(pebblePage, 2, 60_000)
    await waitForPaneIdentitySnapshot(pebblePage, 2)
    const webglActive = await forceTerminalWebgl(pebblePage)
    test.skip(!webglActive, 'Codex skill preview artifact repro needs WebGL rendering')

    const leftPane = await focusLeftTerminalPane(pebblePage)
    const rightPane = await getRightTerminalPane(pebblePage)
    ...
    await dismissCodexPromptsIfPresent(pebblePage, leftPane)
    await waitForPaneContent(pebblePage, leftPane.tabId, leftPane.ptyId, CODEX_READY_RE, 60_000)
    await waitForPaneContent(
      pebblePage,
      leftPane.tabId,
      leftPane.ptyId,
    ...
    await pebblePage.waitForTimeout(CODEX_READY_SETTLE_MS)
    await waitForPaneVisibleContentChanges(pebblePage, rightPane, 1, 8_000)

    await focusLeftTerminalPane(pebblePage)
    await pebblePage.keyboard.type('test $e', { delay: 70 })
    await waitForPaneContent(
      pebblePage,
      leftPane.tabId,
      leftPane.ptyId,
    ...
    const evidence = await captureClickEvidence(pebblePage, leftPane, testInfo)
