...
async function seedCreatePREligibleBranch(
  ...
  return page.evaluate(async ({ createResult }) => {
    ...
    const pr = {
      ...
      state: 'open' as const,
      url: 'https://github.com/acme/pebble/pull/73',
...
test.describe('Source Control create pull request', () => {
  ...
  test('creates the pull request from the Source Control primary action', async ({ pebblePage }) => {
    const { branch, worktreeId } = await seedCreatePREligibleBranch(pebblePage)
    await openSourceControl(pebblePage, worktreeId)
    await forceCreatePREligibleStatus(pebblePage, worktreeId, branch)

    const titleInput = pebblePage.getByRole('textbox', { name: 'Pull request title' })
    const descriptionInput = pebblePage.getByRole('textbox', {
      name: 'Pull request description'
    })
    const createButton = getCreatePRComposerSubmitButton(pebblePage)
    await expect(createButton).toBeVisible({ timeout: 10_000 })
    await expect(createButton).toBeEnabled()
    await expect(titleInput).toHaveValue('E2e secondary')
    await expect(pebblePage.getByRole('textbox', { name: 'Pull request base branch' })).toHaveValue(
      'main'
    )
  ...
  test('surfaces create failures without clearing the pull request composer', async ({
    ...
    await expect(pebblePage.getByText(failureMessage)).toBeVisible()
