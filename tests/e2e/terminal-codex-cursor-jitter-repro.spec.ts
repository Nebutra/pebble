...
test.describe('Codex terminal cursor jitter repro', () => {
  test('keeps queued-message cursor out of the Working status row in native Windows Codex @headful', async ({
    ...
    await installPtyWriteDiagnostics(pebblePage)
    rmSync(ARTIFACT_DIR, { recursive: true, force: true })
    mkdirSync(ARTIFACT_DIR, { recursive: true })

    const shellCase = SHELL_CASES[0]!
    const { tabId, ptyId } = await prepareCodexTerminal(pebblePage, shellCase)
    await installPtyOutputDiagnostics(pebblePage)
    await pebblePage.keyboard.type(CODEX_REPO_PROMPT)
    await pebblePage.waitForTimeout(250)
    await pebblePage.keyboard.press('Enter')
    await pebblePage.waitForTimeout(1_000)
    if (!CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(pebblePage, tabId, 8_000))) {
      await pebblePage.keyboard.press('Enter')
    }
    await pebblePage.waitForTimeout(3_000)
    const submittedContent = await getTerminalContentForTab(pebblePage, tabId, 8_000)
    writeFileSync(path.join(ARTIFACT_DIR, 'queued-message-after-submit.txt'), submittedContent)
    await expect
      .poll(
        async () =>
          CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(pebblePage, tabId, 8_000)),
        {
          timeout: 30_000,
      ...
      .toBe(true)
    await applyCursorProbeTheme(pebblePage, tabId)
    ...
    await pebblePage.keyboard.insertText('s')
    await expect
      .poll(
        async () =>
          (await readScreenLines(pebblePage, tabId)).some((line) => isQueuedInputLine(line.text)),
        {
          timeout: 5_000,
    ...
    const snapshot = await readScreenSnapshot(pebblePage, shellCase.label, tabId, ptyId)
    const rawChunks = await readPtyOutputDiagnostics(pebblePage)
