...
set -euo pipefail
base="$(mktemp -d /tmp/pebble-apphang-repro.XXXXXX)"
repo="$base/repo"
mkdir -p "$repo"
cd "$repo"
git init -q
...
mkdir -p src docs
printf '# Pebble Windows AppHang repro\n' > README.md
...
printf 'plain folder for Pebble AppHang repro\n' > "$plain/README.txt"
...
export function createCompletedOnboardingProfile(userDataDir) {
  ...
  writeFileSync(path.join(userDataDir, 'pebble-data.json'), `${JSON.stringify(profile, null, 2)}\n`)
}
