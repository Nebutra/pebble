...
export const KO_VALUE_OVERRIDES = {
  ...
  'Add Pebble attribution to commits, PRs, and issues.': '커밋, PR 및 이슈에 Pebble 속성을 추가합니다.',
  ...
  'Pebble uses the GitHub CLI (gh) to show pull requests, issues, and checks.':
    'Pebble는 GitHub CLI(gh)를 사용하여 PR, 이슈 및 검사를 표시합니다.',
  ...
  'CLI skill': 'CLI 스킬',
  'Pebble CLI skill': 'Pebble CLI 스킬',
  ...
  'Review inactive workspaces before deleting their local files and Pebble state.':
    '로컬 파일과 Pebble 상태를 삭제하기 전에 비활성 워크스페이스를 확인하세요.',
  ...
  'Install the Pebble skill so agents know to use the Pebble CLI.':
    '에이전트가 Pebble CLI를 사용하도록 Pebble 스킬을 설치하세요.',
  ...
  "Control Pebble from your phone. Check on agents, review changes, and kick off tasks while you're away from your desk.":
    '휴대폰에서 Pebble를 제어하세요. 자리를 비운 동안 에이전트를 확인하고, 변경 사항을 리뷰하고, 작업을 시작하세요.',
