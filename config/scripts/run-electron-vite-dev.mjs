...
const useStableElectronName =
  process.env.PEBBLE_DEV_STABLE_NAME === '1' ||
  rawForwardedArgs.includes(STABLE_NAME_FLAG)
const forwardedRaw = rawForwardedArgs.filter((arg) => arg !== STABLE_NAME_FLAG)
if (useStableElectronName) {
  process.env.PEBBLE_DEV_STABLE_NAME = '1'
}

function getPebbleDevEnv(name) {
  return process.env[`PEBBLE_DEV_${name}`]
}

function setPebbleDevEnv(name, value) {
  process.env[`PEBBLE_DEV_${name}`] ||= value
}
...
function createDockTitle(branch, label) {
  return `Pebble: ${branch || label || 'dev'}`
}

function seedDevInstanceIdentityEnv() {
  const branch =
    getPebbleDevEnv('BRANCH') ||
    readGitValue(['symbolic-ref', '--quiet', '--short', 'HEAD']) ||
    readGitValue(['rev-parse', '--short', 'HEAD'])
  // Why: the checkout directory can keep a legacy name; the dev label is
  // product-facing terminal output and window metadata.
  const worktreeName = getPebbleDevEnv('WORKTREE_NAME') || 'pebble'
  const label = getPebbleDevEnv('INSTANCE_LABEL') || formatDevInstanceLabel(branch, worktreeName)
  const identitySeed = getPebbleDevEnv('INSTANCE_KEY') || repoRoot
  const dockTitle = getPebbleDevEnv('DOCK_TITLE') || createDockTitle(branch, label)

  setPebbleDevEnv('REPO_ROOT', repoRoot)
  setPebbleDevEnv('INSTANCE_KEY', identitySeed)
  if (branch) {
    setPebbleDevEnv('BRANCH', branch)
  }
  if (worktreeName) {
    setPebbleDevEnv('WORKTREE_NAME', worktreeName)
  }
  if (label) {
    ...
    setPebbleDevEnv('INSTANCE_LABEL', label)
  }
  setPebbleDevEnv('DOCK_TITLE', dockTitle)
}
...
function prepareMacDevElectronApp() {
  ...
  const title = getPebbleDevEnv('DOCK_TITLE') || 'Pebble: dev'
  const identityKey = getPebbleDevEnv('INSTANCE_KEY') || repoRoot
  ...
  const markerPath = path.join(distDir, 'pebble-dev-electron-app.json')
  const bundleId = `nebutra.pebble.dev.${sanitizeBundleIdPart(hash)}`
  process.env.PEBBLE_DEV_MACOS_BUNDLE_ID = bundleId
...
function getDevUserDataPath() {
  if (process.env.PEBBLE_DEV_USER_DATA_PATH) {
    return process.env.PEBBLE_DEV_USER_DATA_PATH
  }
  if (process.platform === 'darwin') {
...
function prepareDevCliWrapper() {
  ...
  if (process.platform === 'win32') {
    const cmdContent = `@echo off\r\nset "PEBBLE_USER_DATA_PATH=${userDataPath}"\r\nset "PEBBLE_APP_EXECUTABLE=${electronBin}"\r\nset "PEBBLE_APP_EXECUTABLE_NEEDS_APP_ROOT=1"\r\nnode "${cliPath}" %*\r\n`
    writeFileSync(
      path.join(binDir, 'pebble-dev.cmd'),
      cmdContent,
      'utf8'
    )
  } else {
    const wrapperContent = `#!/usr/bin/env bash\nexport PEBBLE_USER_DATA_PATH=${JSON.stringify(userDataPath)}\nexport PEBBLE_APP_EXECUTABLE=${JSON.stringify(electronBin)}\nexport PEBBLE_APP_EXECUTABLE_NEEDS_APP_ROOT=1\nexec node ${JSON.stringify(cliPath)} "$@"\n`
    const wrapperPath = path.join(binDir, 'pebble-dev')
    writeFileSync(wrapperPath, wrapperContent, 'utf8')
    chmodSync(wrapperPath, 0o755)

    mkdirSync(userDataBinDir, { recursive: true })
    for (const commandName of ['pebble-dev', 'pebble']) {
      const userDataWrapperPath = path.join(userDataBinDir, commandName)
      // Why: dev Pebble terminals prepend this directory to PATH; refreshing
      // aliases prevents stale global/userData wrappers from hijacking
      // Pebble-owned commands such as `pebble claude-teams`.
  ...
  console.log(`[pebble-dev] Prepared wrapper in ${binDir}`)
}
...
if (process.env.PEBBLE_SKIP_DEV_CLI_PREPARE !== '1') {
  prepareDevCliWrapper()
}

seedDevInstanceIdentityEnv()
if (
  !useStableElectronName &&
  process.env.PEBBLE_SKIP_DEV_ELECTRON_APP_PREPARE !== '1'
) {
...
function prepareDevWebClient() {
  if (
    process.env.PEBBLE_SKIP_DEV_WEB_PREPARE === '1' ||
    isHelpOrVersion
  ) {
    return
  }
  ...
  if (
    ...
    process.env.PEBBLE_DEV_WEB_PREPARE !== '1'
  ) {
    console.error(
      '[pebble-dev] Web client bundle missing; skipping pairing web build. Run `pnpm run build:web` or set PEBBLE_DEV_WEB_PREPARE=1 when you need browser pairing.'
    )
    return
  }
  ...
  console.error('[pebble-dev] Building web client for pairing...')
  execFileSync(
    process.execPath,
...
if (!isHelpOrVersion && getPebbleDevEnv('INSTANCE_LABEL')) {
  console.error(`[pebble-dev] Instance: ${getPebbleDevEnv('INSTANCE_LABEL')}`)
}
let forwardedExtras = []
if (!userPassedPort && !isHelpOrVersion) {
  ...
  if (envPortRaw) {
    ...
    if (port === null) {
      console.error(
        `[pebble-dev] Ignoring invalid REMOTE_DEBUGGING_PORT=${JSON.stringify(envPortRaw)}; falling back to probe.`
      )
    }
  }
  if (port === null) {
  ...
  if (port !== null) {
    ...
    console.error(`[pebble-dev] Remote debugging on http://127.0.0.1:${port}`)
  } else {
    console.error(
      '[pebble-dev] No free debug port found in sweep; starting without --remote-debugging-port.'
    )
  }
}
prepareDevWebClient()
