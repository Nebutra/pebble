...
describe('locale-translation-policy zh round 5', () => {
  it('fixes brand spacing, hosted review, and Pebble Mobile regressions', () => {
    expect(
      repairTranslatedValue({
        key: 'auto.components.stats.ShareUsageCard.0eb31e79ee',
        enValue: 'Pebble IDE',
        localeValue: 'Pebble集成开发环境',
        locale: 'zh'
      })
    ).toBe('Pebble IDE')
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Pebble first',
        localeValue: 'Pebble第一',
        locale: 'zh'
      })
    ).toBe('Pebble 优先')
    expect(
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Show Pebble Mobile Button',
        localeValue: '显示 Pebble 移动按钮',
        locale: 'zh'
      })
    ).toBe('显示 Pebble Mobile 按钮')
    expect(
      repairTranslatedValue({
        key: 'auto.components.settings.GitPane.e02ea23a32',
        enValue: 'Pebble Attribution',
        localeValue: 'Pebble归属',
        locale: 'zh'
      })
    ).toBe('Pebble 归因')
    expect(
  ...
  it('keeps workflow terms in English', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue:
          'Git could not safely delete branch "{{value0}}"{{value1}}, so Pebble kept it to avoid losing local commits.',
        localeValue:
          'Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Pebble 保留它以避免丢失本地提交。',
        locale: 'zh'
      })
    ).toBe('Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Pebble 保留它以避免丢失本地 commits。')
  })
