#!/usr/bin/env node
import { _electron as electron } from '@nebutra/playwright-test'
...
function createIdleRepo(worktreeCount) {
  const repoDir = mkdtempSync(path.join(os.tmpdir(), 'pebble-idle-cpu-repo-'))
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# Pebble idle CPU benchmark\n')
  writeFileSync(
  ...
  for (let i = 2; i <= worktreeCount; i += 1) {
    const worktreeDir = path.join(
      ...
      `pebble-idle-cpu-worktree-${i}-${Date.now()}`
    )
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'pebble-idle-cpu-userdata-'))
  ...
  const app = await electron.launch({
    ...
    env: {
      ...
      ...(options.headful ? { PEBBLE_E2E_HEADFUL: '1' } : { PEBBLE_E2E_HEADLESS: '1' })
    }
  })
