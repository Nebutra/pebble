...
# no saved sessions). Your real `pebble-dev` profile is left untouched.
#
# Usage:
...
#   PEBBLE_FRESH_PROFILE_DIR=/some/path ./config/scripts/dev-fresh-profile.sh   # use a fixed dir
set -euo pipefail
...
PROFILE_DIR="${PEBBLE_FRESH_PROFILE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/pebble-fresh-profile.XXXXXXXX")}"
mkdir -p "$PROFILE_DIR"

cleanup() {
  if [[ "$KEEP" -eq 0 && -z "${PEBBLE_FRESH_PROFILE_DIR:-}" ]]; then
    # Guard rm -rf against accidental empty/unrelated PROFILE_DIR.
    [[ -n "${PROFILE_DIR:-}" && -d "$PROFILE_DIR" && "$PROFILE_DIR" == */pebble-fresh-profile* ]] || return 0
    rm -rf "$PROFILE_DIR"
...
PEBBLE_DEV_USER_DATA_PATH="$PROFILE_DIR" PEBBLE_DEV_SHOW_FIRST_RUN_EDUCATION=1 pnpm dev
