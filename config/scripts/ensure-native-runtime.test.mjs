    ...
    try {
      ...
      const result = spawnSync(process.execPath, [scriptPath, '--runtime=node'], {
        ...
        env: envWithPrependedPath(binDir, {
          PEBBLE_NATIVE_TEST_LOG: logPath,
          PEBBLE_NATIVE_TEST_MARKER: markerPath
        })
      })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(result.status, result.stderr).toBe(0)
        expect(result.stderr).toContain("expected build/Release so Pebble's node-pty patch is active")
        expect(readFileSync(logPath, 'utf8')).toContain('pnpm rebuild node-pty\n')
      } finally {
        rmSync(projectDir, { recursive: true, force: true })
      }
...
function mkTempProject() {
  const projectDir = mkdtempSync(join(tmpdir(), 'pebble-native-runtime-'))
...
exports.loadNativeModule = function loadNativeModule(nativeName) {
  const markerExists = existsSync(process.env.PEBBLE_NATIVE_TEST_MARKER)
  appendFileSync(
    process.env.PEBBLE_NATIVE_TEST_LOG,
...
exports.loadNativeModule = function loadNativeModule(nativeName) {
  const rebuilt = existsSync(process.env.PEBBLE_NATIVE_TEST_MARKER)
  const dir = rebuilt ? '../build/Release/' : '../prebuilds/' + process.platform + '-' + process.arch + '/'
  appendFileSync(process.env.PEBBLE_NATIVE_TEST_LOG, \`node-pty load \${nativeName} dir=\${dir}\\n\`)
  return { dir, module: {} }
}
`
  )
...
appendFileSync(process.env.PEBBLE_NATIVE_TEST_LOG, \`pnpm \${process.argv.slice(2).join(' ')}\\n\`)
writeFileSync(process.env.PEBBLE_NATIVE_TEST_MARKER, 'rebuilt')
`
  )
