...
export function shouldPreserveEnglishValue(enValue, key = '') {
  ...
  if (/^https?:\/\//.test(enValue) || enValue.startsWith('pebble://')) {
    return true
  }
...
function applyCjkLatinTermSpacing(localeValue, locale) {
  ...
  if (locale === 'ko') {
    ...
    // boundary — re-glue "Pebble 에"/"PR 을"/"에서는" but keep "Jira 이슈"/"Pebble 로고"/"agent 에뮬레이터".
...
export function repairTranslatedValue({ key, enValue, localeValue, locale }) {
  ...
  if (enValue.includes('pebble://')) {
    result = result.replace(/虎鲸:\/\//g, 'pebble://')
  }

  if (enValue === 'Pebble' || enValue.startsWith('Pebble ')) {
    result = result
      .replaceAll('虎鲸', 'Pebble')
      .replaceAll('逆戟鲸', 'Pebble')
      .replaceAll('シャチ', 'Pebble')
  }

  if (enValue.includes('pebble://')) {
    result = result.replace(/シャチ:\/\//g, 'pebble://')
  }
...
export function repairCatalog(enCatalog, localeCatalog, locale) {
  ...
  if (localeCatalog.menu) {
    if (locale === 'zh') {
      if (localeCatalog.menu.explorePebble !== '探索 Pebble') {
        localeCatalog.menu.explorePebble = '探索 Pebble'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Pebble 入门') {
        localeCatalog.menu.gettingStarted = 'Pebble 入门'
        repaired += 1
      }
    }
    if (locale === 'ko') {
      if (localeCatalog.menu.explorePebble !== 'Pebble 둘러보기') {
        localeCatalog.menu.explorePebble = 'Pebble 둘러보기'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Pebble 시작하기') {
        localeCatalog.menu.gettingStarted = 'Pebble 시작하기'
        repaired += 1
      }
    }
  }
