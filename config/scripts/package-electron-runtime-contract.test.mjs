...
describe('Electron runtime package contract', () => {
  ...
  it('guards release publishing before electron-builder runs', () => {
    ...
    expect(macReleaseCommand).toContain(' && PEBBLE_MAC_RELEASE=1 ')
    ...
    expect(releaseCommands.get('linux-arm64')).toContain('PEBBLE_LINUX_ARM64_RELEASE=1')
  ...
  it('runs the macOS release build in an isolated Blacksmith workflow', () => {
    ...
    expect(publishStep.with.command).toContain('PEBBLE_MAC_RELEASE=1')
  ...
  it('bumps separate Homebrew casks for stable and RC desktop tags', () => {
    ...
    expect(resolveCaskStep.run).toContain('token="pebble@rc"')
    expect(resolveCaskStep.run).toContain('token="pebble"')
  ...
  it('keeps terminal scale perf wired to the report budget gate', () => {
    ...
    const manualProfileKnobs = [
      ['PEBBLE_TERMINAL_PERF_FRAME_COUNT', 'frame_count', 'PEBBLE_E2E_OPENCODE_FRAME_COUNT'],
      [
        'PEBBLE_TERMINAL_PERF_FRAME_INTERVAL_MS',
        'frame_interval_ms',
        'PEBBLE_E2E_OPENCODE_FRAME_INTERVAL_MS'
      ],
      [
        'PEBBLE_TERMINAL_PERF_PRESSURE_OUTPUT_CHARS',
        'pressure_output_chars',
        'PEBBLE_E2E_OPENCODE_PRESSURE_OUTPUT_CHARS'
      ],
      ['PEBBLE_TERMINAL_PERF_SCALE_PANES', 'scale_panes', 'PEBBLE_E2E_OPENCODE_SCALE_PANES'],
      [
        'PEBBLE_TERMINAL_PERF_SCALE_CROSS_WORKSPACE_PANES',
        'scale_cross_workspace_panes',
        'PEBBLE_E2E_OPENCODE_SCALE_CROSS_WORKSPACE_PANES'
      ],
      [
        'PEBBLE_TERMINAL_PERF_SCALE_PRESSURE_PANES',
        'scale_pressure_panes',
        'PEBBLE_E2E_OPENCODE_SCALE_PRESSURE_PANES'
      ],
      [
        'PEBBLE_TERMINAL_PERF_SCALE_HIDDEN_PRESSURE_PANES',
    ...
    expect(uploadStep.with.path).toBe('${{ env.PEBBLE_E2E_TERMINAL_PERF_REPORT_PATH }}')
  })
  ...
  it('keeps terminal rendering regressions in the fast golden E2E gate', () => {
    ...
    expect(releaseGoldenJob.steps.map((step) => step.run ?? '')).toContain(
      'xvfb-run --auto-servernum env SKIP_BUILD=1 PEBBLE_E2E_FORWARD_APP_LOGS=1 pnpm run test:e2e:terminal-rendering-golden'
    )
    ...
    expect(releaseEvidenceJob.steps.map((step) => step.run ?? '')).toContain(
      'xvfb-run --auto-servernum env SKIP_BUILD=1 PEBBLE_E2E_FORWARD_APP_LOGS=1 pnpm run test:e2e:terminal-rendering-release-evidence'
    )
  })
})
