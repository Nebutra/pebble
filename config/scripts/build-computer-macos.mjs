...
const binaryPath = path.join(packagePath, '.build', 'release', 'pebble-computer-use-macos')
const appPath = path.join(packagePath, '.build', 'release', 'Pebble Computer Use.app')
const appExecutablePath = path.join(appPath, 'Contents', 'MacOS', 'pebble-computer-use-macos')
...
const bundleId =
  process.env.PEBBLE_COMPUTER_MACOS_BUNDLE_ID ??
  process.env.PEBBLE_COMPUTER_MACOS_BUNDLE_ID ??
  'nebutra.pebble.computer-use'
const displayName = 'Pebble Computer Use'
const isMacRelease =
...
function buildUniversalBinary() {
  const builtBinaries = universalTriples.map((triple) => {
    ...
    return path.join(packagePath, '.build', triple, 'release', 'pebble-computer-use-macos')
  })
...
<dict>
  ...
  <string>pebble-computer-use-macos</string>
  ...
  <string>Pebble Computer Use needs Accessibility permission to read and interact with app interfaces when you ask Pebble to use apps.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>Pebble Computer Use needs Screen Recording permission to capture app windows when you ask Pebble to inspect your screen.</string>
</dict>
</plist>
`
}
