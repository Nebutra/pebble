...
async function installElectronPackageBinary() {
  ...
  const tempDir = mkdtempSync(resolve(tmpdir(), 'pebble-electron-'))
...
function getExtractorCommand(zipPath, extractDir) {
  if (process.env.PEBBLE_ELECTRON_PACKAGE_EXTRACTOR) {
    return {
      file: process.execPath,
      args: [process.env.PEBBLE_ELECTRON_PACKAGE_EXTRACTOR, zipPath, extractDir],
      label: `node ${process.env.PEBBLE_ELECTRON_PACKAGE_EXTRACTOR}`
    }
  }

  if (osPlatform() === 'win32') {
    return {
      file: process.env.PEBBLE_POWERSHELL_BIN || 'powershell',
      args: [
        '-NoProfile',
        '-NonInteractive',
        '-ExecutionPolicy',
  ...
  return {
    file: process.env.PEBBLE_UNZIP_BIN || 'unzip',
