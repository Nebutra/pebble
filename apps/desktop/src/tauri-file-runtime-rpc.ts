...
import {
  ...
  resolveRuntimePath
} from '../../../packages/product-core/shared/cross-platform-path'
import type { RuntimeRpcResponse } from '../../../packages/product-core/shared/runtime-rpc-envelope'
import type { RuntimeTerminalPathResolution } from '../../../packages/product-core/shared/runtime-types'
import type {
  DirEntry,
  MarkdownDocument,
  SearchResult
...
import {
  emitTauriOpenDiffFromMobile,
  emitTauriOpenFileFromMobile
} from './tauri-settings-event-api'
...
type RuntimeMobileFileEntry = {
  relativePath: string
  basename: string
  kind: 'text' | 'binary'
}

type RuntimeMobileFileListResult = {
  worktree: string
  rootPath: string
  files: RuntimeMobileFileEntry[]
  totalCount: number
  truncated: boolean
}
...
const mobileFileListLimit = 5000
...
const legacySshRelayFileMethods = new Set([
  'files.list',
  'files.listAll',
  'files.read',
  'files.readDir',
...
// Why: Electron opens only raster data-URI images from mobile; SVG/PDF preview
// support is separate and must not be reported as an image tab open.
const mobilePreviewableImageExtensions = new Set([
  '.png',
  '.jpg',
  '.jpeg',
  '.gif',
...
export async function callTauriFileRuntimeRpc(
  ...
  switch (method) {
    case 'files.list':
      return handled(await listMobileFiles(params))
    case 'files.open':
      return handled(await openMobileFile(params))
    case 'files.openDiff':
      return handled(await openMobileDiff(params))
    case 'files.read':
...
async function listMobileFiles(params: unknown): Promise<RuntimeMobileFileListResult> {
  const input = readObject(params)
  const worktree = await readRuntimeWorktree(input)
  const listed = await listAllFiles(params)
  const files = listed.files
    .filter((entry) => isSafeMobileRelativePath(entry.relativePath))
    .sort((left, right) => left.relativePath.localeCompare(right.relativePath))
    .slice(0, mobileFileListLimit)
    .map((entry) => ({
      relativePath: entry.relativePath,
      basename: basenameFromRelativePath(entry.relativePath),
      kind: isMobileBinaryPath(entry.relativePath) ? ('binary' as const) : ('text' as const)
    }))
  return {
    worktree: worktree.id,
    rootPath: worktree.path,
    files,
    totalCount: listed.files.length,
    truncated: listed.files.length > mobileFileListLimit
  }
}

async function openMobileFile(params: unknown): Promise<RuntimeFileOpenResult> {
  const input = readObject(params)
  const worktree = await readRuntimeWorktree(input)
  const relativePath = readRequiredString(input.relativePath, 'file path')
  const filePath = resolveSafeMobilePath(worktree.path, relativePath)
  const kind = mobileFileOpenKind(relativePath)
  if (kind === 'binary') {
...
async function openMobileDiff(params: unknown): Promise<RuntimeFileOpenResult> {
  const input = readObject(params)
  const worktree = await readRuntimeWorktree(input)
  const relativePath = readRequiredString(input.relativePath, 'file path')
  const filePath = resolveSafeMobilePath(worktree.path, relativePath)
...
async function browseServerDirectory(params: unknown): Promise<{
  ...
  return requestRuntimeJson<{ resolvedPath: string; entries: DirEntry[] }>(
    `/v1/files/browse-dir?${new URLSearchParams({
...
async function commitUpload(params: unknown): Promise<{ ok: true }> {
  const input = readObject(params)
  const scope = await readRuntimeFileScope(input)
  await requestRuntimeJson<{ ok: true }>('/v1/files/commit-upload', {
    method: 'POST',
    body: {
      ...scope,
...
async function listMarkdownDocuments(params: unknown): Promise<MarkdownDocument[]> {
  ...
  return requestRuntimeJson<MarkdownDocument[]>(`/v1/files/markdown?${fileQuery(scope, '')}`, {
    method: 'GET',
    timeoutMs: 5000
  })
}
...
async function callRemoteFileRuntimeRpc(
  ...
  try {
    ...
    // Why: hostId can identify a legacy SSH target rather than a paired runtime.
    // Only methods in this set have live Go relay routes for legacy SSH targets.
    if (legacySshRelayFileMethods.has(method)) {
      return { handled: false }
    }
    throw error
  }
...
async function readTerminalArtifact(params: unknown): Promise<RuntimeFileReadResult> {
  ...
  if (remoteGrant) {
    if (remoteGrant.transport === 'relay') {
      return requestRuntimeJson<RuntimeFileReadResult>('/v1/files/terminal-artifact/read', {
        method: 'POST',
        timeoutMs: 60_000,
        body: relayTerminalArtifactParams(input, remoteGrant)
      })
    }
...
async function readTerminalArtifactPreview(params: unknown): Promise<RuntimeFilePreviewResult> {
  ...
  if (remoteGrant) {
    if (remoteGrant.transport === 'relay') {
      return requestRuntimeJson<RuntimeFilePreviewResult>('/v1/files/terminal-artifact/preview', {
        method: 'POST',
        timeoutMs: 60_000,
        body: relayTerminalArtifactParams(input, remoteGrant)
      })
...
async function writeTerminalArtifact(params: unknown): Promise<{ ok: true }> {
  ...
  if (remoteGrant) {
    if (remoteGrant.transport === 'relay') {
      return requestRuntimeJson<{ ok: true }>('/v1/files/terminal-artifact/write', {
        method: 'POST',
        timeoutMs: 60_000,
        body: {
          ...relayTerminalArtifactParams(input, remoteGrant),
...
async function resolveRemoteTerminalPath(
  ...
  try {
    ...
    rememberRemoteTerminalArtifactGrant(result, connectionId, worktree.id, 'paired')
    return result
  } catch {
    return resolveLegacySshTerminalPath(input, worktree, connectionId)
  }
}

async function resolveLegacySshTerminalPath(
  input: Record<string, unknown>,
  worktree: RuntimeWorktreeRecord,
  connectionId: string
): Promise<RuntimeTerminalPathResolution> {
  ...
  const expandedPath = uriPath ?? pathText
  const base = session?.cwd || readString(input.cwd) || worktree.path
  const absolutePath =
    expandedPath.startsWith('~/') || expandedPath.startsWith('~\\')
      ? expandedPath
      : resolveRuntimePath(base, expandedPath)
  const relativePath = absolutePath.startsWith('~')
    ? null
    : relativePathInsideRoot(worktree.path, absolutePath)
  const empty = emptyTerminalPathResolution(worktree.id, relativePath, absolutePath)

  if (relativePath !== null && relativePath !== '' && isSafeRuntimeRelativePath(relativePath)) {
    try {
      const stats = await statPath({ ...input, worktree: worktree.id, relativePath })
      return {
        worktree: worktree.id,
        relativePath,
        absolutePath,
        ...
        openTarget: stats.isDirectory
          ? undefined
          : { kind: 'worktree-file', provider: 'ssh', relativePath, absolutePath }
      }
    } catch (error) {
      if (isRuntimeNotFoundError(error)) {
        return empty
  ...
  const grant = await requestRuntimeJson<TerminalArtifactGrantResult>(
    '/v1/files/terminal-artifact/grant',
    {
      method: 'POST',
      timeoutMs: 60_000,
      body: { projectId: worktree.projectId, worktreeId: worktree.id, absolutePath }
    }
  )
  const result: RuntimeTerminalPathResolution = {
    worktree: worktree.id,
    relativePath: null,
    absolutePath: grant.absolutePath,
    exists: true,
  ...
  rememberRemoteTerminalArtifactGrant(result, connectionId, worktree.id, 'relay')
  return result
}
...
function rememberRemoteTerminalArtifactGrant(
  ...
  connectionId: string,
  worktreeId: string,
  transport: RemoteTerminalArtifactGrant['transport']
): void {
...
function relayTerminalArtifactParams(
  input: Record<string, unknown>,
  grant: RemoteTerminalArtifactGrant
): { worktreeId: string; grantId: string; absolutePath: string } {
  return {
...
function resolveSafeMobilePath(rootPath: string, relativePath: string): string {
  if (!isSafeMobileRelativePath(relativePath)) {
    throw new Error('invalid_relative_path')
  }
  const filePath = resolveRuntimePath(rootPath, relativePath)
  if (relativePathInsideRoot(rootPath, filePath) === null) {
    throw new Error('invalid_relative_path')
  }
  return filePath
}
...
function mobileFileOpenKind(relativePath: string): RuntimeFileOpenResult['kind'] {
  if (isMobilePreviewableImagePath(relativePath)) {
    return 'image'
  }
  if (isMobileBinaryPath(relativePath)) {
    return 'binary'
  }
  return isMobileMarkdownPath(relativePath) ? 'markdown' : 'text'
}

function isMobileMarkdownPath(path: string): boolean {
  const extension = fileExtension(path)
  return extension === '.md' || extension === '.mdx' || extension === '.markdown'
}

function isMobilePreviewableImagePath(path: string): boolean {
  return mobilePreviewableImageExtensions.has(fileExtension(path))
}
