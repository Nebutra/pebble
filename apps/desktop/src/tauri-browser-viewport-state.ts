import type { BrowserViewportResult } from '../../../packages/product-core/shared/runtime-types'
import type { BrowserViewportOverride } from '../../../packages/product-core/shared/types'
import { applyBrowserPageViewportOverride } from '../../../packages/product-core/renderer/src/components/browser-pane/browser-page-viewport'
...
export function setTauriBrowserViewportOverride(args: {
  ...
  if (!override) {
    ...
    applyBrowserPageViewportOverride(pageId, null)
    return
  }
  viewportOverridesByPageId.set(pageId, override)
  applyBrowserPageViewportOverride(pageId, override)
}
...
export function readTauriBrowserViewport(params: unknown): BrowserViewportResult {
  ...
  return {
    width: readPositiveNumber(input.width) ?? stored?.width ?? DEFAULT_TAURI_BROWSER_VIEWPORT.width,
    height:
