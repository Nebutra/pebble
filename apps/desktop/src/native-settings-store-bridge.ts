...
/** Fixed localStorage keys the renderer persists for desktop state
...
const NATIVE_DOCUMENTS: Record<string, string> = {
  ...
  'pebble.web.githubCache.v1': 'github-cache',
  'pebble.web.ui.v1': 'ui',
  'pebble.web.workspaceSession.v1': 'workspace-session'
}
...
const tauriIo: NativeDocumentIo = {
  ...
  writeLegacy: (key, contents) => window.localStorage.setItem(key, contents)
}

/** Install native file-backed persistence and kick off async prime+migration. */
export function installNativeSettingsStore(): void {
  for (const [key, documentName] of Object.entries(NATIVE_DOCUMENTS)) {
    const backend = new NativeDocumentBackend(
      key,
      documentName,
      tauriIo,
      undefined,
      documentName === 'workspace-session'
    )
