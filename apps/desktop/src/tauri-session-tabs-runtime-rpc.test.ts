...
describe('callTauriSessionTabsRuntimeRpc', () => {
  ...
  it('renames a terminal and persists the custom tab title', async () => {
    mockRuntimeSessions([{ id: 'sess-1', tabId: 'tab-1', leafId: 'leaf-1' }])

    await expect(
      callTauriSessionTabsRuntimeRpc('terminal.rename', {
        terminal: 'sess-1',
        title: 'Release verification'
      })
    ).resolves.toEqual({
      handled: true,
      result: { handle: 'sess-1', tabId: 'tab-1', title: 'Release verification' }
    })
    expect(scheduleLayoutSaveMock).toHaveBeenLastCalledWith(
      'wt-1',
      expect.objectContaining({
        tabPropsByTabId: { 'tab-1': { customTitle: 'Release verification' } }
      })
    )
    await expect(
      callTauriSessionTabsRuntimeRpc('session.tabs.list', { worktree: 'id:wt-1' })
      ...
      handled: true,
      result: { tabs: [{ customTitle: 'Release verification' }] }
    })
  })

  it('clears a terminal custom title with null', async () => {
    mockRuntimeSessions([{ id: 'sess-1', tabId: 'tab-1', leafId: 'leaf-1' }])
    loadLayoutMock.mockResolvedValue({
      tabPropsByTabId: { 'tab-1': { customTitle: 'Old title' } }
    })

    await callTauriSessionTabsRuntimeRpc('terminal.rename', {
      handle: 'sess-1',
      title: null
    })
    ...
    expect(scheduleLayoutSaveMock).toHaveBeenLastCalledWith(
      'wt-1',
      expect.objectContaining({ tabPropsByTabId: { 'tab-1': { customTitle: null } } })
    )
  })

  it('restores a persisted terminal custom title after state rehydration', async () => {
    mockRuntimeSessions([{ id: 'sess-1', tabId: 'tab-1', leafId: 'leaf-1' }])
    loadLayoutMock.mockResolvedValue({
      tabPropsByTabId: { 'tab-1': { customTitle: 'Persisted title' } }
    })
    ...
    await expect(
      ...
      handled: true,
      result: { tabs: [{ customTitle: 'Persisted title' }] }
    })
  })
