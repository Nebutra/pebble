...
import { addRuntimePtyDeliveryListener, enqueueRuntimePtyData } from './tauri-runtime-pty-delivery'
...
const ptyReplayListeners = new Set<(data: { id: string; data: string }) => void>()
...
const PTY_OUTPUT_POLL_MIN_MS = 16
const PTY_OUTPUT_POLL_MAX_MS = 250
const PTY_STATUS_POLL_IDLE_MS = 500
...
export function addRuntimePtyDataListener(callback: (data: PtyData) => void): () => void {
  ...
  return addRuntimePtyDeliveryListener(callback)
}

export function addRuntimePtyReplayListener(
  callback: (data: { id: string; data: string }) => void
): () => void {
  ptyReplayListeners.add(callback)
  return () => ptyReplayListeners.delete(callback)
}

export function emitRuntimePtyReplay(id: string, data: string): void {
  for (const listener of ptyReplayListeners) {
    listener({ id, data })
  }
}
...
export function reportRuntimePtyUnavailable(
  sessionId: string,
  status: RuntimeSession['status'] = 'stopped'
): void {
  emitRuntimePtyExit({ id: sessionId, status })
}
...
async function startRuntimePtyEventDelivery(): Promise<void> {
  const { supported } = await subscribeRuntimeEventPush(
    (entry) => {
      ...
      if (entry.topic === 'browser.driver' || !entry.topic) {
        handleRuntimeBrowserDriverEntry(entry)
      }
    },
...
function handleRuntimeBrowserDriverEntry(event: RuntimeEventStreamEntry): void {
  const driverEvent = mapRuntimeBrowserDriverEntry(event)
  if (!driverEvent) {
    return
  }
  deliverRuntimeBrowserDriver(driverEvent.browserPageId, driverEvent.driver)
}

async function pumpRuntimePtyOutput(generation: number): Promise<void> {
  let idleDelayMs = PTY_OUTPUT_POLL_MIN_MS
  while (isPollGenerationCurrent(generation)) {
    const events = await readRuntimeEvents('session.output')
    if (events.length === 0) {
      await delay(idleDelayMs)
      idleDelayMs = nextRuntimePollDelay(
        idleDelayMs,
        PTY_OUTPUT_POLL_MIN_MS,
        PTY_OUTPUT_POLL_MAX_MS
      )
    } else {
      // Why: fallback polling is user-facing terminal I/O. Return to frame-rate
      // latency after activity instead of retaining the idle backoff.
      idleDelayMs = PTY_OUTPUT_POLL_MIN_MS
    }
...
async function pumpRuntimePtyStatus(generation: number): Promise<void> {
  while (isPollGenerationCurrent(generation)) {
    ...
    if (events.length === 0) {
      await delay(PTY_STATUS_POLL_IDLE_MS)
    }
    for (const event of events) {
