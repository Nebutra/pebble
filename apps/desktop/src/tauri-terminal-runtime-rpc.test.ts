...
describe('callTauriTerminalRuntimeRpc', () => {
  it('uses the Go-owned transcript cursor instead of rebuilding lines from chunk tails', async () => {
    requestRuntimeJsonMock.mockImplementation(async (path) => {
      ...
      if (path === '/v1/sessions/sess-1/transcript?limit=2&cursor=3') {
        return {
          tail: ['line 3', 'line 4'],
          truncated: true,
          limited: true,
    ...
    await expect(
      callTauriTerminalRuntimeRpc('terminal.read', {
        handle: 'sess-1',
        cursor: '3',
        limit: 2
      })
  ...
  it('inspects the live foreground process through the bounded Go status probe', async () => {
    requestRuntimeJsonMock.mockImplementation(async (path) => {
      if (path === '/v1/sessions') {
        return [
          {
      ...
      if (path === '/v1/sessions/sess-1/status') {
        return {
          id: 'sess-1',
          cwd: '/repo/worktree',
          command: ['zsh'],
    ...
    await expect(
      callTauriTerminalRuntimeRpc('terminal.inspectProcess', { handle: 'sess-1' })
    ).resolves.toEqual({
      handled: true,
      result: {
    ...
    expect(requestRuntimeJsonMock).toHaveBeenCalledWith('/v1/sessions/sess-1/status', {
      method: 'GET'
    })
  })
  ...
  it('creates an inline setup terminal without a persisted worktree', async () => {
    requestRuntimeJsonMock.mockResolvedValue({
      id: 'sess-setup',
      projectId: 'ephemeral-setup-terminal:computer-use',
    ...
    await expect(
      callTauriTerminalRuntimeRpc('terminal.create', {
        worktree: 'id:ephemeral-setup-terminal:computer-use',
        cwd: '/Users/tester',
        tabId: 'tab-setup',
        leafId: 'leaf-setup'
      })
    ...
    expect(readWorktreesMock).not.toHaveBeenCalled()
    expect(requestRuntimeJsonMock).toHaveBeenCalledWith('/v1/sessions', {
      method: 'POST',
      timeoutMs: 15_000,
      body: expect.objectContaining({
  ...
  it('rejects an inline setup terminal without a native-resolved cwd', async () => {
    await expect(
      callTauriTerminalRuntimeRpc('terminal.create', {
        worktree: 'id:ephemeral-setup-terminal:computer-use'
      })
    ).rejects.toThrow('ephemeral_terminal_create_requires_cwd')
