...
export function toRuntimeDispatchResultRequest(
  ...
  if (result.outputSnapshot) {
    body.outputSnapshot = result.outputSnapshot
  }
  if (result.error) {
...
export function mapRuntimeOutputSnapshot(
  snapshot: AutomationRunOutputSnapshot | null | undefined
): AutomationRunOutputSnapshot | null {
  if (
    snapshot?.format !== 'plain_text' ||
