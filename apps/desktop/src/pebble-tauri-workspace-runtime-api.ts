...
import { projectHostSetupProjectionFromRepos } from '../../../packages/product-core/shared/project-host-setup-projection'
import type {
...
import { MANAGED_WORKTREE_OWNERSHIP } from '../../../packages/product-core/shared/worktree-ownership'
...
type RuntimeProjectHostSetup = Omit<ProjectHostSetup, 'createdAt' | 'updatedAt'> & {
  createdAt: string
  updatedAt: string
}
...
type CloneProgressListener = Parameters<PreloadApi['repos']['onCloneProgress']>[0]
...
const cloneProgressListeners = new Set<CloneProgressListener>()
...
let workspaceEventDeliveryStarted = false
let workspacePollingActive = false
let workspacePollingGeneration = 0

export function createPebbleProjectsApi(base: PreloadApi['projects']): PreloadApi['projects'] {
  return {
    ...base,
    list: readLogicalProjects,
    listHostSetups: readProjectHostSetups,
    update: async ({ projectId, updates }) => {
      ...
      if (updates.localWindowsRuntimePreference) {
        await Promise.all(
          project.sourceRepoIds.map((repoId) =>
            requestRuntimeJson<PebbleRuntimeProject>(`/v1/projects/${encodeURIComponent(repoId)}`, {
              method: 'PATCH',
              body: {
                localWindowsRuntimePreference: updates.localWindowsRuntimePreference
              }
            })
          )
        )
      ...
      return (
        projectHostSetupProjectionFromRepos(await readRepos()).projects.find(
          (entry) => entry.id === projectId
        ) ?? null
      )
    },
    createHostSetup: async (args) => {
      const project = await requireLogicalProject(args.projectId)
      const setup = mapRuntimeProjectHostSetup(
        await requestRuntimeJson<RuntimeProjectHostSetup>('/v1/project-host-setups', {
          method: 'POST',
          body: args
        })
      )
    ...
    setupExistingFolder: async (args) => {
      await requireLogicalProject(args.projectId)
      const host = parseProjectSetupHost(args.hostId)
      const runtimeProject = await createRuntimeProject({
        name: args.displayName?.trim() || pathBasename(args.path),
        path: args.path,
        locationKind: host.locationKind,
        hostId: host.hostId,
        logicalProjectId: args.projectId,
        provider: args.kind,
        projectHostSetupMethod: args.setupMethod ?? 'imported-existing-folder'
      })
      const repo = mapRuntimeProjectToRepo(runtimeProject, args.kind)
      const projection = projectHostSetupProjectionFromRepos(await readRepos())
      const project = projection.projects.find((entry) => entry.id === args.projectId)
      const setup = projection.setups.find((entry) => entry.repoId === repo.id)
      if (!project || !setup) {
        throw new Error('Runtime project host setup did not join the requested logical project.')
      }
      return { project, setup, repo }
    },
    updateHostSetup: async ({ setupId, updates }) => {
      const repos = await readRepos()
      const projection = projectHostSetupProjectionFromRepos(repos)
      const projectedSetup = projection.setups.find((entry) => entry.id === setupId)
      if (projectedSetup) {
        if (updates.path !== undefined && updates.path !== projectedSetup.path) {
          throw new Error(
        ...
        const runtimeProject = await requestRuntimeJson<PebbleRuntimeProject>(
          `/v1/projects/${encodeURIComponent(projectedSetup.repoId)}`,
          {
            method: 'PATCH',
            body: {
              ...(updates.displayName !== undefined ? { name: updates.displayName } : {}),
              ...(updates.kind !== undefined ? { provider: updates.kind } : {}),
              ...
              ...(updates.setupMethod !== undefined
                ? { projectHostSetupMethod: updates.setupMethod }
                : {})
            }
          }
        )
        const repo = mapRuntimeProjectToRepo(runtimeProject, updates.kind ?? projectedSetup.kind)
        const refreshed = projectHostSetupProjectionFromRepos(await readRepos())
        return {
          project: requireProjectionEntry(refreshed.projects, projectedSetup.projectId, 'project'),
          setup: requireProjectionEntry(refreshed.setups, setupId, 'project host setup'),
          repo
        }
      }
      const setup = mapRuntimeProjectHostSetup(
        await requestRuntimeJson<RuntimeProjectHostSetup>(
          `/v1/project-host-setups/${encodeURIComponent(setupId)}`,
          { method: 'PATCH', body: updates }
        )
      )
      return { project: await requireLogicalProject(setup.projectId), setup }
    },
    deleteHostSetup: async ({ setupId }) => {
      const repos = await readRepos()
      const projection = projectHostSetupProjectionFromRepos(repos)
      const projectedSetup = projection.setups.find((entry) => entry.id === setupId)
      if (projectedSetup) {
        const project = requireProjectionEntry(
          projection.projects,
          projectedSetup.projectId,
          'project'
        )
        const repo = requireProjectionEntry(repos, projectedSetup.repoId, 'repo')
        await requestRuntimeJson(`/v1/projects/${encodeURIComponent(projectedSetup.repoId)}`, {
          method: 'DELETE'
        })
        return { project, setup: projectedSetup, repo }
      }
      const independentSetups = await requestRuntimeJson<RuntimeProjectHostSetup[]>(
        '/v1/project-host-setups',
        { method: 'GET' }
      )
      const existingSetup = independentSetups.find((entry) => entry.id === setupId)
      if (!existingSetup) {
        throw new Error(`Project host setup not found: ${setupId}`)
      }
      const project = await requireLogicalProject(existingSetup.projectId)
      const setup = mapRuntimeProjectHostSetup(
        await requestRuntimeJson<RuntimeProjectHostSetup>(
          `/v1/project-host-setups/${encodeURIComponent(setupId)}`,
          { method: 'DELETE' }
...
async function readProjectHostSetups(): Promise<ProjectHostSetup[]> {
  const [repos, independent] = await Promise.all([
    readRepos(),
    requestRuntimeJson<RuntimeProjectHostSetup[]>('/v1/project-host-setups', { method: 'GET' })
  ])
  return [
    ...projectHostSetupProjectionFromRepos(repos).setups,
    ...independent.map(mapRuntimeProjectHostSetup)
  ]
}

async function readLogicalProjects(): Promise<Project[]> {
  const [repos, independent] = await Promise.all([
    readRepos(),
    requestRuntimeJson<RuntimeProjectHostSetup[]>('/v1/project-host-setups', { method: 'GET' })
  ])
  const projects = projectHostSetupProjectionFromRepos(repos).projects
  const knownIds = new Set(projects.map((project) => project.id))
  const setupsByProject = new Map<string, ProjectHostSetup[]>()
  for (const setup of independent.map(mapRuntimeProjectHostSetup)) {
    const siblings = setupsByProject.get(setup.projectId) ?? []
    siblings.push(setup)
    setupsByProject.set(setup.projectId, siblings)
  }
  for (const [projectId, siblings] of setupsByProject) {
    if (knownIds.has(projectId)) {
      continue
    }
    const setup = siblings[0]
    if (!setup) {
      continue
    }
    projects.push({
      ...
      sourceRepoIds: [],
      createdAt: Math.min(...siblings.map((candidate) => candidate.createdAt)),
      updatedAt: Math.max(...siblings.map((candidate) => candidate.updatedAt))
    })
...
async function requireLogicalProject(projectId: string): Promise<Project> {
  const project = (await readLogicalProjects()).find((entry) => entry.id === projectId)
  if (!project) {
...
function mapRuntimeProjectHostSetup(setup: RuntimeProjectHostSetup): ProjectHostSetup {
  return {
    ...setup,
    createdAt: Date.parse(setup.createdAt),
    updatedAt: Date.parse(setup.updatedAt)
  }
}

function parseProjectSetupHost(hostId: string): { locationKind: 'local' | 'ssh'; hostId?: string } {
  if (hostId === 'local') {
    return { locationKind: 'local' }
  }
  if (hostId.startsWith('ssh:') && hostId.slice(4).trim()) {
    return { locationKind: 'ssh', hostId: hostId.slice(4).trim() }
  }
  throw new Error(`Runtime project host setup must be routed through its owning host: ${hostId}`)
}

function requireProjectionEntry<T extends { id: string }>(
  entries: T[],
  id: string,
  label: string
): T {
...
export function createPebbleReposApi(base: PreloadApi['repos']): PreloadApi['repos'] {
  return {
    ...
    create: async ({ parentPath, name, kind }) => {
      try {
        const project = await requestRuntimeJson<PebbleRuntimeProject>(
          '/v1/projects/create-on-host',
          {
            method: 'POST',
            timeoutMs: 60_000,
            body: { parentPath, name, kind }
          }
        )
        return { repo: mapRuntimeProjectToRepo(project, kind) }
      } catch (error) {
    ...
    cloneRemote: async ({ connectionId, url, destination }) => {
      const project = await requestRuntimeJson<PebbleRuntimeProject>('/v1/projects/clone', {
        method: 'POST',
        timeoutMs: 10 * 60_000,
        body: { hostId: connectionId, url, destination }
      })
      return mapRuntimeProjectToRepo(project, 'git')
    },
    cloneAbort: async () => {
      await requestRuntimeJson('/v1/projects/clone/abort', { method: 'POST' })
    },
    remove: async ({ repoId }) => {
    ...
    update: async ({ repoId, updates }) => {
      ...
      if ('issueSourcePreference' in updates) {
        // Why: JSON omits undefined; encode auto explicitly so Go can clear a
        // previously persisted origin/upstream choice.
        body.issueSourcePreference = updates.issueSourcePreference ?? 'auto'
      }
      const project = await requestRuntimeJson<PebbleRuntimeProject>(
    ...
    getGitUsername: async ({ repoId }) => {
      const result = await requestRuntimeJson<{ username: string }>(
        `/v1/projects/${encodeURIComponent(repoId)}/git-username`,
        { method: 'GET' }
      )
    ...
    onCloneProgress: (callback) => subscribeCloneProgress(callback),
...
async function createRuntimeProject(args: {
  ...
  return requestRuntimeJson<PebbleRuntimeProject>('/v1/projects', {
    ...
    body: {
      ...
      ...(args.logicalProjectId ? { logicalProjectId: args.logicalProjectId } : {}),
      ...(args.provider ? { provider: args.provider } : {}),
      ...(args.projectHostSetupMethod
        ? { projectHostSetupMethod: args.projectHostSetupMethod }
        : {})
    }
  })
}
...
function subscribeCloneProgress(callback: CloneProgressListener): () => void {
  cloneProgressListeners.add(callback)
  ensureWorkspaceEventDelivery()
  return () => cloneProgressListeners.delete(callback)
}
...
function ensureWorkspaceEventDelivery(): void {
  if (workspaceEventDeliveryStarted) {
    return
  }
  workspaceEventDeliveryStarted = true
  void startWorkspaceEventDelivery()
}

async function startWorkspaceEventDelivery(): Promise<void> {
  const { supported } = await subscribeRuntimeEventPush(handleWorkspaceRuntimeEvent, (pushActive) =>
    setWorkspacePolling(!pushActive)
  )
  if (!supported) {
...
function setWorkspacePolling(active: boolean): void {
  if (active === workspacePollingActive) {
    return
  }
  workspacePollingActive = active
  workspacePollingGeneration += 1
  if (!active) {
    return
  }
  const generation = workspacePollingGeneration
  void pumpProjectEvents(generation)
  void pumpCloneProgressEvents(generation)
  void pumpWorktreeEvents(generation)
  void pumpWorktreeBaseStatusEvents(generation)
  void pumpWorktreeRemoteBranchConflictEvents(generation)
}

function workspacePollIsCurrent(generation: number): boolean {
  return workspacePollingActive && generation === workspacePollingGeneration
}

async function pumpProjectEvents(generation: number): Promise<void> {
  let idleDelayMs = 50
  while (workspacePollIsCurrent(generation)) {
    const events = await readRuntimeEvents('project.changed')
    if (events.length === 0) {
      await delay(idleDelayMs)
      idleDelayMs = nextRuntimePollDelay(idleDelayMs, 50, 500)
    } else {
      idleDelayMs = 50
    }
...
async function pumpCloneProgressEvents(generation: number): Promise<void> {
  let idleDelayMs = 50
  while (workspacePollIsCurrent(generation)) {
    const events = await readRuntimeEvents('project.cloneProgress')
    if (events.length === 0) {
      await delay(idleDelayMs)
      idleDelayMs = nextRuntimePollDelay(idleDelayMs, 50, 500)
    } else {
    ...
    for (const entry of events) {
      emitCloneProgress(entry)
    }
  }
}

async function pumpWorktreeEvents(generation: number): Promise<void> {
  let idleDelayMs = 50
  while (workspacePollIsCurrent(generation)) {
...
async function pumpWorktreeBaseStatusEvents(generation: number): Promise<void> {
  await pumpTypedWorktreeEvents(generation, 'worktree.base-status.changed')
}

async function pumpWorktreeRemoteBranchConflictEvents(generation: number): Promise<void> {
  await pumpTypedWorktreeEvents(generation, 'worktree.remote-branch-conflict.changed')
}

async function pumpTypedWorktreeEvents(generation: number, topic: string): Promise<void> {
  let idleDelayMs = 50
  while (workspacePollIsCurrent(generation)) {
    const events = await readRuntimeEvents(topic)
    if (events.length === 0) {
      await delay(idleDelayMs)
      idleDelayMs = nextRuntimePollDelay(idleDelayMs, 50, 500)
    } else {
    ...
    for (const entry of events) {
      handleWorkspaceRuntimeEvent(entry)
    }
  }
}

function handleWorkspaceRuntimeEvent(entry: RuntimeEventStreamEntry): void {
  if (emitCloneProgress(entry)) {
    return
  }
  const event = parseRuntimeEvent(entry)
  if (isRuntimeTopic(entry, 'worktree.base-status.changed')) {
    const status = decodeRuntimeWorktreeBaseStatus(event?.payload)
    if (status) {
      emitRuntimeWorktreeBaseStatus(status)
    }
    return
  }
  if (isRuntimeTopic(entry, 'worktree.remote-branch-conflict.changed')) {
    const conflict = decodeRuntimeRemoteBranchConflict(event?.payload)
    if (conflict) {
      emitRuntimeWorktreeRemoteBranchConflict(conflict)
    }
    return
  }
  if (isRuntimeTopic(entry, 'project.changed')) {
...
function emitCloneProgress(entry: RuntimeEventStreamEntry): boolean {
  if (!isRuntimeTopic(entry, 'project.cloneProgress')) {
    return false
  }
  const payload = readObject(parseRuntimeEvent(entry)?.payload)
  const phase = readString(payload.phase)
  const percent = typeof payload.percent === 'number' ? payload.percent : Number.NaN
  if (!phase || !Number.isFinite(percent)) {
    return true
  }
  emitTo(cloneProgressListeners, { phase, percent: Math.min(100, Math.max(0, percent)) })
  return true
}

async function readRuntimeEvents(topic: string): Promise<RuntimeEventStreamEntry[]> {
  const result = await readRuntimeEventStream(
    // Why: this is only the disconnected fallback; the connected path uses
    // native push so idle workspaces do not cross WebKit IPC every second.
