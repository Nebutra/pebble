...
import { SPEECH_MODEL_CATALOG } from '../../../packages/product-core/shared/speech-model-catalog'
import type * as SpeechApiModuleNamespace from './tauri-speech-api'
...
type SpeechApiModule = typeof SpeechApiModuleNamespace
...
describe('createPebbleSpeechApi', () => {
  ...
  it('rejects downloads for unknown and cloud models without invoking', async () => {
    ...
    expect(invokeMock).not.toHaveBeenCalledWith('speech_download_model', expect.anything())
  })

  it('starts cloud dictation natively while warming local capability', async () => {
    const api = await loadSpeechApi()
    ...
    expect(invokeMock).toHaveBeenCalledWith('speech_local_inference_supported')
  })
