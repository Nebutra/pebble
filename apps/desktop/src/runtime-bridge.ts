...
export function createRuntimeStatusProbeCommand(
  ...
  return {
    ...
    bearerToken: input.bearerToken ?? LOCAL_RUNTIME_BEARER_TOKEN,
...
export async function writeRuntimePtyInput(sessionId: string, text: string): Promise<boolean> {
  return invoke<boolean>('write_runtime_pty_input', {
    input: {
      sessionId,
      text,
      runtimeUrl: LOCAL_RUNTIME_ENDPOINT.url,
      bearerToken: LOCAL_RUNTIME_BEARER_TOKEN
    }
  })
}
