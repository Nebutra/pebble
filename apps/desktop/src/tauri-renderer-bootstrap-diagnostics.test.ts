...
describe('Tauri renderer bootstrap diagnostics', () => {
  it('renders a visible bootstrap failure for guarded synchronous startup stages', () => {
    ...
    expect(
      document.querySelector('[data-pebble-tauri-bootstrap-failure="install-window-api"]')
    ).not.toBeNull()
    ...
    expect(
      (window as TauriRendererBootstrapTestWindow).__pebbleTauriBootstrapFailure
    ).toMatchObject({
  ...
  it('renders import-time failures before the React app is loaded', () => {
    ...
    expect(
      document.querySelector('[data-pebble-tauri-bootstrap-failure="load-renderer-entry"]')
    ).not.toBeNull()
