...
async function startPushPipeline(): Promise<boolean> {
  try {
    const eventChannel = new Channel<RuntimeEventStreamEntry>()
    const statusChannel = new Channel<RuntimeEventStatus>()
    eventChannel.onmessage = dispatchEvent
    statusChannel.onmessage = (status) => setPushConnected(status.connected)
    // Keep global listeners as a compatibility path for older native shells;
    // current shells deliver the high-rate stream through the channels below.
