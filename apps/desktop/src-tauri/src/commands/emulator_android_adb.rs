...
/// (runtime/go/internal/runtimecore/domain.go), same enum shape as
...
impl AdbCommand {
    ...
    pub fn to_argv(&self) -> Vec<String> {
        match self {
            ...
            Self::ScreenSize { serial } => shell_argv(serial, ["wm", "size"]),
            Self::Tap { serial, x, y } => shell_argv(
                serial,
                [
                    "input".to_string(),
                    "tap".to_string(),
            ...
            Self::KeyEvent { serial, keycode } => shell_argv(
                serial,
                [
                    "input".to_string(),
                    "keyevent".to_string(),
            ...
            Self::LogcatSnapshot { serial, lines } => vec![
                "-s".to_string(),
                serial.clone(),
                "logcat".to_string(),
                "-d".to_string(),
            ...
            Self::AccessibilityDump {
                serial,
                remote_path,
            } => shell_argv(serial, ["uiautomator", "dump", remote_path.as_str()]),
            Self::ReadRemoteFile {
                serial,
                remote_path,
            } => shell_argv(serial, ["cat", remote_path.as_str()]),
            Self::Exec { serial, argv } => shell_argv(serial, argv.clone()),
            Self::Permission {
                serial,
                operation,
                package,
                ...
                let mut command = vec!["pm".to_string(), operation.clone()];
                if let Some(package) = package {
                    command.push(package.clone());
                }
                if let Some(permission) = permission {
                    command.push(permission.clone());
                }
                shell_argv(serial, command)
            }
            Self::Shutdown { serial } => vec![
                "-s".to_string(),
                serial.clone(),
                "emu".to_string(),
                "kill".to_string(),
...
fn shell_argv<I, S>(serial: &str, command: I) -> Vec<String>
where
    I: IntoIterator<Item = S>,
    S: Into<String>,
{
    let mut result = vec!["-s".to_string(), serial.to_string(), "shell".to_string()];
    result.extend(command.into_iter().map(Into::into));
    result
}
...
\n";
    ...
    #[test]
    fn builds_shutdown_argv() {
        let command = AdbCommand::Shutdown {
            serial: "emulator-5554".to_string(),
        };
        assert_eq!(
            command.to_argv(),
            vec!["-s", "emulator-5554", "emu", "kill"]
        );
    }

    #[test]
    fn builds_accessibility_dump_and_read_argv() {
        let dump = AdbCommand::AccessibilityDump {
            serial: "emulator-5554".to_string(),
            remote_path: "/sdcard/window_dump.xml".to_string(),
        };
        assert_eq!(
            dump.to_argv(),
            [
        ...
        let read = AdbCommand::ReadRemoteFile {
            serial: "emulator-5554".to_string(),
            remote_path: "/sdcard/window_dump.xml".to_string(),
        };
        assert_eq!(
    ...
    #[test]
    fn builds_exec_argv_without_host_shell_reparsing() {
        let command = AdbCommand::Exec {
            serial: "emulator-5554".to_string(),
            argv: vec![
                "printf".to_string(),
                "%s".to_string(),
                "one argument; still one argument".to_string(),
            ],
        };
        assert_eq!(
            command.to_argv(),
            [
                ...
                "printf",
                "%s",
                "one argument; still one argument"
            ]
            .map(str::to_string)
        );
    }
