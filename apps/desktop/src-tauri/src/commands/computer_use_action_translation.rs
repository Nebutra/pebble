...
/// The bundled helper and Tauri host must agree on this protocol version.
...
/// Matches the shared clipboard safety contract without copying the payload.
pub const CLIPBOARD_TEXT_WRITE_MAX_BYTES: usize = 16 * 1024 * 1024;
const CLIPBOARD_TEXT_WRITE_TOO_LARGE_ERROR: &str = "Clipboard text is too large to copy safely.";
...
/// stable runtime error vocabulary (accessibility_error,
...
pub fn plan_computer_action(kind: &str) -> ComputerActionPlan {
    ...
    match method {
        ...
        // "snapshot" is a persisted client alias for the same helper call.
            ...
            if method == "pasteText" {
                if let Some(text) = params.get("text").and_then(Value::as_str) {
                    if text.len() > CLIPBOARD_TEXT_WRITE_MAX_BYTES {
                        return ActionCompletion::Failed {
                            error_message: format!(
                                "invalid_argument: {CLIPBOARD_TEXT_WRITE_TOO_LARGE_ERROR}"
                            ),
                        };
                    }
                }
...
mod tests {
    ...
    #[test]
    fn rejects_oversized_paste_text_without_calling_helper() {
        let mut executor = FakeExecutor::returning(Ok(Value::Null));
        let completion = execute_claimed_action(
            &mut executor,
            &action(
                "computer.pasteText",
                serde_json::json!({"text": "x".repeat(CLIPBOARD_TEXT_WRITE_MAX_BYTES + 1)}),
            ),
        );
        assert!(executor.calls.is_empty());
        assert_eq!(
            completion,
            ActionCompletion::Failed {
                error_message: format!("invalid_argument: {CLIPBOARD_TEXT_WRITE_TOO_LARGE_ERROR}")
            }
        );
    }

    #[test]
    fn accepts_exact_paste_byte_limit() {
        let mut executor = FakeExecutor::returning(Ok(serde_json::json!({"pasted": true})));
        let text = "é".repeat(CLIPBOARD_TEXT_WRITE_MAX_BYTES / "é".len());
        let completion = execute_claimed_action(
            &mut executor,
            &action("computer.pasteText", serde_json::json!({"text": text})),
        );
        assert_eq!(executor.calls.len(), 1);
        assert!(matches!(completion, ActionCompletion::Completed { .. }));
    }
