//! File-backed store for named renderer state documents under the app data directory.
//! Mirrors the Electron persistence
...
/// Return a renderer-owned document path without exposing arbitrary app-data files.
#[tauri::command]
pub fn settings_document_path(app: tauri::AppHandle, name: String) -> Result<String, String> {
    if !is_allowed(&name) {
        return Err(StoreError::ForbiddenName.to_string());
    }
    Ok(document_path(&app_data_dir(&app)?, &name)
        .to_string_lossy()
        .into_owned())
}
...
mod tests {
    ...
    #[test]
    fn agent_migration_state_is_an_allowed_native_document() {
        let base = temp_base();
        write_document(&base, "agent-migration-unsupported", "[]").unwrap();
        assert_eq!(
            read_document(&base, "agent-migration-unsupported").unwrap(),
            Some("[]".to_string())
        );
        fs::remove_dir_all(&base).ok();
    }
    ...
    #[test]
    fn github_cache_is_an_allowed_native_document() {
        let base = temp_base();
        write_document(&base, "github-cache", r#"{"pr":{},"issue":{}}"#).unwrap();
        assert!(read_document(&base, "github-cache").unwrap().is_some());
        fs::remove_dir_all(&base).ok();
    }

    #[test]
    fn document_paths_stay_under_the_settings_store() {
        let base = temp_base();
        assert_eq!(
            document_path(&base, "keybindings"),
            base.join("settings-store").join("keybindings.json")
        );
        assert!(!is_allowed("../keybindings"));
        fs::remove_dir_all(&base).ok();
    }
    ...
    #[test]
    fn ui_and_local_workspace_session_are_native_documents() {
        let base = temp_base();
        write_document(&base, "ui", r#"{"leftSidebarVisible":true}"#).unwrap();
        write_document(&base, "workspace-session", r#"{"activeWorktreeId":"wt-1"}"#).unwrap();
        assert!(read_document(&base, "ui").unwrap().is_some());
        assert!(read_document(&base, "workspace-session").unwrap().is_some());
        fs::remove_dir_all(&base).ok();
    }
