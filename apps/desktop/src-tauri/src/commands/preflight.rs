...
/// Bounds auth probes and shell hydration so broken CLIs cannot block startup.
...
/// Probe `<cli> auth status` for each requested provider CLI:
...
/// prompts and banners before parsing command output.
