...
fn resolve_computer_permissions_status(
    ...
    // Why: computer-use permissions are macOS TCC concepts; other platforms
    // must report both as unsupported rather than silently granting them.
...
fn open_computer_permissions_blocking(
    ...
    // Why: non-macOS hosts have no TCC settings pane to open.
    if !cfg!(target_os = "macos") {
...
fn launch_permission_status_helper(
    ...
    wait_for_permission_helper_launch(
        &mut child,
        Instant::now() + STATUS_HELPER_LAUNCH_TIMEOUT,
        Instant::now,
        thread::sleep,
    )
}
...
trait PermissionHelperChild {
    fn try_exit(&mut self) -> Result<Option<ProcessExit>, String>;
    fn kill_and_reap(&mut self);
}

impl PermissionHelperChild for Child {
    fn try_exit(&mut self) -> Result<Option<ProcessExit>, String> {
        self.try_wait()
            .map(|status| {
...
fn wait_for_permission_helper_launch(
    child: &mut impl PermissionHelperChild,
    deadline: Instant,
    ...
    loop {
        match child.try_exit() {
            ...
            Ok(None) if now() < deadline => sleep(Duration::from_millis(50)),
            Ok(None) => {
                child.kill_and_reap();
                return Err("Timed out launching permission helper".to_string());
            }
            Err(error) => {
                // A failed wait leaves child state unknown; always reap it before
                // returning so repeated permission checks cannot leak launchers.
                child.kill_and_reap();
                return Err(format!("Could not check permissions: {error}"));
            }
        }
    }
}
...
mod tests {
    ...
    use super::{wait_for_permission_helper_launch, PermissionHelperChild, ProcessExit};

    struct ScriptedChild {
        exits: VecDeque<Result<Option<ProcessExit>, String>>,
        cleaned_up: bool,
    }

    impl PermissionHelperChild for ScriptedChild {
        fn try_exit(&mut self) -> Result<Option<ProcessExit>, String> {
    ...
    fn child(
        exits: impl IntoIterator<Item = Result<Option<ProcessExit>, String>>,
    ) -> ScriptedChild {
        ScriptedChild {
    ...
    #[test]
    fn permission_helper_launch_accepts_success_after_polling() {
        let mut child = child([
            Ok(None),
            Ok(Some(ProcessExit {
                success: true,
        ...
        let result = wait_for_permission_helper_launch(&mut child, deadline, Instant::now, |_| {});
    ...
    #[test]
    fn permission_helper_launch_reports_nonzero_exit() {
        let mut child = child([Ok(Some(ProcessExit {
            success: false,
            code: Some(7),
        }))]);

        let error =
            wait_for_permission_helper_launch(&mut child, Instant::now(), Instant::now, |_| {})
                .unwrap_err();

        assert_eq!(error, "Could not check permissions: exit 7");
        assert!(!child.cleaned_up);
    }

    #[test]
    fn permission_helper_launch_timeout_kills_and_reaps() {
        let mut child = child([Ok(None)]);
        let deadline = Instant::now();

        let error = wait_for_permission_helper_launch(&mut child, deadline, || deadline, |_| {})
            .unwrap_err();

        assert_eq!(error, "Timed out launching permission helper");
        assert!(child.cleaned_up);
    }

    #[test]
    fn permission_helper_wait_failure_kills_and_reaps() {
        let mut child = child([Err("wait failed".to_string())]);

        let error =
            wait_for_permission_helper_launch(&mut child, Instant::now(), Instant::now, |_| {})
                .unwrap_err();

        assert_eq!(error, "Could not check permissions: wait failed");
        assert!(child.cleaned_up);
    }
}
