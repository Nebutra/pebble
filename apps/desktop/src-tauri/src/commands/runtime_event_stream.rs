...
trait AbortRuntimeEventTask {
    fn abort(self);
}

impl AbortRuntimeEventTask for JoinHandle<()> {
    fn abort(self) {
...
/// each event to the renderer. A repeated call replaces the task so a reloaded webview gets
/// fresh Tauri channel bindings instead of inheriting the destroyed renderer's channels.
#[tauri::command]
pub async fn start_runtime_event_stream(
    app: AppHandle,
    state: State<'_, RuntimeEventStreamState>,
    input: RuntimeEventStreamStartCommand,
    on_event: Channel<RuntimeEventPush>,
    on_status: Channel<RuntimeEventStreamStatus>,
    ...
    // Why: the new renderer must start in polling mode until its replacement SSE connection
    // is live; the prior renderer's connected state cannot be transferred across channels.
    deliver_status(&app, Some(&on_status), false);
    ...
    replace_runtime_event_task(&state.task, handle)?;
...
fn replace_runtime_event_task<T: AbortRuntimeEventTask>(
    slot: &Mutex<Option<T>>,
    next: T,
) -> Result<(), String> {
    let previous = match slot.lock() {
        Ok(mut slot) => slot.replace(next),
        Err(_) => {
            next.abort();
            return Err("runtime event stream state is unavailable".to_string());
        }
    };
...
async fn run_event_stream(
    ...
    on_event: Option<Channel<RuntimeEventPush>>,
    on_status: Option<Channel<RuntimeEventStreamStatus>>,
) {
    let url = format!(
        "{}{}",
...
async fn connect_and_stream(
    ...
    deliver_status(app, on_status, true);
    let outcome = drain_stream(app, &mut response, on_event).await;
    // Any exit from the drain means the stream dropped; re-arm the renderer's polling.
    deliver_status(app, on_status, false);
    outcome
}
        ...
        match response.chunk().await {
            Ok(Some(bytes)) => {
                for event in parser.push(&bytes) {
                    ...
                    // channels avoid the global event bus serialization and listener fanout.
...
fn deliver_status(
    app: &AppHandle,
    channel: Option<&Channel<RuntimeEventStreamStatus>>,
    connected: bool,
) {
    let status = RuntimeEventStreamStatus { connected };
    if let Some(channel) = channel {
        let _ = channel.send(status);
    } else {
        let _ = app.emit(RUNTIME_EVENT_STATUS, status);
    }
}
...
mod tests {
    ...
    struct FakeTask(Arc<AtomicBool>);

    impl AbortRuntimeEventTask for FakeTask {
        fn abort(self) {
    ...
    #[test]
    fn replacing_stream_task_aborts_stale_renderer_binding() {
        let previous_aborted = Arc::new(AtomicBool::new(false));
        let next_aborted = Arc::new(AtomicBool::new(false));
        let slot = Mutex::new(Some(FakeTask(previous_aborted.clone())));

        replace_runtime_event_task(&slot, FakeTask(next_aborted.clone())).unwrap();

        assert!(previous_aborted.load(Ordering::SeqCst));
