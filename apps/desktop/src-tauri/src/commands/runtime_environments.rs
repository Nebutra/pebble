...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RuntimeEnvironmentUpdatePairingInput {
    selector: String,
    pairing_code: String,
}
...
pub fn runtime_environments_add_from_pairing_code(
    ...
    let mut environment = create_environment_from_pairing_offer(id, name.to_string(), now, offer);
    if input.source.as_deref() == Some("ephemeral-vm") {
        environment.source = input.source;
    }
...
#[tauri::command]
pub fn runtime_environments_update_pairing_code(
    app: tauri::AppHandle,
    input: RuntimeEnvironmentUpdatePairingInput,
) -> Result<RuntimeEnvironmentResult, String> {
    let path = environment_store_path(&app)?;
    let mut store = read_store(&path)?;
    let existing = resolve_environment(&store, input.selector.trim())?.clone();
    let offer = parse_pairing_code(&input.pairing_code)?;
    let now = current_time_millis();
    let mut updated = create_environment_from_pairing_offer(
        existing.id.clone(),
        existing.name.clone(),
        existing.created_at,
        offer,
    ...
    updated.runtime_id = existing.runtime_id;
    updated.source = existing.source;
    store.environments = store
        .environments
        .into_iter()
        .map(|environment| {
            if environment.id == updated.id {
                updated.clone()
            } else {
                environment
            }
