...
Date: 2026-06-09
Issue: https://github.com/nebutra/pebble/issues/5049
...
- `packages/product-core/transport/rpc-client.ts` — new `notifyForeground()`:
...
- `packages/product-core/transport/client-context.tsx`:
...
- `packages/product-core/transport/rpc-client.test.ts` → `foreground recovery` describe block:
...
- `packages/product-core/transport/rpc-client-live-recovery.test.ts`: opt-in live harness running the
  REAL rpc-client (real sockets, real tweetnacl E2EE, real timers) against an
  in-process ws server with a blackhole toggle:
  - `PEBBLE_MOBILE_LIVE_REPRO=1 pnpm vitest run packages/product-core/transport/rpc-client-live-recovery.test.ts`
    — half-open-link scenario (~15s).
  - `PEBBLE_MOBILE_LIVE_REPRO_FULL=1 …` — full parked-loop scenario (~8.5 min): waits
