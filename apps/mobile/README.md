...
React Native companion app for Pebble. Monitor worktrees, view terminal output, and send commands from your phone.
...
## Pair With Desktop Pebble

1. Open Pebble desktop.
...
pebble fill --element @e1 --value "ls" --json
...
```bash
cd mobile
PEBBLE_MOBILE_WS_URL=ws://127.0.0.1:6768 pnpm exec tsx scripts/test-subscribe.ts <deviceToken> <serverPublicKeyB64>
```
...
```bash
cd mobile
PEBBLE_MOBILE_WS_URL=ws://127.0.0.1:6768 pnpm exec tsx scripts/repro-terminal-colors.ts \
...
- `packages/product-core/shared/protocol-version.ts` — `DESKTOP_PROTOCOL_VERSION`, `MIN_COMPATIBLE_MOBILE_VERSION`
...
When a verdict is `blocked`, `apps/mobile/src/components/ProtocolBlockScreen.tsx` renders a screen pointing the user at either the App Store (mobile too old) or GitHub Releases (desktop too old).
...
Develop the mobile app without a running Pebble desktop instance:
...
## Connecting to Real Pebble

1. Start Pebble desktop with WebSocket transport enabled
2. In Pebble, go to Settings > Mobile and scan the QR code with this app
