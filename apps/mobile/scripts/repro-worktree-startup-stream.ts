...
const WS_URL = process.env.PEBBLE_MOBILE_WS_URL ?? 'ws://127.0.0.1:6768'
const USER_DATA =
  process.env.PEBBLE_USER_DATA ?? `${process.env.HOME}/Library/Application Support/pebble-dev`
...
const devices = readJson<Array<{ token: string }>>(join(USER_DATA, 'pebble-devices.json'))
const token = devices[0]?.token
const keypair = readJson<{ publicKeyB64: string }>(join(USER_DATA, 'pebble-e2ee-keypair.json'))
