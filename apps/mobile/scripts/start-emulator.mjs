#!/usr/bin/env node
/**
 * Start Pebble Mobile server and load it in the iOS emulator.
...
const PEBBLE_CLI = process.env.PEBBLE_CLI || 'pebble'
...
// Execute pebble CLI command
async function pebble(args, options = {}) {
  const { stdout, stderr } = await execFileAsync(PEBBLE_CLI, args, {
...
async function getWorktree() {
  ...
  try {
    const { stdout } = await pebble(['worktree', 'current', '--json'])
...
async function attachEmulator(worktree, device, runtime) {
  ...
  try {
    await pebble(['emulator', 'attach', device.udid, '--worktree', worktree, '--focus', '--json'], {
      cwd: worktree,
...
function devClientUrlForMetroUrl(url) {
  return `exp+pebble-mobile://expo-development-client/?url=${encodeURIComponent(url)}`
}
...
async function startMetro(worktree) {
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      ...
      const urlMatch = line.match(/exp\+pebble-mobile:\/\/expo-development-client\/\?url=([^\s]+)/)
...
async function openPairingUrlInSimulator(pairingUrl, deviceUdid, runtime, worktree) {
  ...
  await pebble(['emulator', 'tap', '0.5', '0.56', '--worktree', worktree, '--json'], {
    cwd: worktree,
    env: runtime?.env || process.env,
    timeout: 30000
  })
...
async function takeScreenshot(
  deviceUdid,
  outputPath = path.join(os.tmpdir(), 'pebble-mobile-ios.png')
) {
...
// Main function
async function main() {
  log(colors.bright + 'Starting Pebble Mobile in Emulator\n' + colors.reset)
  ...
  try {
    ...
    // Why: emulator helpers are worktree-scoped in Pebble; attach is idempotent
