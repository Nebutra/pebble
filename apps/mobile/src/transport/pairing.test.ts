...
describe('pairing deep links', () => {
  it('extracts the QR pairing code from the hash payload', () => {
    expect(extractPairingCodeFromUrl('pebble://pair#abc123')).toBe('abc123')
  })

  it('extracts the pairing code from a query param', () => {
    expect(extractPairingCodeFromUrl('pebble://pair?code=abc123')).toBe('abc123')
  })

  it('accepts scanner casing and surrounding whitespace', () => {
    expect(extractPairingCodeFromUrl('  PEBBLE://PAIR?code=abc123\n')).toBe('abc123')
  })
  ...
  it('prefers the query pairing code when both query and hash are present', () => {
    expect(extractPairingCodeFromUrl('pebble://pair?code=query-code#hash-code')).toBe('query-code')
  })

  it('ignores empty and unrelated URLs', () => {
    expect(extractPairingCodeFromUrl('pebble://pair')).toBeNull()
  ...
  it('decodes desktop QR payloads when atob requires base64 padding', () => {
    ...
    expect(decodePairingUrl(`pebble://pair?code=${encodeOffer()}`)).toEqual(offer)
  })
