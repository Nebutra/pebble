...
const RELEASES_URL = 'https://github.com/nebutra/pebble/releases'
const IOS_APP_STORE_URL = 'itms-apps://apps.apple.com/app/pebble-ide/id6766130217'
...
export function ProtocolBlockScreen({ verdict }: Props) {
  ...
  const title = isMobileTooOld ? 'Update Pebble Mobile' : 'Update Pebble on your computer'
  const body = isMobileTooOld
    ? `This desktop needs a newer Pebble Mobile app. Update Pebble Mobile from ${mobileUpdateTarget.storeName}, then try this host again.`
    : 'This paired desktop app is too old for your current Pebble Mobile app. Update Pebble on your computer, then try this host again.'
  const recoveryNote =
