...
} from '../../../../packages/product-core/shared/terminal-file-link-conformance'
...
describe('matchFilePathAtColumn', () => {
  ...
  it('matches a spaced path with line and column suffixes', () => {
    const line = 'wrote /tmp/pebble report/result.json:12:3 for you'
    const result = matchFilePathAtColumn(line, colOf(line, 'result'))
    expect(result).toEqual({
      pathText: '/tmp/pebble report/result.json',
      line: 12,
      column: 3
    })
  })
